...
<!-- This is trick. eModal breaks on jquery 3 so load another campatable version. In this case (missing function) load order doesn't matter but other cases may -->
    ...
    var navListItems = $('div.setup-panel div a'),
      ...
      $("#submitPatient").click(function(){
          ...
          if(isOk){
              ...
              // rest update will determine if new or existing pid. in register step-1 we catch existing pid but,
...
function doCredentials(pid){ //@TODO Get rid of and call callServer direct.
    callServer('do_signup', pid);
}

function showEnd(pid){ // @TODO not in repository because this goes away in production.
     var title = 'Patient Portal';
     var params = {
               buttons: [
               ...
               title: title,
               useBin: false,
               url: './create_pass.php?portalsite=on&patient=' + pid
           };
...
function callServer(action, value, dob, last, first){
       var data = {'action':action, 'value': value, 'dob': dob, 'last': last, 'first': first}
       if(action == 'do_signup'){
           data = {'action':action, 'pid': value};
       }
    $.ajax({
        type: 'GET',
        url: 'account.lib.php',
        data: data
            ...
            if(action == "get_newpid"){
                if( rtn > 1 ){ // set pid for restfull save
                    newPid = rtn;
                    $("#profileFrame").contents().find('input#pubpid').val(newPid);
                    $("#profileFrame").contents().find('input#pid').val(newPid);
                    //showEnd(newPid); // testing hook
                    //doCredentials(newPid); // testing hook
                }
                else{
                    alert(rtn) // Existing user. Error message is translated in account.lib.php. :)
                    alert("<?php echo xlt("Our records show that you already have an account.") ?>");
            ...
            if(action == 'do_signup'){
                ...
                // Production finished signup alert.
                var message  = "<?php echo xlt("Your credentials have been sent. Check your email inbox and also possibly your spam folder. " .
                ...
                alert(rtn); // rtn holds username and password for testing. @TODO disable for production.
                ...
                <div class="col-xs-7 col-md-offset-3 text-center">
                    <fieldset>
                        ...
                        <div class="form-group">
                                <label for="selLanguage"><?php echo xlt('Language'); ?></label> <select class="form-control" id="selLanguage" name=languageChoice size="1">
                            <?php
                ...
                <div class="col-md-12">
                    <fieldset>
                        ...
                        <div class="well" style="text-align: center">
                            <h4 class='bg-success'><?php echo xlt("All set. Click submit button to finish setup") ?></h4>
                            <hr>
                            <p>
                            <?php echo xlt("An e-mail with your new account credentials will be sent to the e-mail address supplied earlier.
                            You may still review or edit any part of your information by using the top step buttons to go to the appropriate panels. Be sure you have given your correct e-mail address.

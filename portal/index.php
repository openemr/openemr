    ...
    <div id="wrapper" class="container-fluid text-center">
        <?php if (isset($_SESSION['password_update']) || isset($_GET['password_update'])) {
            ...
            <form class="form pb-5" action="get_patient_info.php" method="POST" onsubmit="return process_new_pass()">
                ...
                <?php } ?>
                <input class="btn btn-secondary float-left" type="button" onclick="document.location.replace('./index.php?woops=1&site=<?php echo urlencode($_SESSION['site_id']); ?>');" value="<?php echo xla('Cancel'); ?>" />
                ...
                <div class="row">
                    <div class="col-12">
                        <?php if ($GLOBALS['portal_onsite_two_register']) { ?>
                            <button class="btn btn-secondary float-left" onclick="location.replace('./account/register.php?site=<?php echo urlencode($_SESSION['site_id']); ?>')"><?php echo xlt('Register'); ?></button>
                        <?php } ?>
                        <?php if ($GLOBALS['portal_two_pass_reset'] && isset($_GET['w']) && (isset($_GET['u']) || isset($_GET['p']))) { ?>
                            <button class="btn btn-danger ml-2" onclick="location.replace('./index.php?requestNew=1&site=<?php echo urlencode($_SESSION['site_id']); ?>')"><?php echo xlt('Reset Credentials'); ?></button>
                        <?php } ?>
    ...
    <script>
        ...
        function callServer(action, value = '', value2 = '') {
            ...
            $.ajax({
                ...
                if (action === "cleanup") {
                    window.location.href = "./index.php?site=<?php echo urlencode($_SESSION['site_id']); ?>" // Goto landing page.

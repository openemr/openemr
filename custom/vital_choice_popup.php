...
// IN CASE WE NEED THIS VITAL TYPE FOR LATER, LIKE 'diabetes' OR 'dip'
$type = '';
...
<?php
  ...
  while($row = sqlFetchArray($forms)) {
    ...
    $getValues[8] = $getVdate = $row{'date'};
		...
		if(isset($row{'HgbA1c'})) $getValues[14] = $row{'HgbA1c'};
    ...
    $getValues[20] = $row{'note'};
		...
		if($use_abnormal_flag) {
			...
			if($bmi_abn) $getValues[25] = 1;
		}

    $anchor = "<a href='javascript:;' onclick=\"return setVitals('".join("','", $getValues)."')\" />";
    $bmi_anchor = "<a href='javascript:;' $bmi_abn onclick=\"return setVitals('".join("','", $getValues)."')\" />";
    ...
    echo "  <td>$bmi_anchor $getVbmi</a></td>\n";

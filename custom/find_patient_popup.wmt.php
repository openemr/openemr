...
//
 ...
 if(!isset($_GET['pid_field'])) { $_GET['pid_field'] = ''; }
 if(!isset($_GET['pname_field'])) { $_GET['pname_field'] = ''; }
 $target_pid = $_GET['pid_field'];
 $target_name = $_GET['pname_field'];

 if(!isset($_REQUEST['searchby'])) { $_REQUEST['searchby'] = ''; }
 if(!isset($_REQUEST['searchparm'])) { $_REQUEST['searchparm'] = ''; }
 ...
 $base_action="find_patient_popup.wmt.php?pid_field=$target_pid&pname_field=$target_name";
?>
...
<script language="JavaScript">
 ...
 function selpid(pid, lname, fname, dob, pid_field, pname_field) {
  if (opener.closed || ! opener.setpatient)
   ...
   opener.setpatient(pid, lname, fname, dob, pid_field, pname_field);
  window.close();
  return false;
 }
...
<form method='post' name='theform' id="theform" action='<?php echo $base_action; ?><?php if(isset($_GET['pflag'])) echo "&pflag=0"; ?>'>
 ...
 <?php echo htmlspecialchars( xl('for:'), ENT_NOQUOTES); ?>
   <input type='text' id='searchparm' name='searchparm' class="form-control" size='12' value='<?php echo htmlspecialchars( $_REQUEST['searchparm'], ENT_QUOTES); ?>'
    title='<?php echo htmlspecialchars( xl('If name, any part of lastname or lastname,firstname'), ENT_QUOTES); ?>'>
   &nbsp;
   <input type='submit' id="submitbtn" class="btn btn-primary ml-1" value='<?php echo htmlspecialchars( xl('Search'), ENT_QUOTES); ?>'>
...
<a class="noresult" href='<?php echo $base_action; ?>&res=noresult' <?php if(isset($_GET['pflag'])) { ?> style="display:none;" <?php } ?>  ><?php echo htmlspecialchars( xl('Click Here to add a new patient.'), ENT_NOQUOTES); ?></a>
</div>
...
foreach ($result as $iter) {
    ...
    echo " <tr class='".$trClass."' id='" .
        htmlspecialchars( $iterpid."~".$iterlname."~".$iterfname."~".$iterdob."~".$target_pid."~".$target_name, ENT_QUOTES) . "'>";
...
var SelectPatient = function (eObj) {
    ...
    return selpid(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5]);
}

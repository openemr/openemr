...
function getHeaderQRDA1($xml, $patient_id, $provider_id): void
{
    ...
    $xml->self_customTag('birthTime', array('value' => date('Ymd', strtotime((string) $patientRow['DOB']))));
    ...
    $timeArr = array('low' => date('Ymdhis', strtotime((string) $from_date)), 'high' => date('Ymdhis', strtotime((string) $to_date)));
    ...
    foreach ($medArr as $medRow) {
        ...
        foreach ($vset as $v) {
            ...
            //act open
                ...
                $timeArr = array('low' => date('Ymdhis', strtotime((string) $medRow['administered_date'])), 'high' => date('Ymdhis', strtotime((string) $medRow['administered_date'])));
    ...
    foreach ($vitArr as $vitRow) {
        ...
        foreach ($measures as $measure_key => $measure) {
            ...
            //code Close
                ...
                $timeArr = array('low' => date('Ymdhis', strtotime((string) $vitRow['date'])), 'high' => date('Ymdhis', strtotime((string) $vitRow['date'])));
...
function getAllRiskCatAssessment($xml, $patient_id): void
{
    ...
    foreach ($procArr as $procRow) {
        ...
        if (!empty($vset['valueset'])) {
            ...
            $timeArr = array('low' => date('Ymdhis', strtotime((string) $procRow['date_ordered'])), 'high' => date('Ymdhis', strtotime((string) $procRow['date_ordered'])));
            $xml->add_entryEffectTimeQRDA($timeArr);
...
function getAllOrderMedications($xml, $patient_id): void
{
    ...
    foreach ($medArr as $medRow) {
        ...
        if (!empty($vset['valueset'])) {
            ...
            $timeArr = array('low' => date('Ymdhis', strtotime((string) $medRow['start_date'])), 'high' => date('Ymdhis', strtotime((string) $medRow['end_date'])));
            $xml->add_entryEffectTimeQRDAMed($timeArr);
...
function getAllMedicalProbs($xml, $patient_id): void
{
    ...
    foreach ($diagArr as $diagRow) {
        ...
        if (!empty($vset['valueset'])) {
            ...
            $timeArr = array('low' => date('Ymdhis', strtotime((string) $diagRow['begdate'])), 'high' => date('Ymdhis', strtotime((string) $endate)));
            $xml->add_entryEffectTime($timeArr);
...
function getAllPatientEncounters($xml, $patient_id): void
{
    ...
    foreach ($encArr as $encRow) {
        ...
        foreach ($vset as $v) {
            ...
            $timeArr = array('low' => date('Ymdhis', strtotime((string) $encRow['date'])), 'high' => date('Ymdhis', strtotime((string) $encRow['date'])));
            $xml->add_entryEffectTime($timeArr);
...
function patCharactersticQRDA($xml, $patient_id): void
{
    ...
    if (!empty($vset['valueset'])) {
        ...
        $timeArr = array('low' => date('Ymdhis', strtotime((string) $patHist['date'])), 'high' => date('Ymdhis', strtotime((string) $patHist['date'])));
        $xml->add_entryEffectTime($timeArr);

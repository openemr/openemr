...
echo "Secret: $AUTH_TOKEN"
response=$(npx testrigor-cli test-suite run "$TEST_SUITE_ID" --token "$AUTH_TOKEN" --localhost --url "$LOCALHOST_URL" --test-cases-path "$TEST_CASES_PATH" --rules-path "$RULES_PATH" --explicit-mutations) || failTest=true

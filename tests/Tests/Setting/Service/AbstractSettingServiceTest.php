...
class AbstractSettingServiceTest extends TestCase
{
    ...
    public function checkSectionHasSettingTest(
        string $settingServiceClass,
        string $sectionName,
        string $settingKey,
        string $expectedExceptionMessage,
    ): void {
        Assert::oneOf($settingServiceClass, [
            GlobalSettingService::class,
            UserSpecificSettingService::class,
        ]);

        if (UserSpecificSettingService::class === $settingServiceClass) {
            $settingService = new UserSpecificSettingService(
                GlobalsServiceFactory::getInstance(),
                UserSpecificSettingSectionService::getInstance(),
                $this->createMock(CompositeSettingManager::class),
            );
        } else {
            $settingService = new GlobalSettingService(
                GlobalsServiceFactory::getInstance(),

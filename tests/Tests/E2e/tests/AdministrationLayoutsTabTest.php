...
class AdministrationLayoutsTabTest extends PantherTestCase
{
    private $e2eBaseUrl;
    ...
    /** @test */
    public function testNewOnAdministrationLayoutTabOpensModal()
    {
        $session = $this->start();
        (new TabTestElement)->open($session, 'Administration', 'Forms', 'Layouts');
        $session = (new TabTestElement)->focus($session, 'adm');
        (new ButtonTestElement)->clickByText($session, 'New Layout');

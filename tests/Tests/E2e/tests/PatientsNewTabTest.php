...
class PatientsNewTabTest extends PantherTestCase
{
    private $e2eBaseUrl;
    ...
    /** @test */
    public function testSearchOnPatientsNewTabOpensModal()
    {
        $session = $this->start();
        (new TabTestElement)->open($session, 'Patient/Client', 'New/Search');
        $session = (new TabTestElement)->focus($session, 'pat');
        (new ButtonTestElement)->clickById($session, 'search');

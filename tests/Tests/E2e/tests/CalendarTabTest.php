...
class CalendarTabTest extends PantherTestCase
{
    private $e2eBaseUrl;
    ...
    {
        $session = $this->start();
        (new TabTestElement)->open($session, 'Calendar');
        $session = (new TabTestElement)->focus($session, 'cal');
        (new ButtonTestElement)->clickByTitle($session, 'New Appointment');

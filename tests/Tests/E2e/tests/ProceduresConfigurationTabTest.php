...
class ProceduresConfigurationTabTest extends PantherTestCase
{
    private $e2eBaseUrl;
    ...
    {
        $session = $this->start();
        (new TabTestElement)->open($session, 'Procedures', 'Configuration');
        $session = (new TabTestElement)->focus($session, 'pat');
        (new ButtonTestElement)->clickById($session, 'add_node_button');

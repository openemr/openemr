...
/**
 * FHIR RelatedPerson API (HTTP write) tests.
 *
 * Drives real HTTP POST/PUT through OAuth against /apis/default/fhir/RelatedPerson
...
class RelatedPersonFhirWriteApiTest extends TestCase
{
    private const RESOURCE_URL = '/apis/default/fhir/RelatedPerson';
    private const RESOURCE_TYPE = 'RelatedPerson';
    ...
    protected function setUp(): void
    {
        ...
        $fixtureData = json_decode(
            (string) file_get_contents(__DIR__ . '/../Fixtures/FHIR/related-person.json'),
            true
        );
    ...
    public function testPostCreatesRelatedPerson(): void
    {
    ...
    public function testPutUpdatesRelatedPerson(): void
    {
    ...
    /**
     * PersonService rejects a second person with the same first_name + last_name + birth_date.
     * That is a business rule rejecting the payload, not the server failing: it has to reach the
     * client as a 4xx naming the reason, not as a 500 with the detail buried in the PHP log.
     */
    public function testPostDuplicateReturnsValidationErrorNotServerError(): void
    {
        $first = $this->testClient->post(self::RESOURCE_URL, $this->fhirFixture);
        $this->assertSame(
            Response::HTTP_CREATED,
            $first->getStatusCode(),
            'First POST should return 201. Body: ' . $first->getBody()->getContents()
        );

        $second = $this->testClient->post(self::RESOURCE_URL, $this->fhirFixture);
        $secondBody = $second->getBody()->getContents();
        $this->assertNotSame(
            Response::HTTP_INTERNAL_SERVER_ERROR,
            $second->getStatusCode(),
            'A duplicate must not be reported as a server error. Body: ' . $secondBody
        );
        $this->assertSame(
            Response::HTTP_BAD_REQUEST,
            $second->getStatusCode(),
            'Duplicate POST should return 400. Body: ' . $secondBody
        );
        $contents = json_decode($secondBody, true);
        $this->assertIsArray($contents, 'Body: ' . $secondBody);
        $this->assertArrayHasKey('validationErrors', $contents, 'Body: ' . $secondBody);
        $this->assertNotSame([], $contents['validationErrors'], 'The rejection reason should reach the client');
    }

    public function testPostWithoutPatientReturnsError(): void
    {
        $invalid = $this->fhirFixture;
        unset($invalid['patient']);
        $response = $this->testClient->post(self::RESOURCE_URL, $invalid);
        $this->assertSame(
            Response::HTTP_BAD_REQUEST,
            $response->getStatusCode(),
            'POST without patient should return 400. Body: ' . $response->getBody()->getContents()
        );
    }
}

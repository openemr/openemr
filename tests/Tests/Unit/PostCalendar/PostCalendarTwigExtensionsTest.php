...
class PostCalendarTwigExtensionsTest extends TestCase
{
    ...
    {
        parent::setUp();
        // Load calendar helper functions (is_weekend_day, is_holiday, etc.)
        require_once dirname(__DIR__, 4) . '/library/calendar.inc.php';
    ...
    public function testGenerateDOWCalendarMarksOtherMonthDays(): void
    {
        ...
        foreach ($result as $week) {
            foreach ($week as $day) {
                ...
                // Check if non-weekend days from other months are marked
                // Weekend styling takes priority over other-month styling
                if ($dayDate->format('m') != '06' && $dayOfWeek != '0' && $dayOfWeek != '6') {
        ...
        $this->assertTrue($foundOtherMonth, 'Should mark non-weekend days from other months');
    }
}

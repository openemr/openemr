...
final class OidcRpIsolatedTest extends TestCase
{
    ...
    public static function unsafeIssuerProvider(): array
    {
        return [
            ...
            // fe80::/10 runs to febf:ffff:..., so a prefix test on "fe80:"
            // leaves most of the range reachable.
            'ipv6 link local upper nibble' => ['https://[fe90::1]/realms/openemr', false],
            'ipv6 link local range end' => ['https://[febf:ffff::1]/realms/openemr', false],
            // The same address wearing an IPv4-mapped encoding.
            'ipv4 mapped link local' => ['https://[::ffff:169.254.169.254]/latest/meta-data', false],
            'ipv4 mapped link local hex' => ['https://[::ffff:a9fe:a9fe]/latest/meta-data', false],
    ...
    #[Test]
    public function issuerUrlAllowsAddressesOutsideTheLinkLocalRange(): void
    {
        // fec0:: is the next range up and must not be caught by the /10 mask,
        // nor must a global address that merely starts with the same bytes.
        OidcIssuerUrl::assertSafe('https://[fec0::1]/realms/openemr', false);
        OidcIssuerUrl::assertSafe('https://[fe7f::1]/realms/openemr', false);
        OidcIssuerUrl::assertSafe('https://[2001:db8::1]/realms/openemr', false);
        OidcIssuerUrl::assertSafe('https://169.255.0.1/realms/openemr', false);
        $this->assertSame('idp.example', OidcIssuerUrl::hostOf('https://idp.example/realms/openemr'));
    }
     ...
     * @codeCoverageIgnore Data providers run before coverage instrumentation starts.
     */
    public static function unusableJwksResponseProvider(): array
    {
        return [
            // json_decode() yields null and JsonWebKeySet then calls
            // property_exists(null, 'keys'), which is a TypeError, not a
            // JWKValidatorException.
            'invalid json' => [new Response(200, ['Content-Type' => 'application/json'], 'not json')],
            'missing keys property' => [new Response(200, ['Content-Type' => 'application/json'], '{}')],
            'provider error status' => [new Response(503, [], 'upstream down')],
        ];
    }

    #[Test]
    #[DataProvider('unusableJwksResponseProvider')]
    public function unusableJwksIsReportedAsAnOidcFailure(Response $jwksResponse): void
    {
        // The JWKS is fetched inside the JsonWebKeySet constructor, so these
        // failures have to surface as OidcRpException: the callback only renders
        // its login-failure page for that type and rethrows anything else, which
        // would turn a provider outage into a 500.
        $bundle = $this->signedIdToken();
        $http = new Client([
            'handler' => HandlerStack::create(new MockHandler([$jwksResponse])),
        ]);
        $validator = new OidcIdTokenValidator($http, $bundle['clock']);
        $this->expectException(OidcRpException::class);
        $validator->validate($bundle['jwt'], $bundle['settings'], $bundle['metadata'], $bundle['nonce']);
    }

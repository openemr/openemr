...
/**
 * Tests the orchestration of openemr:patch-prep: option parsing, side
 * dispatching, MutatorContext build (esp. fromVersion override), and
 * mutator invocation order. Mutators themselves are exercised by their
 * per-class tests.
 *
 * @package   OpenEMR
...
use OpenEMR\Common\Command\PatchPrepCommand;
...
#[Group('isolated')]
#[Group('release-prep')]
final class PatchPrepCommandTest extends TestCase
{
    public function testRelSideDispatchesRelMutatorsInOrder(): void
    {
        $a = new PatchPrepRecordingMutator('rel-a');
        $b = new PatchPrepRecordingMutator('rel-b');
    ...
    {
        $rel = new PatchPrepRecordingMutator('rel');
        $master = new PatchPrepRecordingMutator('master');
    ...
    public function testRequiresPrevVersion(): void
    {
        $tester = $this->buildTester([], []);
        $exit = $tester->execute([
            '--target-version' => '8.1.1',
    ...
    public function testMalformedPrevVersionReturnsInvalid(): void
    {
        $tester = $this->buildTester([], []);
        $exit = $tester->execute([
            '--target-version' => '8.1.1',
    ...
    public function testContextPassedToMutatorsCarriesFromVersionAndRelBranch(): void
    {
        $spy = new PatchPrepContextRecordingMutator();
        ...
        self::assertSame(1, $spy->lastContext->minor);
        self::assertSame(1, $spy->lastContext->patch);
        self::assertSame('rel-810', $spy->lastContext->relBranch);
        self::assertSame('8.1.0', $spy->lastContext->fromVersion);
    }

    public function testMasterSideContextAlsoHasFromVersion(): void
    {
        $spy = new PatchPrepContextRecordingMutator();
        $tester = $this->buildTester([], [$spy]);
        $tester->execute([
    ...
    {
        $command = new PatchPrepCommand($relMutators, $masterMutators);
        $app = new Application();
        $app->addCommand($command);
        return new CommandTester($app->find('openemr:patch-prep'));
    }
}

final class PatchPrepRecordingMutator implements MutatorInterface
{
...
final class PatchPrepContextRecordingMutator implements MutatorInterface
{

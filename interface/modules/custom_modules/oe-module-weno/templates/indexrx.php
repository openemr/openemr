        ...
        $(function () {
            ...
            $(function () {
                ...
                asyncAlertMsg(warnMsg, 8000, 'danger', 'lg').then(() => {
                    window.location.href = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') ?>/interface/patient_file/summary/demographics.php?set_pid=<?php echo urlencode(attr($_SESSION['pid'] ?? $pid ?? '')) ?>";
                });
            });
...
<body>
    <div id="trigger-debug" class="container-xl">
        <div class="container-xl sticky-container bg-light text-dark">
            <form>
                <header class="bg-light text-dark text-center">
                    <h3>
                        <a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') ?>/interface/patient_file/summary/demographics.php?resync=true&set_pid=<?php echo urlencode(attr($_SESSION['pid'] ?? $pid)) ?>" class="text-primary" title="<?php echo xla("Return to Patient Demographics"); ?>"><?php echo xlt("e-Prescribe"); ?>
        ...
        <footer>
            <a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') ?>/interface/patient_file/summary/demographics.php?resync=true&set_pid=<?php echo urlencode(attr($_SESSION['pid'] ?? $pid)) ?>" class="btn btn-primary float-right mt-2 mb-4 mr-3"><?php echo xlt("Return to Demographics"); ?></a>

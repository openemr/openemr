...
class Bootstrap
{
    ...
    {
        return \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . self::MODULE_INSTALLATION_PATH . $this->moduleDirectoryName . "/public/assets/";
    }
    ...
    {
        $xmlConfigPath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webserver_root') . DIRECTORY_SEPARATOR . 'Documentation' . DIRECTORY_SEPARATOR . 'EHI_Export';
        // . DIRECTORY_SEPARATOR . 'docs' . DIRECTORY_SEPARATOR . 'openemr.openemr.xml';
        return new EhiExporter(
            \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webserver_root') . $this->getPublicPath(),
            $this->getPublicPath(),
            $xmlConfigPath,
            $this->getTwig()
        );

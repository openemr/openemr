...
class EhiExporter
{
    ...
    private function exportAdditionalAssets(\ZipArchive $zip, $tableName)
    {
        $additionalAssets = [
            'form_painmap' => [
                ['name' => 'images/painmap.png', 'path' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webserver_root') . "/interface/forms/painmap/templates/painmap.png"]
            ]
        ];
    ...
    public function getExportSizeSettings()
    {
        ...
        $freeSpace = disk_free_space(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITES_BASE'));
        if ($freeSpace === false) {
    ...
    private function addDocumentationReadme(\ZipArchive $zip)
    {
        $readmeContents = $this->twig->render(Bootstrap::MODULE_NAME . '/README.text.twig', [
            'webBaseUrl' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('site_addr_oath') . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot')

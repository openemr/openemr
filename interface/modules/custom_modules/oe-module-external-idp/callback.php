...
/**
 * External IdP callback endpoint.
 *
 * @package OpenEMR
...
try {
    $redirectTarget = (new OidcAuthenticationService())->finish($_GET);
    header('Location: ' . $redirectTarget);
    exit;
} catch (\Throwable $exception) {
    EventAuditLogger::getInstance()->newEvent('external_login_failure', '', $providerIdForAudit, 0, 'authorization callback failed');
    ServiceContainer::getLogger()->error('External IdP callback failed', ['exception' => $exception]);

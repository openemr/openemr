...
export const UtilService = {
    getPid:()=>{
        ...
        //     ? params.get("pid") : 1;
        return getCookie("pid_vehr") !== "" && getCookie("pid_vehr") !== null ? getCookie("pid_vehr") : 20;
    ...
    defaultWidth : 1800,
    tilesWidth:(size)=>{
      return (size > 5) ? UtilService.zoomMinWidth(1) : (UtilService.defaultWidth / size)
    },
    containerWidth:(size)=>{
        return (size > 5) ? (UtilService.zoomMinWidth(1) * size) : UtilService.defaultWidth
    },
    serializeEndDate:(enddate,viewType)=>{return serializeEndDate(enddate,viewType)},
    handleIsInDateRange:(start,end,date,viewType)=>{
        return handleIsInDateRange(start,end,date,viewType)
    },
    serializeDate:(date)=>{return serializeDate(date)},
    hasTitleShow:(endate,date)=>{
        return hasTitleShow(endate,date)
    },
    makeKey:(length)=> {return makeKeyString(length)}
};

...
class NotificationEventListener implements EventSubscriberInterface
{
    ...
    {
        $this->isSmsEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('oefax_enable_sms') ?? 0);
        $this->isFaxEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('oefax_enable_fax') ?? 0);
        $this->isEmailEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('oe_enable_email') ?? 0);
        $this->isVoiceEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('oe_enable_voice') ?? 0);
    ...
    public function renderPhoneButton()
    {
        ...
        if ($loginCred['appKey'] && $loginCred['appSecret'] && $loginCred['jwt'] && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('oe_enable_voice') ?? false) {
            echo '
    ...
    public function renderPhoneWidget(RenderEvent $event): void
    {
        ...
        $moduleBaseUrl = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/modules/custom_modules/oe-module-faxsms";
        $context = [
    ...
    public function onNotifyDocumentRenderOneTime(SendNotificationEvent $event): string
    {
        ...
        $parameters = [
            'pid' => $pid,
            'redirect_link' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . "/portal/patient/onsitedocuments?pid=" . urlencode($pid) .
    ...
    public function onNotifyUniversalOneTime(SendNotificationEvent $event): string
    {
        ...
        $site_id = ($_SESSION['site_id'] ?? null) ?: 'default';
        $pid = $event->getPid();
        $defaultUrl = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . "/portal/home.php?site=" . urlencode((string) $site_id) . "&landOn=MakePayment";

...
class EtherFaxActions extends AppDispatch
{
    ...
    public function sendFax(): string
    {
        ...
        $smtpEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('SMTP_PASS') ?? null) && !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('SMTP_USER') ?? null);
    ...
    public function forwardFax(): string
    {
        ...
        $hasEmail = $this->validEmail($email);
        $smtpEnabled = !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('SMTP_HOST') ?? null);
        $user = $this::getLoggedInUser();
        $facility = substr((string)$user['facility'], 0, 20);
        $csid = $this->formatPhone($this->credentials['phone']);
        $tag = xlt("Forwarded");

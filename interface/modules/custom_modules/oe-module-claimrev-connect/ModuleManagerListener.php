...
{
    private array $tasks = array('ClaimRev_Send', 'ClaimRev_Receive', 'ClaimRev_Elig_Send_Receive');
     ...
     * so it will be instantiated in Lamina's Manager.
     *
    ...
    private function install($modId, $currentActionStatus): mixed
    {
        ...
        */
        self::setModuleState($modId, $this->tasks, '0', '1');
        return $currentActionStatus;
    }
    ...
    {
        self::setModuleState($modId, $this->tasks, '1', '0');
        return $currentActionStatus;
    }
    ...
    private function unregister($modId, $currentActionStatus): mixed
    {
        ...
        self::setTaskState('0', $this->tasks, false, true);
        return $currentActionStatus;
    }
    ...
    /**
     * Grab all Module setup or by table column values.
     *
     * @param        $modId
     * @param string $col
     * @return array
    ...
    /**
     * Set background tasks to active or inactive.
     * If reset is true, set next_run to now.
     * If removeTask is true, delete the task.
     *
     * @param      $flag
     * @param      $serviceArray
     * @param bool $reset
    ...
    /**
     * Convenience method to set module state and background services.
     * If module is disabled, set all background services to inactive.
     * If module is enabled, set all background services to active.
     *
     * @param $modId   int|string module id or directory name
     * @param $serviceArray array list of background services to set their state.
     ...
     */
    private static function setModuleState(int|string $modId, array $serviceArray, int|string $flag, int|string $flag_ui): array|bool|null
    {
        if (($flag_ui == '1') || ($flag == '0')) {
            self::setTaskState('0', $serviceArray);
        } else {
            // set BG tasks to active if module is active.
            self::setTaskState('1', $serviceArray);
        }
        // set module state.

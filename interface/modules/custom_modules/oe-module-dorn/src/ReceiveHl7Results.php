...
class ReceiveHl7Results
{
    ...
    public function receiveSingleResults($result, $rejectResult): ReceiveResultsResponseModel
    {
        ...
        $resultPath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/procedure_results";
        ...
        $logpath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/procedure_results/logs/$lab_npi";
    ...
    private function receiveHl7Results(&$hl7, &$matchreq, $lab_id = 0, $direction = 'B', $dryrun = false, $patientId = null)
    {
        ...
        $catrow = sqlQuery(
            ...
            [\OpenEMR\Core\OEGlobalsBag::getInstance()->get('lab_results_category_name')]
        );
        if (empty($catrow['id'])) {
            ...
            $catrow = sqlQuery(
                "SELECT id FROM categories WHERE name = ?",
                [\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_mdm_category_name')]
            );
            if (!empty($catrow['id'])) {

    ...
    (function($) {
        ...
        const ContextAdmin = {
            ...
            bindEvents: function() {
                ...
                $('#btnCreateContext').on('click', function() { self.showContextDialog(); });
                $('#filterFacility, #filterUserType').on('change', function() { self.loadUsers(); });

                let searchTimeout;
                $('#filterSearch').on('input', function() {
                    clearTimeout(searchTimeout);
                    searchTimeout = setTimeout(function() { self.loadUsers(); }, 300);
                });
                ...
                $('#btnBulkAssign').on('click', function() { self.showBulkAssignDialog(); });
                $('#btnExportContexts').on('click', function() { self.exportContexts(); });
                $('#btnImportContexts').on('click', function() { self.importContexts(); });
            },
            ...
            showContextDialog: function(contextData) {
                        ...
                        $('<div>', { class: 'dialog-body' }).append(
                            ...
                            $('<div>', { class: 'form-group' }).append(
                                ...
                                $('<input>', {
                                    ...
                                    class: 'form-control',
                                    id: 'contextName',
                                    value: isEdit ? contextData.context_name : ''
                                })
                            ),
                            $('<div>', { class: 'form-group' }).append(
                                ...
                                $('<input>', {
                                    ...
                                    class: 'form-control',
                                    id: 'contextKey',
                                    value: isEdit ? contextData.context_key : '',
                                    readonly: isEdit,
                                    placeholder: self.xl.autoGenerated
                                })
                            ),
                            $('<div>', { class: 'form-group' }).append(
                ...
                $('.dialog-close').on('click', function() { $('#contextDialog').remove(); });
                $('#contextDialog').on('click', function(e) {
                    if (e.target === this) $(this).remove();
                });

                $('#btnSaveContext').on('click', function() { self.saveContext(); });
            },
            ...
            editContext: function(contextId) {
                const self = this;
                const context = this.config.contexts.find(function(c) { return c.id == contextId; });
                if (context) {
            ...
            renderUsers: function(users) {
                ...
                users.forEach(function(user) {
                    const context = self.config.contexts.find(function(c) { return c.context_key === user.active_context; });
            ...
            showAssignDialog: function(userId, userName, currentContext) {
                ...
                $('.dialog-close').on('click', function() { $('#assignDialog').remove(); });
                $('#assignDialog').on('click', function(e) {
                    if (e.target === this) $(this).remove();
                });

                $('#btnConfirmAssign').on('click', function() { self.assignContext(); });
            },
            ...
            showBulkAssignDialog: function() {
                ...
                $('.dialog-close').on('click', function() { $('#bulkAssignDialog').remove(); });
                $('#bulkAssignDialog').on('click', function(e) {
                    if (e.target === this) $(this).remove();
                });

                $('#btnConfirmBulkAssign').on('click', function() { self.bulkAssign(); });
            },
            ...
            renderRoleDefaults: function(defaults, userTypes) {
                ...
                const defaultMap = {};
                defaults.forEach(function(d) { defaultMap[d.role_type] = d.context_key; });
            ...
            showAlert: function(message, type) {
                ...
                const $alert = $('<div>', {
                    ...
                    $('<button>', {
                        type: 'button',
                        class: 'close ml-2'
                    }).html('&times;').on('click', function() { $(this).parent().remove(); })
                );
                $('body').append($alert);

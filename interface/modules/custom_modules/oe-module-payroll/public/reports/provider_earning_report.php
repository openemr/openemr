...
require_once dirname(__FILE__, 7) . "/library/patient.inc";
require_once dirname(__FILE__, 7) . "/custom/code_types.inc.php";
...
<form method='post' id='theform' action='provider_earning_report.php' onsubmit='return top.restoreSession()'>
    ...
    <table class='text'>
        ...
        <div class="checkbox">
                <label><input type='checkbox' name='form_details'
                  value='1'<?php echo (!empty($_POST['form_details'])) ? " checked" : "checked"; ?>><?php echo xlt('Details') ?></label>
        </div>
            </td>
        </tr>
    </table>
   ...
   <td>
      &nbsp;<?php
        ...
        ?>
         ...
         <td>
                <?php getDocsList($encounter); ?>&nbsp;
         </td>
         <td>
                <?php echo text($billed); ?>
         </td>
         <!--<td style='color:#cc0000'>
                <?php echo text($errmsg); ?>&nbsp;
         </td>-->
        </tr>
                <?php
            } // end of details line
    ...
    <div>
        <p class="text-center">
            <?php echo xlt('Payouts are based on the number of billed visits. Unbilled visits are not counted.'); ?>
        </p>
    </div>
</div> <!-- end the apptenc_report_results -->
<?php } else { ?>
<div class='text-center'>
    <strong><?php echo xlt('Please input search criteria above, and click Submit to view results.'); ?></strong>
</div>
<?php } ?>

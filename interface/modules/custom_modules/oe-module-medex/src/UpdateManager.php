...
class UpdateManager
{
    ...
    public function checkForUpdates(bool $forceCheck = false): ?array
    {
        ...
        try {
            ...
            $updateInfo = [
                'update_available' => version_compare($response['latest_version'], self::CURRENT_VERSION, '>'),
                'current_version' => self::CURRENT_VERSION,
                'latest_version' => $response['latest_version'] ?? self::CURRENT_VERSION,
    ...
    public function installUpdate(string $downloadUrl, bool $createBackup = true): array
    {
        ...
        if (!$this->api->isConfigured()) {
            return ['success' => false, 'error' => 'MedEx not configured'];
        }
        ...
        try {
            ...
            if (!$this->verifyPackage($tempFile)) {
                unlink($tempFile);
                return ['success' => false, 'error' => 'Update package verification failed'];
            }
    ...
    public function createBackup(): array
    {
        ...
        $backupFile = $backupDir . '/medex_v' . self::CURRENT_VERSION . '_' . $timestamp . '.zip';

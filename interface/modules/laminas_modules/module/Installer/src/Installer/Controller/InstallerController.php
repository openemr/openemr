<?php
/**
 * interface/modules/laminas_modules/module/Installer/src/Installer/Controller/InstallerController.php
 *
 * @package   OpenEMR
...
class InstallerController extends AbstractActionController
{
    ...
    public function registerAction()
    {
        ...
        if ($request->isPost()) {
            if ($request->getPost('mtype') == 'laminas') {
                ...
                // list that is loaded using the OpenEMR db connector and not the laminas db connector, cache the modules, and then
                ...
                if ($this -> getInstallerTable() -> register($request->getPost('mod_name'), $rel_path, 0, $GLOBALS['laminasModDir'])) {
                    $status = true;
                }
            } else {

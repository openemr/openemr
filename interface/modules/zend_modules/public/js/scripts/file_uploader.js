...
function sendFileToServer(formData, status, progress_bar, success_function) {
    const uploadURL = `${basePath}/documents/documents/upload`; // Upload URL
...
function createStatusbar(obj) {
    ...
    const resultTranslated = js_xl('Abort');
    ...
    this.setFileNameSize = function (name, size) {
        ...
        if (Number.parseInt(sizeKB, 10) > 1024) {
    ...
    this.setProgress = function (progress) {
        ...
        if (Number.parseInt(progress, 10) >= 100) {
            this.abort.hide();
        }
    };
...
function AjaxFileUploader(settings) {
    ...
    if ($('.file-uploader-progress').length < 1 && settings.progress_bar) {
        const progress_div = `
            <div class='file-uploader-progress' style='width:${progress_div_width}px;height:${progress_div_height}px;'>
                <div class='file-uploader-progress-div_head'>
                    <div class='file-uploader-progress-close'>
                    </div>
                    <div class='file-uploader-progress-minimize'>
                    </div>
                    <div class='file-uploader-progress-expand'>
                    </div>
                </div>
    ...
    $(`#${uplader_id}_upload`).click(function () {
        ...
        const files = $(`#${uplader_id}_browse`).prop('files');

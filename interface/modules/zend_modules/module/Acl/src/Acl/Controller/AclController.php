        ...
        foreach ($tempList as $groupID => $tempListRow) {
            ...
            if(!empty($tempListRow['items'])) {
              ...
              foreach ($tempListRow['items'] as $key => $itemRow){
                 $output_string .='<li '.$li_class.' id="li_'.$id.$itemRow['group_id'].'-'.$itemRow['user_id'].'" style="'.$visibility.'"><div class="'.$dragabble.'" id="'.$id.$itemRow['group_id'].'-'.$itemRow['user_id'].'">' . $itemRow['display_name'] . '</div></li>';
              }
              $output_string .='</ul>';
            }
            $output_string .='</li>';

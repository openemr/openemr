...
class InstallerController extends AbstractActionController
{
    ...
    {
        $baseModuleDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir');
        $customDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('customModDir');
        $zendModDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('zendModDir');
        $coreModules = ['Application', 'Acl', 'Installer', 'FHIR', 'PatientFlowBoard'];
        $allModules = [];
        ...
        $dir_path = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../$baseModuleDir$customDir/";
        $dp = opendir($dir_path);
        $inDirCustom = [];
        ...
        $dir_path = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../$baseModuleDir$zendModDir/module";
        $dp = opendir($dir_path);
        $inDirLaminas = [];
        for ($i = 0; false != ($file_name = readdir($dp)); $i++) {
    ...
    public function registerAction()
    {
        ...
        if (method_exists($request, 'isPost')) {
            if ($request->getPost('mtype') == 'zend') {
                ...
                if ($this->InstallerTable->register($request->getPost('mod_name'), $rel_path, 0, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('zendModDir'))) {
                    $status = true;
                }
            } else {
    ...
    {
        $modPath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "custom_modules/" . $dirModule;
    ...
    function getModuleVersionFromFile($modId)
    {
        ...
        $ModulePath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "zend_modules/module/" . $dirModule->modDirectory;
        if (!is_dir($ModulePath)) {
            $ModulePath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "custom_modules/" . $dirModule->modDirectory;
        }
    ...
    {
        $ModulePath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "zend_modules/module/" . $modDirectory;
        $versions = [];
        $dh = opendir($sqldir);
        if (!$dh) {
            return false;
    ...
    public function InstallModuleSQL($modId = '')
    {
        ...
        if ($this->InstallerTable->installSQL($modId, $modType, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . $modUri . $dirModule)) {
    ...
    public function UpgradeModuleSQL($modId = '')
    {
        ...
        $modDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . $modUri . $Module->modDirectory;
    ...
    {
        $Module = $this->InstallerTable->getRegistryEntry($modId, "mod_directory");
        $modDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "zend_modules/module/" . $Module->modDirectory;
        $div = [];
    ...
    public function InstallModule($modId = '', $mod_enc_menu = '', $mod_nick_name = '')
    {
        ...
        $sqlInstalled = false;
        if ($modType == InstModuleTable::MODULE_TYPE_CUSTOM) {
            $fullDirectory = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('customModDir') . "/" . $dirModule;
            ...
            $fullDirectory = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/../" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') . "zend_modules/module/" . $dirModule;
            if ($this->InstallerTable->installSQL($modId, $modType, $fullDirectory)) {
                $sqlInstalled = true;
            } else {

...
<participant index=\"{$index}\">
    ...
    private function convertAdvanceDirectivesToXml($data, $pid): string
    {
        ...
        foreach ($data['documents'] as $document) {
            ...
            <extension>" . xmlEscape(base64_encode($siteId . $document['id'])) . "</extension>
            <sha_extension>" . xmlEscape($this->formatUid($siteId . $document['id'] . $document['type'])) . "</sha_extension>
    ...
    */
    ...
    public function getAllergies($pid)
    {
        ...
        foreach ($res as $row) {
            ...
            foreach ($split_codes as $single_code) {
                ...
                <id>" . xmlEscape(base64_encode($session->get('site_id') . $row['id'] . $single_code)) . "</id>
    ...
    <extension>" . xmlEscape(base64_encode($session->get('site_id') . $row['id'])) . "</extension>
    ...
    where l.type = ? and l.pid = ? AND l.outcome != ?"; // patched out /* AND l.id NOT IN(SELECT list_id FROM issue_encounter WHERE pid = ?)*/
        ...
        foreach ($res as $row) {
            ...
            foreach ($split_codes as $single_code) {
                ...
                <extension>" . xmlEscape(base64_encode($siteId . $row['id'])) . "</extension>
    ...
    where l.type = ? and l.pid = ? AND l.outcome != ? AND l.id NOT IN(SELECT list_id FROM issue_encounter WHERE pid = ?)";
        ...
        foreach ($res as $row) {
            ...
            foreach ($split_codes as $single_code) {
                ...
                <extension>" . xmlEscape(base64_encode($siteId . $row['id'])) . "</extension>
                <sha_extension>" . xmlEscape($this->formatUid($siteId . $row['udi'])) . "</sha_extension>
                <title>" . xmlEscape($row['title']) . ($single_code ? " [" . xmlEscape($single_code) . "]" : '') . "</title>
                <code>" . ($code ? xmlEscape($code) : '') . "</code>
                <code_type>" . ($code ? xmlEscape($code_type) : '') . "</code_type>
                <code_text>" . xmlEscape(($code_text ?: '')) . "</code_text>
    ...
    WHERE $wherCon (po.procedure_order_type = 'order' OR po.procedure_order_type = 'procedure') AND po.patient_id = ? AND po.activity = ?";
        ...
        foreach ($rows as $row) {
            ...
            <extension>" . xmlEscape(base64_encode($siteId . ($row['encounter'] ?? ''))) . "</extension>
            ...
            <zip>" . xmlEscape($row['zip']) . "</zip>
            <work_phone>" . xmlEscape($row['phonew1']) . "</work_phone>
            <facility_extension>" . xmlEscape(base64_encode($siteId . $row['fid'])) . "</facility_extension>
    ...
    public function getResults($pid, $encounter)
    {
        ...
        <extension>' . xmlEscape(base64_encode($siteId . $row['test_code'])) . '</extension>
        ...
        <order_status>' . xmlEscape($order_status) . '</order_status>';
            foreach ($row['subtest'] as $row_1) {
                ...
                $results .= '
            <subtest>
            <extension>' . xmlEscape(base64_encode($siteId . $row_1['result_code'])) . '</extension>
            <root>' . xmlEscape("7d5a02b0-67a4-11db-bd13-0800200c9a66") . '</root>
    ...
    */
    ...
    public function getEncounterHistory($pid)
    {
        ...
        foreach ($res as $row) {
            ...
            $encounter_ext = base64_encode($siteId . $row['encounter']);
            $encounter_root = $this->formatUid($siteId . $row['encounter']);
            $problem = '';
            $primary_diagnosis = '';
            $issue_codes = '';
            if (count($res_issues ?? []) > 0) {
                ...
                foreach ($res_issues as $issue) {
                    ...
                    $issue_ext = base64_encode($siteId . $issue['list_id']);
                    $issue_codes .= "
                    <problem>
        ...
        <facility_extension>" . xmlEscape(base64_encode($siteId . $row['fid'])) . "</facility_extension>
        <facility_sha_extension>" . xmlEscape($this->formatUid($siteId . $row['fid'])) . "</facility_sha_extension>
    ...
    */
    ...
    public function getVitals($pid)
    {
        ...
        foreach ($res as $row) {
            ...
            <extension_temperature>" . xmlEscape(base64_encode($siteId . $row['id'] . 'temperature')) . "</extension_temperature>
            <bpd>" . xmlEscape(($row['bpd'] ?: '')) . "</bpd>
            <extension_bpd>" . xmlEscape(base64_encode($siteId . $row['id'] . 'bpd')) . "</extension_bpd>
            <bps>" . xmlEscape(($row['bps'] ?: '')) . "</bps>
            <extension_bps>" . xmlEscape(base64_encode($siteId . $row['id'] . 'bps')) . "</extension_bps>
            <bp_avg>" . xmlEscape(($bp_avg_value !== '' ? $bp_avg_value : '')) . "</bp_avg>
            <extension_bp_avg>" . xmlEscape(base64_encode($siteId . $row['id'] . 'bp_avg')) . "</extension_bp_avg>
            <avg_systolic>" . xmlEscape(($avg_systolic !== null ? round($avg_systolic, 1) : '')) . "</avg_systolic>
            <extension_avg_systolic>" . xmlEscape(base64_encode($siteId . $row['id'] . 'avg_systolic')) . "</extension_avg_systolic>
            <avg_diastolic>" . xmlEscape(($avg_diastolic !== null ? round($avg_diastolic, 1) : '')) . "</avg_diastolic>
            <extension_avg_diastolic>" . xmlEscape(base64_encode($siteId . $row['id'] . 'avg_diastolic')) . "</extension_avg_diastolic>

            <head_circ>" . xmlEscape(((float)$row['head_circ'] ?: '')) . "</head_circ>
            <extension_head_circ>" . xmlEscape(base64_encode($siteId . $row['id'] . 'head_circ')) . "</extension_head_circ>
            <pulse>" . xmlEscape(((float)$row['pulse'] ?: '')) . "</pulse>
            <extension_pulse>" . xmlEscape(base64_encode($siteId . $row['id'] . 'pulse')) . "</extension_pulse>
            <height>" . xmlEscape($height_value ?: '') . "</height>
            <extension_height>" . xmlEscape(base64_encode($siteId . $row['id'] . 'height')) . "</extension_height>
            <unit_height>" . xmlEscape($height_unit ?: '') . "</unit_height>
            <oxygen_saturation>" . xmlEscape(((float)$row['oxygen_saturation'] ?: '')) . "</oxygen_saturation>
            <extension_oxygen_saturation>" . xmlEscape(base64_encode($siteId . $row['id'] . 'oxygen_saturation')) . "</extension_oxygen_saturation>
            <breath>" . xmlEscape(((float)$row['respiration'] ?: '')) . "</breath>
            <extension_breath>" . xmlEscape(base64_encode($siteId . $row['id'] . 'breath')) . "</extension_breath>
            <weight>" . xmlEscape($weight_value ?: '') . "</weight>
            <extension_weight>" . xmlEscape(base64_encode($siteId . $row['id'] . 'weight')) . "</extension_weight>
            <unit_weight>" . xmlEscape($weight_unit ?: '') . "</unit_weight>
            <BMI>" . xmlEscape(((float)$row['BMI'] ?: '')) . "</BMI>
            <extension_BMI>" . xmlEscape(base64_encode($siteId . $row['id'] . 'BMI')) . "</extension_BMI>
            <BMI_status>" . xmlEscape(($row['BMI_status'] ?: '')) . "</BMI_status>
            <extension_oxygen_flow_rate>" . xmlEscape(base64_encode($siteId . $row['id'] . 'oxygen_flow_rate')) . "</extension_oxygen_flow_rate>
            <oxygen_flow_rate>" . xmlEscape(((float)$row['oxygen_flow_rate'] ?: '')) . "</oxygen_flow_rate>
            <extension_ped_weight_height>" . xmlEscape(base64_encode($siteId . $row['id'] . 'ped_weight_height')) . "</extension_ped_weight_height>
            <ped_weight_height>" . xmlEscape(((float)$row['ped_weight_height'] ?: '')) . "</ped_weight_height>
            <extension_ped_bmi>" . xmlEscape(base64_encode($siteId . $row['id'] . 'ped_bmi')) . "</extension_ped_bmi>
            <ped_bmi>" . xmlEscape(((float)$row['ped_bmi'] ?: '')) . "</ped_bmi>
            <extension_ped_head_circ>" . xmlEscape(base64_encode($siteId . $row['id'] . 'ped_head_circ')) . "</extension_ped_head_circ>
            <ped_head_circ>" . xmlEscape(((float)$row['ped_head_circ'] ?: '')) . "</ped_head_circ>
            <extension_inhaled_oxygen_concentration>" . xmlEscape(base64_encode($siteId . $row['id'] . 'inhaled_oxygen_concentration')) . "</extension_inhaled_oxygen_concentration>
    ...
    public function getSocialHistory($pid)
    {
        ...
        foreach ($res as $row) {
            ...
            $social_history .= "<history_element>" . $provenanceXml . "
                                  <extension>" . xmlEscape(base64_encode('smoking' . $siteId . $row['id'])) . "</extension>
            ...
            $social_history .= "<history_element>" . $provenanceXml . "
                                  <extension>" . xmlEscape(base64_encode('alcohol' . $siteId . $row['id'])) . "</extension>
    ...
    public function getPlanOfCare($pid, $encounter)
    {
        ...
        try {
            ...
            if ($sdoh) {
                ...
                $extension = base64_encode($siteId . $encId);

  ...
  */
  ...
  public function allergy($component)
	...
	public function fetch_medication_value($medication_data)
	{
		...
		$this->ccda_data_array['field_name_value_array']['lists3'][$i]['indication']          = $medication_data['substanceAdministration']['entryRelationship'][0]['observation']['value']['displayName'];
	...
	public function fetch_vital_sign_value($vital_sign_data)
	{
		...
		$this->ccda_data_array['field_name_value_array']['vital_sign'][$i]['date']      = $vital_sign_data['organizer']['effectiveTime']['value'];
		$vitals_array = array(
			'8310-5'	=> 'temperature',
                        '8462-4'	=> 'bpd',
			'8480-6'	=> 'bps',
	...
	{  
		...
		$this->ccda_data_array['field_name_value_array']['referral'][$i]['body'] = $referral_data['text']['paragraph'];
  ...
  */
          ...
          for($i=0;$i<count($val);$i++){
            if($val[$i] == 'insert'){ 
              ...
              elseif(substr($key,0,-4) == 'lists2'){
                ...
                $arr_med_pblm['lists1'][$d]['observation_code'] = $data['lists1-observation'][$i];
                $d++;
              }
              ...
              $arr_procedures['procedure'][$y]['root']                                  = $data['procedure-root'][$i];
              $arr_procedures['procedure'][$y]['codeSystemName']                        = $data['procedure-codeSystemName'][$i];
              ...
              $arr_procedures['procedure'][$y]['represented_organization1']             = $data['procedure-represented_organization1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_address1']     = $data['procedure-represented_organization_address1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_city1']        = $data['procedure-represented_organization_city1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_state1']       = $data['procedure-represented_organization_state1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_postalcode1']  = $data['procedure-represented_organization_postalcode1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_country1']     = $data['procedure-represented_organization_country1'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_telecom1']     = $data['procedure-represented_organization_telecom1'][$i];
              
              $arr_procedures['procedure'][$y]['represented_organization2']             = $data['procedure-represented_organization2'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_address2']     = $data['procedure-represented_organization_address2'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_city2']        = $data['procedure-represented_organization_city2'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_state2']       = $data['procedure-represented_organization_state2'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_postalcode2']  = $data['procedure-represented_organization_postalcode2'][$i];
              $arr_procedures['procedure'][$y]['represented_organization_country2']     = $data['procedure-represented_organization_country2'][$i];
              $y++;  
            }
  ...
  public function InsertEncounter($enc_array,$pid,$revapprove=1)
  {
    ...
    foreach($enc_array as $key=>$value) {
      ...
      if($value['encounter_diagnosis_issue'] != '') {
       ...
       $res_sel_iss_enc = $appTable->zQuery($q_sel_iss_enc,array($pid,$list_id,$enc_id));
       if($res_sel_iss_enc->count() == 0) {
        $insert = "INSERT INTO issue_encounter(pid,list_id,encounter,resolved) VALUES (?,?,?,?)";
        $appTable->zQuery($insert, array($pid,$list_id,$enc_id,0));
       }
     }
  ...
  public function InsertProcedures($proc_array,$pid,$revapprove=1)
  {
    ...
    foreach($proc_array as $key=>$value) {
      ...
      if($result_selectB->count() == 0) {
        ...
        $appTable->zQuery($qB_insert, array($value['date'],$value['code'],$value['codeSystemName'],$value['code_text'],$pid,$encounter_for_billing,$facility_id2,$value['extension']));
      }
    }
  }
  ...
  public function InsertPrescriptions($pres_array,$pid,$revapprove=1)
  {
    ...
    foreach($pres_array as $key=>$value) { 
      ...
      //unit
        ...
        $appTable->zQuery($q_insert_units_option,array($unit_option_id,
                                                       $value['rate_unit']));
        } 
    ...
    foreach($allergy_array as $key=>$value) {
      ...
      if($res_q_sel_allergies->count() == 0) {
        ...
        $result = $appTable->zQuery($query, array($pid, 
                                        ...
                                        $value['codeSystemName'].':'.$value['list_code'], 
                                        $severity_option_id,
                                        $active,
    ...
    foreach($med_pblm_array as $key=>$value) {
      ...
      $query_select = "SELECT * FROM list_options WHERE list_id = ? AND title = ?";
        ...
        if($result->count()>0) {
         ...
         $appTable->zQuery($q_update, array('outcome',$value['observation_text'],'SNOMED-CT:'.$value['observation_code']));
         foreach ($result as $value1) {
          ...
          $appTable->zQuery($q_insert, array('outcome',$o_id,$value['observation_text'],'SNOMED-CT:'.$value['observation_code'],1));
        }
  ...
  public function InsertFunctionalCognitiveStatus($functional_cognitive_status_array,$pid,$revapprove=1)
  {
    ...
    foreach($functional_cognitive_status_array as $key=>$value) {
      if($value['date'] != '') $date = $this->formatDate($value['date']);

...
class CcdaServiceDocumentRequestor
{
    ...
    public function socket_get($data)
    {
        ...
        // we're local service
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ccda_alt_service_enable') > 0) {
        } else {
        ...
        if ($server_active === false) {
            ...
            $path = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/ccdaservice";
            if (IS_WINDOWS) {
                // node server is quite with errors(hidden process) so we'll do redirect of tty
                // to generally Windows/Temp.
                $redirect_errors = " > " .
                    $system->escapeshellcmd(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/ccdaserver.log") . " 2>&1";

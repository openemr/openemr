    ...
    while ($glrow = sqlFetchArray($glres)) {
      ...
      // Adjust for user specific settings
        ...
        if ($gl_name == 'language_menu_other') {
            ...
            $GLOBALS[$gl_name] = $GLOBALS['OE_SITE_WEBROOT'].'public/themes/'.attr($gl_value).'?v='.$v_js_includes;
...
// @todo eliminate 1 SQL query
    ...
    if ($rtl_override) {
        ...
        if (file_exists($GLOBALS['OE_SITE_WEBROOT'].'/public/themes/'.$new_theme)) {
            //Escape css file name using 'attr' for security (prevent XSS).
            $GLOBALS['css_header'] = $GLOBALS['OE_SITE_WEBROOT'].'public/themes/'.attr($new_theme).'?v='.$v_js_includes;
        } else {
            // throw a warning if rtl'ed file does not exist.
            error_log("Missing theme file ".text($GLOBALS['OE_SITE_WEBROOT']).'/public/themes/'.text($new_theme));
        }
    }
  ...
  // first release containing this table.
    ...
    $css_header = $GLOBALS['OE_SITE_WEBROOT'] . "/public/themes/style_default.css";

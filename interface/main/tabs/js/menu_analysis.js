...
function set_acl_reqs(entry) {
    if ('url' in entry) {
        if (entry.label in acl_reqs) {
            const reqs = acl_reqs[entry.label];
            if (reqs.length === 3) {
...
function setTarget(entry, target) {
    if ('url' in entry) {
        ...
        for (let idx = 0; idx < entry.children.length; idx += 1) {
...
function post_process(menu_entries) {
    for (let idx = 0; idx < menu_entries.length; idx += 1) {
        const curEntry = menu_entries[idx];
...
function parse_link(link, entry) {
    if (link) {
        let parameters = link.substring(link.indexOf('(') + 1, link.indexOf(')'));
        if (parameters === '') {
            parameters = link;
        }
        if (link.indexOf('loadFrame2') === -1) {
            const url = parameters.replace(/\'/g, '').replace(/\"/g, '').replace('../', '/interface/');
            entry.url = url;
            entry.target = 'report';
        } else {
...
function analyze_menu() {
    ...
    const jqLeft = window.top.left_nav.$(window.top.left_nav.document);
    const $ = window.top.left_nav.$;
    jqLeft.ready(function () {
        const menuTop = jqLeft.find('#navigation-slide');
        menuTop.children().each(function (idx, elem) {
            // Header or content
            const jqElem = $(elem);
            const anchor = jqElem.children('a');
            const subMenu = jqElem.children('ul');
            const newEntry = menu_entry_from_jq(anchor);

            if (subMenu.length > 0) {
                // 2 (Second) level menu items
                subMenu.children('li').each(function (idx, elem) {
                    const sub_anchor = $(elem).children('a');
                    const sub_entry = menu_entry_from_jq(sub_anchor);
        ...
        // Scan popup select
        const popups = jqLeft.find("select[name='popups'] option");
        const popups_menu_header = new menu_entry('Popups', '', 'popup');
        menu_entries.push(popups_menu_header);
        popups.each(function (idx, elem) {
            const jqElem = $(elem);
            if (jqElem.val() !== '') {
                const popup_entry = new menu_entry(jqElem.text(), jqElem.val(), `Popup:${jqElem.text()}`);
...
const toID = setTimeout(analyze_menu, 1000);

...
left_nav.setTherapyGroup = function (group_id, group_name) {
    if ((app_view_model.application_data.therapy_group() !== null)
    ...
    const new_therapy_group = new therapy_group_view_model(group_id, group_name);
...
left_nav.setPatientEncounter = function (EncounterIdAry, EncounterDateAry, CalendarCategoryAry) {
...
left_nav.removeOptionSelected = function (EncounterId) {
    const self = app_view_model.application_data[attendant_type]();
    for (let encIdx = 0; encIdx < self.encounterArray().length; encIdx += 1) {
        const curEnc = self.encounterArray()[encIdx];

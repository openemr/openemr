...
function viewPtFinder(myMessage, searchAnyType, data, event) {
    ...
    const srchBox = document.getElementById('anySearchBox');
    srchBox.focus();

    let finderUrl = `${webroot_url}/interface/main/finder/dynamic_finder.php`;
    const srchBoxVal = srchBox.value.trim();
    const srchBoxWidth = srchBox.offsetWidth;
    // to let input box with values be displayed on mousedown on Smartphones
    const srchBoxLength = srchBoxWidth < 50 ? 0 : srchBoxVal.length;

...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format') == 1) {
...
<script>
    ...
    function validateDate(fromDate,toDate){
        ...
        document.location='<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/main/display_documents.php?' + params;
    }
...
<div class="container mt-3">
    ...
    <div class='col-12' id='docdiv' <?php echo $display_div; ?>>
        ...
        $catIDRs = sqlQuery($query, [\OpenEMR\Core\OEGlobalsBag::getInstance()->get('lab_results_category_name')]);
        $catID = $catIDRs['rght'];
            ...
            <table class="table table-bordered">
                ...
                if (sqlNumRows($resultSet)) {
                    while ($row = sqlFetchArray($resultSet)) {
                        $url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?document&retrieve&patient_id=" . attr_url($row["foreign_id"]) . "&document_id=" . attr_url($row["id"]) . '&as_file=false';

...
if ($GLOBALS['login_into_facility']) {
    ...
    if ($GLOBALS['set_facility_cookie']) {
        ...
        setcookie("pc_facility", (string) $session->get('facilityId'), ['expires' => time() + (3600 * 365), 'path' => $GLOBALS['webroot']]);
    }
}
...
if ($is_expired) {
    ...
    array_unshift($_tabs, [
        'notes' => "pwd_expires_alert.php?csrf_token_form=" . attr_url(CsrfUtils::collectCsrfToken('default', $session)),
        'id' => "adm",
...
$session->set('token_main_php', RandomGenUtils::createUniqueToken());
header('Location: ' . $web_root . "/interface/main/tabs/main.php?token_main=" . urlencode((string) $session->get('token_main_php')));
exit();
?>

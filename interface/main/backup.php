...
$TMP_BASE = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/openemr_web_backup";
$BACKUP_DIR = $TMP_BASE . "/emr_backup";
$TAR_FILE_PATH = $TMP_BASE . DIRECTORY_SEPARATOR . $backup_file_prefix . $backup_file_suffix;
$EXPORT_FILE = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/openemr_config.sql";
$MYSQL_PATH = realpath(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('mysql_bin_dir'));
$PERL_PATH = realpath(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('perl_bin_dir'));
...
if ($form_step == 402) {
    if (!empty($_POST['form_end_date'])) {
        ...
        $outfile = tempnam(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir'), 'OET');
        if ($outfile === false) {
            die("tempnam('" . text(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir')) . "','OET') failed.\n");
        }
        ...
        if (true) {
            $zip = new ZipArchive();
            $zippedoutfile = tempnam(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir'), 'OEZ');
            if ($zippedoutfile === false) {
                die("tempnam('" . text(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir')) . "','OEZ') failed.\n");
            }
...
$mysql_ssl = '';
if ($dbOptions->sslCaPath !== null) {
    $mysql_ssl = " --ssl-ca=" . escapeshellarg($dbOptions->sslCaPath) . " ";
    if ($dbOptions->sslClientCert !== null) {
        $mysql_ssl .= "--ssl-cert=" . escapeshellarg($dbOptions->sslClientCert['cert']) . " ";
        $mysql_ssl .= "--ssl-key=" . escapeshellarg($dbOptions->sslClientCert['key']) . " ";
    }
}
...
if ($form_step == 0) {
  ...
  // The config import/export feature is optional.
    if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('configuration_import_export'))) {
        echo " <tr>\n";
...
if ($form_step == 5) {   // create the final compressed tar containing all files
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('audit_events_backup')) {
...
if ($form_step == 102) {
    ...
    if ($tables || is_array($_POST['form_sel_lists'] ?? '') || is_array($_POST['form_sel_layouts'] ?? '')) {
        ...
        $dumppfx = escapeshellcmd($mysql_dump_cmd) . " -u " . escapeshellarg($dbOptions->user) .
                 " -p" . escapeshellarg($dbOptions->password) .
                 " -h " . escapeshellarg((string) $dbOptions->host) .
                 " --port=" . escapeshellarg((string) $dbOptions->port) .
  ...
  // Process uploaded config file.
    if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
        if (move_uploaded_file($_FILES['userfile']['tmp_name'], $EXPORT_FILE)) {
            ...
            echo brCustomPlaceholder(text($form_status));
            $cmd = escapeshellcmd($mysql_cmd) . " -u " . escapeshellarg($dbOptions->user) .
            " -p" . escapeshellarg($dbOptions->password) .
            " -h " . escapeshellarg((string) $dbOptions->host) .
            " --port=" . escapeshellarg((string) $dbOptions->port) .
            " $mysql_ssl " .
...
# Eventlog backup directory
    $BACKUP_EVENTLOG_DIR = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('backup_log_dir');

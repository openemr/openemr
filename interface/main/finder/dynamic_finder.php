...
<script>
    ...
    $(function () {
        ...
        var oTable = $('#pt_table').dataTable({
            ...
            "pageLength": <?php echo empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_pt_list_page_size')) ? '10' : \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_pt_list_page_size'); ?>,
        ...
        <?php
        $checked = (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_pt_list_new_window'))) ? 'checked' : '';
        ?>
...
<?php
    ...
    $eventDispatcher->addListener(PageHeadingRenderEvent::EVENT_PAGE_HEADING_RENDER, function ($event): void {
        ...
        $event->setPrimaryMenuItem(new BaseMenuItem([
            ...
            'id' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . '/interface/new/new.php',
...
function rp()
{
    ...
    foreach ($headers as $v) {
        ...
        if (in_array($col_name, $date_cols) || in_array($col_name, $datetime_cols)) {
            switch (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format')) {
            ...
            if (in_array($col_name, $datetime_cols)) {
                switch (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format')) {
                    case 0: // 24 Hr fmt
                        $dt_format .= " %T";
                        break;
                    case 1: // AM PM fmt

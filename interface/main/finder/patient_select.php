...
if ($popup) {
  ...
  // Collect patient listing from cdr report
    if ($print_patients) {
        ...
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set('PATIENT_INC_COUNT', count($result));
        $MAXSHOW = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('PATIENT_INC_COUNT');
    } else {
        // collect the total listing count
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set('PATIENT_INC_COUNT', collectItemizedPatientsCdrReport($report_id, $itemized_test_id, $pass_id, $numerator_label, true));
...
<?php if (empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_search_results_style'))) { ?>
...
if ($result) {
    foreach ($result as $iter) {
        ...
        if (empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_search_results_style'))) {

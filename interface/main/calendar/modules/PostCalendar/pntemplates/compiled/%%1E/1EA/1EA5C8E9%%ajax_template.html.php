...
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/day/ajax_template.html', 10, false),array('function', 'pc_sort_events', 'default/views/day/ajax_template.html', 381, false),array('modifier', 'date_format', 'default/views/day/ajax_template.html', 376, false),array('modifier', 'string_format', 'default/views/day/ajax_template.html', 377, false),)), $this); ?>
...
<script language='JavaScript'>
 ...
 function oldGroupEvt(eventdate, eventid, pccattype){
  top.restoreSession();
  dlgopen('add_edit_event.php?group=true&date='+eventdate+'&eid=' + eventid+'&prov=' + pccattype, '_blank', 775, 500);
 }
...
<?php 
        ...
        "+ '?method=groupDetails&group_id=' + gid \n ";
 ?>
 }
...
<?php 
echo "   <a href='#' title='" . xl("New Appointment") . "' onclick='newEvt(1, 9, 00, $Date, 0, 0)' class='css_button'>
            <i class='fa fa-plus' aria-hidden='true'></i></a>\n";
echo "   <a href='#' title='" . xl("Search Appointment") . "'
...
echo "   <a id='prevday' href='$PREV_DAY_URL' onclick='top.restoreSession()' title='" . xl("Previous Day") . "'>
            <i class='fa fa-arrow-left' aria-hidden='true'></i></a>\n";
$atmp = array_keys($A_EVENTS);
echo dateformat (strtotime($atmp[0]),true);
echo "   <a id='nextday' href='$NEXT_DAY_URL' onclick='top.restoreSession()' title='" . xl("Next Day") . "'>
...
<div id="viewPicker">
<?php 
echo "   <a href='#' id='printview' title='" . xl("View Printable Version") . "' class='css_button'>
            <i class='fa fa-print' aria-hidden='true'></i></a>\n";
echo "   <a href='#' title='" . xl("Refresh") . "' onclick='javascript:refreshme()' class='css_button'>
...
foreach ($providers as $provider) {
    ...
    echo "<td class='schedule " . $classForWeekend . "' title='".htmlspecialchars($provider["fname"],ENT_QUOTES)." ".htmlspecialchars($provider["lname"],ENT_QUOTES)."' date='".date("Ymd",$caldate )."' provider='$providerid'>";
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        foreach ($events as $event) {
            ...
            if($groupid)
                $divTitle = xlt('Counselors') . ": \n" . htmlspecialchars($groupcounselors,ENT_QUOTES). " \n";
            else
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                if($groupid)
                    $divTitle .= "\r\n[".$catname.' '.htmlspecialchars($comment, ENT_QUOTES) ."]".htmlspecialchars($groupname, ENT_QUOTES);
                else
       ...
       if($_SESSION['pc_facility'] == 0){
          ...
          // For "OUT" events, applying the background color in CSS.
            ...
            if ($event['catid']!="6" && $event['catid']!="4" ){
             ...
             $background_string= ";background-color:".$color;

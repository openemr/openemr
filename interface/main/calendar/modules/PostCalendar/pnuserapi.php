...
require_once(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/library/group.inc.php");
require_once(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/library/encounter_events.inc.php");
...
function postcalendar_userapi_buildView($args)
{
    ...
    extract($args);
    unset($args);
    $schedule_start = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('schedule_start');
    $schedule_end = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('schedule_end');
    ...
    for ($blocknum = $schedule_start; $blocknum <= $schedule_end; $blocknum++) {
        ...
        for ($minutes = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('calendar_interval'); $minutes <= 60; $minutes += \OpenEMR\Core\OEGlobalsBag::getInstance()->get('calendar_interval')) {
            if ($minutes <= '9') {
                $under_ten = "0" . $minutes;
    ...
    if (true) {
        ...
        $pc_short_day_names = [_CALSUNDAYSHORT, _CALMONDAYSHORT,
        ...
        $tpl->assign('interval', \OpenEMR\Core\OEGlobalsBag::getInstance()->get('calendar_interval'));
...
//RM add address
  ...
  // put the information into an array for easy access
    ...
    foreach ($result->iterateNumeric() as $row) {
        ...
        $events[$i]['intervals']   = ceil(($tmp['duration'] / 60) / \OpenEMR\Core\OEGlobalsBag::getInstance()->get('calendar_interval'));

...
function postcalendar_userapi_buildView($args)
{
    ...
    for ($blocknum = $schedule_start; $blocknum <= $schedule_end; $blocknum++) {
        ...
        foreach ($minute as $m) {
            array_push($times, ["hour" => $blocknum, "minute" => $m, "mer" => $mer]);
        }
    }
    ...
    if (true) {
        ...
        //=================================================================
        $pc_month_names = [_CALJAN,_CALFEB,_CALMAR,_CALAPR,_CALMAY,_CALJUN,
            _CALJUL,_CALAUG,_CALSEP,_CALOCT,_CALNOV,_CALDEC];

        $pc_short_day_names = ['0' => xl('Sun'), '1' => xl('Mon'), '2' => xl('Tue'), '3' => xl('Wed'), '4' => xl('Thu'), '5' => xl('Fri'), '6' => xl('Sat')];

        $pc_long_day_names = [_CALSUNDAY, _CALMONDAY,
        ...
        //=================================================================
        if ($viewtype != 'year') {
            $eventsByDate =& postcalendar_userapi_pcGetEvents(['start' => $starting_date,'end' => $ending_date, 'viewtype' => $viewtype, 'provider_id' => $provIDs]);
        } else {
            $eventsByDate = [];
        }
        ...
        $pc_prev_day = pnModURL(
            ...
            ['tplview' => $template_view,
            'viewtype' => 'day',
            'Date' => date("Ymd", strtotime($prev_day)),
            'pc_username' => $pc_username,
            'pc_category' => $category,
            'pc_topic' => $topic]
        );
        ...
        $pc_next_day = pnModURL(
            ...
            ['tplview' => $template_view,
            'viewtype' => 'day',
            'Date' => date("Ymd", strtotime($next_day)),
            'pc_username' => $pc_username,
            'pc_category' => $category,
            'pc_topic' => $topic]
        );
        ...
        $tpl->assign('DATE_TIMESTAMP', strtotime((string) $Date));
        ...
        $tpl->assign("openhour", OEGlobalsBag::getInstance()->getInt('schedule_start'));
        $tpl->assign("closehour", OEGlobalsBag::getInstance()->getInt('schedule_end'));

        if (OEGlobalsBag::getInstance()->get('time_display_format') == 1) {
        ...
        pnModURL(__POSTCALENDAR__,'user','view', ['tplview'=>($template_view ?? ''),
                'viewtype'=>$viewtype,
                'Date'=> '~REPLACEME~',
                'pc_username'=>($pc_username ?? ''),
                'pc_category'=>($category ?? ''),
    ...
    //  Parse the template
    //=================================================================
    // Check for Twig template first, then fall back to Smarty
    $twig_template = "calendar/$template_name/views/$viewtype/$template_view_load.html.twig";
    $smarty_template = "$template_name/views/$viewtype/$template_view_load.html";

    // Try Twig template first
    $template = $tpl->twig && $tpl->twig->getLoader()->exists($twig_template) ? $twig_template : $smarty_template;
...
function postcalendar_userapi_selector_provider_vars($tpl, $providers): void
{
    $session = SessionWrapperFactory::getInstance()->getActiveSession();
    ...
    if (!empty($session->get('authorizeduser')) && ($session->get('authorizeduser') == 1)) {
    ...
    $pcFacility = $session->get('pc_facility');
    if (count($facilities) > 1) {
        if (!OEGlobalsBag::getInstance()->getBoolean('restrict_user_facility')) {
            $facilitySelectorList[] = ['value' => 0, 'color' => ''
                , 'selected' => !$pcFacility, 'text' => xlt('All Facilities')];
        }
        foreach ($facilities as $fa) {
            $facilitySelectorList[] = ['value' => $fa['id'], 'color' => $fa['color']
                , 'selected' => $pcFacility == $fa['id'], 'text' => $fa['name']];
        }
    } else {
        $facilitySelectorList[] = ['value' => $facilities[key($facilities)]['id'], 'color' => $facilities[key($facilities)]['color']
            , 'selected' => true, 'text' => $facilities[key($facilities)]['name']];
    }

    //FACILITY FILTERING (CHEMED)
    $provinfo = $pcFacility ? getProviderInfo('%', true, $pcFacility) : getProviderInfo();
    ...
    // Check if "All Users" should be selected
    ...
    if (empty($pc_username) ||
        ...
        empty($providerUsernames)) {
        $allUsersSelected = true;
    }

    $providerSelectorList[] = ['value' => '__PC_ALL__', 'text' => xlt("All Users"), 'selected' => $allUsersSelected];
    foreach ($provinfo as $doc) {
        $username = $doc['username'];
        $providerSelectorList[] = [
            'value' => $username
            , 'selected' => !$allUsersSelected && isset($providerUsernames[$username])
    ...
    $tpl->assign('pc_facility', $pcFacility);
    $tpl->assign('facilities', $facilities);
}

function postcalendar_userapi_monthly_calendar_selector_vars($tpl, $eventsByDate, $Date, $provinfo, $times): void
{
    $DOWlist = [];
    ...
    $caldate = strtotime((string) $atmp[0]);
    ...
    foreach ($eventsByDate as $date => $events) {
        if ($defaultDate == "") $defaultDate = date("Ymd", strtotime((string) $date));
        $dowListNames[] = date("D", strtotime((string) $date));
        if ($dowCount++ == 6) { break; }
    }
    ...
    foreach ($provinfo as $provider) {
        ...
        foreach ($eventsByDate as $date => $events) {
            ...
            $dateAsTime = strtotime((string) $date);
            ...
            $gotoURL = pnModURL(__POSTCALENDAR__, 'user', 'view',
                ['tplview' => ($template_view ?? null),
                    'viewtype' => 'day',
                    'Date' => date("Ymd", strtotime((string) $date)),
                    'pc_username' => ($pc_username ?? null),
                    'pc_category' => ($category ?? null),
                    'pc_topic' => ($topic ?? null)]);
            $classForWeekend = is_weekend_day(date('w', strtotime((string) $date))) ? 'weekend-day' : 'work-day';
            $day = [
                'title' => xl("Go to ") . date('d M Y', strtotime((string) $date))
                , 'text' => date('d', strtotime((string) $date))
                , 'url' => $gotoURL
            ...
            foreach ($events as $event) {
                ...
                if ($event['alldayevent'] == 1) {
                    $tmpTime = $times[0];
                    if (strlen((string) $tmpTime['hour']) < 2) { $tmpTime['hour'] = "0".$tmpTime['hour']; }
                    if (strlen((string) $tmpTime['minute']) < 2) { $tmpTime['minute'] = "0".$tmpTime['minute']; }
...
function postcalendar_userapi_day_view_setup_vars($tpl, $eventsByDate): void
{

    $atmp = array_keys($eventsByDate);
    $calDate = strtotime((string) $atmp[0]);
    $tpl->assign("TODAY_DATE_NAV_HEADER", dateformat($calDate, true));
    $tpl->assign("firstEventDFY", date("d F Y", strtotime((string) $atmp[0])));
    $tpl->assign("firstEventDayOfWeek", date("l", strtotime((string) $atmp[0])));
    ...
    $tpl->assign('prevMonthFormatted', date("F", strtotime((string) $prevMonth)));

    $tpl->assign('nextMonth', $nextMonth);
    $tpl->assign('nextMonthFormatted', date("F", strtotime((string) $nextMonth)));
}
...
function postcalendar_userapi_day_view_calculate_previous_month($cDay, $cMonth, $cYear)
{
    ...
    while (! checkdate($pMonth, $pDay, $pYear)) { $pDay -= 1; }
...
function postcalendar_userapi_day_view_calculate_next_month($cDay, $cMonth, $cYear)
{
    ...
    while (! checkdate($nMonth, $nDay, $nYear)) { $nDay -= 1; }
    return sprintf("%d%02d%02d",$nYear,$nMonth,$nDay);
}

function postcalendar_userapi_month_view_setup_vars($tpl, $eventsByDate, $Date, $provinfo, $times): void
{
    // This function sets up template variables for the month view
    // It combines functionality from both day_view_setup_vars and monthly_calendar_selector_vars

    // First, set up basic date information
    $atmp = array_keys($eventsByDate);
    if (empty($atmp)) {
        // If no events, use the provided Date parameter
        $calDate = strtotime((string) $Date);
    } else {
        $calDate = strtotime((string) $atmp[0]);
    }

    // Assign basic date variables
    $tpl->assign("TODAY_DATE_NAV_HEADER", dateformat($calDate, true));
    $tpl->assign('caldate', $calDate);
    $tpl->assign('caldateFormatted', date('F Y', $calDate));
    $tpl->assign('Month', date('Y-m-d', $calDate)); // Add Month variable for header display
    $cMonth = date("m", $calDate);
    $cYear = date("Y", $calDate);
    $cDay = date("d", $calDate);
    $tpl->assign('cMonth', $cMonth);
    ...
    $prevMonth = date('Ym01', strtotime('-1 month', strtotime((string) $Date)));
    $nextMonth = date('Ym01', strtotime('+1 month', strtotime((string) $Date)));
    $tpl->assign('prevMonth', $prevMonth);
    $tpl->assign('prevMonthFormatted', date("F Y", strtotime($prevMonth . '01')));
    $tpl->assign('nextMonth', $nextMonth);
    $tpl->assign('nextMonthFormatted', date("F Y", strtotime($nextMonth . '01')));

    // Set default selected provider to the logged-in user if no provider is selected
    $session = SessionWrapperFactory::getInstance()->getActiveSession();
    $authUserID = $session->get('authUserID');
    if ($authUserID !== null && !isset($_REQUEST['pc_username'])) {
        // Find the provider matching the logged-in user
        foreach ($provinfo as $provider) {
            if ($provider['id'] == $authUserID) {
                $tpl->assign('selectedProvider', $provider);
                $tpl->assign('displayProviders', true);
                break;
            }
        }
    }

    // Now incorporate the necessary parts from monthly_calendar_selector_vars
    $DOWlist = [];
    $tmpDOW = pnModGetVar(__POSTCALENDAR__, 'pcFirstDayOfWeek');
    // bound check and auto-correction
    if ($tmpDOW < 0 || $tmpDOW > 6) {
    ...
    // Get day of week names for header
    $dowCount = 0;
    $defaultDate = "";
    $dowListNames = [];
    foreach ($eventsByDate as $date => $events) {
    ...
    // Organize calendars by provider
    $calendarsByProvider = [];
    foreach ($provinfo as $provider) {
        $providerCalendar = [
            'provider' => [
        ...
        foreach ($eventsByDate as $date => $events) {
            ...
            foreach ($events as $event) {
                ...
                $starth = (int) substr((string) $event['startTime'], 0, 2);
                $startm = (int) substr((string) $event['startTime'], 3, 2);
                $startDateTime = strtotime((string) $date." ".$event['startTime']);
...
function &postcalendar_userapi_pcQueryEvents($args)
{
  ...
  // put the information into an array for easy access
    ...
    foreach ($result->iterateNumeric() as $row) {
        ...
        $events[$i]['owner_name']  = $tmp['owner_name'];
        $events[$i]['patient_dob'] = $tmp['patient_dob'];
        $events[$i]['patient_age'] = (int) date("Y") - (int) substr(((string) $tmp['patient_dob']), 0, 4);
        $events[$i]['facility']    = getFacility($tmp['facility']);
        $events[$i]['sharing']     = $tmp['sharing'];
        $events[$i]['prefcatid']   = $tmp['prefcatid'];
        $events[$i]['aid']         = $tmp['aid'];
...
function &postcalendar_userapi_pcGetEvents($args)
{
    ...
    if (!empty($faFlag) && !isset($events)) {
        $a = ['faFlag' => true,'start' => $start_date,'end' => $end_date,'s_keywords' => $s_keywords,'s_category' => $s_category,'s_topic' => $s_topic,'viewtype' => $viewtype, 'provider_id' => $provider_id, 'event_status' => $event_status];
        $events = pnModAPIFunc(__POSTCALENDAR__, 'user', '<strong></strong>pcQueryEventsFA', $a);
    } elseif (!empty($collideFlag) && !isset($events)) {
        $a = ['collideFlag' => true,'start' => $start_date,'end' => $end_date, 'provider_id' => $provider_id, 'collide_stime' => $stime, 'collide_etime' => $etime];
    ...
    $event->setKeywords($s_keywords ?? null);
    $event->setCategory($s_category ?? null);
    $event->setStartDate($start_date ?? null);
    $event->setEndDate($end_date ?? null);

...
function postcalendar_userapi_buildView($args)
{
    ...
    if (true) {
        ...
        $pc_short_day_names = array('0' => xl('Sun'), '1' => xl('Mon'), '2' => xl('Tue'), '3' => xl('Wed'), '4' => xl('Thu'), '5' => xl('Fri'), '6' => xl('Sat'));
        ...
        $tpl->assign('JAVASCRIPT_BASE_URL', pnModURL(
            __POSTCALENDAR__,
            'user',
            'view',
            array('tplview' => ($template_view ?? ''),
        ...
        $tpl->assign('JAVASCRIPT_PRINT_URL', pnModURL(
            __POSTCALENDAR__,
            'user',
            'view',
            array('tplview' => ($template_view ?? ''),
...
function postcalendar_userapi_monthly_calendar_selector_vars($tpl, $eventsByDate, $Date, $provinfo, $times)
{
    ...
    foreach ($eventsByDate as $date => $events) {
        if ($defaultDate == "") {
            $defaultDate = date("Ymd", strtotime($date));
        }
...
function postcalendar_userapi_day_view_calculate_next_month($cDay, $cMonth, $cYear)
{
    ...
    while (! checkdate($nMonth, $nDay, $nYear)) {
        $nDay = $nDay - 1;
    }

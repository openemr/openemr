...
$default_catid = !empty($_GET['catid']) ? $_GET['catid'] : (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('default_visit_category') ?? '') ? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('default_visit_category') : '5');
...
if (isset($_GET['starttimeh'])) {
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format')  == 1 && isset($_GET['startampm'])) {
...
if ((!$g_edit && !$g_view) || (!\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_group_therapy'))) {
    $_GET['group'] = false;
...
function DOBandEncounter($pc_eid): void
{
    ...
    if (!empty($_POST['form_pid'])) {
        ...
        if (
            (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('auto_create_new_encounters')
            ...
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->has('temporary-eid-for-manage-tracker') || !empty($_GET['eid'])) {
                ...
                $temp_eid = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary-eid-for-manage-tracker') ?? $_GET['eid'];
    ...
    if (!empty($_POST['form_gid'])) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('auto_create_new_encounters') && $event_date == date('Y-m-d') && $_POST['form_apptstatus'] == '=') {
...
if ($eid) {
    ...
    if (is_array($facility) && !$facility['pc_facility']) {
        ...
        } elseif (!$facility['pc_facility'] && $_COOKIE['pc_facility'] && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('set_facility_cookie')) {
...
if (!empty($_POST['form_action']) && ($_POST['form_action'] == "save")) {
    ...
    if ($eid) {
        ...
        // multiple providers
        // ====================================
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('select_multi_providers') && $row['pc_multiple']) {
...
if ($groupid) {
 ...
 // Fix up the time format for AM/PM.
    ...
    if ($starttimeh >= 12) { // p.m. starts at noon and not 12:01
        $startampm = '2';
        if ($starttimeh > 12 && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format') == 1) {
            $starttimeh -= 12;
        }
    }
...
function set_allday() {
    ...
    <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format') == 1) { ?>
...
function setappt(year,mon,mday,hours,minutes) {
    ...
    var f = document.forms[0];
    <?php
    $currentDateFormat = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format');
...
function find_available(extra) {
    ...
    dlgopen('<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root'); ?>/interface/main/calendar/find_appt_popup.php' +
        '?providerid=' + s +
...
<div class="jumbotron jumbotron-fluid px-3 py-4 my-2">
    ...
    <div class="form-row mb-sm-2">
        ...
        <select class='col-sm form-control form-control-sm' name='form_repeat_type'>
            ...
            foreach ([0 => xl('day'), 4 => xl('workday'), 1 => xl('week'), 2 => xl('month'), 3 => xl('year'), 5 => '?', 6 => '?'] as $key => $value) {
                echo "<option value='" . attr($key) . "'";
        ...
        if (!empty($repeatexdate)) {
            ...
            $max = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('number_of_ex_appts_to_show');
...
<div class="form-row mx-2">
    <div class="col-sm form-group">
        ...
        <?php
        if ($_GET['group'] != true) {
            generate_form_field(['data_type' => 1, 'field_id' => 'apptstatus', 'list_id' => 'apptstat', 'empty_title' => 'SKIP'], ($row['pc_apptstatus'] ?? null));
        } else {
            generate_form_field(['data_type' => 1, 'field_id' => 'apptstatus', 'list_id' => 'groupstat', 'empty_title' => 'SKIP'], ($row['pc_apptstatus'] ?? null));
        }
        ?>
...
<div class="form-row mx-2">
    <div id="recurr_popup" class="col-sm input-group alert bg-warning text-left" style="display: none; position: relative; max-width: 400px;">
        ...
        <br />
        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('submit_changes_for_all_appts_at_once')) {?>
...
<div class="form-row mx-2 mt-3">
    ...
    <?php if (!(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('select_multi_providers'))) { //multi providers appt is not supported by check slot avail window, so skip ?>
...
function validateform(event,value){
    ...
    <?php if (!\OpenEMR\Core\OEGlobalsBag::getInstance()->get('allow_early_check_in')) { ?>
...
function SubmitForm() {
    var f = document.forms[0];
    <?php if (!(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('select_multi_providers')) && empty($_GET['prov'])) { // multi providers appt is not supported by check slot avail window, so skip. && is not provider tab. ?>

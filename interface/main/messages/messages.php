...
?><html>
  <head>
    ...
    <link rel="stylesheet" href="<?php echo $webroot; ?>/interface/main/messages/css/reminder_style.css?v=<?php echo rand(0); ?>" type="text/css">
  ...
  <body class="body_top" style="font-size:0.8em;">
    <div class="row">
        <?php
                    ...
                    foreach($assigned_to_list as $assigned_to){
                        if ($noteid && $assigned_to != '-patient-') {
                            ...
                            if($noteid && $assigned_to == '-patient-'){
                                ...
                                if (! $row) die("getPnoteById() did not find id '".text($noteid)."'");//?translate
                    ...
                    <table border='0' cellspacing='8'>
                      <tr>
                        <td class='text'>
                          ...
                          <a class="patLink" onclick="goPid('<?php echo attr($result['pid']);?>')"><?php echo xlt('Patient'); ?>:</a>
                            <?php } else { ?>
                          <b class='<?php echo ($task=="addnew"?"required":"") ?>'><?php echo xlt('Patient'); ?>:</b>
                            <?php
}
if ($reply_to) {
    ...
    $patientname = xl('Click to select');
                      ...
                      <tr>
                        <td class='text'>
                         ...
                         value='<?php echo xla("Select Users From The Dropdown List"); ?>' >
                         <input type='hidden' name='assigned_to' id='assigned_to' >
                         <select name='users' id='users' onchange='addtolist(this);' >
                            <?php
                            echo "<option value='" . htmlspecialchars( '--', ENT_QUOTES) . "'";//really?
                      ...
                      <tr>
                        <td>
                            ...
                            ?>
                            <textarea name='note' id='note' rows='8' style="width: 660px; "><?php echo nl2br(text( $note )); ?></textarea>
                        </td>
                      </tr>
                    </table>
                    ...
                    <input type="button" id="newnote" value="<?php echo xla('Send message'); ?>">
                    <input type="button" id="printnote" value="<?php echo xla('Print message'); ?>">
            ...
            if(sel.value != '--'){
              ...
              // This is for sorting the records.
                ...
                for($i = 0; $i < count($sort); $i++) {
                    if($sortby == $sort[$i]) {
                        switch($sortorder) {
                            case "asc"      : $sortlink[$i] = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=desc&$activity_string_html\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortup.gif\" border=0 alt=\"".xla('Sort Up')."\"></a>";
      break;
                            case "desc"     : $sortlink[$i] = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=asc&$activity_string_html\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".xla('Sort Down')."\"></a>";
      break;
                        } break;
                    }
                }
      ...
      <td width=\"15%\" style=\"border-bottom: 1px #000000 solid; \" class=bold>&nbsp;<b>" .
        ...
        <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\">" .
                    text($myrow['title']) . "</td><td width=5></td></tr></table></td>
        <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\">" .
        ...
        <td style=\"border-bottom: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\">" .
                    text( $myrow['message_status'] ) . "</td><td width=5></td></tr></table></td>
        </tr>";
                }
      ...
      <td align=right class=\"text amount-msg\">$prevlink &nbsp; $end of $total &nbsp; $nextlink</td>
              ...
              <script language="javascript">
                ...
                function confirmDeleteSelected() {
                  if(confirm("<?php echo xlt('Do you really want to delete the selection?'); ?>")) {

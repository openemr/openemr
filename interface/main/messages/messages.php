  ...
  function addtolist(sel){
    ...
    for ($i = 0; $i < count($sort); $i++) {
        if ($sortby == $sort[$i]) {
            switch ($sortorder) {
                case "asc":
                    $sortlink[$i] = "<a class='arrowhead' href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=desc&$activity_string_html\" onclick=\"top.restoreSession()\"><i class='fa fa-sort-asc fa-lg' aria-hidden='true'></i>";
                    break;
                case "desc":
                    $sortlink[$i] = "<a class='arrowhead' href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=asc&$activity_string_html\" onclick=\"top.restoreSession()\"><i class='fa fa-sort-desc fa-lg' aria-hidden='true'></i>";
                    break;
            } break;
        }
    }

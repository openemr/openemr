...
if (!CsrfUtils::verifyCsrfToken($csrf)) {
    ...
    try {
        ...
        $isValid = false;
        (\OpenEMR\BC\ServiceContainer::getLogger())->error("Access was denied", ['trace' => $error->getTraceAsString(), 'message' => $error->getMessage()]);
    } catch (\Throwable $error) {
        $result['errorCode'] = 'invalidRequest';
        (\OpenEMR\BC\ServiceContainer::getLogger())->error("Data was invalid", ['trace' => $error->getTraceAsString(), 'message' => $error->getMessage()]);
        $isValid = false;
    }
}
...
try {
    (\OpenEMR\BC\ServiceContainer::getLogger())->debug("trusted-messages-ajax.php result object", $result);
    echo json_encode($result, JSON_THROW_ON_ERROR);
} catch (\Throwable $error) {
    (\OpenEMR\BC\ServiceContainer::getLogger())->error("Failed to encode json response", ['trace' => $error->getTraceAsString(), 'message' => $error->getMessage(), 'result' => $result]);

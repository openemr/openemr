...
<script>
    ...
    function topatient(newpid, enc) {
        if ($('#setting_new_window').val() === 'checked') {
            ...
            top.restoreSession();
            if (enc > 0) {
                window.location = "<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid) + "&set_encounterid=" + encodeURIComponent(enc);
            } else {
                window.location = "<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid);
            }
        }
    }
</script>
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center">
            <img src="<?php echo $GLOBALS['images_static_relative']; ?>/flowboard.png" id="head_img" alt="OpenEMR <?php echo xla('Calendar'); ?>">
        </div>
                                    ...
                                    <?php
                                        if (strtotime($newend) != '') {
                                            ...
                                            <input type=checkbox disabled='disable' class="drug_screen_completed"
                                                   id="<?php echo attr($appointment['pt_tracker_id']) ?>" <?php if ($appointment['drug_screen_completed'] == "1") {
                                                echo "checked";
                                            } ?>>
                                            <?php
                                        } else {

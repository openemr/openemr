...
if (!\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_date_range')) {
    $from_date = date('Y-m-d');
} elseif (!is_null($_REQUEST['form_from_date'] ?? null)) {
    $from_date = DateToYYYYMMDD($_REQUEST['form_from_date']);
} elseif ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_start_date')) == 'D0') {
    $from_date = date('Y-m-d');
} elseif ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_start_date')) == 'B0') {
    if (date('w') == \OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_day_week')) {
        //today is the first day of the week
        $from_date = date('Y-m-d');
    } elseif (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_day_week') == 0) {
        //Sunday
        $from_date = date('Y-m-d', strtotime('previous sunday'));
    } elseif (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_day_week') == 1) {
        //Monday
        $from_date = date('Y-m-d', strtotime('previous monday'));
    } elseif (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_day_week') == 6) {
        //Saturday
...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_date_range')) {
    if (str_starts_with((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 'Y')) {
        $ptkr_time = substr((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 1, 1);
        $ptkr_future_time = mktime(0, 0, 0, date('m'), date('d'), date('Y') + $ptkr_time);
    } elseif (str_starts_with((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 'M')) {
        $ptkr_time = substr((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 1, 1);
        $ptkr_future_time = mktime(0, 0, 0, date('m') + $ptkr_time, date('d'), date('Y'));
    } elseif (str_starts_with((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 'D')) {
        $ptkr_time = substr((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_end_date'), 1, 1);
...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('medex_enable') == '1') {
...
<head>
    ...
    <?php if ($_SESSION['language_direction'] == "rtl") { ?>
      <link rel="stylesheet" href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('themes_static_relative'); ?>/misc/rtl_bootstrap_navbar.css?v=<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('v_js_includes'); ?>" />
    <?php } else { ?>
      <link rel="stylesheet" href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('themes_static_relative'); ?>/misc/bootstrap_navbar.css?v=<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('v_js_includes'); ?>" />
    <?php } ?>

    <script src="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root'); ?>/interface/main/messages/js/reminder_appts.js?v=<?php echo $v_js_includes; ?>"></script>
</head>

<body>
    <?php
    if ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('medex_enable') == '1') && (empty($_REQUEST['nomenu']))) {
                        ...
                        <div class="row">
                          <input type="hidden" name="csrf_token_form" value="<?php echo attr(CsrfUtils::collectCsrfToken()); ?>" />
                            <div class="text-center col-4 align-items-center">
                              ...
                              <?php
                                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_date_range') == '1') {
                                    $type = 'date';
                                    $style = '';
                                } else {
                                    $type = 'hidden';
                            ...
                            <div class="col-4 mt-3 row">
                                ...
                                <div class="col-sm-12 mx-auto">
                                    <div class="text-center pt-3 mx-auto">
                                        ...
                                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('medex_enable') == '1') { ?>
                                          <b>MedEx:</b>
    ...
    ?>
                ...
                <div id="pull_kiosk_right" class="text-right">
                    ...
                    <tr class="small font-weight-bold text-center">
                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_show_pid')) { ?>
                        ...
                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_visit_reason') == '1') { ?>
                            <td class="dehead text-center text-ovr-dark" name="kiosk_hide">
                                <?php echo xlt('Appt Comment'); ?>
                            </td>
                        <?php } ?>
                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_show_encounter')) { ?>
                            <td class="dehead text-center text-ovr-dark" name="kiosk_hide">
                                <?php echo xlt('Encounter'); ?>
                            </td>
                        <?php } ?>

                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_date_range') == '1') { ?>
                            <td class="dehead text-center text-ovr-dark" name="kiosk_hide">
                        ...
                        <?php
                        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_show_staff')) { ?>
                            <td class="dehead text-center text-ovr-dark" name="kiosk_hide">
                    ...
                    foreach ($appointments as $appointment) {
                        ...
                        // reason for visit
                        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_visit_reason')) {
                        ...
                        if (is_checkout($status) && ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('checkout_roll_off') > 0) && strlen($form_apptstatus) != 1)) {
                            $to_time = strtotime((string) $newend);
                            $from_time = strtotime($datetime);
                            $display_check_out = round(abs($from_time - $to_time) / 60, 0);
                            if ($display_check_out >= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('checkout_roll_off')) {
                                continue;
                            }
                        }
                        ...
                        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_show_pid')) {
                            ?>
                        ...
                        <!-- reason -->
                        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_visit_reason')) { ?>
                            <td class="detail text-center" name="kiosk_hide">
                        ...
                        <?php
                        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ptkr_show_staff') == '1') {
                            ?>
                                <td class="detail text-center" name="kiosk_hide">
    ...
    <script>
        ...
        function toggleSelectors() {
            ...
            if ($("#flb_selectors").css('display') === 'none') {
                $.post("<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/patient_tracker/patient_tracker.php"; ?>", {
        ...
        function topatient(newpid, enc) {
            if ($('#setting_new_window').val() === 'checked') {
                ...
                top.restoreSession();
                if (enc > 0) {
                    top.RTop.location = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/patient_file/summary/demographics.php?set_pid=" + encodeURIComponent(newpid) + "&set_encounterid=" + encodeURIComponent(enc);
                }
                else {
        ...
        $(function () {
            ...
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pat_trkr_timer') != '0') { ?>
                var reftime = <?php echo js_escape(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pat_trkr_timer')); ?>;

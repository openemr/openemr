...
//Included Ext Utility
include_once($GLOBALS['fileroot'].'/modules/ext_utility/extu/Utility.php');
$extuObj = new \extu\Utility();

use OpenEMR\Core\Header;

//Save Filter Value
$extuObj->saveFilterValueOfPatientTracker($_SESSION['authId'], $_POST);
...
function setPreservedValuesForFlowBoard() {
    ...
    $flItems = $extuObj->getSectionValues($_SESSION['authId']);
...
// function fetch_appt_signatures_data($appointments = array()) {
//     $response = array();
//     if(!empty($appointments)) {
//         $encounterIds = array();
//         foreach ($appointments as $akey => $appt) {
//             if(isset($appt['encounter']) && !empty($appt['encounter']) && $appt['encounter'] != '0') {
//                 $encounterIds[] = $appt['encounter'];
//             }
//         }

//         if(!empty($encounterIds)) {
//             $encounterIdsStr = implode(",", $encounterIds);
//             $eSql = "SELECT FE.encounter, E.id, E.tid, E.table, E.uid, U.fname, U.lname, E.datetime, E.is_lock, E.amendment, E.hash, E.signature_hash 
//                 FROM form_encounter FE 
//                 LEFT JOIN esign_signatures E ON (case when E.`table` ='form_encounter' then FE.encounter = E.tid else  FE.id = E.tid END)
//                 LEFT JOIN users U ON E.uid = U.id 
//                 WHERE FE.encounter IN(".$encounterIdsStr.") 
//                 ORDER BY E.datetime ASC";
//             $result = sqlStatement($eSql);
        
//             while ($row = sqlFetchArray($result)) {
//                 $response['TID_'.$row['encounter']]['data'][] =  $row;

//                 if(!isset($response['TID_'.$row['encounter']]['is_lock'])) {
//                     $response['TID_'.$row['encounter']]['is_lock'] = 0;
//                 }

//                 if(isset($row['is_lock']) && $row['is_lock'] == '1') {
//                     $response['TID_'.$row['encounter']]['is_lock'] = $row['is_lock'];
//                 }
//             }
//         }
//     }

    ...
    for ($i = 0; $i < $bg_count; $i++) {
        if (($_POST['access_group'][$i] == "Emergency Login") && ($_POST['active'] == 'on') && ($_POST['pre_active'] == 0)) {
            if (($_POST['get_admin_id'] == 1) && ($_POST['admin_id'] != "")) {
                ...
                $uname = $row['username'];
                $mail = new MyMailer();
                $mail->From = \OpenEMR\Core\OEGlobalsBag::getInstance()->get("practice_return_email_path");
...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] == "user_admin") {
    if ($_POST["mode"] == "update") {
        ...
        if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_fac_warehouse_restrictions')) || !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('restrict_user_facility'))) {
...
function resetCounter(username) {
    ...
    fetch("<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get("webroot"); ?>/library/ajax/login_counter_ip_tracker.php", {
        method: 'POST',
...
<div class="container">
    ...
    <div class="row">
        <div class="col-12">
                ...
                <table class="table table-striped table-sm">
                    <thead>
                        <tr>
                            ...
                            <?php
                            $checkPassExp = false;
                            if ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_expiration_days') != 0) && (check_integer(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_expiration_days'))) && (check_integer(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_grace_time')))) {
                                $checkPassExp = true;
                    ...
                    <tbody>
                        ...
                        foreach ($result4 as $iter) {
                            ...
                            if ($checkPassExp && !empty($iter["active"])) {
                                ...
                                $pwd_expires = date("Y-m-d", strtotime($userSecure['last_update_password'] . "+" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_expiration_days') . " days"));
                                $grace_time = date("Y-m-d", strtotime($pwd_expires . "+" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_grace_time') . " days"));
                            }
                            ...
                            if (empty($iter["active"])) {
                                ...
                                if (!empty($queryCounter['login_fail_counter'])) {
                                    ...
                                    $autoBlocked = false;
                                    $autoBlockEnd = null;
                                    if ((int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_max_failed_logins') != 0 && ($queryCounter['login_fail_counter'] > (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('password_max_failed_logins'))) {
                                        if ((int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_reset_password_max_failed_logins') != 0) {
                                            if ($queryCounter['seconds_last_login_fail'] < (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_reset_password_max_failed_logins')) {
                                                $autoBlocked = true;
                                                $autoBlockEnd = date('Y-m-d H:i:s', (time() + ((int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_reset_password_max_failed_logins') - $queryCounter['seconds_last_login_fail'])));
                                            }
                                        } else {
                                            $autoBlocked = true;
                                        }
            ...
            <?php
            if (empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('disable_non_default_groups'))) {

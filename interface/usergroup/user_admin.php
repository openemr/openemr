...
<?php  require_once(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/validation/validation_script.js.php"); ?>
<?php
...
function submitform() {
    ...
    top.restoreSession();
    var flag=0;
<?php if (empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_ldap_enabled')) || empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_ldap_exclusions'))) { ?>
...
<input type=hidden name="get_admin_id" value="<?php echo attr(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('Emergency_Login_email')); ?>" >
<input type=hidden name="admin_id" value="<?php echo attr(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('Emergency_Login_email_id')); ?>" >
...
<?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('restrict_user_facility')) { ?>
<tr>
 <td colspan=2>&nbsp;</td>
...
<?php if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('inhouse_pharmacy'))) { ?>
<tr>
...
<?php if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_fac_warehouse_restrictions')) || !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('restrict_user_facility'))) { ?>
 <tr title="<?php echo xla('If nothing is selected here then all are permitted.'); ?>">
  <td class="text"><?php echo !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_fac_warehouse_restrictions')) ?
  ...
  <td colspan="3">
   <select name="schedule_facility[]" multiple style="width:490px;">
    <?php
    $userFacilities = getUserFacilities($_GET['id'], 'id', \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_fac_warehouse_restrictions'));
    $ufid = [];
    foreach ($userFacilities as $uf) {
        $ufid[] = $uf['id'];
    }
    ...
    if ($fres) {
        while ($frow = sqlFetchArray($fres)) {
            ...
            if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_fac_warehouse_restrictions'))) {
                $lres = sqlStatement(

...
try {
    ...
    $contents = TwigFactory::createInstance()->render('core/unauthorized.html.twig', ['pageTitle' => xl("Alerts Log")]);
    // Send the error response
    $response = new Response($contents, 403);
} catch (NotFoundHttpException $e) {
    // Log the exception
    (new SystemLogger())->errorLogCaller($e->getMessage(), ['trace' => $e->getTraceAsString()]);
    $contents = TwigFactory::createInstance()->render('error/404.html.twig');
    // Send the error response
    $response = new Response($contents, 404);
} catch (\Throwable $e) {
    // Log the exception
    (new SystemLogger())->errorLogCaller($e->getMessage(), ['trace' => $e->getTraceAsString()]);
    $contents =  TwigFactory::createInstance()->render('error/general_http_error.html.twig');
    // Send the error response

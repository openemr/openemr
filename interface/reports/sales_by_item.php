...
function salesByItemLineItem(int $patient_id, int $encounter_id, string $rowcat, string $description, string $transdate, int $qty, float $amount, string $irnumber = ''): void
{
  ...
  <td class="detail" colspan="3">
                ...
                <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 0 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) {?>
  <td>
  &nbsp;
  </td>
    <?php } ?>
  ...
  <td class="text-right font-weight-bold">
    ...
    if (!empty($_POST['form_details'])) {
        if ($_POST['form_csvexport']) {
            ...
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 1 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) {
                echo csvEscape($pat_name) . ',';
            }

            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 0 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) {
                echo csvEscape(display_desc($invnumber)) . ',';
            }

            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 1) {
  ...
  <td>
   &nbsp;
        ...
        <?php } ?>
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 1 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) { ?>
  <td>
  ...
  <td class="detail">
   ...
   <a href='../patient_file/pos_checkout.php?ptid=<?php echo attr_url($patient_id); ?>&enc=<?php echo attr_url($encounter_id); ?>' target='_blank' rel='noopener'>
    ...
    <?php }
            ...
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 0) {?>
  <td>
   &nbsp;
  </td>
        <?php } ?>
...
<thead class='thead-light'>
 ...
 <th>
        ...
        <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) {?>
  <th>
   &nbsp;
  </th>
    <?php } ?>
 <th>
        <?php
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 0) {
            if ($form_details) {
                echo ' ';
            }
            ?>
  ...
  <th>
        <?php
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sales_report_invoice') == 2) {
            if ($form_details) {
                echo xlt('Invoice');
            }
        }

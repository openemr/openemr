...
<head>
    ...
    <style>
        ...
        <?php if ($type != 'Service Codes' || $type == '') { ?>
...
<span class='title'><?php echo htmlspecialchars(xl('Report - Clinical'), ENT_NOQUOTES); ?></span>
<!--
...
<div id="report_parameters_daterange"><?php echo text(oeFormatDateTime($sql_date_from, "global", true)) . " &nbsp; " . xlt("to{{Range}}") . " &nbsp; " . text(oeFormatDateTime($sql_date_to, "global", true)); ?></div>
    ...
    <div id="report_parameters">
        ...
        <table><tr>
                ...
                <table class='text'>
                    ...
                    <tr>
                        ...
                        <td><select name="type" class='form-control' id="type" onChange="checkType();">
                            ...
                            <option value="Procedure" <?php echo $type == 'Procedure' ? "selected" : ""; ?>><?php echo xlt('Procedure'); ?></option>
                            <option value="Medical History" <?php echo $type == 'Medical History' ? "selected" : ""; ?>><?php echo xlt('Medical History'); ?></option>
                            <option value="Service Codes" <?php echo $type == 'Service Codes' ? "selected" : ""; ?>><?php echo xlt('Service Codes'); ?></option>
                        </select></td>
                        <td class='col-form-label'><?php echo xlt('Communication'); ?>:</td>
                        <td><select name="communication" class='form-control' id="communication" title="<?php echo xla('Select Communication Preferences'); ?>">
            ...
            <td height="100%" valign='middle' width="175">
                <table class='w-100 h-100' style='border-left:1px solid;'><tr><td><div class="text-center"><div class="btn-group" role="group">
                    <a href='#' class='btn btn-secondary btn-save' onclick='submitForm();'><?php echo xlt('Submit'); ?></a>
...
<?php // SQL scripts for the various searches
$sqlBindArray = array();
if (!empty($_POST['form_refresh'])) {
    $sqlstmt = "select concat(pd.fname, ' ', pd.lname) AS patient_name, "
            ...
            . "REPLACE(REPLACE(concat_ws(',', IF(pd.hipaa_allowemail = 'YES', 'Allow Email', 'NO'), IF(pd.hipaa_allowsms = 'YES', 'Allow SMS', 'NO'), "
    ...
    if (!empty($form_diagnosis)) {
        $sqlstmt .= ", li.date AS lists_date, li.diagnosis AS lists_diagnosis, li.title AS lists_title";
    }

    if (strlen($form_drug_name) > 0 || !empty($_POST['form_drug'])) {
        $sqlstmt .= ", r.id as id, "
                . "r.date_modified AS prescriptions_date_modified, "
                . "r.dosage as dosage, "
                . "r.route as route, "
                . "r.interval as hinterval, "
                . "r.refills as refills, "
                . "r.drug as drug, "
                . "r.form as hform, "
                . "r.size as size, "
                . "r.unit as hunit, "
                . "d.name as name, "
                . "d.ndc_number as ndc_number, "
    ...
    if ($type == 'Procedure') {
        $sqlstmt .= ", po.date_ordered AS procedure_order_date_ordered, "
                . "pt.procedure_type AS procedure_type, "
    ...
    if ($type == 'Service Codes') {
        $sqlstmt .= ", c.code as code, c.code_text as code_text, fe.encounter as encounter, b.date as date";
    }

    if (strlen($form_immunization) > 0) {
        $sqlstmt .= ", immc.code_text as imm_code, "
                . "immc.code_text_short as imm_code_short, "
                . "immc.id as cvx_code, "
                . "imm.administered_date as imm_date, "
                . "imm.amount_administered, "
                . "imm.amount_administered_unit, "
    ...
    if ($type == 'Procedure' || ( strlen($form_lab_results) != 0) || !empty($_POST['lab_results'])) {
        ...
        if (strlen($form_lab_results) != 0 || !empty($_POST['lab_results'])) {
            $sqlstmt .= " left outer join procedure_report as pp on pp.procedure_order_id = po.procedure_order_id "
    ...
    if ($type == 'Medical History') {
        $sqlstmt .= " left outer join history_data as hd "
                . "on hd.pid = pd.pid and (isnull(hd.tobacco) = 0 or isnull(hd.alcohol) = 0 or isnull(hd.recreational_drugs) = 0)";
    }
    ...
    if ($type == 'Procedure') {
        ...
        if (strlen($form_lab_results) != 0 || !empty($_POST['lab_results'])) {
            $whr_stmt .= " AND pt.procedure_type = 'res'";
        }
    }
    ...
    if (strlen($form_drug_name) > 0 || !empty($_POST['form_drug'])) {
        $whr_stmt .= " AND (d.name LIKE ? OR r.drug LIKE ?)";
    ...
    <div id = "report_results">
        ...
        <table class='border-0' width='90%' align="center" cellpadding="5" cellspacing="0" style="font-family: tahoma;">
            <tr bgcolor="#CCCCCC" style="font-size:15px;">
                <td><strong><?php echo xlt('Summary of') . " " . text($row['patient_name']); ?></strong></td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td align="center">
            ...
            $row_id++; ?>
                <tr bgcolor="#C3FDB8" align="left">
                    ...
                    <td width="15%" <?php echo (strlen($communication) == 0 || !empty($_POST['communication_check'])) ? "colspan='5'" : ""; ?>><strong><?php echo xlt('Provider'); ?></strong></td>
                ...
                <tr bgcolor="#FFFFFF">
                    ...
                    <td <?php echo (strlen($communication) == 0 || (!empty($_POST['communication_check']))) ? "colspan='5'" : "" ?>><?php echo text($row['users_provider']); ?>&nbsp;</td>
                    <?php if (strlen($communication) > 0 || !empty($_POST['communication_check'])) { ?>
                    <td colspan='4'><?php echo text($row['communications']); ?></td>
                    <?php } ?>
                </tr>
                <!-- Diagnosis Report Start-->
                <?php if (!empty($form_diagnosis)) { ?>
                <tr bgcolor="#C3FDB8" align="left">
                    <td colspan='12'><strong><?php echo "#" . xlt('Diagnosis Report'); ?></strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align="left">
                ...
                <?php if (strlen($form_drug_name) > 0 || !empty($_POST['form_drug'])) {
                ...
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan='12'><strong><?php echo "#" . xlt('Prescription Report'); ?><strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">
                ...
                <?php if (strlen($form_lab_results) > 0 || !empty($_POST['lab_results'])) { ?>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan='12'><strong><?php echo "#" . xlt('Lab Results Report'); ?><strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td><strong><?php echo xlt('Date'); ?></strong></td>
                ...
                <?php if ($type == 'Procedure') {
                    if (!empty($row['procedure_type_standard_code'])) {
                        ...
                        $procedure_type_standard_code = $row['procedure_type_standard_code'];
                    } ?>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan=12><strong><?php echo "#" . xlt('Procedure Report'); ?><strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td><strong><?php echo xlt('Date'); ?></strong></td>
                ...
                <tr class='bg-white'>
                    ...
                    <td><?php echo generate_display_field(array('data_type' => '1','list_id' => 'proc_type'), $row['procedure_type']); ?>&nbsp;</td>
                ...
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan=12><strong><?php echo "#" . xlt('Medical History'); ?></strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">
                ...
                <?php if ($type == 'Service Codes') { ?>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan='11'><strong><?php echo "#" . xlt('Service Codes'); ?><strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td><strong><?php echo xlt('Date'); ?></strong></td>
                    <td><strong><?php echo xlt('Code'); ?></strong></td>
                    <td><strong><?php echo xlt('Encounter ID'); ?></strong></td>
                    <td colspan='8'><strong><?php echo xlt('Code Text'); ?></strong></td>
                </tr>
                <tr class='bg-white'>
                ...
                <?php if (strlen($form_immunization) > 0) { ?>
                <tr bgcolor="#C3FDB8" align= "left">
                    <td colspan='12'><strong><?php echo "#" . xlt('Immunization Report'); ?></strong></td>
                </tr>
                <tr bgcolor="#C3FDB8" align= "left">

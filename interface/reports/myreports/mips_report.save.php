...
require_once("$srcdir/ins_payor_funcs.php");
...
$query .= 'u.lname AS drlast, u.fname AS drfirst, u.mname AS drmiddle, '.
	...
	'v1dr.npi AS v1drnpi, v1dr.federaltaxid AS v1drtin, v1dr.lname AS v1drlast, '.
	'v1dr.fname AS v1drfirst, v1dr.mname AS v1drmiddle, '.
	'v2dr.npi AS v2drnpi, v2dr.federaltaxid AS v2drtin, v2dr.lname AS v2drlast, '.
	'v2dr.fname AS v2drfirst, v2dr.mname AS v2drmiddle '.
	'FROM patient_data AS p '.
	'LEFT JOIN form_encounter AS v1 ON (v1.id = '.
		'(SELECT form_encounter.id FROM form_encounter LEFT JOIN forms ON '.
		'(form_encounter.id = forms.form_id AND forms.formdir = "newpatient") '.
		'WHERE form_encounter.pid = p.pid AND forms.deleted = 0 '.
		'AND form_encounter.date >= ? AND form_encounter.date <= ? '.
		'ORDER BY form_encounter.date ASC LIMIT 1) ) ';
...
$query .= 'LEFT JOIN form_encounter AS v2 ON (v2.id = '.
		...
		'WHERE form_encounter.pid = p.pid AND forms.deleted = 0 '.
		'AND form_encounter.date >= ? AND form_encounter.date <= ? '.
		'ORDER BY form_encounter.date DESC LIMIT 1) ) ';
$binds[] = $form_from_date_time;
$binds[] = $form_to_date_time;
...
if(in_array('diag_filter', $report_flags)) {
	$query .= 'RIGHT JOIN lists AS l ON (l.pid = p.pid AND l.activity > 0 '.
		'AND l.type = "medical_problem"  AND ';
	if($form_from_date) $query .= '( ';
	$query .= '(l.enddate IS NULL OR '.
		'l.enddate = "0000-00-00") ';
	if($form_from_date) {
		$query .= 'OR l.enddate >= ? ) ';
...
if(in_array('diag_filter', $report_flags)) {
	$part = build_lists_where($diags, $ranges);
	$query .= $part . ') ';
}
...
if($query && ($_POST['form_refresh'] || $_REQUEST['exportcsv'])) $res = sqlStatement($query, $binds);

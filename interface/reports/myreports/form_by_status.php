...
$first = true;
if($custom_forms && (count($custom_forms) > 0)) {
	$query .= 'AND (';	
	foreach($custom_forms as $frm) {
		if($frm['form_dir'] == 'inject1') continue;
		if(!$first) $query .= 'OR '; 
		$query .= 'forms.formdir = ? ';
		$binds[] = $frm['form_name'];
		$first = false;
...
<span class='title'><?php xl('Report','e'); ?> - <?php xl('Forms by Status','e'); ?></span>

<div id="report_parameters_daterange">
<?php echo date("d F Y", strtotime($form_from_date)) ." &nbsp; to &nbsp; ". date("d F Y", strtotime($form_to_date)); ?>
</div>

<form method='post' name='theform' id='theform' action='form_by_status.php?mode=search'>
...
<table>
 <tr>
         ...
         <tr>
          ...
          <td><?php
            ...
            echo "    <option value=''>-- " . xl('All') . " --</option>\n";
						foreach($custom_forms as $frm) {
							if($frm['form_name'] == '') continue;
							if($frm['form_name'] == 'inject1') continue;
							echo "		<option value='".$frm['form_dir']."'";
							if($frm['form_name'] == $form_name) echo " selected='selected'";
  ...
  <td style="vertical-align: middle; text-align: left; width: 180px;" height="100%">
...
if ($res) {
  ...
  while ($row = sqlFetchArray($res)) {
    ...
    $this_form= 'form_'.$row['formdir'];
   	$ftitle = $row['form_name'];
		if($row['formdir'] == 'mc_wellsub' || $row['formdir'] == 'mc_wellness') { 
			$this_form = 'form_mc_wellness';
			if($row['form_dir'] == 'mc_wellsub') { $ftitle = 'Medicare Subsequent Visit'; }
		}
    $sql = "SELECT form_complete FROM $this_form WHERE `id` = ?";
    $farray = sqlStatement($sql, array($this_id));

...
$query = "SELECT " .
	"procedure_result.procedure_type_id AS pt_id, procedure_result.result, ".
	"procedure_order.provider_id, procedure_order.patient_id, ".
	"procedure_order.date_ordered, procedure_report.date_report, ".
	"procedure_report.report_status ".
  "FROM procedure_result " .
  "LEFT JOIN procedure_report USING (procedure_report_id) " .
  "LEFT JOIN procedure_order ON (procedure_report.procedure_order_id = procedure_order.procedure_order_id) " .
  "WHERE ".
	"( procedure_result.procedure_type_id='11817' OR ".
	"procedure_result.procedure_type_id='12054' OR ".
	"procedure_result.procedure_type_id='81952' OR ".
	"procedure_result.procedure_type_id='81953' OR ".
	"procedure_result.procedure_type_id='1172' OR ".
	"procedure_result.procedure_type_id='140097' OR ".
	"procedure_result.procedure_type_id='140288' OR ".
	"procedure_result.procedure_type_id='1464' ) AND ".
	"( procedure_result.result != 0 AND procedure_result.result != '' AND ".
	"procedure_result.result IS NOT NULL ) ";
if ($form_to_date) {
...
$query .= "ORDER BY procedure_order.provider_id, procedure_order.patient_id, procedure_result.procedure_type_id";
// echo "Query: ",$query,"\n";
...
<form method='post' name='theform' id='theform' action='berk_lab_summary.php?mode=search'>
...
if ($lres) {
  ...
  while ($row = sqlFetchArray($lres)) {
		...
		$lastdoc= $row{'provider_id'};
		// Add this patient to the correct test/result category
		if($row{'patient_id'} != $lastpat) $DocAccumulator->ChangePat();

		$lastpat = $row{'patient_id'};
		if($row{'pt_id'} == '1464') {
			$DocAccumulator->A1c_tests++;
			$DocAccumulator->A1c_pat_total = $DocAccumulator->A1c_pat_total + $row{'result'};
			$DocAccumulator->A1c_pat_tests++;
		}
		if($row{'pt_id'} == '12054') {
			// echo "<tr><td>Test 11817 Result: ".$row{'result'}."</td></tr>\n";
			$DocAccumulator->LDL_tests++;
			$DocAccumulator->LDL_pat_total = $DocAccumulator->LDL_pat_total + $row{'result'};
			$DocAccumulator->LDL_pat_tests++;
		}
		if($row{'pt_id'} == '11817') {
			$DocAccumulator->HDL_tests++;
			$DocAccumulator->HDL_pat_total = $DocAccumulator->HDL_pat_total + $row{'result'};
			$DocAccumulator->HDL_pat_tests++;
			// echo "<tr><td>Patient Count: ".$test_11817_pat."</td></tr>\n";
		}
		if($row{'pt_id'} == '1172') {
			// echo "<tr><td>Test 1172 Result: ".$row{'result'}."</td></tr>\n";
			if($row{'result'} < 250 && !$DocAccumulator->Tri_pat_lt_250) {
			...
			// echo "<tr><td>Patient Count: ".$test_1172_pat."</td></tr>\n";
		}
		if($row{'pt_id'} == '81952' || $row{'pt_id'} == '81953') {
			$DocAccumulator->D_tests++;
			if(!$DocAccumulator->D_pat_used) { $DocAccumulator->D_pats++; }
			$DocAccumulator->D_pat_used++;
			// echo "<tr><td>Patient Count: ".$test_81952_pat."</td></tr>\n";
		}
		if($row{'pt_id'} == '140097' || $row{'pt_id'} == '140288') {
			$DocAccumulator->Urine_tests++;
			if(!$DocAccumulator->Urine_pat_used) { $DocAccumulator->Urine_pats++; }
			$DocAccumulator->Urine_pat_used++;
			// echo "<tr><td>Patient Count: ".$test_81952_pat."</td></tr>\n";
		}
	}
	if($lastdoc != '') {

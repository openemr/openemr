...
$query = "SELECT " .
  'p.pid, p.pubpid, p.fname, p.mname, p.lname, p.street, p.city, p.state, ' .
	'p.postal_code, p.phone_home, p.phone_cell, p.email, ' .
	'lo.title AS ref_cource, p.providerID, ' .
  'u.lname AS ulname, u.fname AS ufname, u.mname AS umname ' .
  'FROM patient_data AS p ' .
  'LEFT JOIN users AS u ON u.id = p.providerID WHERE 1 ' .
if ($form_provider) {
  $query .= "AND p.providerID = '$form_provider' ";
}

...
$lab_code_stmt = "SELECT * FROM list_options WHERE list_id='HepC_Test_Name' ".
				"ORDER BY seq";
$lab_desc_stmt = "SELECT * FROM list_options WHERE list_id='HepC_Test_Name' ".
				"AND option_id=?";
...
$query = "SELECT " .
	"ora.id AS ora_id, ora.hpc_collect_dt, ora.hpc_report_dt, ora.hcp_test, ".
	"ora.hcp_result, ora.hpc_ref, ora.hpc_comments, ora.pid AS ora_pid, ".
	...
	"FROM form_hepC AS ora ".
	"LEFT JOIN patient_data AS p ON ora.pid=p.pid ".
	"LEFT JOIN form_dashboard AS db ON ora.pid=db.pid ".
  "WHERE ";
$query .= "r.pid > 0 AND ";
$first = true;
...
if ($form_to_date) {
  $query .= "(ora.hpc_report_dt >= '$form_from_date 00:00:00' AND ".
			"ora.hpc_report_dt <= '$form_to_date 23:59:59') ";
} else {
  $query .= "(ora.hpc_report_dt >= '$form_from_date 00:00:00' AND ".
			"ora.hpc_result_dt <= '$form_from_date 23:59:59') ";
}
...
if($create == 'csv' && $lres) {
	...
	fputcsv($output, $columns);
 	while ($row = sqlFetchArray($lres)) {
		...
		$race = '';
		$relation = '';
		if(strtolower($row{'hepc_result'}) == 'r') { $result = 'POSITIVE'; }
		if(strtolower($row{'hepc_result'}) == 'd') { $result = 'POSITIVE'; }
		if(strtolower($row{'hepc_result'}) == 'nr') { $result = 'NEGATIVE'; }
		if(strtolower($row{'hepc_result'}) == 'nd') { $result = 'NEGATIVE'; }
		...
		$ires = sqlStatement($sql, array($row{'ora_pid'}));
		...
		$sql = "SELECT forms.encounter, form_id, forms.pid, facility, ".
				...
				"form_id=? AND formdir='form_hepC' AND ".
				"forms.pid=? AND deleted=0";
		$vres = sqlStatement($sql, array($row{'ora_id'}, $row{'ora_pid'}));
		...
		unset($data);
		$data = array($row{'ora_pid'}, substr($row{'result_datetime'},0,10), 
...
if ($lres) {
  while ($row = sqlFetchArray($lres)) {
		...
		$pat_first = ($row{'pat_first'} != '') ? $row{'pat_first'} : $row{'fname'};
		echo "<tr>\n";
		echo "<td>".substr($row{'hpc_report_dt'},0,10)."&nbsp;</td>\n";
		echo "<td>".$row{'ora_pid'}."&nbsp;</td>\n";
		echo "<td>$pat_last&nbsp;</td>\n";
		echo "<td>$pat_first&nbsp;</td>\n";
		echo "<td>".ListLook($row{'hpc_test'}, 'HepC_Test_Name')."&nbsp;</td>\n";
		echo "<td>".ListLook($row{'hpc_result'}, 'HepC_Test_Results')."&nbsp;</td>\n";
		echo "</tr>\n";
	}
}
?>

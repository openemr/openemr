...
if (empty($formid)) {
    $sql = "SELECT form_id, encounter FROM `forms` WHERE formdir = 'care_plan2' AND pid = ? AND encounter = ? AND deleted = 0 LIMIT 1";
...
if (!empty($formid)) {
    $sql = "SELECT * FROM `form_care_plan2` WHERE id=? AND pid = ? AND encounter = ?";
    ...
    <head>
        ...
        <script>
            ...
            function removeVal(rowid) {
                ...
                document.getElementById("care_plan2_type_" + rowid1[1]).value = '';
        ...
        <div class="container mt-3">
            <div class="row">
                <div class="col-12">
                    ...
                    <form method='post' name='my_form' action='<?php echo $rootdir?>/forms/care_plan2/save.php?id=<?php echo attr_url($formid) ?>'>
                                ...
                                if (!empty($check_res)) {
                                    ...
                                    <div class="tb_row" id="tb_row_<?php echo attr($key) + 1; ?>">
                                        <div class="form-row">
                                            ...
                                            <div class="forms col-md-2">
                                                <label for="care_plan2_type_<?php echo attr($key) + 1; ?>" class="h5"><?php echo xlt('Type'); ?>:</label>
                                                <select name="care_plan2_type[]" id="care_plan2_type_<?php echo attr($key) + 1; ?>" class="form-control care_plan2_type">
                                                    <option value=""></option>
                                                    <?php foreach ($care_plan2_type as $value) :
                                                        $selected = ($value['value'] == $obj["care_plan2_type"]) ? 'selected="selected"' : '';
                                                        if (!empty($selected)) {
                                                            $context = $value['title'];
                                                        }
                                                        ?>
                                ...
                                <?php }
                                    ...
                                    <div class="tb_row" id="tb_row_1">
                                        <div class="form-row">
                                            ...
                                            <div class="forms col-md-2">
                                                <label for="care_plan2_type_1" class="h5"><?php echo xlt('Type'); ?>:</label>
                                                <select name="care_plan2_type[]" id="care_plan2_type_1" class="form-control care_plan2_type">
                                                    <option value=""></option>
                                                    <?php foreach ($care_plan2_type as $value) :
                                                        $selected = ($value['value'] == ($obj["care_plan2_type"] ?? '')) ? 'selected="selected"' : '';
                                                        ?>

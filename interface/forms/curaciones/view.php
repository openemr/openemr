...
if ($id > 0) {
    ...
    $rows = QueryUtils::fetchRecords("SELECT * FROM form_curaciones WHERE id = ? AND pid = ? AND encounter = ? LIMIT 1", [$id, $pid, $encounter]);
} else {
    /** @var list<array<string, string|int|null>> $rows */
    $rows = QueryUtils::fetchRecords("SELECT * FROM form_curaciones WHERE pid = ? AND encounter = ? ORDER BY date DESC", [$pid, $encounter]);
}
?>
    ...
    <div class="registro-card">
        ...
        <div class="curacion-detalle">
            <div class="curacion-header <?php echo $cls; ?>">
                <div class="curacion-nombre"><?php echo text((string)$label); ?></div>

...
$query .= "fqo.id AS ord_id, fqr.id AS res_id, fqo.order_number AS number FROM form_quest_order fqo ";
$query .= "LEFT JOIN forms ON forms.form_id = fqo.id AND forms.formdir = 'quest_order' AND forms.deleted = 0 ";
$query .= "LEFT JOIN form_quest_result fqr ON fqo.order_number = fqr.request_order ";
$query .= "UNION ";
$query .= "SELECT forms.id AS forms_id, forms.date AS forms_date, forms.encounter AS forms_encounter, forms.form_name AS forms_name, forms.pid AS forms_pid, forms.user AS forms_user, forms.groupname AS forms_groupname, forms.authorized AS forms_authorized, ";
...
$query .= "LEFT JOIN form_quest_order fqo ON fqo.order_number = fqr.request_order) XXX ";
...
$last_number = '';
$count = 0;
while ( $old = sqlFetchArray ( $forms ) && $count ++ < 10 ) {
	...
	if ($old ['ord_id']) {
		...
		if ($order ['id']) { // skip if order not found
			...
			$params [] = $order_number;
			$record .= "order_req_id = ?, ";
			$params [] = $order ['order_req_id'];
			$record .= "order_abn_id = ?, ";
			$params [] = $order ['order_abn_id'];
	...
	$params [] = $odate;
	$record .= "date_pending = ?, ";
	$params [] = $order ['order_pending'];
	$record .= "specimen_fasting = ?, ";
	$params [] = $order ['order_fasting'];
	$record .= "specimen_duration = ?, ";
	$params [] = $order ['order_duration'];

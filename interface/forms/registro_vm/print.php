...
<div class="page">
    ...
    <div style="background:#eef2ff; border:1px solid #c7d2fe; padding:10px 14px; margin-bottom:12px;">
        ...
        <span style="font-size:12px; font-weight:bold; color:#1976d2; margin-left:8px;">
            <?php echo htmlspecialchars((string) $modo_display); ?>
        </span>
        <?php if (($row['obs_modo'] ?? '') !== '') :
            ?>
        <div style="margin-top:6px; font-size:9px; color:#444;"><?php echo htmlspecialchars((string) $row['obs_modo']); ?></div>
            <?php
        endif; ?>
    </div>
    ...
    foreach ($bool_fields as $label => $data) {
        echo $vmRow($label, (int)($data['val']), (string)($data['obs']));
    }
    ?>
    </tbody></table>
...
$mpdf->SetTitle(xlt('Mechanical Ventilation Record') . ' - ' . ($paciente !== false ? (string)($paciente['full_name'] ?? '') : ''));
$mpdf->WriteHTML((string)$html);
$filename = 'RegistroVM_' . preg_replace('/\s+/', '_', $paciente !== false ? (string)($paciente['full_name'] ?? 'paciente') : 'paciente') . '_' . date('Ymd_His') . '.pdf';

...
function display_QP($zone, $provider_id)
{
    ...
    ?>
      <a href="JavaScript:void(0);"
         onclick="openNewForm('<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/super/edit_list.php?list_id=Eye_QP_<?php echo attr($zone) . "_" . attr($provider_id); ?>','QP Editor');"
    ...
    <div id="Draw_<?php echo attr($zone); ?>" name="Draw_<?php echo attr($zone); ?>" class="Draw_class canvas">
        ...
        <div id="<?php echo attr($zone); ?>_canvas">
            <div class="tools">
                ...
                $base_filetoshow = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . "/interface/forms/" . $form_folder . "/images/" . $side . "_" . $zone . "_BASE.jpg";
                if ((($doc['id'] ?? null) > '0')) {
                    $filetoshow = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . "/controller.php?document&retrieve&patient_id=" . attr($pid) . "&document_id=" . attr($doc['id']);
                } else {
                    //base image.
...
function document_engine($pid)
{
    ...
    while ($row2 = sqlFetchArray($sql2)) {
        ...
        $row2['display_url'] = preg_replace("|file:///.*/sites/|", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/sites/", (string) $row2['url']);
...
function display($pid, $encounter, $category_value)
{
    ...
    for ($j = 0; $j < count($documents['zones'][$category_value]); $j++) {
        ...
        <td>
            <a onclick=\"openNewForm('" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?document&upload&patient_id=" . attr($pid) . "&parent_id=" . attr($documents['zones'][$category_value][$j]['id']) . "&', '" . xla('Upload') . " " . attr($documents['zones'][$category_value][$j]['name']) . "');\" href='#'>
        ...
        if ($count_here > '0') {
            $episode .= '<a onclick="openNewForm(\'' . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . '/controller.php?document&view&patient_id=' . $pid . '&doc_id=' . $id_to_show . '\',\'' . xla('Documents') . ': ' . attr($documents['zones'][$category_value][$j]['name']) . '\');"><img src="../../forms/' . $form_folder . '/images/jpg.png" class="little_image" /></a>';
        }
    ...
    <nav class="navbar fixed-top navbar-custom navbar-bright navbar-inner navbar-expand-lg" data-role="page banner navigation"
        ...
        <div class="container-fluid" style="margin-top:0px;padding:2px;">
            <div class="navbar-brand" style="color:black;">
                &nbsp;
                <img src="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/sites/default/images/login_logo.gif" class="little_image">
            ...
            <div class="navbar-collapse oer-navbar-collapse mr-auto mt-2 mt-lg-0" id="oer-navbar-collapse-1">
                <ul class="navbar-nav mr-auto">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li id="menu_Defaults" name="menu_Defaults" class="">
                                <a class="nav-link black"
                                   ...
                                   onclick="openNewForm('<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/super/edit_list.php?list_id=Eye_defaults_<?php echo attr($provider_id); ?>', '<?php echo xla('Default Exam Values'); ?>');"
                                   name="provider_todo"
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                            ...
                            <li>
                                <a class="nav-link black" tabindex="-1" target="_shorthand" href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/forms/eye_mag/help.php">
                ...
                <ul class="nav navbar-nav navbar-right my-2 my-lg-0">
                    <li><span style="margin-right:15px;color:black;"  onclick="editScripts('<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root'); ?>/controller.php?prescription&list&id=<?php echo attr_url($pid); ?>');">eRx</button>
                        </span></li>
    ...
    <div class="borderShadow row" id="title_bar">
        ...
        <div id="left_menu3" name="left_menu3" class="col-sm-3" style="font-size:1.0em;">
            ...
            if (!empty($documents['docs_in_name']['Patient Photograph'][0])) {
                ?>
                <object><embed
                            src="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/controller.php?document&retrieve&patient_id=<?php echo attr($pid); ?>&document_id=<?php echo attr($documents['docs_in_name']['Patient Photograph'][0]['id']); ?>&as_file=false&original_file=true&disable_exit=false&show_original=true&context=patient_picture"

                            Xsrc="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/controller.php?document&amp;retrieve&amp;patient_id=<?php echo attr($pid); ?>&amp;document_id=<?php echo attr($documents['docs_in_name']['Patient Photograph'][0]['id']); ?>&amp;as_file=false" frameborder="0"
                ...
                ?>
            <object><embed src="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root'); ?>/interface/forms/<?php echo $form_folder; ?>/images/anon.gif" frameborder="0"
    ...
    <table style="width:100%;">
        <tr>
            <td style='width:150px;text-align:top;'>
                <?php
                if ($direction == "web") {
                    $practice_logo = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/sites/default/images/practice_logo.gif";
                    ...
                    foreach ($documents['docs_in_name']['VF'] as $VF) {
                        if ($count < 1) {
                            ...
                            $current_VF = '<tr><td class="GFS_td_1 blue">
                                <a onclick="openNewForm(\'' . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . '/controller.php?document&view&patient_id=' . attr($pid) . '&doc_id=' . attr($VF['id']) . '\',\'Documents\');">
                        ...
                        if ($count_OCT > '0') { //need to decide how many to show on open, and hide the rest?  For now show first, hide rest.
                            ...
                            foreach ($documents['docs_in_name']['OCT'] as $OCT) {
                                ...
                                if ($count < 1) {
                                    $current_OCT = '<tr>
                                            <td class="GFS_td_1">
                                            <a onclick="openNewForm(\'' . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . '/controller.php?document&view&patient_id=' . attr($pid) . '&doc_id=' . attr($OCT['id']) . '\',\'Documents\');"><img src="../../forms/' . $form_folder . '/images/jpg.png" class="little_image" style="width:15px; height:15px;" /></a>
                                            </td>
                      ...
                      <table id="wearing_<?php echo attr($W); ?>" >
                        ...
                        <tr class="WNEAR">
                          ...
                          <td name="W_wide" title="<?php echo xla('Lens Material'); ?>" colspan="2">
                            <a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/super/edit_list.php?list_id=Eye_Lens_Material" target="RTop"
                          ...
                          <td name="W_wide2" colspan="4" rowspan="4">
                            <a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/super/edit_list.php?list_id=Eye_Lens_Treatments" target="RTop"

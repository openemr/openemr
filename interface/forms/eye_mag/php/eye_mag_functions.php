...
function display_GlaucomaFlowSheet($pid, $bywhat = 'byday')
{
    ...
    if ((array)$priors) {
        ...
        if ($encounter_data['ODKTHICKNESS'] || $encounter_data['OSKTHICKNESS']) {
            $KTHICKNESS_date[$i] = $dated;
        }
    ...
    for ($a = 0; $a < count($date_OU); $a++) {
        ...
        if (!empty($KTHICKNESS_date)) {
            foreach ($KTHICKNESS_date as $KTHICKNESS) {
                if ($date_OU[$a] == $KTHICKNESS) {
                    $KTHICKNESS_values[$a] = "1";
                    break;
                }
            }
        }

        if (!($KTHICKNESS_values[$a] ?? null)) {
            $KTHICKNESS_values[$a] = "";
        }
        ...
        <div id="GFS_table" name="GFS_table" class="table-responsive borderShadow" style="position:relative;display:table;float:left;margin-top:10px;padding:15px;text-align:left;vertical-align:center;width:30%;">
            <table class="GFS_table">
                ...
                <tr>
                    ...
                    if ($hideme) {
                        echo '<td><span class="top right" id="toggle_kthickness"><i class="fa fa-toggle-down"></i></span></td>';
                    }
                    ?>
                </tr>
                    <?php echo $pachymetry;

...
function process_kb_1(field, text, appendix) {
    ...
    if ((field === 'PMH') || (field === 'Allergy') || (field === 'Medication')
         || (field === 'Surgery') || (field === 'POH') || (field === 'POS')) {
        ...
        const url = '../../forms/eye_mag/save.php?PMSFH_save=1&mode=update&form_save=1';
        const text_seg = text.match(/[^\.]*/g);
        for (let index = 0; index < text_seg.length; index += 1) {
            if (text_seg[index] === '') {
                continue;
            }
    ...
    if ((field === 'BCNV') || (field === 'BCN5') || (field === 'CNV') || (field === 'CN5')) {
        field = 'RCNV';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
        field = 'LCNV';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
    ...
    if ((field === 'BG') || (field === 'G') || (field === 'GONIO')) {
        field = 'ODGONIO';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
        field = 'OSGONIO';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
    ...
    if ((field === 'BD') || (field === 'BDISC') || (field === 'BDISCS')) {
        field = 'ODDISC';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
        field = 'OSDISC';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
    ...
    if ((field === 'BC') || (field === 'C') || (field === 'BCUP') || (field === 'BCUPS')) {
        field = 'ODCUP';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
        field = 'OSCUP';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
    ...
    if ((field === 'BMAC') || (field === 'MAC') || (field === 'BM')) {
        field = 'ODMACULA';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
        field = 'OSMACULA';
        (appendix === ".a") ? ($('#' + field).val($('#' + field).val() + ', ' + text)) : $('#' + field).val(text);
    ...
    if (field.match(/^(\d{1,2})$/)) {
        const data = text.match(/(\d{0,2}||ortho)(.*)/i);
        let PD = data[1];
        ...
        const strab = data[2].toUpperCase().replace(/I(.)/g, '$1(T)').replace(/\s*(\d)/, '\n$1');
...
function process_kb(field, text, appendix, prior_field, prior_text) {
    let response = process_kb_1(field, text, appendix);
    // If the field is found, the text is added (or appended) to the field
    if (response.field === 'error') {
        // Human error correcting:
        // If the field is not found, append the "field + text" data to the prior_field's prior_text
        // if it exists.
        // In essence the ";" is believed to be an error so it is replaced by ", " for most fields.
        // For POH/PMH/MEDS/ALLERGIES/PSURG it NOT appended but is instead added
        // as a new entry to the list.

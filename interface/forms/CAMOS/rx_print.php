...
if ($_POST['print_pdf'] || $_POST['print_html']) {
    ...
    foreach ($_POST as $key => $val) {
        if (substr((string) $key, 0, 3) == 'ch_') {
            $query = sqlStatement("select content from " . mitigateSqlTableUpperCase("form_CAMOS") . " where id =?", array(substr((string) $key, 3)));
            if ($result = sqlFetchArray($query)) {
        ...
        if (substr((string) $key, 0, 5) == 'chrx_') {

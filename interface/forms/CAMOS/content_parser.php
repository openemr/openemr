...
function replace($pid, $enc, $content)
{
 //replace placeholders with values
    ...
    $ret = preg_replace(
        ...
        array($name,$age,strtolower((string) $gender),$doctorname),
        (string) $content
    );
...
function patient_age($birthday, $date)
{
 //calculate age from birthdate and a given later date
    list($birth_year,$birth_month,$birth_day) = explode("-", (string) $birthday);
    list($date_year,$date_month,$date_day) = explode("-", (string) $date);

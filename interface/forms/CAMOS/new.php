...
if (!$out_of_encounter) {
    ?>
    f2.action = '<?php print \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') ?>/interface/patient_file/encounter/load_form.php?formname=CAMOS';
    <?php
} else {
    ?>
    f2.action = '<?php print \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') ?>/interface/forms/CAMOS/new.php?mode=external';
    <?php
}
?>
    f2.target = '_self';
...
//    alert(str);
    processajax ('<?php print \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') ?>/interface/forms/CAMOS/ajax_save.php', myobj, "post", str);
//    alert("submitted!");
//ajax code
}
...
<select name='stepback'>
  ...
  <option value='12'><?php echo xlt('Back twelve visits'); ?></option>
    ...
    <?php
    echo "<a href='" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('form_exit_url') . "' onclick='top.restoreSession()'>[" . xlt('Leave The Form') . "]</a>";
    ?>
...
if (!$out_of_encounter) { //do not do stuff that is encounter specific if not in an encounter
    echo "<a href='" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('form_exit_url') . "' onclick='top.restoreSession()'>[" . xlt('Leave The Form') . "]</a>";
    echo "<a href='" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/forms/CAMOS/help.html' target='new'> | [" . xlt('Help') . "]</a>";

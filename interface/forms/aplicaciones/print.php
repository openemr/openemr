...
<div class="page">
    ...
    foreach ($fields as $label => $data) {
        echo $appRow($label, $data['val'], (string)$data['obs']);
    }
    ?>
    </tbody></table>
...
$mpdf->SetTitle(xlt('Nursing Applications') . ' - ' . ($paciente !== false ? (string)($paciente['full_name'] ?? '') : ''));
$mpdf->WriteHTML((string)$html);
$filename = 'Aplicaciones_' . preg_replace('/\s+/', '_', $paciente !== false ? (string)($paciente['full_name'] ?? 'paciente') : 'paciente') . '_' . date('Ymd_His') . '.pdf';

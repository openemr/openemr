...
if ($id > 0) {
    $result = sqlStatement("SELECT * FROM form_aplicaciones WHERE id = ? AND pid = ? AND encounter = ? LIMIT 1", [$id, $pid, $encounter]);
} else {
    $result = sqlStatement("SELECT * FROM form_aplicaciones WHERE pid = ? AND encounter = ? ORDER BY date DESC", [$pid, $encounter]);
}
?>
...
<head>
    ...
    <style>
        .aplicaciones-view *, .aplicaciones-view *::before, .aplicaciones-view *::after { box-sizing: border-box; }
        ...
        .aplicacion-detalle { border: 1px solid rgba(128,128,128,0.2); border-radius: 3px; margin-bottom: 12px; overflow: hidden; }
        .aplicacion-header  { display: flex; justify-content: space-between; align-items: center; padding: 12px 15px; }
        .aplicacion-header.si { background-color: rgba(40,167,69,0.15); border-left: 4px solid #28a745; }
        .aplicacion-header.no { border-left: 4px solid #6c757d; }
        .aplicacion-nombre  { font-size: 14px; font-weight: 600; }
        ...
        .aplicacion-obs { padding: 18px 20px; }
        .aplicacion-obs.con-contenido { background: rgba(13,110,253,0.08); border-top: 3px solid #0d6efd; }
        .aplicacion-obs h5 { font-size: 12px; opacity: 0.6; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; font-weight: 700; }
        .aplicacion-obs p  { font-size: 14px; line-height: 1.7; white-space: pre-wrap; }
    ...
    <div class="registro-card">
        <div class="registro-header">
            ...
            <div class="registro-acciones">
                <a href="<?php echo attr(OEGlobalsBag::getInstance()->getString('webroot') . '/interface/forms/aplicaciones/new.php?pid=' . $pid . '&encounter=' . $encounter . '&id=' . (int)($row['id'] ?? 0)); ?>"
                   class="btn btn-primary mr-1" onclick="top.restoreSession()">
                    <i class="fas fa-pen mr-1"></i><?php echo xlt('Edit'); ?>
                </a>
                <a href="<?php echo attr(OEGlobalsBag::getInstance()->getString('webroot') . '/interface/forms/aplicaciones/print.php?pid=' . $pid . '&encounter=' . $encounter . '&id=' . (int)($row['id'] ?? 0)); ?>"
        ...
        <div class="aplicacion-detalle">
            ...
            <div class="aplicacion-obs <?php echo ($obs !== '') ? 'con-contenido' : ''; ?>">

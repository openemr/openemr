...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_LOINCnote') ?? 0) {
    switch (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_LOINCnote') ?? 0) {
        case '0':
            $top_note = true;
...
if ($isPortal) {
    $theme = stripos((string)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('portal_css_header'), 'dark') !== false ? 'dark' : 'light';
} else {
    $theme = stripos((string)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('css_header'), 'dark') !== false ? 'dark' : 'light';
}

if ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_style') ?? 0) == 3) {
    $theme = 'light';
} elseif ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_style') ?? 0) == 4) {
    $theme = 'dark';
}

if ($isModule || $isDashboard || $isPortal) {
    $container = 'container-fluid';
} elseif (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_fullscreen') ?? 0)) {
    $container = 'container';
} else {
    $container = 'container-fluid';
}
...
<head>
    ...
    <script>
        ...
        let allowCopyright = <?php echo js_escape(!((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('questionnaire_display_LOINCnote') ?? 0) == '3')); ?>;
        let formOptions = {

...
function echoServiceLines(): void
{
    ...
    foreach ($fs->serviceitems as $lino => $li) {
        ...
        if ($billed) {
            ...
            if (!empty(OEGlobalsBag::getInstance()->get('gbl_auto_create_rx'))) {
...
function echoProductLines(): void
{
    ...
    foreach ($fs->productitems as $lino => $li) {
        ...
        if ($billed) {
            ...
            echo "  <td class='billcell text-center' $usbillstyle>&nbsp;</td>\n"; // note codes
            echo "  <td class='billcell text-center' $usbillstyle>&nbsp;</td>\n"; // auth
            if (OEGlobalsBag::getInstance()->get('gbl_auto_create_rx')) {
                            ...
                            <table name='selected_codes' id='selected_codes' class="table table-striped table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <?php if (!empty(OEGlobalsBag::getInstance()->get('gbl_auto_create_rx'))) { ?>
                    ...
                    <fieldset>
                        ...
                        <div class="row mx-5">
                            <div class="form-row col">
                                ...
                                <div class="col-10">
                                    <?php
                                    if (empty(OEGlobalsBag::getInstance()->get('default_rendering_provider'))) {
                                        $default_rid = $fs->provider_id ?: 0;
                                        if (!$default_rid && $userauthorized) {
                                            $default_rid = $session->get('authUserID');
                                        }
                                    } elseif (OEGlobalsBag::getInstance()->get('default_rendering_provider') == '1') {
                        ...
                        <div class="col-sm-12 position-override">
                            <div class="btn-group" role="group">
                                ...
                                <?php if (!$isBilled) { // visit is not yet billed ?>
                                    ...
                                    <?php if (OEGlobalsBag::getInstance()->get('ippf_specific') && (AclMain::aclCheckForm('admin', 'super') || AclMain::aclCheckForm('acct', 'bill') || AclMain::aclCheckForm('acct', 'disc'))) { // start ippf-only stuff ?>

...
function start_edit(data, event) {
    data.edit_mode(true);
    const elem = $(event.target).siblings('input').get(0);
}
...
function justify_entry(json_object) {
    const retval = new code_entry(json_object);
...
function update_diagnosis_options(diagnosis_options, data) {
    let chosen = 0;
    let cur;
    for (let idx = 0; idx < diagnosis_options().length; idx += 1) {
        cur = diagnosis_options()[idx];
        if (cur.selected()) {
            chosen += 1;
        }
    }
    let old_priority = 99999;
    if (data.selected()) {
...
function choose_search_diag(data, event, parent) {
    const code_key = `${data.code_type}|${data.code}`;
    const existing = parent.added_keys[code_key];
    if (typeof existing !== 'undefined') {
        ...
        const new_justify = justify_entry({
            code: data.code,
        ...
        new_justify.allowed_to_create_problem_from_diagnosis(
            data.allowed_to_create_problem_from_diagnosis,
        );
...
function search_change(data, model) {
    ...
    const search_type = model.searchType().key;
    const search_type_id = model.searchType().id;
    if (search_query.length > 0) {
        ...
        function (result) {
            ...
            if (result.codes != null) {
                for (let idx = 0; idx < result.codes.length; idx += 1) {
                    const cur_code = result.codes[idx];
...
function update_justify(data, event) {
    ...
    const justify = [];
    for (let idx = 0; idx < data.diagnosis_options().length; idx += 1) {
        const cur = data.diagnosis_options()[idx];
        if (cur.selected()) {
            justify.push(cur.jsonify());
        }
    }
    const skip_issues = data.duplicates().length > 0;
...
function lookup_justify(current_justifications, entry) {
    for (let idx = 0; idx < current_justifications.length; idx += 1) {
        const cur = current_justifications[idx];
...
function setup_justify(model, current, patient, common) {
    ...
    for (let idx = 0; idx < current.length; idx += 1) {
        cur_entry = current[idx];
        const new_justify = new justify_entry(cur_entry);
        if (typeof model.added_keys[new_justify.key()] === 'undefined') {
            ...
            new_justify.allowed_to_create_problem_from_diagnosis(
                cur_entry.allowed_to_create_problem_from_diagnosis,
            );
            model.diagnosis_options.push(new_justify);
        }
    }
    for (let idx = 0; idx < patient.length; idx += 1) {
        ...
        if ((cur_entry.code != null) || cur_entry.code_type !== '') {
            ...
            if (typeof model.added_keys[new_justify.key()] === 'undefined') {
                ...
                const entry = model.added_keys[new_justify.key()];
                if ((entry.prob_id() != null) && (entry.prob_id() !== cur_entry.db_id)) {
                    ...
                    entry.allowed_to_create_problem_from_diagnosis(
                        cur_entry.allowed_to_create_problem_from_diagnosis,
                    );
    ...
    for (let idx = 0; idx < common.length; idx += 1) {
...
function fee_sheet_justify_view_model(billing_id, enc_id, pat_id, current_justify) {
    this.justify = {
        ...
        search_show: ko.observable(false).extend({
            throttle: 300,
        }),
    ...
    const vm = this.justify;
    vm.search_query_throttled = ko.computed(vm.search_query).extend({
        throttle: 300,

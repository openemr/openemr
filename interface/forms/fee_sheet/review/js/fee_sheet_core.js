...
function parse_row_justify(row) {
    const codes = row.find("select[onchange^='setJustify']").val().split(',');
    const retval = [];
    for (let idx = 0; idx < codes.length; idx += 1) {
        const cur_code_string = codes[idx];
        if (cur_code_string.length > 0) {
            const code_parts = cur_code_string.split('|');
            const justify = new code_entry({
                description: '',
                code: code_parts[1],
...
function justify_start(evt) {
    ...
    const myForm = document.getElementById('fee_sheet_form');
    ...
    $.ajax({
        ...
        const current_justify_choices = parse_row_justify(parent.parent());
        const justify_model = new fee_sheet_justify_view_model(parent.attr('billing_id'), enc, pid, current_justify_choices);
...
function tag_justify_rows(display) {
    const justify_selectors = display.find("select[onchange^='setJustify']").parent();
    const justify_rows = justify_selectors.parent('tr');
    const justify_td = justify_rows.children('td:first-child').addClass('has_justify');
    justify_td.each(function (idx, elem) {
        ...
        if (jqElem.find('a.justify_label').length === 0) {
            ...
            const label = jqElem.text();
            const html = jqElem.html().substr(label.length);
            jqElem.html(html);
            $(`<a class='justify_label'>${label}</a>`).appendTo(jqElem).on({
                click: justify_start,
            }).attr('title', justify_click_title);
        }
        return false;
    });
    const id_fields = justify_rows.find("input[type='hidden'][name$='[id]']");
    id_fields.each(function (idx, elem) {
        const jqElem = $(elem);
        const td = jqElem.parent();
        td.addClass('has_id');
...
function update_display_table(data) {
    let rc = true; // this will be our return value
    ...
    const new_table = new_info.find(display_table_selector);
    ...
    const new_checksum = new_info.find('input[name="form_checksum"]').val();
    ...
    const new_alertmsg = new_info.find('input[name="form_alertmsg"]').val();
    if (new_alertmsg) {
        alert(new_alertmsg);
        rc = false;
    }
    ...
    const diag_regex = new RegExp('diags.push(.*);', 'g');
    const diags_matches = data.match(diag_regex);
    if (diags_matches != null) {
        diags = []; // clear the existing diags array
        for (let i = 0; i < diags_matches.length; i += 1) {
            // eslint-disable-next-line no-eval
            eval(diags_matches[i]);
        }
    }
    const justifications = $("select[onchange='setJustify(this)']");
...
function codeselect_and_save(selobj) {
    const i = selobj ? selobj.selectedIndex : -1;
    if (i) {
        const f = document.forms[0];
        if (selobj) f.newcodes.value = selobj.options[i].value;
        // Submit the newly selected code.
        window.top.restoreSession();
        const form_data = `${$('form').serialize()}&running_as_ajax=1`;
        $.post(fee_sheet_new, form_data,
            function (data) {
                ...
                if (update_display_table(data)) {
                    ...
                    const form_data = `${$('form').serialize()}&bn_save=Save&running_as_ajax=1`;
                    $.post(fee_sheet_new, form_data,
                        function (data) {
                            update_display_table(data);
                        });

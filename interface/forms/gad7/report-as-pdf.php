 ...
 */
   ...
   $formated_date = oeFormatShortDate($data['date']);
  ...
  /*  $client_file_name = $client_name_fn . '_gad7_' . $id . '.pdf' ; /* downloaded file name */
    $client_file_name = convert_safe_file_dir_name($client_name_fn . '_gad7_' . $id . '.pdf') ; /* downloaded file name using only legal characters*/

    if ($_SESSION['language_direction'] == 'rtl') {
        $mpdf->SetDirectionality('rtl');
    }
    $mpdf->WriteHTML('<head></head><body>'); /*include full web page tags - recommended by JP */
    $mpdf->WriteHTML('<p><h2 align=center>' . xlt("Generalised Anxiety Disorder 7 (GAD-7)") . '</h2></p>');
    $mpdf->WriteHTML('<p>' . xlt("Patient/Client") .': ' . text($client_name) . '</p>');
    $mpdf->WriteHTML('<p>' . xlt("Form completed on") . ': ' . text($formated_date) . '</p>');
    $mpdf->WriteHTML('<p><h3>' . xlt("The number of times client has been bothered by the following issues") . '</h3></p>');
    $mpdf->WriteHTML('<p>' . $str_nervous . ": " . $str_values [$data['nervous_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_control_worry . ": " . $str_values [$data['control_worry_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_worry . ": " . $str_values [$data['worry_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_relax . ": " . $str_values [$data['relax_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_restless . ": " . $str_values [$data['restless_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_annoyed . ": " .  $str_values [$data['irritable_score']] . '</p>');
    $mpdf->WriteHTML('<p>' . $str_afraid . ": " . $str_values [$data['fear_score']] . '</p>');
    $mpdf->WriteHTML('<hr><b>   ' . $str_total . ": " . text($gad7_total) . " - " . text($exp) . '</b><hr>');
    $mpdf->WriteHTML('<p><br>' . $str_q8 . ": " . $str_difficulty_values [$data['difficulty']]);
    $mpdf->WriteHTML($str_q8_2 .  '</p>');
    $mpdf->WriteHTML('</body>');

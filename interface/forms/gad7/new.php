...
function create_q8 ( question, menue){
 // create the 8th question - the second part is italicised. Only displayed if score > 0
       ...
       var question_2 = document.createTextNode('<?php echo ( $str_q8_2); ?>');
...
<input type="Submit" value="<?php echo xla('Save Form'); ?>" style="color: #483D8B" >
 &nbsp &nbsp
 <input type="button" value="<?php echo xla($str_nosave_exit);?>" onclick="top.restoreSession();return( nosave_exit());" style="color: #483D8B">
<br></br>
...
<select name="nervous_score" onchange="update_score(0, my_form.nervous_score.value);">
    <option selected value="undef"><?php echo xlt($str_default); ?></option>
    <option value="0"><?php echo xlt($str_not); ?></option>
    <option value="1"><?php echo xlt($str_several); ?></option>
    <option value="2"><?php echo xlt($str_more); ?></option>
    <option value="3"><?php echo xlt($str_nearly); ?></option>
    </select>
<br>
</br>
</tr>
...
function  check_all (){
   // has each question been answered and save scores
    ...
    if (flag) {
          ...
          alert("Please answer all of the questions: " + list + " are unanswered");
           return false;
     }
 //create a string that holds the array of scores - we're ok as each score <10 - use it to pass array to viewer
// although there must be a way to pass an array?
   //  for (i=0;  i<no_qs; i++){
...
var conf = confirm ( <?php echo '"' . xlt($str_nosave_confirm) . '"'; ?> );
//if (conf) alert("confirmation "+conf);
if (conf) {

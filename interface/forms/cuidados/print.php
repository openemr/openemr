...
<div class="page">
    ...
    foreach ($bool_fields as $label => $data) {
        echo $careRow($label, $data['val'], (string)$data['obs']);
    }
    ?>
    </tbody></table>
...
$mpdf->SetTitle(xlt('Nursing Care Bundle') . ' - ' . ($paciente !== false ? (string)($paciente['full_name'] ?? '') : ''));
$mpdf->WriteHTML((string)$html);
$filename = 'Cuidados_' . preg_replace('/\s+/', '_', $paciente !== false ? (string)($paciente['full_name'] ?? 'paciente') : 'paciente') . '_' . date('Ymd_His') . '.pdf';

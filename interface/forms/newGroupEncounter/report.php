...
function newGroupEncounter_report($group_id, $encounter, $cols, $id)
{
    ...
    while ($result = sqlFetchArray($res)) {
        print "<span class='fw-bold'>" . xlt('Facility') . ": </span><span class='text'>" . text($result["facility"]) . "</span><br />\n";
        if (AclMain::aclCheckCore('sensitivities', $result['sensitivity'])) {
            print "<span class='fw-bold'>" . xlt('Reason') . ": </span><span class='text'>" . nl2br(text($result["reason"])) . "</span><br />\n";
            $counselors = '';
            ...
            print "<span class='fw-bold'>" . xlt('Counselors') . ": </span><span class='text'>" . nl2br(text($counselors)) . "</span><br />\n";
        }
    }

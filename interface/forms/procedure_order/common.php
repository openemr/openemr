...
function getDetails(e, id) {
    ...
    url += "?type=codedetail)&code=" + encodeURIComponent(code) +
...
function createLabels(e, el) {
    e.preventDefault();
    let prmt = <?php echo xlj("How many sets of specimen labels to create? Each test in order gets a label.") ?>;
    ...
    let url = top.webroot_url + "/interface/procedure_tools/libs/labs_ajax.php";
    // this escapes above
    let uri = encodeURI("?type=label&count=" + count + "&order=" + order + "&pid=" + pid + "&acctid=" + acctid + "&patient=" + patient + "&specimen=" + tarray) + "&csrf_token_form=" + <?php echo js_url(CsrfUtils::collectCsrfToken()); ?>;

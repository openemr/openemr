...
$req_url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . '/controller.php?document&retrieve&patient_id=' . urlencode((string) $pid) . '&document_id=';
$reqStr = "";
...
if (($_POST['bn_save'] ?? null) || !empty($_POST['bn_xmit']) || !empty($_POST['bn_save_exit'])) {
    ...
    $log_file = \OpenEMR\Core\OEGlobalsBag::getInstance()->get("OE_SITE_DIR") . "/documents/labs/" . check_file_dir_name($lab_name) . "/logs/" . check_file_dir_name($formid) . "_order_log.log";
...
if (!empty($row['lab_id'])) {
    ...
    $log_file = \OpenEMR\Core\OEGlobalsBag::getInstance()->get("OE_SITE_DIR") . "/documents/labs/" . check_file_dir_name($lab_name) . "/logs/";
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', function () {
            if (oeUI.reasonCodeWidget) {
                oeUI.reasonCodeWidget.init(<?php echo js_url(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot')); ?>, <?php echo js_url(collect_codetypes("medical_problem", "csv")) ?>);
            } else {
                            ...
                            <div class="position-override mt-2">
                                ...
                                <div class="btn-group" role="group">
                                    ...
                                    <button type="button" class="btn btn-secondary btn-cancel"
                                        onclick="top.restoreSession();location='<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('form_exit_url'); ?>'"><?php echo xlt('Cancel/Exit'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>

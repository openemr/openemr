...
class C_EncounterVisitForm
{
    ...
    {
        // hate that we are relying on globals here... but leaving it alone for now.
        global $viewmode;
        ...
        if ($viewmode) {
            global $encounter;
            $def_facility = $encounter['facility_id'];
...
// Get facilities list
    function getFacilitiesForTemplate(FacilityService $facilityService, $viewmode, $encounter)
    {
        $facilities = $facilityService->getAllServiceLocations();
        $def_facility = $this->getDefinedFacility();

        return array_map(function ($facility) use ($def_facility) {
            return [
                'id' => $facility['id'],
                'name' => $facility['name'],
                'pos_code' => $facility['pos_code'],
                'selected' => ($def_facility == $facility['id'])
            ];
        }, $facilities);
    }
...
// Get sensitivity options
    function getSensitivitiesForTemplate()
    {
        global $viewmode, $encounter;
...
// Prepare data for template
        $issuesEnabled = $GLOBALS['enc_enable_issues'] && AclMain::aclCheckCore('patients', 'med', '', 'write');
        $issuesAuth = true;
...
//Note that for technical reasons, we are bypassing the standard validateUsingPageRules() call.
        ...
        if (!$viewmode) {
            ...
            $encounter = [
                ...
                ,'facility_id' => $this->getDefinedFacility()
        ...
        $facilities = $this->getFacilitiesForTemplate($facilityService, $viewmode, $encounter);
        $posCode = '';
...
// END AI GENERATED CODE
        ...
        $viewArgs = [
            ...
            'sensitivities' => $this->getSensitivitiesForTemplate(),
            ...
            'selectedFacilityId' => $this->getDefinedFacility(),

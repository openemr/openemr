...
function cancelClicked() {
 if (window.name == 'RBot') {
  parent.setRadio(window.name, 'ens');
  parent.loadFrame('ens1', window.name, 'patient_file/history/encounters.php');
 }
 else {
  parent.setRadio(window.name, 'dem');
  parent.loadFrame('dem1', window.name, 'patient_file/summary/demographics.php');
 }
 return false;
}

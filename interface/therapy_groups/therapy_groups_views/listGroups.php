    ...
    <!---------- TABLE SECTION -------------->
        <table id="therapy_groups_list" class="dataTable display">
            ...
            <?php foreach ($therapyGroups as $group) { ?>
                <tr>
                    <td><a href="<?php echo OEGlobalsBag::getInstance()->getKernel()->getRootDir() . '/therapy_groups/index.php?' . \OpenEMR\Common\Http\QueryString::buildUntyped(['method' => 'groupDetails', 'group_id' => $group['group_id']]); ?>"><?php echo text($group['group_name']);?></a></td>
                    ...
                    <td>
                        ...
                        if ($group['group_status'] == 10) { ?>
                            <a href="<?php echo OEGlobalsBag::getInstance()->getKernel()->getRootDir() . '/therapy_groups/index.php?' . \OpenEMR\Common\Http\QueryString::buildUntyped(['method' => 'listGroups', 'deleteGroup' => '1', 'group_id' => $group['group_id']]); ?>"><?php
                            if ($edit) { ?>

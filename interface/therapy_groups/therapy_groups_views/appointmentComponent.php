        ...
        <div class="col-md-12">
            <?php foreach ($events as $event) { ?>
                ...
                <div class="event_details">
                        <?php if ($edit) { ?>
                            <a onclick="goToEvent(<?php echo attr_js(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . "/main/calendar/add_edit_event.php?group=true&groupid=" . urlencode((string) $groupId) . "&date=" . urlencode((string) $date_for_url) . "&eid=" . urlencode((string) $event['pc_eid'])); ?>)">
                        <?php } ?>
                        ...
                        <span>
                            ...
                            if ($event['pc_recurrtype'] > 0) {
                                echo "<img src='" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/main/calendar/modules/PostCalendar/pntemplates/default/images/repeating8.png' border='0' style='margin:0px 2px 0px 2px;' title='" . xla("Repeating event") . "' alt='" . xla("Repeating event") . "'>";
                            }
...
<script>
    ...
    $('#addEvent').on('click', function(){
        top.restoreSession();
        var url = <?php echo js_escape(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir')); ?> + '/main/calendar/add_edit_event.php?group=true&groupid=' + <?php echo js_url($groupId); ?>;

            ...
            <div id="main-component" class="col-md-9 col-sm-12">
                <div class="row">
                    <div class="col-md-8 col-sm-12">
                        <ul class="tabNav">
                            <li><a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=groupDetails&group_id=' . attr_url($groupId); ?>"><?php echo xlt('General data');?></a></li>
                            <li class="current"><a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=groupParticipants&group_id=' . attr_url($groupId); ?>"><?php echo xlt('Participants ');?></a></li>
                        </ul>
                    </div>
                    <div class="col-md-4 col-sm-4">
                        <?php if ($edit) {?>
                            ...
                            <?php }?>
                            <?php if ($readonly == '') { ?>
                            <button class="btn btn-secondary" onclick="location.href='<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=groupParticipants&group_id=' . attr_url($groupId); ?>'"><?php echo xlt('Cancel');?></button>
                            <button class="btn btn-primary" id="saveForm"><?php echo xlt('Save');?></button>
                        <?php } else { ?>
                            <button class="btn btn-primary" onclick="location.href='<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=groupParticipants&editParticipants=1&group_id=' . attr_url($groupId); ?>'"><?php echo xlt('Update');?></button>
                        <?php } ?>
                      <?php } ?>
                    </div>
                </div>
              <div id="component-border">
                  <div class="row">
                      <form id="add-participant-form" name="add-participant-form" class="<?php echo isset($addStatus) ? 'showAddForm' : '' ?>" action="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=addParticipant&group_id=' . attr_url($groupId)?>" method="post">
                      ...
                      <table id="participants_table" class="dataTable display">
                          ...
                          <?php foreach ($participants as $i => $participant) { ?>
                              <tr>
                                  ...
                                  <?php if ($readonly == '') { ?>
                                      <td class="delete_btn">
                                          <a href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rootdir') . '/therapy_groups/index.php?method=groupParticipants&group_id=' . attr_url($groupId) . '&deleteParticipant=1&pid=' . attr_url($participant['pid']); ?>">&times;</a>
                                      </td>
                                  <?php } ?>
                              </tr>
                          <?php } ?>
...
<script>
    $(function () {
        ...
        $('#participant_name').on('click', function(){
            top.restoreSession();
            var url = <?php echo js_escape(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot')); ?> + '/interface/main/calendar/find_patient_popup.php';
    ...
    function refreshme() {
        top.restoreSession();
        location.href = <?php echo js_escape(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot')); ?> + '/interface/therapy_groups/index.php?method=groupParticipants&group_id=' + <?php echo js_url($groupId); ?>;
    }

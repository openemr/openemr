<?php
/**
* Maintenance for the list of laboratory providers.
*
...
<form method='post' name='theform' action='laboratory_edit.php?ppid=<?php echo attr($ppid) ?>'>
<center>
...
?>
 ...
 <tr>
  <td nowrap><b><?php echo xlt('Type'); ?>:</b></td>
  <td>
   <select name='form_type'>
<?php
$res = sqlStatement("SELECT * FROM list_options WHERE list_id LIKE ? ORDER BY seq",array("Provider_Type"));
while ($data = sqlFetchArray($res)) {
  echo "    <option value='" . attr($data['option_id']) . "'";
  if ($data['option_id'] == $row['type']) echo " selected";
  echo ">" . text($data['title']) . "</option>\n";
}
?>
   </select>
  </td>
...
<?php
$res = sqlStatement("SELECT * FROM list_options WHERE list_id LIKE ? ORDER BY seq",array("Provider_Protocol"));
while ($data = sqlFetchArray($res)) {
//foreach(array(
...
//{
  echo "    <option value='" . attr($data['option_id']) . "'";
  if ($data['option_id'] == $row['protocol']) echo " selected";
  echo ">" . text($data['title']) . "</option>\n";
}
?>
   </select>
...
<?php /* NEED THE PORT NUMBER TOO */ ?>
 <tr>
  <td nowrap><b><?php echo xlt('Remote Port'); ?>:</b></td>
  <td>
   <input type='text' size='10' name='form_remote_port' maxlength='255'
    value='<?php echo attr($row['remote_port']); ?>' class='inputtext' />
  </td>
 </tr>

                            ...
                            if ($form_vendor == '1598760985' || $form_vendor == '1235138868') {
                                if ($form_action == 1) { // load compendium
                                    ...
                                    while (!feof($fhcsv)) {
                                        ...
                                        if (empty($trow['procedure_type_id'])) {
                                                            sqlStatement(
                                                                ...
                                                                "parent = ?, name = ?, lab_id = ?, procedure_code = ?, procedure_type = ?, " .
                                                                "activity = 1",
                                                                array($form_group, trim((string) $acsv[1]), $lab_id, $ordercode, 'ord')
                                                            );
                                        } else {
                                                                sqlStatement(
                                                                    "UPDATE procedure_type SET " .
                                    ...
                                    while (!feof($fhcsv)) {
                                        ...
                                        // Figure out field type.
                                        $fldtype = 'T';
                                        if (strpos((string) $acsv[4], 'Drop') !== false) {
                                            $fldtype = 'S';
                                        } elseif (strpos((string) $acsv[4], 'Multiselect') !== false) {
                                            $fldtype = 'S';
                                        }
                                        ...
                                        if (empty($qrow['procedure_code'])) {
                                                    sqlStatement(
                                                        ...
                                                        array($lab_id, $pcode, $qcode, trim((string) $acsv[2]), $fldtype, $required, $options, $seq)
                                                    );
                                        } else {
                                            if ($qrow['activity'] == '1' && $qrow['options'] !== '' && $options !== '') {
                                                $options = $qrow['options'] . ';' . $options;
                                                    ...
                                                    sqlStatement(
                                                        ...
                                                        "lab_id = ? AND procedure_code = ? AND question_code = ?",
                                                        array(trim((string) $acsv[2]), $fldtype, $required, $options, $lab_id, $pcode, $qcode)
                                                    );
                                        }
                                    } // end while
                                } // end load questions

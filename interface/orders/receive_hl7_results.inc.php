...
function receive_hl7_results(&$hl7, &$matchreq, $lab_id = 0, $direction = 'B', $dryrun = false, $matchresp = null)
{
    ...
    if (empty($catrow['id'])) {
        return rhl7LogMsg(xl('Document category for lab results does not exist') .
            ': ' . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('lab_results_category_name'), true);
    } else {
    ...
    while ($pprow = sqlFetchArray($ppres)) {
        ...
        if ($protocol == 'SFTP') {
            ...
            foreach ($files as $file) {
                ...
                $prpath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/procedure_results";

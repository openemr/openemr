...
function collectorTotal(payDate, payCollector){
	...
	for(var i=1;i<ccc.rows.length;i++) {// make i = 1 to not count the thead row
		...
		if(trs.cells.length > 2 ){ // !!IMPORTANT: remember to look for cells that exist in a VALID row, adjust logic accordingly
			...
			if(trs.cells.length == 11 ){
				...
				var cellValMethod=trs.cells[10].firstChild.nodeValue;// changed on 1/27/2016 to accomodate for foreign languages that change value of Pmt Method Cell
			}
			else if(trs.cells.length > 11 ){
				var cellValAmount= parseFloat(trs.cells[10].firstChild.data);
				var cellValMethod=trs.cells[11].firstChild.nodeValue;// changed on 1/27/2016 to accomodate for foreign languages that change value of Pmt Method Cell
			}
	...
	modalHeader.innerHTML = payCollector + ' - ' + '{$lang['all_collections']}' + ' - ' + payDate ;
			
	modalBodyLine1Left.innerHTML = '{$lang['total_cash_collections']}:';
	modalBodyLine1Right.innerHTML = '{$lang['currency']}'+ cashTotal.toFixed(2);
	
	modalBodyLine2Left.innerHTML = '{$lang['total_check_collections']}:';
	modalBodyLine2Right.innerHTML = '{$lang['currency']}'+ checkTotal.toFixed(2);
	
	modalBodyLine3Left.innerHTML = '{$lang['total_credit_collections']}:';
	modalBodyLine3Right.innerHTML = '{$lang['currency']}'+ creditTotal.toFixed(2);
							
	modalBodyLine4Left.innerHTML = '{$lang['all_collections_by']} ' + payCollector + ' {$lang['for']} <span style="font-weight:bold">' + payDate +': </span> ';
	modalBodyLine4Right.innerHTML = '{$lang['currency']}'+ cellTotal.toFixed(2);

...
$lrArr = ['left', 'right'];
$logoarea .= (in_array(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('login_page_layout'), $lrArr)) ? " col-md-6" : " col-md-12";
$formarea .= (in_array(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('login_page_layout'), $lrArr)) ? " col-md-6" : " col-md-12";

// More finite control on a per-setting basis
switch (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('login_page_layout')) {
    case 'left':
        $logoarea .= " order-md-2";
        $formarea .= " order-md-1";
        break;
...
<form id="app_form" method="POST" autocomplete="off">
    <div class="<?php echo $loginrow; ?> card m-5">
        <div class="<?php echo attr($logoarea); ?>">
            <?php $extraLogo = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('extra_logo_login'); ?>
            <?php if ($extraLogo) { ?>
                <div class="text-center">
                    <span class="d-inline-block w-40">
                        <?php echo file_get_contents(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('images_static_absolute') . "/login-logo.svg"); ?>
                    </span>
            ...
            <?php } ?>
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('show_label_login')) { ?>
            ...
            $t1 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('tiny_logo_1');
            $t2 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('tiny_logo_2');
            if ($t1 && !$t2) {
                echo $tinylogocode1;
            } if ($t2 && !$t1) {
                echo $tinylogocode2;

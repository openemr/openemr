...
?>: <BR><?php echo text(basename($file)) ?><span class="msg" id="<?php echo attr($db); ?>_unsupportedmsg">!</span></div>
            <?php
            }
        }
    }
...
?><span class="msg" id="<?php echo attr($db); ?>_dirmsg">!</span></div>
    <?php
}
if (count($files_array) === 0) {
    ?>
   <div class="error_msg"><?php echo xlt("No files staged for installation");
?><span class="msg" id="<?php echo attr($db); ?>_msg">!</span></div>
  ...
  // If new version is being offered, then provide install/upgrade options
    if ($success_flag === 1) {
        ...
        if ($installed_flag === 1) {
            if ($current_name=="SNOMED" && $current_version!=$file_revision && $file_revision!="US Extension" && $current_version=="US Extension") {
                ...
                // a complete SNOMED pacakge.
                ?>
                <div class="stg"><?php echo text(basename($file_revision_path));
    ?> <?php echo xlt("is a different version of the following database") . ": " . text($db); ?></div>
            <?php
            $action=xl("REPLACE");
                ...
                ?>
                <div class="stg"><?php echo text(basename($file_revision_path));
    ?> <?php echo xlt("is a different language version of the following database") . ": " . text($db); ?></div>
            <?php
            $action=xl("REPLACE");
    ...
    ?><span class="msg" id="<?php echo attr($db); ?>_instrmsg">?</span></div>
            <?php
                ...
                ?>
                <div class="stg"><?php echo text(basename($file_revision_path));
    ?> <?php echo xlt("is an extension of the following database") . ": " . text($db); ?></div>
            <?php
            $action=xl("UPGRADE");
...
?><span class="msg" id="<?php echo attr($db); ?>_instrmsg">?</span></div>
            <?php
                ...
                ?>
                <div class="stg"><?php echo text(basename($file_revision_path));
    ?> <?php echo xlt("is a more recent version of the following database") . ": " . text($db); ?></div>
            <?php
            $action=xl("UPGRADE");
            }
        } else {
    ...
    ?><span class="msg" id="<?php echo attr($db); ?>_instrmsg">?</span></div>
        ...
        if (strlen($action) > 0) {
        ?>
        <input id="<?php echo attr($db);
    ?>_install_button" version="<?php echo attr($file_revision);

...
function toencounter(pid, pubpid, pname, enc, datestr, dobstr) {
    top.restoreSession();
    encurl = 'patient_file/encounter/encounter_top.php?set_encounter=' + enc +
        '&pid=' + pid;
...
function topatient(pid, pubpid, pname, enc, datestr, dobstr) {
    ...
    parent.left_nav.loadFrame('ens1', 'enc',
        'patient_file/history/encounters.php?pid=' + pid);
...
function popUB04(pid, enc) {
    ...
    top.restoreSession();
    var href =
        "<?php echo $GLOBALS['web_root']?>/interface/billing/ub04_form.php?pid=" +
        pid + "&enc=" + enc
    var h = (screen.height - 130).toString();
    window.open(href, '',
        'location=0,scrollbars=yes,centerscreen=yes,width=1200,height=' + h +
        '');
    //window.open(href);
    return true;
}
    ...
    <div class="container">
        <div class="row">
            <div class="page-header">
                <h2><?php echo xlt('Billing Manager') ?> &nbsp <i id="show_hide" class="fa fa-eye-slash fa-2x small" title="<?php echo xla('Click to Hide'); ?>"></i></h2>
            </div>
        </div>
        <div class="row">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group col-xs-12">
                            ...
                            <div  class= 'text'>
                                &nbsp;
                                <ul>
                                   <li><a class="link_submit"  href="#" onclick="javascript:return SubmitTheScreen();"><strong><?php echo xlt('Update List') ?></strong> </a><i id='update-tooltip' class="fa fa-info-circle text-primary" aria-hidden="true"></i></li>
                                    <li><a class='link_submit'  href="#" onclick="javascript:return SubmitTheScreenExportOFX();"><strong><?php echo  xlt('Export OFX'); ?></strong></a></li>
                                    <li><a class='link_submit' href="#" onclick="javascript:return SubmitTheScreenPrint();"><strong><?php echo xlt('View Printable Report'); ?></strong></a></li>
                                    <span >
                                    <?php if ($daysheet) { ?>
                                        <li><a class='link_submit'  href="#" onclick="javascript:return SubmitTheEndDayPrint();"><strong><?php echo xlt('End Of Day Report').' - ' ?></strong></a>
                                        ...
                                        <?php } ?> 
                                        <?php if ($provider_run) {
?> <span class="text"><strong><?php echo xlt('Provider'); ?></strong></span><input name="end_of_day_provider_only" type="checkbox" value="1"></li> <?php
} ?>
                                    <?php } ?>
                                    </span> 
                                    <?php if (! file_exists($EXPORT_INC)) { ?>
                ...
                <div class="form-group clearfix">
                    <div class="col-xs-12">
                        <div class=" col-xs-8 btn-group btn-group-pinch position-override" role="group">
                            <div class = "btn-group">
                               <button type = "button" class = "btn btn-default dropdown-toggle" data-toggle = "dropdown" name="bn_x12_support"
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    <?php if (file_exists($EXPORT_INC)) { ?>
                                    <li><button type="submit" data-open-popup="true" class="btn btn-default btn-download"name="bn_external"
                                         title="<?php echo xla('Export to external billing system') ?>"value="<?php echo xla("Export Billing")?>"><?php echo xla("Export Billing")?></button></li>
                                    <li><button type="submit" data-open-popup="true" class="btn btn-default btn-download"name="bn_mark"
                                         title="<?php echo xla('Mark as billed but skip billing') ?>"><?php echo xla("Mark as Cleared")?></button></li>
                                    <?php } else { ?>
                                    <li><button type="submit" class="btn btn-default btn-download"name="bn_x12" onclick="MarkAsCleared(1)"
                                         title="<?php echo xla('Generate and download X12 batch')?>"><?php echo xla('Generate X12')?></button></li>
                                    <?php } ?>
                                    <?php if ($GLOBALS['ub04_support']) { ?>
                                    <li><button type="submit" class="btn btn-default btn-download"name="bn_ub04_x12"
                                        title="<?php echo xla('Generate Institutional X12 837I')?>"><?php echo xla('Generate X12 837I')?></button></li>
                                    <?php } ?>
                                    <?php if ($GLOBALS['support_encounter_claims']) { ?>
                                    <li><button type="submit" class="btn btn-default btn-download"name="bn_x12_encounter" onclick="MarkAsCleared(1)"
                                        title="<?php echo xla('Generate and download X12 encounter claim batch')?>" ><?php echo xla('Generate X12 Encounter')?></button></li>
                                    <?php } ?>
                                </ul>
                            </div>
                            <div class = "btn-group">
                               <button type = "button" class = "btn btn-default dropdown-toggle" data-toggle = "dropdown" name="bn_process_hcfa_support"
                                    title="<?php echo xla('A claim must be selected to enable this menu.')?>"><?php echo xla('HCFA FORM')?> 
                                <span class="caret"></span></button>
                                <ul class="dropdown-menu" role="menu">
                                    <li><button type="submit" class="btn btn-default btn-download"name="bn_process_hcfa" onclick="MarkAsCleared(2)"
                                        title="<?php echo xla('Generate and download CMS 1500 paper claims')?>"><?php echo xla('CMS 1500 PDF')?></button></li>
                                    <?php if ($GLOBALS['preprinted_cms_1500']) { ?>
                                    <li><button type="submit" class="btn btn-default btn-download"name="bn_process_hcfa_form" onclick="MarkAsCleared(2)"
                                    ...
                                    <?php } ?>
                                    <li><button type="submit" class="btn btn-default btn-download" name="bn_hcfa_txt_file" onclick="MarkAsCleared(3)"
                            ...
                            <?php if ($GLOBALS['ub04_support']) { ?>
                            <div class = "btn-group">
                                <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" name="bn_process_ub04_support"
                                    title="<?php echo xla('A claim must be selected to enable this menu.')?>"><?php echo xla('UB04 FORM')?> <span class="caret"></span></button>
                                <ul class="dropdown-menu" role="menu">
                                    <li><button type="submit" class="btn btn-default btn-download" name="bn_process_ub04_form"
                                        title="<?php echo xla('Generate and download UB-04 CMS1450 with form')?>"><?php echo xla('UB04 FORM PDF')?></button></li>
                                    <li><button type="submit" class="btn btn-default btn-download" name="bn_process_ub04"

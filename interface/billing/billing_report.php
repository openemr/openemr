...
<script type="text/javascript"
    src="../../library/js/common.js?v=&lt;?php echo $v_js_includes; ?&gt;"></script>
<style type="text/css">
#ajax_div_insurance {
    position: absolute;
    z-index: 10;
...
input[type="submit"].subbtn-warning {
    background-color: #ec971f !important;
    color: black !important;
}

input[type="submit"].subbtn-warning:hover {
    background-color: #da8104 !important;
    color: #fff !important;
}
</style>
<script type="text/javascript">
...
<body class="body_top" onLoad="TestExpandCollapse()">
    ...
    <form name='the_form' method='post' action='billing_report.php'
        onsubmit='return top.restoreSession()' style="display: inline">

        <script type="text/javascript"
            src="../../library/dialog.js?v=&lt;?php echo $v_js_includes; ?&gt;"></script>
        <script type="text/javascript"
            src="../../library/textformat.js?v=&lt;?php echo $v_js_includes; ?&gt;"></script>
        <script type="text/javascript"
...
?>
<table
            style="width: 100%; border-collapse: collapse; background: #f9f9ff;">
            <tr class="hideAway">
                <td width="25%">&nbsp;</td>
                <td width="35%">
...
?>
      <table class="pull-left"
                        ...
                        <tr>
                            <td>&nbsp;</td>
                            <td width="100%"><span class='text'><a
                                    onClick="javascript:return SubmitTheScreen();" href="#"
                                    class='css_button_small btn btn-xs'><?php echo xlt('Update List') ?></a>
                                    <a onClick="javascript:return SubmitTheScreenExportOFX();"
                                    href="#" class='css_button_small btn btn-xs'><?php echo xlt('Export OFX') ?></a></span>
                            </td>
                        </tr>
                        <tr>
                            <td>&nbsp;</td>
                            <td><a onClick="javascript:return SubmitTheScreenPrint();"
                                href="#" class='css_button_small btn btn-xs'><?php echo xlt('View Printable Report') ?></a></td>
                        </tr>
        <?php if ($daysheet) { ?>
          <tr>
                            <td>&nbsp;</td>
                            <td><a onClick="javascript:return SubmitTheEndDayPrint();"
                                href="#" class='css_button_small btn btn-xs'><?php echo xlt('End Of Day Report') ?></a>
    <?php if ($daysheet_total) { ?>
        <span class="text"><?php echo xlt('Totals'); ?> </span> <input
                                type="checkbox" name="end_of_day_totals_only" value="1"
                                <?php if ($obj['end_of_day_totals_only'] === '1') echo "checked";?>>
    <?php } ?>
    <?php if ($provider_run) { ?>
        <span class="text"><?php echo xlt('Provider'); ?> </span> <input
                                type="checkbox" name="end_of_day_provider_only" value="1"
                                <?php if ($obj['end_of_day_provider_only'] === '1') echo "checked";?>>
    <?php } ?>
    </td>
                        </tr>
        <?php } ?>
          <tr>
            ...
            <?php if (! file_exists($EXPORT_INC)) { ?>
               ...
               &nbsp;
               --> <a href='#' id="view-log-link"
                                class='css_button_small btn btn-xs'
            ...
            <?php } ?>
                        ...
                        <tr>
                            <td>&nbsp;</td>
                            <td><a href="javascript:select_all()"
                                class="css_button_small btn btn-xs"><?php  echo xlt('Select All') ?></a></td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <td><button type="button" class="btn btn-xs btn-danger"
                        onclick="$('.hideAway').toggle()"><?php echo xlt("Criteria Toggle") ?></button></td>
            </tr>
        </table>
    </form>
    <form name='update_form' method='post' action='billing_process.php'
        onsubmit='return top.restoreSession()' style="display: inline">
        <span style="text-align: center"> <span class='text'
            style="display: inline">
<?php if (file_exists($EXPORT_INC)) { ?>
<input type="submit" data-open-popup="true" class="subbtn"
                name="bn_external" value="Export Billing"
                title="&lt;?php echo xla('Export to external billing system') ?&gt;">
                <input type="submit" data-open-popup="true" class="subbtn"
                name="bn_mark" value="Mark as Cleared"
                title="&lt;?php echo xla('Mark as billed but skip billing') ?&gt;">
<?php } else { ?>
<input type="submit" class="subbtn" name="bn_x12"
                value="&lt;?php echo xla('Generate X12')?&gt;"
                title="&lt;?php echo xla('Generate and download X12 batch')?&gt;"
                onclick="MarkAsCleared(1)">
<?php if ($GLOBALS['support_encounter_claims']) { ?>
<input type="submit" class="subbtn" name="bn_x12_encounter"
                value="&lt;?php echo xla('Generate X12 Encounter')?&gt;"
                title="&lt;?php echo xla('Generate and download X12 encounter claim batch')?&gt;"
                onclick="MarkAsCleared(1)">
<?php } ?>
<input type="submit" class="subbtn" name="bn_process_hcfa"
                value="&lt;?php echo xla('CMS 1500 PDF')?&gt;"
                title="&lt;?php echo xla('Generate and download CMS 1500 paper claims')?&gt;"
                onclick="MarkAsCleared(2)">
    <?php if ($GLOBALS['preprinted_cms_1500']) { ?>
<input type="submit" class="subbtn" name="bn_process_hcfa_form"
                value="&lt;?php echo xla('CMS 1500 incl FORM')?&gt;"
                title="&lt;?php echo xla('Generate and download CMS 1500 paper claims on Preprinted form')?&gt;"
                onclick="MarkAsCleared(2)">
    <?php } ?>
<input type="submit" class="subbtn" name="bn_hcfa_txt_file"
                value="&lt;?php echo xla('CMS 1500 TEXT')?&gt;"
                title="&lt;?php echo xla('Making batch text files for uploading to Clearing House and will mark as billed')?&gt;"
                onclick="MarkAsCleared(3)"> <input type="submit"
                data-open-popup="true" class="subbtn-warning" name="bn_mark"
                value="&lt;?php echo xla('Mark as Cleared')?&gt;"
                title="&lt;?php echo xla('Post to accounting and mark as billed')?&gt;">
                <input type="submit" data-open-popup="true" class="subbtn-warning"
                name="bn_reopen" value="&lt;?php echo xla('Re-Open')?&gt;"
...
</span> <input type='hidden' name='HiddenMarkAsCleared'
            id='HiddenMarkAsCleared' value="" /> <input type='hidden' name='mode'
            value="bill" /> <input type='hidden' name='authorized'
            value="&lt;?php echo attr($my_authorized); ?&gt;" /> <input
            type='hidden' name='unbilled'
            value="&lt;?php echo attr($unbilled); ?&gt;" /> <input type='hidden'
            name='code_type' value="%" /> <input type='hidden' name='to_date'
            value="&lt;?php echo attr($to_date); ?&gt;" /> <input type='hidden'
            name='from_date' value="&lt;?php echo attr($from_date); ?&gt;" />
...
if ($ret = getBillsBetween("%")) {
    if (is_array($ret)) {
        ?>
<tr>
                <td colspan='9' align="right"><table
                        style="width: 250px; border-collapse: collapse;">
                        <tr>
                            <td width="100" id='ExpandAll'><a
                                onclick="expandcollapse('expand');" class='small'
                                href="JavaScript:void(0);"><?php echo '('.htmlspecialchars( xl('Expand All'), ENT_QUOTES).')' ?></a></td>
                            <td width="100" id='CollapseAll'><a
                                onclick="expandcollapse('collapse');" class='small'
                                href="JavaScript:void(0);"><?php echo '('.htmlspecialchars( xl('Collapse All'), ENT_QUOTES).')' ?></a></td>
                            <td width="50">&nbsp;</td>
                        </tr>
                    </table></td>
            </tr>
...
$(document).ready(function() {
    ...
    <input type="hidden" name="divnos" id="divnos"
        value="&lt;?php echo attr($divnos) ?&gt;" />

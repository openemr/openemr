...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_custom_daysheet') != 0) {
    $daysheet = true;
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('daysheet_provider_totals') == 1) {
        $daysheet_total = true;
        $provider_run = false;
    }
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('daysheet_provider_totals') == 0) {
        $daysheet_total = false;
        $provider_run = true;
    }
}
...
$left_margin = $_POST["left_margin"] ?? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('cms_left_margin_default');
$top_margin = $_POST["top_margin"] ?? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('cms_top_margin_default');
...
if ($ub04_support) {
    $left_ubmargin = $_POST["left_ubmargin"] ?? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('left_ubmargin_default');
    $top_ubmargin = $_POST["top_ubmargin"] ?? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('top_ubmargin_default');
}
...
<head>
    ...
    <script>
        ...
        function set_button_states() {
            ...
            f.bn_x12_support.disabled = !can_generate;
                <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('support_encounter_claims')) { ?>
            f.bn_x12_encounter.disabled = !can_generate;
            <?php } ?>
            f.bn_process_hcfa_support.disabled = !can_generate;
                <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('preprinted_cms_1500')) { ?>
            f.bn_process_hcfa_form.disabled = !can_generate;
            <?php } ?>
                <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ub04_support')) { ?>
        ...
        function toEncounter(newpid, enc) {
            ...
            top.RTop.location = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/patient_file/summary/demographics.php?" + params;
        }
        ...
        function popMBO(pid, enc, mboid) {
            ...
            const href = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root')?>/interface/patient_file/encounter/view_form.php?" + params;
        ...
        function popUB04(pid, enc) {
            ...
            const href = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root')?>/interface/billing/ub04_form.php?" + params;
        ...
        function SubmitTheEndDayPrint() { //Action on View End of Day Report link
            ...
            top.restoreSession();
            document.the_form.target = 'new';
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_custom_daysheet') == 1) { ?>
            document.the_form.action = 'print_daysheet_report_num1.php';
            <?php } ?>
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_custom_daysheet') == 2) { ?>
            document.the_form.action = 'print_daysheet_report_num2.php';
            <?php } ?>
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_custom_daysheet') == 3) { ?>
    ...
    <?php require_once \OpenEMR\Core\OEGlobalsBag::getInstance()->get('srcdir') . "/ajax/payment_ajax_jav.inc.php"; ?>
    <style>
        #ajax_div_insurance {
            position: absolute;
            z-index: 10;
                ...
                <table class="table table-sm">
                    ...
                    <script>
                        ...
                        EncounterNoteArray[<?php echo attr($iter['enc_pid']); ?>] = new Array;
                                ...
                                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('notes_to_display_in_Billing') == 2 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('notes_to_display_in_Billing') == 3) {
                                ...
                                if ($iter['id']) {
                                    ...
                                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('notes_to_display_in_Billing') == 1 || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('notes_to_display_in_Billing') == 3) {
                            ...
                            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('display_units_in_billing') != 0) {
                                if ($iter['id']) {

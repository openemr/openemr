...
if (isset($_FILES) && !empty($_FILES)) {
    ...
    if (!isset($message)) {
        ...
        if (OEGlobalsBag::getInstance()->getBoolean('drive_encryption')) {
            $uploadedFile = $cryptoGen->encryptStandard($uploadedFile !== false ? $uploadedFile : null, null, KeySource::Database);
        }
        if (file_put_contents($target, $uploadedFile)) {
            ...
            if ($cryptoGen->cryptCheckStandard($Response271 !== false ? $Response271 : null)) {
                $Response271 = $cryptoGen->decryptStandard($Response271 !== false ? $Response271 : null, null, KeySource::Database);
            }
            if ($Response271) {

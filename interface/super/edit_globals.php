...
function checkCreateCDB()
{
  ...
  ('couchdb_host','couchdb_user','couchdb_pass','couchdb_port','couchdb_dbase','document_storage_method')");
    ...
    while ($globalsrow = sqlFetchArray($globalsres)) {
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set($globalsrow['gl_name'], $globalsrow['gl_value']);
    }

    $directory_created = false;
    if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('document_storage_method'))) {
        // /documents/temp/ folder is required for CouchDB
        if (!is_dir(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/temp/')) {
            $directory_created = mkdir(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/temp/', 0777, true);
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_host') || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_port') || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_dbase')) {
            $couch->createDB();
        }
    }
...
function checkBackgroundServices(): void
{
    ...
    $phimail_active = empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_enable')) ? '0' : '1';
    $phimail_interval = max(0, (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_interval'));
    ...
    $auto_sftp_x12 = empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('auto_sftp_claims_to_x12_partner')) ? '0' : '1';
     ...
     */
    $wenoservices = (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('weno_rx_enable') ?? '') == 1 ? '1' : '0';
...
<head>
    ...
    if (array_key_exists('form_save', $_POST) && $_POST['form_save'] && !$userMode) {
        ...
        $auditLogStatusFieldOld = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_auditlog');
        $forceBreakglassLogStatusFieldOld = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_force_log_breakglass');

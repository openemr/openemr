...
function fuzzyRename($from, $to): void
{
    ...
    sqlStatement($query, array($to, strlen((string) $from) + 1, $layout_id, "$from%"));
...
function tableNameFromLayout($layout_id)
{
    ...
    if (substr((string) $layout_id, 0, 3) == 'LBF' || substr((string) $layout_id, 0, 3) == 'LBT' || $layout_id == "FACUSR") {
        return '';
    }
    if ($layout_id == "DEM") {
        $tablename = "patient_data";
    } elseif (substr((string) $layout_id, 0, 3) == "HIS") {
...
function addOrDeleteColumn($layout_id, $field_id, $add = true): void
{
    ...
    if ($add && !$column_exists) {
        ...
        if (!isset($tmp['field_id']) && !isColumnReserved($tablename, $field_id)) {
            ...
            if (substr((string) $layout_id, 0, 3) == 'HIS') {
                $lotmp = sqlQuery(
...
$lbfonly = substr((string) $layout_id, 0, 3) == 'LBF' ? "" : "style='display:none;'";
...
<?php if (substr((string) $layout_id, 0, 3) == 'LBF') { ?>

...
{
    $query =
        "UPDATE layout_group_properties SET grp_last_update = CURRENT_TIMESTAMP " .
...
$layouts = [];
collectLayoutNames(
    "grp_form_id NOT LIKE 'LBF%' AND grp_form_id NOT LIKE 'LBT%'",
    xl("Core"),
);
...
function genGroupId($parent)
{
    ...
    while ($result = sqlFetchArray($results)) {
        $tmp = substr(
            (string) $result["grp_group_id"],
            strlen((string) $parent),
            1,
        );
        if ($tmp >= $maxnum) {
...
function fuzzyRename($from, $to): void
{
    ...
    $query =
        "UPDATE layout_options SET group_id = concat(?, substr(group_id, ?)) " .
        "WHERE form_id = ? AND group_id LIKE ?";
    sqlStatement($query, [
        $to,
        strlen((string) $from) + 1,
        $layout_id,
        "$from%",
    ]);

    $query =
        "UPDATE layout_group_properties SET grp_group_id = concat(?, substr(grp_group_id, ?)) " .
...
function tableNameFromLayout($layout_id)
{
    ...
    if (
        str_starts_with((string) $layout_id, "LBF") ||
        str_starts_with((string) $layout_id, "LBT") ||
        $layout_id == "FACUSR"
    ) {
        return "";
    }
    if ($layout_id == "DEM") {
        ...
        die(
            xlt("Internal error in tableNameFromLayout") .
                "(" .
                text($layout_id) .
                ")"
        );
...
function isColumnReserved($tablename, $field_id)
{
    if ($tablename == "patient_data") {
        ...
        if (in_array($field_id, ["id", "date", "pid"])) {
            return true;
        }
    }
    return false;
...
function addOrDeleteColumn($layout_id, $field_id, $add = true): void
{
    ...
    if ($add && !$column_exists) {
        sqlStatement(
            "ALTER TABLE `" .
                escape_table_name($tablename) .
                "` ADD `" .
                escape_identifier($field_id, "a-zA-Z0-9_", true) .
                "` TEXT",
        );
        ...
        $tmp = sqlQuery(
            "SELECT `" .
                escape_sql_column_name($field_id, [$tablename]) .
                "` AS field_id FROM `" .
                escape_table_name($tablename) .
                "` WHERE " .
                "`" .
        ...
        if (
            !isset($tmp["field_id"]) &&
            !isColumnReserved($tablename, $field_id)
        ) {
            $lotmp = [];
...
function renameColumn($layout_id, $old_field_id, $new_field_id)
{
    ...
    $colarr = sqlQuery(
        "SHOW COLUMNS FROM `" .
            escape_table_name($tablename) .
            "` LIKE '" .
            add_escape_custom($old_field_id) .
            "'",
    );
    if (empty($colarr)) {
    ...
    $tmp = sqlQuery(
        "SHOW COLUMNS FROM `" .
            escape_table_name($tablename) .
            "` LIKE '" .
            add_escape_custom($new_field_id) .
            "'",
    );
    if (!empty($tmp)) {
    ...
    $query =
        ...
        escape_table_name($tablename) .
        "` CHANGE `" .
        escape_sql_column_name($old_field_id, [$tablename]) .
        "` `" .
        escape_identifier($new_field_id, "a-zA-Z0-9_", true) .
        "` $colstr";
    sqlStatement($query);
    $session = SessionWrapperFactory::getInstance()->getActiveSession();
    EventAuditLogger::getInstance()->newEvent(
...
if (!$thisauth) {
    AccessDeniedHelper::denyWithTemplate(
        "ACL check failed for admin/super: Layout Editor",
        xl("Layout Editor"),
    );
}
...
$layout_id = empty($_REQUEST["layout_id"])
    ? ""
    : (string) $_REQUEST["layout_id"];
...
if (
    !empty($_POST["formaction"]) &&
    $_POST["formaction"] == "save" &&
    $layout_id
) {
    if (
        !CsrfUtils::verifyCsrfToken(
            $_POST["csrf_token_form"],
            session: $session,
        )
    ) {
    ...
    for ($lino = 1; isset($fld[$lino]["id"]); ++$lino) {
        ...
        $listval =
            $data_type == 34
                ? trim((string) $iter["contextName"])
                : trim((string) $iter["list_id"]);
        $action = $iter["action"] ?? "";
        ...
        for (; !empty($iter["condition_id"][$cix]); ++$cix) {
            $andor = empty($iter["condition_andor"][$cix])
                ? ""
        ...
        if ($field_id) {
            if ($field_id != $field_id_original) {
                if (
                    renameColumn($layout_id, $field_id_original, $field_id) > 0
                ) {
    ...
    $listval =
        $data_type == 34
            ? trim((string) $_POST["contextName"])
            : trim((string) $_POST["newlistid"]);
    // AI/Claude Code change: refactored to use query parameters
    sqlStatement(
    ...
    ($_POST["formaction"] ?? "") == "copytolayout" &&
    $layout_id &&
    !empty($_POST["targetlayout"])
) {
    ...
    foreach (explode(" ", (string) $_POST["selectedfields"]) as $onefield) {
        ...
        if (!empty($trow)) {
            echo "<!-- Field '" .
                text($onefield) .
                "' already exists in layout '" .
                text($tlayout) .
                "'. -->\n";
            continue;
        }
        $qstr =
            "INSERT INTO layout_options SET `form_id` = ?, `field_id` = ?, `group_id` = ?";
    ...
    foreach (explode(" ", (string) $_POST["selectedfields"]) as $onefield) {
        if (
            !isColumnReserved(
                tableNameFromLayout($_POST["layout_id"]),
                $onefield,
            )
        ) {
    ...
    if (!empty($fieldsToDelete)) {
        $placeholders = implode(
            ", ",
            array_fill(0, count($fieldsToDelete), "?"),
        );
    ...
    sqlStatement(
        "DELETE FROM layout_options WHERE " . " form_id = ? AND group_id = ?",
        [$_POST["layout_id"], $_POST["deletegroupid"]],
    );
    sqlStatement(
    ...
    $query =
        "UPDATE layout_group_properties SET " .
        "grp_group_id = ?, grp_title = ? " .
        "WHERE grp_form_id = ? AND grp_group_id = ?";
    sqlStatement($query, [
        $newid,
        $_POST["renamegroupname"],
        $layout_id,
        $oldid,
    ]);
}
...
function genFieldOptionList($current = "")
{
    global $layout_id;
    $option_list =
        "<option value=''>-- " . xlt("Please Select") . " --</option>";
    if ($layout_id) {
        $query =
            "SELECT field_id FROM layout_options WHERE form_id = ? ORDER BY group_id, seq";
...
function writeFieldLine($linedata): void
{
    ...
    echo "<input type='text' name='fld[" .
        attr($fld_line_no) .
        "][seq]' id='fld[" .
        attr($fld_line_no) .
        "][seq]' value='" .
        attr($linedata["seq"]) .
        "' size='2' maxlength='4' class='form-control form-control-sm optin' />";
    echo "</td></div>\n";

    echo "  <td class='text-center optcell' $lbfonly>";
    echo "<select name='fld[" .
        attr($fld_line_no) .
        "][source]' class='form-control form-control-sm optin' $lbfonly>";
    ...
    echo "<input type='text' name='fld[" .
        ...
        "][id]' value='" .
        attr($linedata["field_id"]) .
        "' size='15' maxlength='31' " .
        "class='form-control form-control-sm optin' onclick='FieldIDClicked(this)' />";
    echo "</td>\n";

    echo "  <td class='text-center optcell'>";
    echo "<input type='text' id='fld[" .
        attr($fld_line_no) .
        "][title]' name='fld[" .
        attr($fld_line_no) .
        "][title]' value='" .
        attr($linedata["title"]) .
        "' size='15' maxlength='3000' class='form-control form-control-sm optin' />";
    echo "</td>\n";
    ...
    if ($GLOBALS["translate_layout"] && $session->get("language_choice") > 1) {
        echo "<td class='text-center translation'>" .
            xlt($linedata["title"]) .
            "</td>\n";
    }
    ...
    echo "<select name='fld[" .
        attr($fld_line_no) .
        "][uor]' class='form-control form-control-sm optin'>";
    ...
    echo "  <td class='text-center optcell'>";
    echo "<select class='form-control form-control-sm' name='fld[" .
        attr($fld_line_no) .
        "][datatype]' id='fld[" .
        attr($fld_line_no) .
        "][datatype]' onchange=NationNotesContext(" .
        attr_js($fld_line_no) .
        ",this.value)>";
    ...
    if (
        ...
        echo "<input type='text' name='fld[" .
            ...
            "' size='2' maxlength='10' class='form-control form-control-sm optin' title='" .
            xla("Width") .
            "' />";
    ...
    echo "<input type='text' name='fld[" .
        ...
        "title='" .
        xla("Maximum Size (entering 0 will allow any size)") .
        "' />";
    echo "</td>\n";
    ...
    if (
        ...
        echo "<select class='form-control form-control-sm' name='fld[" .
            ...
            ">";
        $res = sqlStatement(
            "SELECT * FROM customlists WHERE cl_list_type=2 AND cl_deleted=0",
        );
        while ($row = sqlFetchArray($res)) {
            $sel = "";
    ...
    //Backup List Begin
    echo "  <td class='text-center optcell'>";
    if (in_array($linedata["data_type"], [1, 26, 33, 36, 43, 46])) {
        echo "<input type='text' name='fld[" .
            attr($fld_line_no) .
            "][list_backup_id]' value='" .
    ...
    echo "<input type='text' name='fld[" .
        attr($fld_line_no) .
        "][titlecols]' value='" .
        attr($linedata["titlecols"]) .
        "' size='3' maxlength='10' class='form-control form-control-sm optin' />";
    echo "</td>\n";
    ...
    echo "<select id='fld[" .
        attr($fld_line_no) .
        "][edit_options]' name='fld[" .
        attr($fld_line_no) .
        "][edit_options][]' class='typeAddons optin' size='3' multiple data-set='" .
        attr(trim((string) $linedata["edit_options"])) .
        "' ></select></td>\n";

    if ($linedata["data_type"] == 31) {
        echo "  <td class='text-center optcell'>";
        echo "<textarea name='fld[" .
            attr($fld_line_no) .
            "][desc]' rows='3' cols='35' class='form-control form-control-sm optin'>" .
            text($linedata["description"]) .
            "</textarea>";
        echo "<input type='hidden' name='fld[" .
            attr($fld_line_no) .
            "][default]' value='" .
        ...
        echo "<input type='text' name='fld[" .
            ...
            "' size='20' class='form-control form-control-sm optin' />";
        echo "<input type='hidden' name='fld[" .
            attr($fld_line_no) .
            "][default]' value='" .
            attr($linedata["default_value"]) .
        ...
        if (
            $GLOBALS["translate_layout"] &&
            $session->get("language_choice") > 1
        ) {
            echo "<td class='text-center translation'>" .
                xlt($linedata["description"]) .
                "</td>\n";
        }
    }
    echo "  <td class='text-center optcell'>";
    echo "<input type='text' name='fld[" .
        ...
        "' title='" .
        xla("Code(s)") .
        "' onclick='select_clin_term_code(this)' size='10' maxlength='255' class='form-control form-control-sm optin' />";
    echo "</td>\n";

    // The "?" to click on for yet more field attributes.
    echo "  <td class='font-weight-bold' id='querytd_" .
        attr($fld_line_no) .
    ...
    $extra_html .=
        ...
        xlt("For") .
        " " .
        text($linedata["field_id"]) .
        " " .
        "<select class='form-control form-control-sm' name='fld[" .
        attr($fld_line_no) .
        "][action]' onchange='actionChanged(" .
        attr_js($fld_line_no) .
        ")'>" .
        "<option value='skip'  " .
        ...
        "</option>" .
        "<option value='hsval' " .
        ($action == "hsval" ? "selected" : "") .
        ">" .
        xlt("hide else set to") .
        "</option>" .
        "</select>" .
        "<input type='text' class='form-control form-control-sm' name='fld[" .
        attr($fld_line_no) .
        "][value]' value='" .
        attr($action_value) .
        "' size='15' />" .
    ...
    foreach ($conditions as $i => $condition) {
        ...
        $extra_html .=
            ...
            attr_js($i) .
            ")'>" .
            genFieldOptionList($condition["id"]) .
            " </select>\n" .
            "  </td>\n" .
            "  <td class='text-left'>\n" .
        ...
        if (!isset($conditions[$i + 1])) {
            $extra_html .=
                ...
                xla("Add a condition") .
                "'>\n" .
                "   <input type='button' class='btn btn-primary btn-sm' value='+' onclick='extAddCondition(" .
                attr_js($fld_line_no) .
                ",this)' />\n" .
                "  </td>\n";
        } else {
    ...
    $extra_html .=
        ...
        text($linedata["field_id"]) .
        "\" " .
        xlt("will have the following validation rules") .
        ":</td>\n" .
        " </tr>\n" .
        " <tr>\n" .
        ...
        " <tr>\n" .
        "  <td class='text-left' title='" .
        xla("Select a validation rule") .
        "'>\n" .
        "   <select class='form-control form-control-sm' name='fld[" .
        attr($fld_line_no) .
        "][validation]' onchange='valChanged(" .
        attr_js($fld_line_no) .
        ")'>\n" .
        "   <option value=''";
...
function inactiveClicked() {
  top.restoreSession();
  location.href = 'edit_layout.php?form_inactive=<?php echo $form_inactive
      ? "0"
      : "1"; ?>';
}
...
function extAddCondition(lino, btnelem) {
  ...
  if (i == 0) alert('f["fld[' + lino + '][condition_id][' + i + ']"]' + <?php echo xlj(
      "not found",
  ); ?>);
  ...
  newtrelem.innerHTML =
    ...
    "<option value='ne'>" + jsText(<?php echo xlj(
        "Does not equal",
    ); ?>) + "</option>" +
    "<option value='se'>" + jsText(<?php echo xlj(
        "Is selected",
    ); ?>) + "</option>" +
    "<option value='ns'>" + jsText(<?php echo xlj(
        "Is not selected",
    ); ?>) + "</option>" +
    "</select>" +
    "</td>" +
...
function setListItemOptions(lino, seq, init) {
  ...
  const params = new URLSearchParams({
    csrf_token_form: <?php echo js_escape(
...
<input type="hidden" name="csrf_token_form" value="<?php echo attr(
    CsrfUtils::collectCsrfToken(session: $session),
); ?>" />
...
<div class="btn-group ml-auto">
    <button type='button' class='btn btn-secondary btn-sm' onclick='edit_layout_props("")'><?php echo xla(
        "Layout Properties",
    ); ?></button>
    <button type='button' class='btn btn-secondary btn-sm addgroup' id='addgroup'><?php echo xla(
        "Add Group",
    ); ?></button>
    <button type='button' class="btn btn-primary btn-save btn-sm" name='save' id='save'><?php echo xla(
        "Save Changes",
    ); ?></button>
</div>
<br>
    <?php echo xlt("With selected"); ?>:&nbsp;
<input type='button' class='btn btn-secondary btn-sm' name='deletefields' id='deletefields' value='<?php echo xla(
    "Delete",
); ?>' disabled="disabled" />
<input type='button' class='btn btn-secondary btn-sm' name='movefields' id='movefields' value='<?php echo xla(
    "Move to...",
); ?>' disabled="disabled" />
...
&nbsp;&nbsp;&nbsp;
<input type='button' class='btn btn-secondary btn-sm' value='<?php echo xla(
    "Tips",
); ?>' onclick='$("#tips").toggle();' />&nbsp;
<input type='button' class='btn btn-secondary btn-sm' value='<?php echo xla(
    "Encounter Preview",
...
<?php } else { ?>
<button type='button' class='btn btn-primary btn-sm btn-add btn-new' onclick='edit_layout_props("")'><?php echo xla(
    "New Layout",
); ?></button>
<?php } ?>
...
?>
   ...
   <button class='btn btn-success btn-sm float-right' onclick='$("#tips").toggle();return false;'><?php echo xlt(
       "Dismiss",
   ); ?></button>
  </div>
</section></div></div>
...
$grparr = [];
$gres = sqlStatement(
    "SELECT * FROM layout_group_properties WHERE grp_form_id = ? " .
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        if ($group_id != $prevgroup) {
            if ($firstgroup == false) {
                ...
                echo "<div id='" .
                    attr($group_id) .
                    "' class='group' style='padding-top:80px'>";
            }
            ...
            $t_vars = [
                ...
                "translate_layout" =>
                    $GLOBALS["translate_layout"] && $language_choice > 1
                        ? xlt($gdispname)
                        : "",
            ...
            HTML;
            $firstgroup = false;
            if (!empty($row["form_id"])) {<?php
                // if this is not an empty group
                ?>
      ...
      <thead>
       <tr class='head'>
        ...
        <th><?php echo xlt(
            "ID",
        ); ?>&nbsp;<span class="help" title='<?php echo xla(
...
<div class="btn-group">
    <input type="button" class="btn btn-primary btn-sm saverenamegroup btn-save" value="<?php echo xla(
        "Rename Group",
    ); ?>" />
    <input type="button" class="btn btn-secondary btn-sm cancelrenamegroup" value="<?php echo xla(
        "Cancel",
    ); ?>" />
</div>
</div>
...
<input type="button" class="btn btn-primary btn-sm savenewgroup" value='<?php echo xla(
    "Save New Group",
); ?>' />
<input type="button" class="btn btn-secondary btn-sm cancelnewgroup" value='<?php echo xla(
    "Cancel",
); ?>' />
</span>
</div>
...
<table class="table table-sm" style="border-collapse: collapse;">
 <thead>
  <tr class='head'>
   <th><?php echo xlt("Order{{Sequence}}"); ?></th>
   <th <?php echo " $lbfonly"; ?>><?php echo xlt("Source"); ?></th>
   <th><?php echo xlt("ID"); ?>&nbsp;<span class="help" title='<?php echo xla(
    "A unique value to identify this field, not visible to the user",
); ?>' >(?)</span></th>
   <th><?php echo xlt(
       "Label",
...
?>
   ...
   <td><input class='form-control form-control-sm' type="text" name="newlengthWidth" id="newlengthWidth" value="" size="1" maxlength="3" title="<?php echo xla(
       "Width",
   ); ?>" /></td>
   <td><input class='form-control form-control-sm' type="text" name="newlengthHeight" id="newlengthHeight" value="" size="1" maxlength="3" title="<?php echo xla(
       "Height",
   ); ?>" /></td>
   <td><input class='form-control form-control-sm' type="text" name="newmaxSize" id="newmaxSize" value="" size="1" maxlength="3" title="<?php echo xla(
  ...
  <tr>
   <td colspan="9">
    <input type="button" class="btn btn-primary btn-sm savenewfield" value='<?php echo xla(
        "Save New Field",
    ); ?>' />
    <input type="button" class="btn btn-secondary btn-sm cancelnewfield" value='<?php echo xla(
        "Cancel",
    ); ?>' />
   </td>
  </tr>
...
echo "{id: 'EP',text:" .
    ...
    ",ctx:['4'],ctxExcp:['0']},
    {id: 'F',text:" .
    xlj("Add Time to Date") .
    ",ctx:['4'],ctxExcp:['0']},
    {id: 'C',text:" .
    xlj("Capitalize") .
    ",ctx:['0'],ctxExcp:['4','15','40']},
    {id: 'D',text:" .
    xlj("Dup Check") .
    "},
    {id: 'E',text:" .
    xlj("Dup Check on only Edit, or Extra billing codes OK") .
    "},
    {id: 'W',text:" .
    xlj("Dup Check on only New") .
    "},
    {id: 'G',text:" .
    xlj("Graphable") .
    "},
    {id: 'J',text:" .
    xlj("Jump to Next Row") .
    "},
    {id: 'K',text:" .
    xlj("Prepend Blank Row") .
    "},
    {id: 'L',text:" .
    xlj("Lab Order") .
    "},
    {id: 'M',text:" .
    xlj("Radio Group Master") .
    "},
    {id: 'm',text:" .
    xlj("Radio Group Member") .
    "},
    {id: 'N',text:" .
    xlj("New Patient Form") .
    "},
    {id: 'O',text:" .
    xlj("Order Processor") .
    "},
    {id: 'P',text:" .
    xlj("Default to previous value") .
    "},
    {id: 'R',text:" .
    xlj("Distributor") .
    "},
    {id: 'T',text:" .
    xlj("Description is default text") .
    "},
    {id: 'DAP',text:" .
    xlj("Description is Placeholder") .
    "},
    {id: 'U',text:" .
    xlj("Capitalize all") .
    "},
    ...
    ",children:[
        {id: 'RS',text:" .
    xlj("Add Bottom Border Row") .
    "},
        {id: 'RO',text:" .
    xlj("Outline Entire Row") .
    "},
        {id: 'DS',text:" .
    xlj("Add Data Bottom Border") .
    "},
        {id: 'DO',text:" .
    xlj("Outline Data Col") .
    "},
        {id: 'SP',text:" .
    xlj("Span Entire Row") .
    "}
    ...
    "},
    {id: '2',text:" .
    xlj("Billing Code Descriptions") .
    "}];\n";

// Language direction for select2
echo "var langDirection = " .
    js_escape($session->get("language_direction")) .
    ";";
?>
...
$(function () {
    ...
    var DeleteFields = function(btnObj) {
        if (!myChangeCheck()) return;
        if (confirm(<?php echo xlj("WARNING"); ?> + " - " + <?php echo xlj(
    ...
    var ShowGroups = function(btnObj) {
        ...
        dlgopen('../patient_file/encounter/find_code_dynamic.php?what=groups&layout_id=' + <?php echo js_url(
            $layout_id,
        ); ?>,
...
<?php foreach ($typesUsingList as $mytype) {

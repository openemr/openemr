...
function row_delete($table, $where): void
{
    ...
    while ($trow = sqlFetchArray($tres)) {
        ...
        EventAuditLogger::instance()->newEvent("delete", $session->get('authUser'), $session->get('authProvider'), 1, "$table: $logstring");
        ++$count;
    }
...
function row_modify($table, $set, $where): void
{
    ...
    if (sqlQuery("SELECT * FROM " . escape_table_name($table) . " WHERE $where")) {
        EventAuditLogger::instance()->newEvent("deactivate", $session->get('authUser'), $session->get('authProvider'), 1, "$table: $where");

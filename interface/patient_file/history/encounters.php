...
$issue = empty($_GET['issue']) ? 0 : 0 + $_GET['issue'];
 ...
 $default_encounter = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('default_encounter_view'); //'0'=clinical, '1' = billing
...
function getDocListByEncID($encounter, $raw_encounter_date, $pid): void
{
    ...
    if (!empty($documents) && count($documents) > 0) {
        foreach ($documents as $documentrow) {
            ...
            $docHref = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?document&view&patient_id=" . attr_url($pid) . "&doc_id=" . attr_url($documentrow['id']);
...
<?php if ($session->get('language_direction') == "rtl") { ?>
  <link rel="stylesheet" href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('themes_static_relative'); ?>/misc/rtl_encounters.css?v=<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('v_js_includes'); ?>" />
<?php } else { ?>
  <link rel="stylesheet" href="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('themes_static_relative'); ?>/misc/encounters.css?v=<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('v_js_includes'); ?>" />
<?php } ?>
<!-- Not sure why we don't want this ui to be B.S responsive. -->
<?php Header::setupHeader(['no_textformat']); ?>

<script src="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') ?>/library/js/ajtooltip.js"></script>
...
function todocument(docid) {
  ...
  h = '<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') ?>/controller.php?' + params;
  top.restoreSession();
  location.href = h;
}
...
<div class="container-fluid mt-3" id="encounters"> <!-- large outer DIV -->
    ...
    if (isset($_GET['pagesize'])) {
        ...
        $pagesize = array_key_exists('encounter_page_size', $GLOBALS) ? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('encounter_page_size') : 0;
    }
        ...
        <table class="table table-hover jumbotron py-4 mt-3">
            <thead>
                <tr class='text'>
                    ...
                    <th colspan='5' scope="col"><?php echo (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('phone_country_code') == '1') ? xlt('Billing') : xlt('Coding'); ?></th>
                    <?php } ?>

                    <?php if ($attendant_type == 'pid' && !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific')) { ?>
                    <th scope="col">&nbsp;<?php echo (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('weight_loss_clinic')) ? xlt('Payment') : xlt('Insurance'); ?></th>
                    <?php } ?>

                    <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_group_therapy') && !$billing_view && $therapy_group == 0) { ?>
                        <th scope="col"><?php echo xlt('Encounter type'); ?></th>
                    <?php }?>

                    <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_follow_up_encounters')) { ?>
                        <th scope="col"></th>
                    <?php }?>
            ...
            while ($result4 = sqlFetchArray($res4)) {
                ...
                if ($thisauth && $auth_sensitivity && $authPostCalendarCategory) {
                    ...
                    // show insurance
                if ($attendant_type == 'pid' && !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific')) {
                ...
                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_group_therapy') && !$billing_view && $therapy_group == 0) {
                ...
                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_follow_up_encounters')) {

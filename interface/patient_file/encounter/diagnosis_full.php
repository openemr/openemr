...
if ($result = BillingUtilities::getBillingByEncounter($pid, $encounter, "*")) {
    ...
    foreach ($result as $iter) {
        if ($iter["code_type"] == "ICD9") {
            ...
            $billing_html[$iter["code_type"]] .= "<td><a class=\"link_submit\" href='diagnosis_full.php?" . \OpenEMR\Common\Http\QueryString::buildUntyped(['mode' => 'clear', 'id' => $iter["id"], 'csrf_token_form' => CsrfUtils::collectCsrfToken(session: $session)]) . "' class='link' onclick='top.restoreSession()'>[" . xlt('Clear Justification') . "]</a></td>";
        }

        $billing_html[$iter["code_type"]] .= "<td><a class=\"link_submit\" href='diagnosis_full.php?" . \OpenEMR\Common\Http\QueryString::buildUntyped(['mode' => 'delete', 'id' => $iter["id"], 'csrf_token_form' => CsrfUtils::collectCsrfToken(session: $session)]) . "' class='link' onclick='top.restoreSession()'>[Delete]</a></td>";

...
<form name=icd9_form method=post action=icd9cm_codes.php>
<input type=hidden name=mode value="search">

<span class="title" href="icd9cm_codes.php">ICD-9-CM Codes</span><br>
...
if ($res = sqlStatement("select * from codes where (code_text like '%".$_POST["text"]."%' or code like'%".$_POST["text"]."%') and code_type = '" . CODE_TYPE_ICD9 . "' order by code limit " . ($M + 1)) ) {
...
echo "<a target=Diagnosis class=text href='diagnosis.php?mode=add&type=".urlencode($code_type_array[$iter{"code_type"}])."&code=".urlencode($iter{"code"})."&text=".urlencode($iter{"code_text"})."'><b>".$iter{"code"}."</b> ".ucwords(strtolower($iter{"code_text"}))."</a><br>\n";

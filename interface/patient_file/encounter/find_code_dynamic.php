    ...
    <script>
        ...
        $(function () {
            ...
            oTable = $('#my_data_table').dataTable({
                ...
                "bServerSide": true,
                "sAjaxSource": "find_code_dynamic_ajax.php?csrf_token_form=" + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>,
                ...
                fnDrawCallback: function (obj) {
                    ...
                    if (obj.json && obj.json.iTotalHasMoreRecords) {
                        $(".dataTables_wrapper .dataTables_info").parent().parent().prepend("<div class='col-sm-12'><p class='alert alert-warning moreResults'>" + <?php echo xlj("Maximum displayable results reached. Narrow your search"); ?> +"</p></div>");
                    }
                },
                ...
                initComplete: function () {
                    const input = $('.dataTables_filter input').unbind(),
                        self = this.api(),
                        $searchButton = $('<button class="btn btn-sm btn-outline-primary fa fa-search p-2">').click(function (e) {
                            e.preventDefault();
                            e.stopPropagation();
                            self.search(input.val()).draw();
                        }),
                        $clearButton = $('<button type="button" class="btn btn-sm btn-outline-warning fa-solid fa-eraser p-2">').click(function () {
                            input.val('');
                            self.search('').draw();
                        });
                ...
                if (Object.values(oChosenIDs).length < limit || limit <= 0) {
                    ...
                    var codeStr = String(jobj['code'] ?? '');
                    var codesel = codeStr.split('|');
                    var selector = (codesel.length > 1 ? codesel[1] : (jobj['selector'] || ''));
                    selcode(jobj['codetype'], codesel[0] || codeStr, selector, jobj['description'], target_element, limit, jobj['modifier']);
                    <?php } elseif ($what == 'fields') { ?>
                    selectField(jobj);

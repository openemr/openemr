...
if ($result = getFormByEncounter($pid, $encounter, "id, date, form_id, form_name,formdir,user")) {
	...
	foreach ($result as $iter) {
		...
		foreach ($form_info as $field_name => $field) {
			if ($field_name == "id" || $field_name == "date" || $field_name == "pid" || $field_name == "user" || $field_name == "groupname" || $field_name == "authorized" || $field_name == "activity" || $field_name == "encounter") {
			...
			elseif ($field == "0000-00-00" || $field == "00:00" || $field == "0000-00-00 00:00") {
				...
				echo '<td ' . $width . ' valign="bottom"><span class="text" style="font-size:8pt;font-weight: bold">' . $field_name . ':</span> <span style="font-size:8pt;">' . $field . '</span></td><td width="5"></td>';
			}
			if ($counter > 5) {
				echo "</tr><tr>";
				$counter = 0;

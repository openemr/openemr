    ...
    $meds = sqlStatement("SELECT `title` FROM `lists` WHERE `type` = 'medication' AND `pid` = $pid ");
     //var_export($pmeds);
        ...
        function getProvider($id){
           ...
           $hunt = sqlStatement($sql);
           $p = sqlFetchArray($hunt);
           $provider = $p['lname'].", ".$p['fname'];
        return $provider;
    }
?>
<!DOCTYPE html>

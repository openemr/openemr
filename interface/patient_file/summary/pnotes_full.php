...
?>&orderid=<?php echo htmlspecialchars($orderid, ENT_QUOTES);
?>&<?php echo attr($activity_string_html);?>' onsubmit='return top.restoreSession()'>
...
?>
    ...
    <div>
        <a href="pnotes_full_add.php?<?php echo $urlparms;
?>" class="css_button iframe" onclick='top.restoreSession()'><span><?php echo xlt('Add'); ?></span></a>
    ...
    <?php } else { ?>
      <a href="pnotes_full.php?<?php echo $urlparms;
?>" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show All'); ?></span></a>
    <?php } ?>
    |
    <?php if ($active == '1') { ?>
      <span><?php echo xlt('Show Active'); ?></span>
    <?php } else { ?>
      <a href="pnotes_full.php?form_active=1&<?php echo $urlparms;
?>" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show Active'); ?></span></a>
    <?php } ?>
    |
    <?php if ($active == '0') { ?>
      <span><?php echo xlt('Show Inactive'); ?></span>
    <?php } else { ?>
      <a href="pnotes_full.php?form_inactive=1&<?php echo $urlparms;
?>" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show Inactive'); ?></span></a>
    <?php } ?>
    </div>
...
?>" ><a onclick="show_div('inbox')" href="#"><?php echo htmlspecialchars(xl('Inbox'),ENT_NOQUOTES); ?></a></li>
  <li class="<?php echo $outbox;
?>" ><a onclick="show_div('outbox')" href="#"><?php echo htmlspecialchars(xl('Sent Items'),ENT_NOQUOTES); ?></a></li>
</ul>
...
<?php if ($result != ""): ?>
 <tr>
  <td colspan='5' style="padding: 5px;" >
    ...
    |
    <a href="pnotes_full.php?<?php echo $urlparms;
?>&<?php echo attr($activity_string_html);
?>" class="" id='Submit' onclick='top.restoreSession()'><span><?php echo htmlspecialchars( xl('Refresh'), ENT_NOQUOTES); ?></span></a>
  </td>
 </tr></table>
<?php endif; ?>
...
<?php if ($result_sent != ""): ?>
 <tr>
  <td colspan='5' style="padding: 5px;" >
    <a href="pnotes_full.php?<?php echo $urlparms;
?>&s=1&<?php echo attr($activity_string_html);?>"
...
if ($noteid /* && $title == 'New Document' */ ) {
    ...
    if (preg_match('/New scanned document (\d+): [^\n]+\/([^\n]+)/', $prow['body'], $matches)) {
        ...
        $docname = $matches[2];
    ?>
   window.open('../../../controller.php?document&retrieve&patient_id=<?php echo htmlspecialchars($patient_id, ENT_QUOTES);
?>&document_id=<?php echo htmlspecialchars($docid, ENT_QUOTES);
?>&<?php echo htmlspecialchars($docname, ENT_QUOTES);?>&as_file=true',

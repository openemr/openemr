...
if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr')) {
    //CDR Engine stuff
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_allergy_check') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_alert_log')) {
    ...
    if ((empty($alertNotifyPid) || ($alertNotifyPid != $pid)) && isset($_GET['set_pid']) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_crp')) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_allergy_check')) {
...
{
    if (
        !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('portal_onsite_two_enable')
    ) {
        return false;
    }
...
function isPortalSiteAddressValid(): bool
{
    if (
        ...
        !\OpenEMR\Core\OEGlobalsBag::getInstance()->has('portal_onsite_two_address')
    ) {
        return false;
    }
...
{
    if (
        \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enforce_signin_email')
    ) {
        return true;
    }
...
function deceasedDays($days_deceased)
{
    ...
    if (strlen($days_deceased['date_deceased'] ?? '') > 10 && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format') < 1) {
...
function image_widget($doc_id, $doc_catg): void
{
    ...
    $image_width = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('generate_doc_thumb') == 1 ? '' : 'width=100';
...
<head>
        ...
        $(function () {
            ...
            if (reloadRequired) {
                document.location.reload();
                    ...
                    <?php
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('erx_import_status_message')) { ?>
            if (msg_updation)
                alert(msg_updation);
                        <?php
                    }
            ...
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_crw')) { ?>
            placeHtml("clinical_reminders_fragment.php", "clinical_reminders_ps_expand", true, true).then(() => {
                ...
                $(".cdr-rule-btn-info-launch").on("click", function (e) {
                    ...
                    let launchUrl = "<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'); ?>/interface/super/rules/index.php?" + params;
                    e.preventDefault();
                    e.stopPropagation();
            ...
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_prw')) { ?>
            ...
            <?php if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_birthday_alert')) {
    ...
    <style>
      ...
      <?php
        if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('right_justify_labels_demographics')) && ($session->get('language_direction') == 'ltr')) { ?>
...
<body class="mt-1 patient-demographic bg-light">
    ...
    <div id="container_div" class="<?php echo $oemr_ui->oeContainer(); ?> mb-2">
        ...
        $idcard_doc_id = false;
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_id_category_name')) {
            $idcard_doc_id = get_document_by_catg($pid, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_id_category_name'), 3);
        }
        ?>
        <div class="main mb-1">
            ...
            <div class="row">
                ...
                $rx = !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('disable_prescriptions') && AclMain::aclCheckCore('patients', 'rx') && !in_array('card_prescriptions', $hiddenCards) ? 1 : 0;
                ...
                if ($allergy === 1) {
                    ...
                    $viewArgs = [
                        ...
                        'auth' => true,
                        'btnLabel' => 'Edit',
                        'btnLink' => "return load_location('" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/patient_file/summary/stats_full.php?active=all&category=allergy')"
                    ];
                    echo "<div class=\"$col\">";
                ...
                if ($pl === 1) {
                    ...
                    $viewArgs = [
                        ...
                        'auth' => true,
                        'btnLabel' => 'Edit',
                        'btnLink' => "return load_location('" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/patient_file/summary/stats_full.php?active=all&category=medical_problem')"
                    ];
                    echo "<div class=\"$col\">";
                ...
                if ($meds === 1) {
                    ...
                    $viewArgs = [
                        ...
                        'auth' => true,
                        'btnLabel' => 'Edit',
                        'btnLink' => "return load_location('" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/patient_file/summary/stats_full.php?active=all&category=medication')"
                    ];
                    echo "<div class=\"$col\">";
                ...
                if ($rx === 1) :
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('erx_enable') && ($display_current_medications_below ?? '') == 1) {
                    ...
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('erx_enable')) {
                        ...
                        $viewArgs['btnLink'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/eRx.php?page=compose";
                    } else {
                        $viewArgs['btnLink'] = "editScripts('" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?prescription&list&id=" . attr_url($pid) . "')";
            ...
            <div class="row">
                ...
                <div class="col-md-8 px-2">
                    ...
                    if (!\OpenEMR\Core\OEGlobalsBag::getInstance()->get('hide_billing_widget')) {
                    ...
                    if (AclMain::aclCheckCore('patients', 'reminder') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_prw')) :
                    ...
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('amendments') && AclMain::aclCheckCore('patients', 'amendment')) :
                        ...
                        $viewArgs = [
                            ...
                            'initiallyCollapsed' => (getUserSetting($id) == 0) ? true : false,
                            'btnLabel' => 'Edit',
                            'btnLink' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/interface/patient_file/summary/list_amendments.php?id=" . attr_url($pid),
                            'btnCLass' => '',
                            'linkMethod' => 'html',
                            'bodyClass' => 'notab collapse show',
                ...
                <div class="col-md-4 px-2">
                    ...
                    $_extAccess = [
                        \OpenEMR\Core\OEGlobalsBag::getInstance()->get('portal_onsite_two_enable'),
                        \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rest_fhir_api'),
                        \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rest_api'),
                        \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rest_portal_api'),
                    ];
                    ...
                    $photos = pic_array($pid, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_photo_category_name'));
                    if ($photos or $idcard_doc_id) {
                        ...
                        $viewArgs = [
                            ...
                            'bodyClass' => 'collapse show',
                            'auth' => false,
                            'patientIDCategoryID' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_id_category_name'),
                            'patientPhotoCategoryName' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_photo_category_name'),
                            'photos' => $photos,
                    ...
                    // Advance Directives
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('advance_directives_warning')) {
                    ...
                    $cdr = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr');
                    $cdr_crw = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_crw');
                    ...
                    if (isset($pid) && !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('disable_calendar') && AclMain::aclCheckCore('patients', 'appt')) {
                        $displayAppts = true;
                        $current_date2 = date('Y-m-d');
                        $events = [];
                        $apptNum = (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('number_of_appts_to_show');
                        $apptNum2 = ($apptNum != 0) ? abs($apptNum) : 10;

                        $mode1 = !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_display_sets_option');
                        $colorSet1 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_display_sets_color_1');
                        $colorSet2 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_display_sets_color_2');
                        $colorSet3 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_display_sets_color_3');
                        $colorSet4 = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_display_sets_color_4');
                        ...
                        foreach ($events as $row) {
                            ...
                            $displayMeridiem = (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format') == 0) ? "" : "am";
                            $disphour = substr((string) $row['pc_startTime'], 0, 2) + 0;
                            $dispmin = substr((string) $row['pc_startTime'], 3, 2);
                            if ($disphour >= 12 && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('time_display_format') == 1) {
                                $displayMeridiem = "pm";
                                if ($disphour > 12) {
                                    $disphour -= 12;
                                }
                    ...
                    $recurr = [];
                    if (isset($pid) && !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('disable_calendar') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('appt_recurrences_widget') && AclMain::aclCheckCore('patients', 'appt')) {
                    ...
                    $direction = '1';
                    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('num_past_appointments_to_show') < 0) {
                        $direction = '2';
                        ($showpast = -1 * \OpenEMR\Core\OEGlobalsBag::getInstance()->get('num_past_appointments_to_show'));
                    } else {
                        $showpast = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('num_past_appointments_to_show');
                    }

                    if (isset($pid) && !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('disable_calendar') && $showpast > 0 && AclMain::aclCheckCore('patients', 'appt')) {
                    ...
                    if ($thisauth) :
                        echo $twig->getTwig()->render('patient/partials/delete.html.twig', [
                            ...
                            'allowPatientDelete' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('allow_pat_delete'),

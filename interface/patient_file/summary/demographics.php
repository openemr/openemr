...
<?php
 ...
 if (!$thisauth):
  $noAuth = htmlspecialchars(xl('Demographics not authorized'),ENT_NOQUOTES); ?>
  <p>(<?php echo $noAuth;?>)</p>
  </body>
  </html>
 <?php
  ...
  if ($thisauth) { // Don't really need this because if the user isn't auth'd line 484 stops execution. Robert Down 5/8/15
?>

<?php
error_reporting(E_ALL);
ini_set('display_errors', 'on');
require_once "$srcdir/Patient/src/Entity.php";
$pt = new OpenEMR\Patient\Entity($pid);
?>

<div class="row">
  <div class="small-12 columns end">
    <h3 class="left"><?php echo $pt->getPatientName('l, f'); ?></h3>
    <div class="right">
      <?php 
      if ($pt->portal): ?>
          <span class="label warning"><?php echo htmlspecialchars(xl('Patient has not authorized the Patient Portal'), ENT_NOQUOTES);?></span>
      <?php
      endif;
      if ($pt->isDeceased): 
        $deceased = htmlspecialchars(xl('Deceased') . ' ' . $pt->daysDeceased . ' ' . xl('days ago'), ENT_NOQUOTES);
      ?>
        <span class="label alert"><?php echo $deceased;?></span>
      <?php
      endif;
      ?>
    </div>
...
<div class="row">
  <div class="small-12 columns end">
    <ul class="button-group">
      ...
      <li><a class="css_button small iframe alert" 
           href="../deleter.php?patient=<?php echo $pt->pid; ?>" 
           onclick="top.restoreSession()">
          <?php echo htmlspecialchars(xl('Delete'),ENT_NOQUOTES); ?>
      </a></li>
      <?php endif; ?>
      <?php if(!$GLOBALS['erx_enable']): ?>
      <li><a class="css_button small" href="../../eRx.php?page=medentry" onclick="top.restoreSession()">
        <?php echo htmlspecialchars(xl('NewCrop MedEntry'),ENT_NOQUOTES); ?>
      </a></li>
      <li><a class="css_button small iframe1" href="../../soap_functions/soap_accountStatusDetails.php" onclick="top.restoreSession()">
      ...
      if($GLOBALS['portal_onsite_enable'] && $GLOBALS['portal_onsite_address']):
        if ($pt->portal):
          ...
          <li><a class='css_button iframe small_modal' href='create_portallogin.php?portalsite=on&patient=<?php echo htmlspecialchars($pid,ENT_QUOTES);?>' onclick='top.restoreSession()'>
          <?php 
          if (empty($portalLogin)):
            echo htmlspecialchars(xl('Create Onsite Portal Credentials'),ENT_NOQUOTES);
          else:
            echo htmlspecialchars(xl('Reset Onsite Portal Credentials'),ENT_NOQUOTES);
          endif; ?>
          </a></li>
      <?php 
        endif;
...
<div class="row">
  <div class="small-12 column">
    <dl class="sub-nav">
      <dd><a href="../history/history.php" onclick="top.restoreSession()"><?php echo htmlspecialchars(xl('History'),ENT_NOQUOTES);?></a></dd>
      <dd><a href="../report/patient_report.php" onclick="top.restoreSession()"><?php echo htmlspecialchars(xl('Report'),ENT_NOQUOTES);?></a></dd>
      <?php //note that we have temporarily removed document screen from the modal view ?>
      <dd><a href="../../../controller.php?document&list&patient_id=<?php echo $pid;?>" onclick="top.restoreSession()"><?php echo htmlspecialchars(xl('Documents'),ENT_NOQUOTES);?></a></dd>
      <dd><a href="../transaction/transactions.php" class="iframe large_modal" onclick="top.restoreSession()"><?php echo htmlspecialchars(xl('Transactions'),ENT_NOQUOTES);?></a></dd>
      <dd><a href="stats_full.php?active=all" class="iframe large_modal" onclick="top.restoreSession()"><?php echo htmlspecialchars(xl('Issues'),ENT_NOQUOTES);?></a></dd>
      ...
      <?php
        ...
        if (sqlNumRows($module_query)) {
          ...
          while ($modulerow = sqlFetchArray($module_query)) {
            ...
            ?>
            <dd><a href="<?php echo $relative_link; ?>" onclick='top.restoreSession()'>
            <?php echo htmlspecialchars($nickname,ENT_NOQUOTES); ?></a></dd>
          <?php 
          }
        }
        ?>
...
<div class="small-6 medium-8 columns">
  <div class="<?php if ($GLOBALS['athletic_team']) echo "hide";?>">
    <?php
      // Billing expand collapse widget
      $forceExpandAlways = ($GLOBALS['force_billing_widget_open']) ? true : false;
      expand_collapse_widget(xl('Billing'), "billing", xl("Edit"), "return newEvt();",
                             "", "javascript", "notab", false, true, $forceExpandAlways); 
      ?>
      <br>
      <?php
      ...
      <table class="table">
        <tr>
          <th><?php echo xlt('Patient Balance Due'); ?></th>
          <td><?php echo text(oeFormatMoney($patientbalance)); ?></td>
        </tr>
        <tr>
          <th><?php echo xlt('Insurance Balance Due');?></th>
          <td><?php echo text(oeFormatMoney($insurancebalance));?></td>
        </tr>
        <tr>
          <th><?php echo xlt('Total Balance Due');?></th>
          <td><?php echo text(oeFormatMoney($totalbalance));?></td>
        </tr>
        <?php if ($result['genericname2'] == 'Billing'): ?>
        <tr>
          <th><?php echo xlt('Billing Note');?></th>
          <td><?php echo text($result['genericval2']);?></td>
        </tr>
        <?php endif; ?>
        <?php if ($result3['provider']): ?>
        <tr>
        ...
        <?php endif; ?>
        <tr>
          <th><?php echo xlt('Effective Date');?></th>
  ...
  <div class="clearfix">
    <?php
      ...
      expand_collapse_widget(xl("Demographics"), "demographics", xl("Edit"), "demographics_full.php",
                             "", "html", "", acl_check('patients', 'demo', '', 'write'), true);
      ?>
      <div id="DEM">
        <ul class="tabNav">
  ...
  <div class="clearfix">
    <?php
      ...
      if ( $insurance_count > 0 ) {
        ...
        expand_collapse_widget(xl("Insurance"), "insurance", xl("Edit"), "demographics_full.php",
                               "", "html", "", acl_check('patients', 'demo', '', 'write'), true);
        if ( $insurance_count > 0 ) {
      ?>
  ...
  <div class="clearfix">
    <?php
      ...
      ?>
      <br/>
      <div style='margin-left:10px' class='text'><i class="fa fa-spinner"></i></div><br/>
      </div> <?php // Needed to close out expand widget; ?>
  </div>
  ...
  <div class="clearfix">
    ...
    <br/>
    <div style='margin-left:10px' class='text'><i class="fa fa-spinner fa-pulse"></i></div><br/>
    </div>
  </div>
  <?php endif; //end if prw is activated  ?>
  <div class="clearfix">
    <?php
    ...
    </div> <?php // Needed for expand/collapse widget; ?>
  </div>
  ...
  <div class="clearfix">
    ...
    <table class="table">
      <tr>
        <td colspan='<?php echo $numcols; ?>' class='text'>&nbsp;&nbsp;<?php echo xlt('None');?></td>
      </tr>
    </table>
    <?php 
    endif;
    ...
    while ($row=sqlFetchArray($result)): ?>
      &nbsp;&nbsp;
      <a class="<?php echo $widgetButtonClass;?>" 
         href="<?php echo $widgetButtonLink . '&id=' . attr($row['amendment_id']);?>" 
         onclick='top.restoreSession()'>
          <?php echo text($row['amendment_date']);?>&nbsp;<?php echo text($row['amendment_desc']);?>
      </a><br>
    <?php endwhile; ?>
    </div> <?php // Needed for expand/collapse widget; ?>
  </div>
  ...
  ?>
    <br/>
    <div style='margin-left:10px' class='text'><i clas="fa fa-pulse fa-spinner"></i></div><br/>
    </div>
  </div>
  <?php if ($vitals_is_registered && acl_check('patients', 'med')): ?>
  <div class="clearfix">
  ...
  <?php 
  endif; // end if ($vitals_is_registered && acl_check('patients', 'med'))
...
<div class="small-6 medium-4 columns">
  ...
  if ($photos or $idcard_doc_id ) {
    echo "<div>";
    expand_collapse_widget(xl("ID Card").'/'.xl("Photos"), "photos", "javascript",
                           "notab-right", false, false);
    ?>
    <br />
    <?php
      if ($idcard_doc_id) {
  ...
  if (isset($pid) && !$GLOBALS['disable_calendar']) {
    ...
    if ((acl_check('patients', 'med')) && ($GLOBALS['enable_cdr'] && $GLOBALS['enable_cdr_crw']) ) {
      ...
      echo "<div>";
        expand_collapse_widget(xl("Clinical Reminders"), "clinical_reminders", xl("Edit"),
                               ".../reminder/clinical_reminders.php?patient_id=".$pid, "",
                               "html", "summary_item small", true, false); ?>

        <br/>
        <div style='margin-left:10px' class='text'><i class="fa fa-pulse fa-spinner"></i></div>
        </div> <?php // Needed for expand/collapse widget;
      echo "</div>";
    } // end if crw
    ...
    echo "<div>";
    expand_collapse_widget(xl("Appointments"), "appointments", xl("Add"), '',
                           "javascript", "summary_item small", (isset($res) && $res != null), false);
    $count = 0;
    while($row = sqlFetchArray($res)) {
      $count++;
    ...
    echo "</div>"; // Needed for expand/collapse widget;
  }
...
<?php if (false && $GLOBALS['athletic_team']) { // This will never evaluate, we should remove this code block?>

...
<body class="mt-1 patient-demographic bg-light">
    ...
    <?php
    // Clinical Co-Pilot iframe rail — embeds the AI agent panel beside the patient dashboard.
    // Looks up the patient's FHIR UUID and passes it via query string so the agent session opens scoped.
    ...
    if (!empty($pid)) {
        ...
        if (!empty($copilotRow['uuid'])) {
            // Binary UUID → string. OpenEMR ships with this helper; works on both MySQL 8 + MariaDB.
    ...
    ?>
    <style>
        /* Closed by default — only a slim handle is visible. Body keeps a 36px gutter. */

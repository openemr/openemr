...
if ($mode == "update") {
	...
	addList($pid,"allergy",$_POST["title"],$_POST["comments"],1);
}
}
...
<a href="patient_summary.php" target="Main"><font class="title">Allergies</font><font class=back><?echo $tback;?></font></a>
<br>

<form border=0 method=post name=new_note action="allergies.php">
<input type=hidden name=mode value="new">
<input type=hidden name=active value="<?echo $active;?>">
<input type=entry size=15 name=title value="Allergy" onfocus="javascript:this.value=''"><br>
<textarea name=comments rows=3 cols=25 wrap=virtual onfocus="javascript:this.value=''">Comments</textarea>
<br>
<a href="javascript:document.new_note.submit();" class=link_submit>[Add New Allergy]</a>
</form>


<form border=0 method=post name=update_activity action="allergies.php">
...
<a href="allergies.php?offset=0&active=all" class=<?echo $all_class;?>>[All]</a>
<a href="allergies.php?offset=0&active=1" class=<?echo $active_class;?>>[Only Active]</a>
<a href="allergies.php?offset=0&active=0" class=<?echo $inactive_class;?>>[Only Inactive]</a>
...
<span class=text>
<?
if ($result = getListByType($pid, "allergy", "id,title,comments,activity,date", $active, "all", 0)){

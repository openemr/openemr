...
<span class="title_bar_top"><?echo $result{"fname"} . " " . $result{"lname"};?></span> <span style="font-size:10pt;">(Age: <?=$age?> ID: <?=$result['pubpid']?>)</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="title">Logged in as: <?echo $provider_results{"fname"}.' '.$provider_results{"lname"};?> </span><span style="font-size:9pt;"> (<?=$_SESSION['authGroup']?>)</span>
</td>
...
$qstring = "&event_startampm=$ampm&event_starttimeh=" . date("H") ."&event_category=".$GLOBALS['default_category']."&provider_id=". $result['providerID'] ."&event_dur_hours=".$event_dur_hours."&event_dur_minutes=".$event_dur_minutes."&patient_id=".$result['pid']."&event_subject=".$event_title;

echo '<a href="../../main/calendar/find_patient.php?no_nav=1&show_pnote_link=1' . $qstring. '" target="Notes" class=title_bar_top>New Appointment</a>';
?>

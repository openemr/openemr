...
function calctotal() {
    ...
    for (var i = 0; i < f.elements.length; ++i) {
        ...
        var ename = elem.name;
        if (ename.indexOf('form_upay[') == 0 || ename.indexOf('form_bpay[') ==
            0) {
...
function coloring() {
    for (var i = 1;; ++i) {
        if (document.getElementById('paying_' + i)) {
            ...
            patient_balance = document.getElementById('duept_' + i).innerHTML *
                1;
            //balance=document.getElementById('balance_'+i).innerHTML*1;
            if (patient_balance > 0 && paying > 0) {
                if (paying > patient_balance) {
                    document.getElementById('paying_' + i).style.background =
                        '#FF0000';
...
function validate() {
    ...
    if (document.getElementById('radio_type_of_payment_self1').checked == false &&
        document.getElementById('radio_type_of_payment_self2').checked == false &&
    ...
    if (document.getElementById('radio_type_of_payment_self1').checked == true ||
        document.getElementById('radio_type_of_payment_self2').checked == true ||
        ...
        for (var i = 0; i < f.elements.length; ++i) {
            ...
            if (ename.indexOf('form_upay[0]') == 0) //Today is this text box.
            {
                if (elem.value * 1 > 0) { //A warning message, if the amount is posted with out encounter.
                    if (confirm(
                            "<?php echo addslashes(xlt('If patient has appointment click OK to create encounter otherwise, cancel and create encounter.')) ?>"
                        )) {
                        ok = 1;
                    } else {
                        elem.focus();
...
function make_it_hide_enc_pay() {
    ...
    for (var i = 1;; ++i) {
        ...
        var td_patient_copay_elem = document.getElementById('td_patient_copay_' +
            i)

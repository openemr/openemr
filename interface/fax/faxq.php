...
function dodclick(ffname) {
 cascwin('fax_view.php?file=' + encodeURIComponent(ffname) + '&csrf_token_form=' + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>, '_blank', 600, 475,
...
function dojclick(jobid) {
 cascwin('fax_view.php?jid=' + encodeURIComponent(jobid) + '&csrf_token_form=' + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>, '_blank', 600, 475,
  "resizable=1,scrollbars=1");
 return false;
}
...
function dosvclick(sfname) {
 cascwin('fax_view.php?scan=' + encodeURIComponent(sfname) + '&csrf_token_form=' + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>, '_blank', 600, 475,
  "resizable=1,scrollbars=1");
 return false;
}
...
function domclick(ffname) {
    dlgopen('fax_dispatch.php?file=' + encodeURIComponent(ffname) + '&csrf_token_form=' + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>, '_blank', 850, 550, '', 'Fax Dispatch');
}

// Process click to pop up the scanned document dispatch window.
function dosdclick(sfname) {
    dlgopen('fax_dispatch.php?scan=' + encodeURIComponent(sfname) + '&csrf_token_form=' + <?php echo js_url((string) CsrfUtils::collectCsrfToken(session: $session)); ?>, '_blank', 850, 550, '', 'Scanned Dispatch');
}
...
foreach ($mlines as $matches) {
    ...
    echo "<a href='fax_view.php?file=" . attr_url($ffname) . "&csrf_token_form=" . attr_url((string) CsrfUtils::collectCsrfToken(session: $session)) . "' onclick='return false'>" . text($ffbase) . "</a></td>\n";
    echo "     <td onclick='domclick(\"" . attr(addslashes($ffname)) . "\")'>";
    echo "<a href='fax_dispatch.php?file=" . attr_url($ffname) . "&csrf_token_form=" . attr_url((string) CsrfUtils::collectCsrfToken(session: $session)) . "' onclick='return false'>" . xlt('Dispatch') . "</a></td>\n";
...
foreach ($dlines as $matches) {
    ...
    echo "     <td onclick='dojclick(\"" . attr(addslashes($jobid)) . "\")'>" .
     "<a href='fax_view.php?jid=" . attr_url($jobid) . "&csrf_token_form=" . attr_url((string) CsrfUtils::collectCsrfToken(session: $session)) . "' onclick='return false'>" .
     "$jobid</a></td>\n";
...
foreach ($slines as $sline) {
    ...
    echo "     <td onclick='dosvclick(\"" . attr(addslashes($sfname)) . "\")'>" .
     "<a href='fax_view.php?scan=" . attr_url($sfname) . "&csrf_token_form=" . attr_url((string) CsrfUtils::collectCsrfToken(session: $session)) . "' onclick='return false'>" .
     "$sfname</a></td>\n";
    echo "     <td onclick='dosdclick(\"" . attr(addslashes($sfname)) . "\")'>";
    echo "<a href='fax_dispatch.php?scan=" . attr_url($sfname) . "&csrf_token_form=" . attr_url((string) CsrfUtils::collectCsrfToken(session: $session)) . "' onclick='return false'>" . xlt('Dispatch') . "</a></td>\n";

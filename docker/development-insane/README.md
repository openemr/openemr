...
- http://localhost:8083 (with Alpine Edge (Apache and now PHP 8.0))
...
- http://localhost:8093 (with Alpine Edge (Apache and now PHP 8.0) with redis)
...
setup input screen:
 - for `Server Host`, use either `mariadb` or `mariadb-ssl` or `mysql` or `mariadb-old` or `mariadb-very-old` or
   `mariadb-very-very-old` or `mariadb-very-very-very-old` or `mariadb-very-very-very-very-old` or `mysql-old` (you have all
   ...
   `mariadb` is version 10.7; `mariadb-ssl` is version 10.7 with support for ssl; `mariadb-old` is version 10.6; `mariadb-very-old` is
   version 10.5; `mariadb-very-very-old` is version 10.4; `mariadb-very-very-very-old` is version 10.3; `mariadb-very-very-very-very-old` is version 10.2)

<?php /* Smarty version 2.6.31, created on 2020-02-27 03:23:34
         compiled from default/views/day/ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/day/ajax_template.html', 11, false),array('function', 'xla', 'default/views/day/ajax_template.html', 172, false),array('function', 'xlt', 'default/views/day/ajax_template.html', 172, false),array('function', 'pc_sort_events', 'default/views/day/ajax_template.html', 380, false),array('modifier', 'date_format', 'default/views/day/ajax_template.html', 375, false),array('modifier', 'string_format', 'default/views/day/ajax_template.html', 376, false),)), $this); ?>
...
<script>
 ...
 function newEvt(startampm, starttimeh, starttimem, eventdate, providerid, catid) {
  dlgopen('add_edit_event.php?startampm=' + encodeURIComponent(startampm) +
   ...
   '&date=' + encodeURIComponent(eventdate) + '&catid=' + encodeURIComponent(catid)<?php 
...
<div id="wrapper">
    <form name='theform' id='theform' action='index.php?module=PostCalendar&func=view&tplview=default&pc_category=&pc_topic=' method='post' onsubmit='return top.restoreSession()'>
        <div id="topToolbarRight" class="bgcolor2">
            <a id="menu-toggle" class="float-left btn btn-calendar"></a>
            <div id="functions">
                ...
                <?php  echo "<a href='#' title='" . xla("New Appointment") . "' onclick='newEvt(1, 9, 00, " . attr_js($Date) . ", 0, 0)' class='btn btn-primary btn-sm'><i class='fa fa-plus' aria-hidden='true'></i></a>\n";
                echo "<a href='#' title='" . xla("Search Appointment") . "' onclick='top.restoreSession();location=\"index.php?module=PostCalendar&func=search\"' class='btn btn-primary btn-sm'><i class='fa fa-search' aria-hidden='true'></i></a>\n";
    if ($Date <> date('Ymd')) {
     ?>
    <a href='#' name='bnsubmit' value='<?php echo smarty_function_xla(array('t' => 'Today'), $this);?>
' onClick='GoToToday(theform);' class='btn btn-primary btn-sm'><?php echo smarty_function_xlt(array('t' => 'Today'), $this);?>
</a>
    <?php 
    }
 ?>
...
<div id="viewPicker">
<?php 
echo "   <a href='#' id='printview' title='" . xla("View Printable Version") . "' class='btn btn-primary btn-sm'>
            <i class='fa fa-print' aria-hidden='true'></i></a>\n";
echo "   <a href='#' title='" . xla("Refresh") . "' onclick='javascript:refreshme()' class='btn btn-primary btn-sm'>
            <i class='fa fa-refresh' aria-hidden='true'></i></a>\n";
echo "   <a href='#' type='button' id='dayview' title='" . xla('Day View') . "' class='btn btn-primary btn-sm'>" . xlt('Day') . "</a>\n";
echo "   <a href='#' type='button' id='weekview' title='" . xla('Week View') . "' class='btn btn-primary btn-sm'>" . xlt('Week') . "</a>\n";
echo "   <a href='#' type='button' id='monthview' title='" . xla('Month View') . "' class='btn btn-primary btn-sm'>" . xlt('Month') . "</a>\n";
 ?>
</div>
...
echo "\n<tr><td><div class='view1 bg-info font-weight-bold text-ovr-dark'>" . text($f['name']) . "</div></td></tr>\n";
}
 ?>
 </table>
</div>
...
echo "<table class='w-100 border-0' cellpadding='1' cellspacing='0'>\n";

// output the TD with the times DIV
echo "<tr>";
echo "<td id='times'><div><table><tr>\n";
...
foreach ($providers as $provider) {
    ...
    echo "<div class='providerheader'>" . text($provfl) . "<a class='providerXbtn' data-username='" . attr($provider['username']) . "'><i class='fa fa-times'></i></a></div>";
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        foreach ($events as $event) {
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                if ($patientid) {
                    ...
                    $content .= "<img src='{$this->_tpl_vars['TPL_IMAGE_PATH']}/user-green.gif' onmouseover=\"javascript:ShowImage(" . attr_js($GLOBALS['webroot']."/controller.php?document&retrieve&patient_id=".attr($patientid)."&document_id=-1&as_file=false&original_file=true&disable_exit=false&show_original=true&context=patient_picture") . ");\" onmouseout=\"javascript:HideImage();\" border='0' title='" . attr($link_title) . "' alt='" . xla('View Patient') . "' />";
                ...
                elseif($groupid){
                    ...
                    $content .= "<img src='{$this->_tpl_vars['TPL_IMAGE_PATH']}/user-blue.gif' border='0' title='" . attr($link_title) . "' alt='" . xla('View Patient') . "' />";
                    if ($catid == 1) $content .= "<strike>";
       ...
       if($_SESSION['pc_facility'] == 0){
          ...
          if ($event['catid'] == 2) {
              ...
              echo "<div class='" . attr($evtClass) . " event in_start' style='top:".$inTop.
          ...
          // For "OUT" events, applying the background color in CSS.
            ...
            if ($event['catid'] != "6") {
              $background_string= ($event['catid'] == 3) ? "" : ";background-color:" . attr($color);
...
<script>
    ...
    $(function(){
        ...
        $('div.providerheader').find('*')
        .css('font-size', $('div.providerheader:first').css('font-size'))
        .addClass(' mx-1 p-0 border-0 align-middle');
        $('a.providerXbtn').addClass('close');

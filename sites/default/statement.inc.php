...
$STMT_TEMP_FILE = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/openemr_statements.txt";
$STMT_TEMP_FILE_PDF = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/openemr_statements.pdf";
$STMT_PRINT_CMD = (ServiceContainer::getCrypto())->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('more_secure')['print_command']);
...
function report_header_2($stmt, $providerID = '1')
{
    ...
    <table style="width:100%;">
        <tr>
            <?php
                $haveLogo = false;
            if (empty(!\OpenEMR\Core\OEGlobalsBag::getInstance()->get('statement_logo'))) {
                $practice_logo = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/" . convert_safe_file_dir_name(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('statement_logo'));
            } else { // 'ya never know.
                    $practice_logo = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/practice_logo.gif"; // can see is safe...
            }
...
#minimum_amount_due_to _print
    if ($stmt['amount'] <= (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('minimum_amount_to_print')) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_statement_print_exclusion') && ($_POST['form_category'] !== "All")) {
        return "";
    }
...
// Billing location modified by Daniel Pflieger at Growlingflea Software
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_dunning_message')) {
        if ($stmt['ins_paid'] != 0 || $stmt['level_closed'] == 4) {
            // do collection messages
            switch ($stmt['age']) {
                case $stmt['age'] <= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_dun_msg_set'):
                    $dun_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('first_dun_msg_text');
                    break;
                case $stmt['age'] <= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('second_dun_msg_set'):
                    $dun_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('second_dun_msg_text');
                    break;
                case $stmt['age'] <= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('third_dun_msg_set'):
                    $dun_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('third_dun_msg_text');
                    break;
                case $stmt['age'] <= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fourth_dun_msg_set'):
                    $dun_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fourth_dun_msg_text');
                    break;
                case $stmt['age'] >= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fifth_dun_msg_set'):
                    $dun_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fifth_dun_msg_text');
                    break;
            }
        }
    }
    ...
    foreach ($stmt['lines'] as $line) {
        $description = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_custom_statement') ? substr((string) $line['desc'], 0, 30) : $line['desc'];
    ...
    for ($age_index = 1; $age_index < ($num_ages - 1); ++$age_index) {
        $ageline .= ' | ' . ($age_index * 30 + 1) . '-' . ($age_index * 30 + 30) . ':' .
            sprintf(" %.2f", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_currency_symbol') . '' . $aging[$age_index]);
    }
    ...
    $label_bill_phone = (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('billing_phone_number')) ? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('billing_phone_number') : $row['phone'] );
    $label_appointments = xl('Future Appointments') . ':';

    // This is the top portion of the page.
    $out .= "\n\n\n";
    if (strlen((string) $stmt['bill_note']) != 0 && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('statement_bill_note_print')) {
        $out .= sprintf("%-46s\n", $stmt['bill_note']);
        $count++;
    }
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('statement_message_to_patient')) {
        $out .= "\n";
        $statement_message = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('statement_msg_text');
        $out .= sprintf("%-40s\n", $statement_message);
        $count++;
    }

    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('show_aging_on_custom_statement')) {
        # code for ageing
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('number_appointments_on_statement') != 0) {
        $out .= "\n";
        $num_appts = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('number_appointments_on_statement');
 ...
 ';
    ...
    $practice_cards = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/visa_mc_disc_credit_card_logos_176x35.gif";
    if (file_exists(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/visa_mc_disc_credit_card_logos_176x35.gif")) {
...
function create_statement($stmt)
{
    ...
    if ($stmt['amount'] <= (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('minimum_amount_to_print')) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_statement_print_exclusion')) {
        return "";
    }
    ...
    $label_prompt = xl('We appreciate prompt payment of balances due.');
    $label_dept = xl('Billing Department');
    $label_bill_phone = (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('billing_phone_number')) ? \OpenEMR\Core\OEGlobalsBag::getInstance()->get('billing_phone_number') : $billing_phone );
    $label_appointments = xl('Future Appointments') . ':';

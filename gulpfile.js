...
var config = {
    ...
    src: {
        styles: {
            ...
            tabs: 'interface/themes/tabs_*_*.scss'
        }
    },
    dist: {
...
gulp.task('styles:tabs', function() {
    return gulp.src(config.src.styles.tabs)
        .pipe(sourcemaps.init())
        .pipe(sass().on('error', sass.logError))
        .pipe(prefix('last 1 version'))
        .pipe(gap.prependText(autoGeneratedHeader))
...
gulp.task('styles', ['styles:style_uni', 'styles:style_color', 'styles:tabs']);

...
5. **Byte-identical drift canary across rel branches** (`validate-byte-identical.yml`). The orchestrator-driven pipeline depends on a set of files being character-for-character identical across master and every rel branch. The current set, defined in [`.github/byte-identical.yml`](../.github/byte-identical.yml), is:

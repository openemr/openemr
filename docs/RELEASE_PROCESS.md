...
**Trigger caveat (as of 2026-07-27).** The acceptance-package `paths:` filter matches `tools/release/**`, `build.xml`, `.gitattributes`, `.github/workflows/acceptance-package.yml`, `tests/Acceptance/**`, `composer.json`, `composer.lock`. It intentionally does NOT match `version.php` or `docker/production/docker-compose.yml` — the two files a typical release-prep PR touches — because those get bumped in many non-release contexts (dev-cycle bumps, backport version ticks) and would over-fire. So today the workflow only actually fires on a release-prep PR when its diff piggybacks something in the paths list (a coincidental `tools/release/**` change, an acceptance-harness edit, etc.) OR when a maintainer manually `workflow_dispatch`es it. The intended coverage pattern is `release-prep.yml` explicitly dispatching acceptance-package after opening/updating the release-prep PR (`gh workflow run acceptance-package.yml --ref release-prep/<branch> -f build_locally=true`) — that follow-up is not yet wired. Once it lands, every release-prep push will fire the full acceptance run and by merge time the exact-same-shape tarball will have passed acceptance end-to-end. Until then, the pre-publish gate at runbook step 10 (Phase 7c, openemr/openemr#13207) is the load-bearing check; Phase 3.5 supplies the head-branch detection that Phase 7c's release-prep-dispatch follow-up will eventually route to.

See runbook step 10 for the pre-publish validation pass Phase 7c adds against the literal shipped tarball. Effective from rel-830 onward (rel-820 intentionally not backported).
...
5. **[Automated]** Continuous cycle: as further commits land on the rel-branch during the dev cycle, `release-prep.yml` re-fires on every push and both draft PRs regenerate (including a fresh `CHANGELOG.md` block). No maintainer action in this window — iterate until QA sign-off and the release-cycle commits have settled. If any push in this window also touches the acceptance-package `paths:` filter (`tools/release/**`, `build.xml`, `.gitattributes`, `.github/workflows/acceptance-package.yml`, `tests/Acceptance/**`, `composer.json`, `composer.lock`) OR if the maintainer manually dispatches acceptance-package against the release-prep branch, the Phase 3.5 gate fires with `build_locally=true` and validates the fresh PR-built tarball against the full acceptance matrix. A red acceptance run on the release-prep PR blocks merge readiness the same way a red integration-test run would. Full every-push coverage is a follow-up — see the [Conductor PR section](#conductor-pr--release-preprel-branch-in-openemropenemr)'s trigger-caveat paragraph. The pre-publish gate at runbook step 10 (Phase 7c) is the currently-load-bearing acceptance check.
...
10. **[Automated]** Create the **GitHub Release object** on `openemr/openemr` for the new tag and attach the build artifacts. This is the canonical (and only) distribution target — SourceForge is no longer supported, and the website's `/downloads/` and `/releases/` pages link directly to assets on the Release object (see step 14). A tag alone is not enough: without a Release object the website's "Download" button 404s.
    ...
    - **`acceptance-gate`** calls [`acceptance-package.yml`](../.github/workflows/acceptance-package.yml) as a reusable workflow, pointing it at the `build-package` artifact. Runs the full acceptance matrix (`fresh-install`, `wizard-install`, `upgrade`, `wizard-upgrade`) against the exact tarball `publish` would upload. Same harness that Phase 3.5 (openemr/openemr#13204) validates release-prep PRs with — this is the same validation applied one more time, this time against the actual release-time tarball rather than the release-prep-time build of the same-shape tree. Skipped on `dry_run` (nothing to publish, so nothing to gate).
    - **`publish`** re-generates the app token (GHA masks token outputs across jobs), re-checks out the release branch (for `git tag`), re-downloads the artifact, then runs the same "Create annotated tag and GitHub release" step (no-op-safe when the tag already exists) and proceeds to `gh release create --verify-tag --notes-file changelog.md` and uploads the packages + checksums + changelog with `gh release upload --clobber`. Depends on both prior jobs succeeding — a failed `acceptance-gate` skips `publish` entirely, leaving the annotated tag in place on `rel-*` with no GitHub Release object created.

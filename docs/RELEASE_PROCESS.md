...
1. On the `release-prep/<rel-branch>` PR, mark it ready-for-review (out of draft). Undrafting is the maintainer's "ready to ship" signal — ship-release preflight requires the Conductor to be non-draft. Approvals are optional (informational only); GitHub's `reviewDecision` gate is only meaningful when the target rel branch has branch-protection review requirements enabled, so ship-release doesn't require it (see `ShipReleaseOrchestrator::requiresApproval` docblock).

...
The automation core spans four repositories and is driven by `repository_dispatch` events emitted by this repo as the conductor. Two of them open reviewable PRs — the conductor PR here (code/version bumps), and the docs PR in `website-openemr` (install/upgrade/release-notes pages). The third (`openemr-devops`) consumes `openemr-tag` to produce the GitHub Release object + per-channel announcement drafts; the fourth (`demo_farm_openemr`) is updated by a direct automated push rather than a PR. Two steps are still done by hand today (merging the `website-openemr` docs PR, social/forum/email announcement fan-out); see [Automation gaps](#automation-gaps).
...
The two PRs merge in strict order **conductor → docs.** The conductor merge creates the annotated tag (which flips the docs PR's banner from DRAFT to FINAL and fires the `openemr-tag` cascade for the Release object, demo-farm bump, and announcement drafts); merging the docs PR ships the now-FINAL pages.
...
| 9 | Auto-mark the `website-openemr` docs PR Ready on `openemr-tag` (or some pre-ship signal); the page banner already flips to FINAL via the manifest, but the PR stays a GitHub draft that an operator must mark Ready before ship-release.yml's preflight will pass. Once marked Ready, ship-release.yml merges it; the manual step is the mark-Ready, not the merge itself. | [openemr/openemr-devops#761](https://github.com/openemr/openemr-devops/issues/761) |

...
**Why there's no acceptance gate inside `docker-build-release.yml` itself (i.e. no Phase 7c-docker to mirror Phase 7c-tarball).** The docker release pipeline is triggered by `docker-release-orchestrator.yml` on a master push touching `.github/release-targets.yml`, which happens when the release-finalize PR merges to master. Finalize can't merge until the tarball GitHub Release object exists, and tarball publish is now gated by tarball acceptance (Phase 7c, openemr/openemr#13207). So a broken release-cycle codebase blocks tarball publish → blocks finalize merge → blocks docker orchestrator: **the docker publish is transitively gated by tarball acceptance already**. What that transitive gate doesn't cover is Dockerfile-side / docker-runtime-only regressions that wouldn't affect the tarball. Phase 7a-docker (the release-prep dispatch above) fills exactly that gap by exercising the docker path at the same release-prep-PR moment tarball acceptance validates the tarball path. So the docker side gets pre-merge coverage without needing a build → validate → publish refactor of the docker-release pipeline itself.

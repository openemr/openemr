...
Land a **single** PR against the rel branch that bumps `$v_patch` by one AND sets `$v_tag = '-dev'` in `version.php` (e.g. `8.1.0` → `8.1.1-dev` on `rel-810`). Leading hyphen required; single PR required (see [G38](release-mechanism-gaps.md#g38--split-fix-on-the-versionphp-trigger-pr-trips-patch-prep-automations-delta-gates--noted-2026-09-16) for the failure mode + recovery command if the fix splits). On merge, [`patch-prep-automation.yml`](../.github/workflows/patch-prep-automation.yml) opens the patch-cycle PRs and [`release-prep.yml`](../.github/workflows/release-prep.yml) opens the release-prep + release-finalize draft pair. See the [patch-prep workflow entry](#lifecycle-event-workflows-siblings).
...
DORA's original "four keys" were expanded in 2024 to five metrics, and MTTR was replaced with the more-precise "Failed Deployment Recovery Time" (recovery from failed *deployments* specifically, not generic operational incidents). DORA groups the five as **Throughput** (3) + **Instability** (2) — the second group name captures that higher values mean more failures / more unplanned rework (worse), not more stability. See [DORA's metrics history](https://dora.dev/insights/dora-metrics-history) for the research behind the rename.

**Proxy definitions (mapped to DORA metrics):**

| Our proxy | DORA metric | Group | How we measure it |
|---|---|---|---|
| **Deployment Frequency** | Deployment Frequency | Throughput | Days between the previous release (on any rel line) and this one. |
| **Change Lead Time (cut → ship)** | Change Lead Time | Throughput | Days from the rel-branch cut date (approximate proxy for "changes stopped landing on this rel line") to the ship date. Doesn't capture pre-cut lead time; a better measure would sample individual PRs but the cut-to-ship value is what we can compute mechanically without instrumentation. |
| **Ship-time Recovery** | Failed Deployment Recovery Time | Throughput | Wall-clock hours from the first acceptance-gate failure (or preflight block) to green publish. `n/a` when no cascade. DORA's metric is about post-deploy production-incident recovery; ours is pre-publish recovery inside the release-mechanism (bugs never reach users because they block the publish itself). |
| **Ship-time Cascade Rate** | Change Fail Rate | Instability | Yes/no per ship + a count of same-day cascade fixes that were required to actually publish. DORA's CFR is about deployments that "reach production but need rollback / intervention"; ours captures pre-publish cascades that also required intervention but never reached users. Same class of signal (latent bug surfaced by deployment), different scope. |
| **Post-ship Rework Rate** | Deployment Rework Rate | Instability | Per-release: **Yes** if a subsequent same-rel-line patch (e.g. 8.3.1 following 8.3.0) shipped primarily to fix regressions the earlier release introduced; **No** if the observation window closed without such a patch; **TBD** while the window is still open. Observation window closes when either (a) a same-rel-line patch ships (examine its contents to decide Yes/No), or (b) the rel line is EOL'd. Cause note captures the specific regression(s) when Yes. **Aggregate rate** across N releases = Yes-count / (Yes + No), excluding TBD. This is the metric that aligns most closely with DORA's original definition — it captures problems caught *after* users have the release. |

**Per-release data:**

| Version | Shipped | From | Dep. freq (days since prev) | Lead time (cut → ship, days) | Ship-time Recovery | Ship-time Cascade | Post-ship Rework? | Gap entry |
|---|---|---|---|---|---|---|---|---|
| 8.2.0 | 2026-07-08 | rel-820 | 147 (since 8.0.0 on 2026-02-11; 8.1.0 was skipped) | ~6 | n/a | n/a (manual click-through) | No | — |
...
- **Ship-time cascade rate on automated ships: 100% (2/2).** Both cascades traced to "first real exercise of code path X since infra change Y." G33 exposed 7 preflight-gate paths that had never been end-to-end-tested until an actual ship needed them. G35 exposed 3 recovery-path bugs that were latent since a 2026-07-30 refactor and surfaced only when the 8.4.0 ship needed the recovery path. Same systemic driver, not per-ship bad luck.
- **Deployment frequency shortening** (147 days 8.0.0 → 8.2.0 during the manual-ship era; 41 then 26 days between automated ships). Moving to bi-weekly would test the DORA "small batches → lower failure rate" hypothesis directly against our proxies.
- **Ship-time Recovery shrinking** (~24h on 8.3.0 → ~2h on 8.4.0) as the recovery playbook + operator familiarity mature. Not enough data yet to know if this holds or was a one-off.
- **Post-ship Rework Rate: 0/2 with closed observation windows** — neither 8.2.0's nor 8.3.0's same-rel-line window has produced a regression-fix patch (no 8.2.1, no 8.3.1 as of this writing). Both count as No. 8.4.0's observation window is open (rel-840 hasn't shipped a patch or been EOL'd), so it's TBD and excluded from the aggregate. Aggregate rate = 0/2 = 0%.
...
9. **[Automated]** Run the **ship-release workflow** in `openemr/openemr` (`workflow_dispatch` on `.github/workflows/ship-release.yml`, or `task ship-release` locally for a dry-run). One operator action: pick the version + rel-branch + mode and trigger.
   ...
   **Modes (`mode` input, default `semi-auto`):**
   ...
   - **`semi-auto`** (default): merges Conductor PR only. Docs + Finalize PRs marked SKIPPED_BY_MODE (still success — exit 0), left for maintainer to review + merge manually. Use for the first 1-2 releases after wiring up the automation so surprising mutator/EHI/finalize output can be caught in review before committing to full-auto. **Manual merge order after the GitHub Release object appears: Finalize first, THEN wait for Docker publish to complete, THEN merge Docs.** Finalize's merge advances master's `.github/release-targets.yml`, which fires `docker-release-orchestrator.yml` to publish the tagged Docker images. Watch that orchestrator's fan-out complete successfully (all `Docker build release` runs for the shipping rel branch — including its `acceptance-gate` matrix and the final `publish + cleanup` job) before merging Docs, because merging Docs fires the `release-announcements.yml` pipeline and the website announcement goes live pointing at a shipped tarball. If Docker publish is still in flight when Docs merges the announcement is premature; if Docker publish broke (Docker Hub 502s, acceptance-gate rejections) the announcement goes live pointing at a release without shipped Docker images. Finalize-before-Docs alone only STARTS docker earlier — the wait between Finalize and Docs is what makes it a completion gate. Full-auto handles this ordering + waits internally.
   ...
   - **`dry-run`**: preflight + full dress-rehearsal build + acceptance. Probes every PR's readiness and prints a report (merges nothing); on preflight success, the workflow layers a `dry-run-build` job that invokes the reusable `build-release.yml` with `dry_run=true`, pinning its checkout to the Conductor PR head (`release-prep/<rel_branch>`) so the packaged tree carries the pending version bump + `## [X.Y.Z]` CHANGELOG entry that `extract-changelog-section.php` requires. Produces the actual tarball, zip, changelog, and checksums as a workflow-run artifact (named `openemr-release-candidate-<version>`) AND runs the full Phase 7c `acceptance-gate` matrix against the built bundle — no git tag is created, no GitHub Release is published, no downstream dispatches fire, only the `publish` job itself is skipped on `dry_run`. Runtime is longer (~30-40 min including acceptance) but rehearsal is the point: catch any acceptance failure now, not on real ship. Download the artifact from the run page to eyeball what would ship before firing a real ship-release. `dry_run: true` still supported as a legacy alias and always wins over `mode` when set. (G43 decoupling 2026-09-19 split the acceptance-gate skip out of `dry_run` — pre-fix `dry_run` silently suppressed acceptance too, defeating the dress-rehearsal intent.)
...
10. **[Automated]** Create the **GitHub Release object** on `openemr/openemr` for the new tag and attach the build artifacts. This is the canonical (and only) distribution target — SourceForge is no longer supported, and the website's `/downloads/` and `/releases/` pages link directly to assets on the Release object (see step 14). A tag alone is not enough: without a Release object the website's "Download" button 404s.
    ...
    - **`acceptance-gate`** calls [`acceptance-package.yml`](../.github/workflows/acceptance-package.yml) as a reusable workflow, pointing it at the `build-package` artifact. Runs the full acceptance matrix against the exact bundle `publish` would upload — **8 cells: `fresh-install`, `wizard-install`, `upgrade`, `wizard-upgrade`, each against both `.tar.gz` AND `.zip` formats** (Phase 3.6 slice 1 + slice 2 landed 2026-07-29). Catches format-specific regressions (line-ending translation, exec-bit preservation, empty-dir handling, path-separator edges) that byte-similar content wouldn't otherwise surface. Same harness that Phase 3.5 (openemr/openemr#13204) validates release-prep PRs with — this is the same validation applied one more time, this time against the actual release-time bundle rather than the release-prep-time build of the same-shape tree. Skipped only when the dedicated `skip_acceptance_gate=true` input is passed (as `recovery-path-smoketest.yml`'s source-artifact dispatch does — it runs the acceptance matrix downstream via its own variant-A/B dispatches so running it here would duplicate the work). Independent of `dry_run` since G43 (2026-09-19); ship-release's `mode=dry-run` intentionally runs the gate as part of the full dress rehearsal.

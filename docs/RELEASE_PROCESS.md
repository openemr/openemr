...
**Pre-merge artifact acceptance validation (Phase 3.5, openemr/openemr#13204).** Every push to `release-prep/<rel-branch>` fires [`acceptance-package.yml`](../.github/workflows/acceptance-package.yml) with `build_locally=true` — `detect-mode` recognises the head-ref pattern and force-enables the flag regardless of what files the PR touches. A dedicated `build-tarball` job runs `PackageAssembler` against the release-prep PR's checkout and hands the resulting `openemr-<version>.tar.gz` to the full acceptance matrix (`fresh-install`, `wizard-install`, `upgrade`, `wizard-upgrade`). The release version is parsed out of the PR title (`chore(release): prep X.Y.Z`) so downstream logs and the artifact filename reflect the actual release being validated. Effect: **by the time the release-prep PR is ready to merge, its exact-same-shape tarball has already passed acceptance end-to-end**, so a real codebase regression is caught pre-merge rather than surfacing at build-release-on-tag time. See runbook step 10 for the second validation pass Phase 7c adds against the literal shipped tarball. Currently effective from rel-830 onward (rel-820 intentionally not backported).
...
5. **[Automated]** Continuous cycle: as further commits land on the rel-branch during the dev cycle, `release-prep.yml` re-fires on every push and both draft PRs regenerate (including a fresh `CHANGELOG.md` block). No maintainer action in this window — iterate until QA sign-off and the release-cycle commits have settled. During this window `acceptance-package.yml` also runs on every push to `release-prep/<rel-branch>` (Phase 3.5 gate — see the [Conductor PR section](#conductor-pr--release-preprel-branch-in-openemropenemr) for detail): every regeneration cycle produces a fresh PR-built tarball and runs it through the full fresh-install + wizard-install + upgrade + wizard-upgrade matrix. A red acceptance run on the release-prep PR blocks merge readiness the same way a red integration-test run would — investigate before proceeding to Phase 3.
...
10. **[Automated]** Create the **GitHub Release object** on `openemr/openemr` for the new tag and attach the build artifacts. This is the canonical (and only) distribution target — SourceForge is no longer supported, and the website's `/downloads/` and `/releases/` pages link directly to assets on the Release object (see step 14). A tag alone is not enough: without a Release object the website's "Download" button 404s.
    ...
    **Failure recovery** (Phase 7c documents the full mechanism in openemr/openemr#13207's PR body):
    ...
    In all four cases no cleanup of a partial-publish state is needed — `publish` is atomic (whole release page lands or nothing does), and the annotated tag being in place ahead of `build-release` is cosmetic in all cases except real codebase regression (where the version bump is the recovery).

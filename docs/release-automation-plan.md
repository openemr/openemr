...
**Also observed (deferred / GitHub-side, not code fixes):** `release-amendment.yml`'s peter-evans step timed out reading GitHub's API response after successful PR create/update (had to rerun three times before all three peter-evans steps + Release-body edit completed cleanly); Docker Hub `docker buildx imagetools create` bounced with an HTTP 502 on master's publish (rel-840's publish 90 seconds later succeeded — retryable transient); `site` + `release-docs-ci` workflows hit `startup_failure` on the docs PR merge (rerun cleared both).

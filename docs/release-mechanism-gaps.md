...
**Root causes (all instances):**
- **Design assumed cooperative maintainer + full-admin merge context.** Every preflight gate that surfaced (mergeStateStatus, reviewDecision, App-token perspective, ruleset bypass, contents:write) was designed against a mental model of "a human admin does the merge from the UI." App tokens have narrower default permission grants than human sessions, and rel-branch protection is not necessarily configured with the review-requirement gate the code assumed.
- **First-ever exercise of a code path.** Every ship-release test dispatch before 2026-08-17 failed at earlier preflight gates, so the merge-time + merge-API-permission gates were never exercised. Same shape as G31's systemic lesson but for the SHIP path: latent bugs accumulate in ship-side infra between real ships (typically 4-6 weeks apart) and surface as a cascade the first time a real ship runs.
- **`ChangelogMutator` misfile risk lifted from `sql_upgrade.php` files.** #13490 authored against the wrong file (`8_2_0-to-8_3_0_upgrade.sql`) because the file-rename-dance at rel-830 cut left the pre-cycle file populated on master. See G32 for the review-time-signal follow-up.

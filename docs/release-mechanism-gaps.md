...
- PR 8 — add CompatibilityMutator (thread CompatibilityDeriver + CompatibilityNotesRenderer through the mutator flow so the CHANGELOG.md gets its Minimum-supported-versions section; also fixes CompatibilityNotesRenderer::inject() non-idempotence at the same time — CR round-2 finding from PR 4). Slotted after PR 7, non-blocking.

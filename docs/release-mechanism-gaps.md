...
- **Preflight-deadlock root cause (applies to the seven code fixes #13570 / #13574 / #13581 / #13582 / #13590 / #13607 / #13619).** Ship-release preflight was designed against a mental model of "a human admin does the merge from the UI." Every gate that surfaced (mergeStateStatus semantics per-token, reviewDecision null when branch protection doesn't require reviews, contents:write vs pull-requests:write for the merge API, rollup dedup for stale re-runs, downstream draft tolerance, finalize-PR-aware validator, ignore-list for upstream-known-broken jobs) reflected an assumption about App-vs-human context that only got tested when a real ship reached merge-time. Prior test dispatches all failed at earlier preflight gates and returned before hitting the merge API, so the merge-time + merge-API-permission gates were never exercised.

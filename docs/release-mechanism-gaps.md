...
- **Regression canaries at layer boundaries pay off.** The `git sparse-checkout list` guard added in #14199 (retained through #14203) fires within seconds of any future re-introduction of the leak, instead of surfacing hours later as a misleading downstream failure (empty sync PR, rename-sweep abort). Same shape as G43's "make gate independent from `dry_run`" and G37's preflight-gate philosophy: assert the invariant at the layer that's actually affected.

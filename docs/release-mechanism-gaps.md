...
- **openemr/openemr#13002 + #13003** — first real amendment PRs (8.2.0-rel-820 + 8.2.0-master), opened by the amendment workflow's first real dispatch on 2026-07-15. Contain the correct amended CHANGELOG diff (Security section with `GHSA-vv5j-6gjw-ffx9`, compare link updated to `v8_1_0...v8_2_0`, date preserved as 2026-07-08). (open, awaiting review)
- **openemr/openemr#13004** — extract-order fix. First real dispatch of the amendment workflow surfaced a second bug: `peter-evans/create-pull-request@v8` resets the calling checkout's working tree back to base-branch HEAD after committing to the amendment branch. The Extract step (which pipes to `gh release edit --notes-file`) was running AFTER peter-evans, so it saw the pre-amendment content. Release body + attachment got overwritten with byte-identical pre-amendment content — no user-visible damage, but the amendment didn't actually take. Fix: reorder Extract to run BEFORE peter-evans; extracted content lives in `$RUNNER_TEMP/section.md` which survives peter-evans's reset. (open, in review)
...
- Real dispatch (run 29397657588, 07:31Z, post-#12999 but pre-#13004): all steps green, amendment PRs #13002 + #13003 opened with correct content, but Release body edit put byte-identical pre-amendment content because of the peter-evans reset. Prompted #13004.
- Re-dispatch after #13004 lands: will force-update #13002 + #13003 (idempotent — same content) and correctly write the amended Security section to the Release body + `changelog.md` attachment.

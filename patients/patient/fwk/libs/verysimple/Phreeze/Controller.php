...
abstract class Controller {
	 ...
	 *        	the view varname to use for assignment
	 */
	...
	/**
	 * Returns a stored CSRF Token from the session.
	 * If no token exists, then generate
	 ...
	 * If not an Exception with be thrown.
	...
	/**
	 * Start observing messages from Phreeze.
	 * If no observer is provided, then
	 * an ObserveToBrowser will be used and debug messages will be output to the browser
	 * 
	 * @param IObserver $observer        	
	 * @param bool $with_styles
	 *        	if true then basic styles will be output to the browser
	 */
	 ...
	 * if a querystring parameter name is
	 * provided, that will be checked first before the 401 auth headers
	 * 
	 * @param string $qs_username_field
	 *        	the querystring parameter to check for username (optional)
	 */
	...
	/**
	 * accept password passed in either headers or querystring.
	 * if a querystring parameter name is
	 * provided, that will be checked first before the 401 auth headers
	 * 
	 * @param string $qs_password_field
	 *        	the querystring parameter to check for password (optional)
	 */
	...
	/**
	 * Gets the user from 401 auth headers (or optionally querystring).
	 ...
	 * Then should overwrite any applicable
	...
	/**
	 * Displays the ListAll view for the primary model object.
	 * Because the
	 ...
	 * ajax will not be used.
	 * The model data is populated
	 *
	 * @param Criteria $criteria        	
	 * @param int $current_page
	 *        	number of the current page (for pagination)
	 * @param int $limit
	...
	/**
	 * Renders a datapage as XML for use with a datagrid.
	 * The optional $additionalProps allows
...
;
	 ...
	 * input.
	 * The input is validated and a ValidationResponse is rendered in JSON format
	 *
	 ...
	 * be cross-application authentication issues.
	 * Additionally, the remote ip
	...
	/**
	 * Returns true if this request is an API request.
	 * This examines the URL to
	 ...
	 * @param string $on_fail_action
	 *        	(optional) The action to redirect if require fails
	 * @param string $not_authenticated_feedback
	 *        	(optional) Feedback to forward to the on_fail_action if user is not logged in
	 * @param string $permission_denied_feedback
	 *        	(optional) Feedback to forward to the on_fail_action if user is logged in but does not have permission
	 ...
	 *
	 * @param string $view
	 *        	(optional) if not provided, the view is automatically bound using the class and method name
	 * @param string $format
	 *        	(optional) defaults to $self::SmartyViewPrefix
	 */
	 ...
	 * @param int $code
	 *        	used for support for this error
	 * @param Exception $exception
	 *        	if exception was thrown, can be provided for more info
	 */
	...
	/**
	 * Does a recursive UTF8 encoding on a string/array/object.
	 * This is used

...
//
// Modified to accomodate invoicing in the patient portal.
...
<?php 
      ...
      global $pid;
      $pname = getPatientData($pid);
      echo $pname['lname'].", ".$pname['fname'];
      $ago = strtotime("-6 months");
      $nDate = date("m/d/Y", $ago);
      
?>
<form method='post' action='portal_sl_eob_search.php' enctype='multipart/form-data'>
<!-- removed lines    447 - 509 -->
...
<table border='0' cellpadding='5' cellspacing='0'>
 ...
 <tr bgcolor='#ddddff'>
  <td>
      <input type='hidden' name='form_name' size='10' value='<?php echo $pname['lname'].", ".$pname['fname']; ?>'
  </td>
  <td>
   <input type='hidden' name='form_date' size='10' value='<?php echo $nDate; ?>'
    title='<?php xl("Date of service mm/dd/yyyy","e"); ?>'>
  </td>
  <td>
   <input type='hidden' name='form_to_date' size='10' value='<?php echo $pToday; ?>'
    
  </td>
  <td>
   <select name='form_category'>
<?php
 foreach (array( xl('All'), xl('Due Pt')) as $value) {
  echo "    <option value='$value'";
  if ('Due Pt' == $value) echo " selected";
  echo ">$value</option>\n";
 }
?>
   </select>
  ...
  <td>
   <input type='submit' name='form_search' value='<?php xl("Get Statement(s)","e"); ?>'>
  </td>
 </tr>
...
<?php  if($row['lname'] == " "){echo "<font color='red'>No invoices found</font>";} ?>
<?php
  $orow = -1;
...
?> 
 <tr bgcolor='<?php echo $bgcolor ?>'>
  ...
  <td class="detail">
   &nbsp;<?php echo $row['pid'] . '.' . $row['encounter']; ?>
  </td>
  <td class="detail">
...
<?php } else { ?>
<input type='submit' name='form_pdf' value='<?php xl('Print PDF Selected Statements','e'); ?>' /> &nbsp;
<?php } ?>
<input type='hidden' name='form_without' value='1' /> 
</p>

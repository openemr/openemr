...
{
    public function __construct(private readonly string $rcb_selectors = 'block', private readonly string $rcb_facility = '', private readonly string $rcb_provider = '')
    {
    }
            ...
            <div class="col-12 jumbotron p-4">
                <div class="showRFlow text-center" id="show_recalls_params">
                    <form name="rcb" id="rcb" method="post">
                        ...
                        <div name="message" id="rcb_message" class="warning"></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        function toggleRcbSelectors() {
            var newState = ($("#rcb_selectors").css('display') === 'none') ? 'block' : 'none';
            $.post("<?php echo $globals->get('webroot') . "/interface/main/messages/messages.php"; ?>", {
                'rcb_selectors': newState,
                'csrf_token_form': typeof csrfTokenForm !== 'undefined' ? csrfTokenForm : ''
            }, function () {
                $("#rcb_selectors").slideToggle();
                $("#rcb_caret").css('color', newState === 'block' ? 'var(--black)' : 'var(--danger)');
            });
        ...
        var translations = {
            ...
            confirm_delete: <?php echo xlj('Are you sure you want to delete this Recall?'); ?>
        };
    </script>
                        ...
                        <div class="form-group">
                            ...
                            <div id="postcardPreview" class="postcard-wrapper" style="max-width:760px;margin:auto;">
                                <div class="postcard-back" id="postcardBack" style="position:relative;width:100%;padding-top:62%;background-size:cover;background-repeat:no-repeat;background-position:center;border:1px solid #ddd;border-radius:4px;overflow:hidden;">
                                    <div id="postcardMessage" style="position:absolute;left:3%;top:5%;width:44%;height:90%;overflow:auto;padding:10px;box-sizing:border-box;color:#000;font-size:14px;line-height:1.35;"></div>
                                    <div id="postcardAddress" style="position:absolute;right:3%;top:30%;width:42%;height:50%;overflow:auto;padding:10px;box-sizing:border-box;color:#000;font-size:13px;line-height:1.25;text-align:left;"></div>
                                </div>
                            </div>
                            <style>
        ...
        <script>
                ...
                var postcardBackUrl = <?php echo json_encode($globals->getString('webroot') . '/public/images/postcard_back.svg'); ?>;
                ...
                function showToast(message, isError) {
                    var toastId = 'rcbToast_' + Date.now();
                    var bg = isError ? 'bg-danger' : 'bg-success';
                    var html = '<div id="' + toastId + '" class="toast align-items-center text-white ' + bg + ' border-0" role="alert" style="position:fixed;top:20px;right:20px;z-index:99999;min-width:250px;">' +
                        '<div class="d-flex"><div class="toast-body">' + message + '</div>' +
                        '<button type="button" class="ml-2 mb-1 close text-white" data-dismiss="toast"><span>&times;</span></button></div></div>';
                    $('body').append(html);
                    var $t = $('#' + toastId);
                    $t.toast({ delay: 3000 }).toast('show');
                    $t.on('hidden.bs.toast', function() { $(this).remove(); });
                }

                $('#savePostcardTemplateBtn').on('click', function() {
                    var val = $('#postcardTemplateText').val();
                    $.post('<?php echo $globals->get('webroot') . "/interface/main/messages/save.php"; ?>', { action: 'save_postcard_template', postcard_top: val, csrf_token_form: csrfTokenForm }, function(resp) {
                        try {
                            var j = (typeof resp === 'string') ? JSON.parse(resp) : resp;
                            if (j.success) {
                                rcb_postcard_template = val;
                                showToast('<?php echo xla('Template saved'); ?>', false);
                                $('#rcbPostcardModal').modal('hide');
                            } else {
                                showToast('<?php echo xla('Save failed'); ?>', true);
                            }
                        } catch (e) { showToast('<?php echo xla('Save failed'); ?>', true); }
                    }).fail(function() { showToast('<?php echo xla('Save failed'); ?>', true); });
                });
        </script>
        <?php
    ...
    /** @param array<array<mixed>>|null $recalls */
    private function recall_board_process($recalls)
    {
        ...
        foreach ($recalls as $recall) {
            ...
            if ($apptCheck) {
                ...
                (new RecallService())->deleteRecallByPidOrId(
                    is_int($recall['r_pid']) ? $recall['r_pid'] : null,
                    is_int($recall['r_ID']) ? $recall['r_ID'] : null
                );
            ...
            $age = RecallService::getAge($result2['DOB']);

            echo '<div class="divTableCell"><a href="#" onclick="show_patient(' . attr(js_escape((string) $recall['r_pid'])) . ');"> ' . text($recall['fname']) . ' ' . text($recall['lname']) . '</a>';
            ...
            echo '<br /><span class="small" data-toggle="tooltip" data-placement="auto" title="' . xla("Date of Birth and Age") . '">' . xlt('DOB') . ': ' . text($DOB) . ' (' . text((string) $age) . ')</span>';
            echo '</div>';
            ...
            if (( $count_facilities > 1 ) && ( $this->rcb_facility == '' ) && isset($facility[$recall['r_facility']])) {
            ...
            if ($recall['phone_cell'] > '') {
                echo xlt('C') . ': ' . text($recall['phone_cell']) . "<br />";
            }
            if ($recall['phone_home'] > '') {
                echo xlt('H') . ': ' . text($recall['phone_home']) . "<br />";
            }
            if ($recall['email'] > '') {
                $mailto = $recall['email'];
                ...
                echo xlt('E') . ': <a data-toggle="tooltip" data-placement="auto" title="' . xla('Send an email to ') . attr($mailto) . '" href="mailto:' . attr($mailto) . '">' . text($recall['email']) . '</a><br />';
            }
            ...
            echo '<input type="checkbox" name="msg_phone" id="msg_phone_' . attr($recall['r_pid']) . '" onclick="process_this(' . attr(js_escape('phone')) . ',' . attr(js_escape((string) $recall['r_pid'])) . ',' . attr(js_escape((string) $recall['r_ID'])) . ')" />';
            echo '</span>';
            echo '<span data-toggle="tooltip" data-placement="auto" title="' . xla('Scheduling') . '" class="far fa-calendar-check fa-fw" style="cursor:pointer;" onclick="newEvt(' . attr(js_escape((string) $recall['r_pid'])) . ',\'\')"></span>';
            echo '</div>';
            ...
            echo '<textarea onblur="process_this(' . attr(js_escape('notes')) . ',' . attr(js_escape((string) $recall['r_pid'])) . ',' . attr(js_escape((string) $recall['r_ID'])) . ');" name="msg_notes" id="msg_notes_' . attr($recall['r_pid']) . '" placeholder="' . xla('Add note...') . '" style="width:98%;height:60px;"></textarea>';
            echo '</div>';
            ...
            <i class="fas fa-pen-to-square" style="cursor:pointer; margin-right:8px;" onclick="goReminderRecall(' . attr(js_escape('addRecall')) . ',' . attr(js_escape((string) $recall['r_pid'])) . ');" title="' . xla('Edit Recall') . '"></i>
            <i class="fas fa-xmark" style="cursor:pointer;" onclick="delete_Recall(' . attr(js_escape((string) $recall['r_pid'])) . ',' . attr(js_escape((string) $recall['r_ID'])) . ');" title="' . xla('Delete Recall') . '"></i>';
            echo '</div>';
            echo '</div>';
    ...
    private function recall_board_top()
    {
        ...
        // Allow any module to inject buttons by dispatching a RecallBoardToolbarEvent.
        // (Replaces the former scandir/include_once pattern which bypassed the module system.)
        $dispatcher = OEGlobalsBag::getInstance()->getKernel()->getEventDispatcher();
        $event = new RecallBoardToolbarEvent();
        $dispatcher->dispatch($event, RecallBoardToolbarEvent::EVENT_NAME);
        echo $event->getHtml();
        ?>
        <div class="divTable" style="width: 100%;">
            <div class="divTableBody">
                <div class="divTableRow divTableHeading">
                    ...
                    <div class="divTableCell text-center">
                        <?php echo xlt('Postcards'); ?>
                            <div class="postcard-icons-core" style="margin-top:6px;">
                                ...
                                <i class="far fa-square fa-fw" style="cursor:pointer;" onclick="checkAll('postcards',true);" id="chk_postcards" title="<?php echo xla('Select All'); ?>"></i>
                    ...
                    <div class="divTableCell text-center">
                        ...
                        <div class="label-icons-core" style="margin-top:6px;">
                            <i class="far fa-square fa-fw" style="cursor:pointer;" onclick="checkAll('labels',true);" id="chk_labels" title="<?php echo xla('Check All'); ?>"></i>
                ...
                <style>
                    ...
                    /* Prevent patient info (Last Visit, DOB) from wrapping */
                    .divTableCell .small { white-space: nowrap; }
    ...
    private function getRecallActions($pid)
    {
        ...
        foreach ($results as $row) {
            $type = is_string($row['msg_type']) ? $row['msg_type'] : '';
            $msgDate = is_string($row['msg_date']) ? $row['msg_date'] : '';
            $ts = strtotime($msgDate) ?: null;
            $date = date('m/d/Y', $ts);
            $time = date('g:iA', $ts);
            $extraText = is_string($row['msg_extra_text']) ? $row['msg_extra_text'] : '';
            ...
            if ($userName) {
                $actionLine .= '<br /><span style="font-size:0.9em; color:#888;">' . xlt('by') . ' ' . text($userName) . '</span>';
            }

...
class OemrUI
{
    ...
    public function __construct($arrOeUiSettings = [])
    {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_help') !== 0 && $this->display_help_icon) {
    ...
    private function helpFileModal($help_file = '')
    {
        ...
        if ($help_file) {
            ...
            $help_file = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/Documentation/help_files/$help_file";
    ...
    private function headerExpandJs($arrFiles = [])
    {
        ...
        $web_root = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot');

...
class ObservationController
{
    ...
    private function renderReport(int $pid, int $encounter, $cols, $id): string
    {
        ...
        $observations = $result->getData();
        $formattedObs = array_map(fn($observation) => $this->formatObservationForDisplay($observation), $observations);
    ...
    public function formatObservationForDisplay(array $observation): array
    {
        return [
            ...
            'subObservations' => array_map(fn($observation) => $this->formatObservationForDisplay($observation), $observation['sub_observations'] ?? [])
        ];
    }

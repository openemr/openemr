...
class SphereRevert
{
    ...
    public function __construct(string $front)
    {
        ...
        if ($front == 'patient') {
            $this->custid = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_patientfront_trxcustid'));
            $this->custpass = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_ecomm_tc_link_pass'));
        } elseif ($front == 'clinic-retail') {
            $this->custid = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_retail_trxcustid'));
            $this->custpass = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_retail_tc_link_pass'));
        } else { // $front == 'clinic-phone'
            $this->custid = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_trxcustid'));
            $this->custpass = $this->cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_moto_tc_link_pass'));
        }

        $this->client = new Client(['base_uri' => Sphere::TRUSTEE_API_URL]);

        // Calculate the OpenEMR server returnUrl
        $this->returnUrl = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('site_addr_oath') . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root') . '/sphere/initial_response.php';
    }
    ...
    public static function renderRevertSphereJs(): string
    {
        return '
            ...
            function processSphereRevert(token, front, action, transId, uuidTx) {
                ...
                //  if modify this, also need to modify there
                let returnUrl = ' . js_escape(\OpenEMR\Core\OEGlobalsBag::getInstance()->get("site_addr_oath") . \OpenEMR\Core\OEGlobalsBag::getInstance()->get("web_root")) . ' + "/sphere/initial_response.php?action=" + encodeURIComponent(action) + "&front=" + encodeURIComponent(front) + "&uuid_tx=" + encodeURIComponent(uuidTx) + "&revert=1&csrf_token=" + ' . js_url(CsrfUtils::collectCsrfToken("sphere_revert")) . ';

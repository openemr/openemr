...
class SpherePayment
{
    ...
    public function __construct(private readonly string $front, private readonly int $patientIdCc)
    {
        ...
        $testing = empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gateway_mode_production'));

        // Collect the correct trxcustid and trxcustid_licensekey and url
        $cryptoGen = ServiceContainer::getCrypto();
        if ($this->front == 'patient') {
            $frontSpecific = 'patient';
            $trxcustid = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_patientfront_trxcustid'));
            $trxcustidLicensekey = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_patientfront_trxcustid_licensekey'));
            if ($testing) {
            ...
            $trxcustid = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_trxcustid'));
            $trxcustidLicensekey = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_trxcustid_licensekey'));
            $trxcustidRetail = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_retail_trxcustid'));
            $trxcustidRetailLicensekey = $cryptoGen->decryptStandard(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('sphere_clinicfront_retail_trxcustid_licensekey'));
            if ($testing) {
        ...
        $this->serverSite = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('site_addr_oath') . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('web_root');

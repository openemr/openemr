...
class UserRoleListener
{
    ...
    public function onKernelRequest(RequestEvent $event)
    {
        ...
        if (!$kernel instanceof OEHttpKernel) {
            // This listener is only for the OpenEMR OEHttpKernel
            return;
        }

        $request = $event->getRequest();
        if (!$request instanceof HttpRestRequest) {
            // This listener is only for Symfony Requests
            return;
        }
        ...
        if (empty($user)) {
            ...
            $event->getSystemLogger()->error("OpenEMR Error - api user account could not be identified, so forced exit", [
                'userId' => $userId,
                'userRole' => $uuidToUser->getUserRole()]);
    ...
    private function setupSessionForUserRole(string $userRole, array $user, HttpRestRequest $request)
    {

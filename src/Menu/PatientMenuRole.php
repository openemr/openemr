...
class PatientMenuRole extends MenuRole
{
    ...
    public function getMenu()
    {
        ...
        if (preg_match("/.json$/", $patientMenuRole)) {
            ...
            $menu_parsed = json_decode(file_get_contents(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/custom_menus/patient_menus/" . $patientMenuRole));
        } else {
            // load a standardized menu (does not include .json in id)
            $menu_parsed = json_decode(file_get_contents(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . "/interface/main/tabs/menu/menus/patient_menus/" . $patientMenuRole . ".json"));
        }
    ...
    public function displayMenuRoleSelector($selected = "")
    {
        ...
        $customMenuDir = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/custom_menus/patient_menus/";
    ...
    private function getAbsoluteWebRoot($rel_url)
    {
        if ($rel_url && !strpos((string) $rel_url, "://")) {
            ...
            return \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/" . $rel_url;
        }
        return $rel_url;
    }

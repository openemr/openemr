...
/**
 * Quest HL7 order generator — delegates to quest_*() procedural functions.
 *
 * @package   OpenEMR
...
class QuestHl7OrderGenerator implements Hl7OrderGeneratorInterface
{
    public function generate(int $orderId): Hl7OrderResult
    {
        return quest_gen_hl7_order($orderId);
    }

    public function send(int $providerId, string $hl7): string
    {
        return quest_send_hl7_order($providerId, $hl7);
    }

    /** @return array<mixed> */
    public function loadPayerInfo(int $patientId, string $date = ''): array
    {
        return quest_loadPayerInfo($patientId, $date);
    }
}

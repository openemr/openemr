...
class OAuth2KeyConfig
{
    ...
    private function createOrRecreateKeys(): void
    {
        ...
        if (empty($this->oaEncryptionKey)) {
            ...
            EventAuditLogger::getInstance()->newEvent("oauth2", $authUser, $authProvider, 0, $logLabel . "random generator broken during oauth2 encryption key generation");
        ...
        if (empty($this->oaEncryptionKey)) {
            // if empty, then log and force exit
            EventAuditLogger::getInstance()->newEvent("oauth2", $authUser, $authProvider, 0, $logLabel . "base64 encoding broken during oauth2 encryption key generation");
        ...
        if (empty($this->passphrase)) {
            // if empty, then log and force exit
            EventAuditLogger::getInstance()->newEvent("oauth2", $authUser, $authProvider, 0, $logLabel . "random generator broken during oauth2 key passphrase generation");

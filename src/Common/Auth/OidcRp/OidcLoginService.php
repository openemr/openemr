...
final readonly class OidcLoginService
{
    public function __construct(
        private OidcRpSettings $settings,
        private OidcRpClient $client,
        private OidcIdTokenValidator $validator,
        private OidcStaffAuthenticator $authenticator,
    ...
    public static function fromContainer(OEGlobalsBag $globals, SessionInterface $session): self
    {
        ...
        $httpClient = OidcRpClient::createHttpClient();
        ...
        $validator = new OidcIdTokenValidator($httpClient, ServiceContainer::getClock());
    ...
    /**
     * Until main_screen finishes local MFA, other authenticated pages must
     * return to the challenge even if the caller navigates there directly.
     */
    public static function pendingLoginRedirect(SessionInterface $session, string $scriptFilename, string $webRoot): ?string
    {
        if ($session->get(OidcRpSettings::SESSION_NEW_LOGIN) !== true) {
            return null;
        }
        $mainScreen = dirname(__DIR__, 4) . '/interface/main/main_screen.php';
        if (realpath($scriptFilename) === realpath($mainScreen)) {
            return null;
        }
        $site = $session->get('site_id');
        return $webRoot . '/interface/main/main_screen.php?site=' . rawurlencode(is_string($site) ? $site : '');
    }
    ...
    public static function postLogoutRedirectUri(OEGlobalsBag $globals): string
    {
        return self::absolutePostLogoutRedirectUri(
            $globals->getString('login_screen'),
            $globals->getString('site_addr_oath'),
            $globals->getWebRoot(),
            CurrentRequest::get()->getSchemeAndHttpHost(),
        );
    }

    public static function absolutePostLogoutRedirectUri(
        string $loginScreen,
        string $siteAddrOath,
        string $webRoot,
        string $requestOrigin,
    ): string {
        if (str_starts_with($loginScreen, 'http://') || str_starts_with($loginScreen, 'https://')) {
            return $loginScreen;
        }
        $origin = rtrim($siteAddrOath !== '' ? $siteAddrOath : $requestOrigin, '/');
        $path = $loginScreen !== '' ? $loginScreen : ($webRoot . '/interface/login/login.php');

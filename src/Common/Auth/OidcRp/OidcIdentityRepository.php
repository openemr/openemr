...
final class OidcIdentityRepository
{
    ...
    public function findUserId(string $issuer, string $subject): ?int
    {
        $row = QueryUtils::querySingleRow(
            'SELECT `user_id` FROM `oidc_external_identity` WHERE `issuer` = ? AND `subject` = ?',
            [$issuer, $subject],
        );
        if ($row === false) {
            return null;
    ...
    public function bind(int $userId, OidcIdTokenClaims $claims): void
    {
        ...
        if ($existingId === $userId) {
            QueryUtils::sqlStatementThrowException(
                'UPDATE `oidc_external_identity` SET `username` = ?, `email` = ?, `last_login` = ? WHERE `issuer` = ? AND `subject` = ?',

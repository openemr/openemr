...
final readonly class OidcStaffAuthenticator
{
    public function __construct(
        private OidcRpSettings $settings,
    ...
    public function login(OidcIdTokenClaims $claims): void
    {
        ...
        if ($boundId !== null) {
            $user = $this->requireActiveLoginUser($this->findUserById($boundId));
        } else {
    ...
    private function resolveUser(OidcIdTokenClaims $claims, string $username): array
    {
        ...
        if ($this->settings->matchEmail && $claims->email !== '' && $claims->emailVerified) {
    ...
    {
        return QueryUtils::inTransaction(function () use ($claims, $username): array {
            $userData = [
                'username' => $username,
            ...
            if ($userId <= 0) {
                throw new OidcRpException('Unable to create a local user');
            }
            ...
            $assigned = AclExtended::setUserAro(
            ...
            if ($assigned !== true) {
                throw new OidcRpException('Unable to assign access control for the new user');
            }

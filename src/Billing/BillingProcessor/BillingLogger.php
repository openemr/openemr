...
class BillingLogger
{
    ...
    public function __construct()
    {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('billing_log_option') == 1) {
            if (file_exists(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/edi/process_bills.log")) {
                $this->hlog = file_get_contents(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/edi/process_bills.log");
            }
    ...
    public function onLogComplete()
    {
        ...
        if (!empty($this->hlog)) {
            ...
            file_put_contents(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/edi/process_bills.log", $this->hlog);
        }

...
// To Show Eligibility Verification data
    public static function showEligibilityInformation($pid, $flag = false)
    {
        ...
        while ($benefit = sqlFetchArray($result)) {
            ...
            $benefit['start_date'] = strpos((string) $benefit['start_date'], "0000") === false ? $benefit['start_date'] : '';
            $benefit['end_date'] = strpos((string) $benefit['end_date'], "0000") === false ? $benefit['end_date'] : '';
            $color = "";
            switch ($benefit['type']) {
                case '1':
                    $color = "darkred";
...
// return array of provider usernames
    ...
    public static function mimeParse(string $formBody, $contentType)
    {
        ...
        list($contentType, $boundaryDirective) = explode(";", trim((string) $contentType));

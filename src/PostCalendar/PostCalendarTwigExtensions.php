...
class PostCalendarTwigExtensions extends AbstractExtension
{
    ...
    public function monthSelector($Date)
    {
        ...
        include_once(OEGlobalsBag::getInstance()->get('fileroot') . '/interface/main/calendar/modules/PostCalendar/pntemplates/default/views/monthSelector.php');
    }
    ...
    /**
     * Render events for print views.
     * TODO: Implement full event rendering for print templates.
     */
    ...
    private function overlapDetection($times, $interval, $events, $calEndMin, $calStartMin, $providerid)
    {
        ...
        foreach ($times as $slottime) {
            ...
            foreach ($events as $e1) {
                // skip non-event items (e.g. 'blocks' sub-arrays from week view)
                if (!is_array($e1) || !isset($e1['catid']) || !isset($e1['eid'])) {
                    continue;
                }
            ...
            if (!empty($events_in_timeslot)) {
                ...
                foreach ($events_in_timeslot as $eid) {
                    ...
                    } elseif ($eventPositions[$eid]->width > $width) {
                        $eventPositions[$eid]->width = $width;
                    }
                    ...
                    } elseif ($eventPositions[$eid]->leftpos < $leftpos) {
                        $eventPositions[$eid]->leftpos = $leftpos;
                    }
    ...
    public function renderProviderTimeSlots($times, $events, $interval, $provider, $providerid, $calEndMin, $calStartMin, $timeformat, $openhour, $closehour, $timeslotHeightVal, $timeslotHeightUnit, $date, $viewtype)
    {
        ...
        foreach ($events as $event) {
            ...
            // we dont want to exclude those events from being displayed
            if (!empty($event['aid']) && ($providerid != $event['aid'])) {
                continue;
            }
            ...
            if ((int) $starth < (int) $openhour || (int) $starth > (int) $closehour) {
                continue;
            }
            ...
            if ($event['catid'] == 2) {
                ...
                foreach ($events as $outevent) {
                    // skip events for other providers
                    if (!empty($outevent['aid']) && ($providerid != $outevent['aid'])) {
                        continue;
                    }
                    // skip events with blank IDs
                    if (empty($outevent['eid'])) {
                        continue;
                    }

                    if ($outevent['eid'] == $event['eid']) {
                        $found = true;
                        continue;
                    }
            ...
            // of the time table
            if ($eEndMin > $calEndMin) {
                $eEndMin = $calEndMin + $interval;
            }
            ...
            $eventtype = QueryUtils::fetchSingleValue("SELECT pc_cattype FROM openemr_postcalendar_categories as oc LEFT OUTER JOIN openemr_postcalendar_events as oe ON oe.pc_catid=oc.pc_catid WHERE oe.pc_eid=?", 'pc_cattype', [$eventid]);
            $pccattype = '';
            if ($eventtype == 1) {
                $pccattype = 'true';
            }
            ...
            if ($comment && OEGlobalsBag::getInstance()->get('calendar_appt_style') < 4) {
                $title .= " " . $comment;
            }
            ...
            $row = QueryUtils::fetchRecords("SELECT name,id,color FROM facility WHERE id=(SELECT pc_facility FROM openemr_postcalendar_events WHERE pc_eid=?)", [$eventid]);
            $row = $row[0] ?? [];
            $color = $event["catcolor"];
            if (OEGlobalsBag::getInstance()->get('event_color') == 2) {
                $color = $row['color'];
            } else {
                ...
                if ($patientid) {
                    ...
                    $content .= "<i class='fas fa-user text-success' onmouseover=\"javascript:ShowImage(" . attr_js(OEGlobalsBag::getInstance()->get('webroot')."/controller.php?document&retrieve&patient_id=".attr($patientid)."&document_id=-1&as_file=false&original_file=true&disable_exit=false&show_original=true&context=patient_picture") . ");\" onmouseout=\"javascript:HideImage();\" title='" . attr($link_title) . "'></i>";
                    if ($catid == 1) {
                        $content .= "<s>";
                    }
                    $content .= text($lname);
                    if (OEGlobalsBag::getInstance()->get('calendar_appt_style') != 1) {
                        $content .= "," . text($fname);
                        if ($event['title'] && OEGlobalsBag::getInstance()->get('calendar_appt_style') >= 3) {
                            $content .= " (" . text($event['title']);
                            if ($event['hometext'] && OEGlobalsBag::getInstance()->get('calendar_appt_style') >= 4) {
            ...
            if (SessionWrapperFactory::getInstance()->getActiveSession()->get('pc_facility') == 0) {
                ...
                echo "</div>\n";
            } elseif (SessionWrapperFactory::getInstance()->getActiveSession()->get('pc_facility') == $row['id']) {
                if ($event['catid'] == 2) {
                    $inTop = 20 + ($eStartPos - $timeslotHeightVal).$timeslotHeightUnit;
                    echo "<div data-eid='" . attr($eventid) . "' class='" . attr($evtClass) . " event in_start' style='top:".$inTop.
                    "; height:".$timeslotHeightVal.$timeslotHeightUnit.
                ...
                // second part for the special IN event
                if ($event['catid'] != 2) {
                    echo "<span class='text-danger text-center font-weight-bold'>" . text($row['name']) . "</span>";
                }
                echo "</div>\n";
            }
        } // end EVENT loop
    ...
    {
        $records = QueryUtils::fetchRecords("SELECT * FROM users WHERE id=?", [$providerID]);
        return $records[0] ?? [];
    }
    ...
    {
        return OEGlobalsBag::getInstance()->get('webroot') . "/interface/main/calendar/modules/PostCalendar/pntemplates/default/images";
    }
}

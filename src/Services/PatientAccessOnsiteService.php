...
class PatientAccessOnsiteService
{
    ...
    public function saveCredentials($pid, $pwd, $userName, $loginUsername, $forced_reset_disable)
    {
        ...
        $res = sqlStatement("SELECT * FROM patient_access_onsite WHERE pid=?", [$pid]);
    ...
    public function sendCredentialsEmail($pid, $pwd, $username, $loginUsername, $emailDirect)
    {
        ...
        $data = [
            'portal_onsite_two_enable' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('portal_onsite_two_enable')
            , 'portal_onsite_two_address' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('portal_onsite_two_address')
            , 'enforce_signin_email' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enforce_signin_email')
            , 'uname' => $username
...
// make unique if needed
        ...
        if (empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('use_email_for_portal_username'))) {
    ...
    {
        $records = QueryUtils::fetchRecords("SELECT pd.*,pao.portal_username, pao.portal_login_username,pao.portal_pwd,pao.portal_pwd_status FROM patient_data AS pd LEFT OUTER JOIN patient_access_onsite AS pao ON pd.pid=pao.pid WHERE pd.pid=?", [$pid]);
    ...
    private function emailLogin($patient_id, $htmlMsg, $plainMsg, Environment $twig)
    {
        ...
        if ($patientData['hipaa_allowemail'] != "YES" || empty($patientData['email']) || empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_reminder_sender_email'))) {
            $this->logger->debug(
                ...
                ['hipaa_allowemail' => $patientData['hipaa_allowemail']
                    ...
                    , 'GLOBALS[patient_reminder_sender_email]' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_reminder_sender_email')]
            );
            return false;
        }
        ...
        if (!($this->validEmail(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_reminder_sender_email')))) {
            $this->logger->debug(

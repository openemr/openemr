...
class AclGroupService
{
    ...
    {
        return null !== $this->getOneById($parentId);
    }

    // @todo Move to AclGroupUserMemberService
    /**
     * @throws InvalidArgumentException
     */
    public function addUserToGroup(string $name, string $username, int $groupId): void
    {
        $this->acl->add_object('users', $name, $username, 0, 0, 'ARO');
    ...
    // @todo Move to AclGroupUserMemberService
    public function removeUserFromAllGroups(string $username): void
    {
        $aroId = $this->acl->get_object_id('users', $username, 'ARO');
        if (!$aroId) {
        ...
        /** @var int[] $groups */
        $groupIds = $this->acl->get_object_groups($aroId, 'ARO');
        if ([] === $groupIds) {
        ...
        foreach ($groupIds as $groupId) {
            $this->acl->del_group_object($groupId, 'users', $username, 'ARO');
        }

        // Delete the ARO entry itself
        $this->acl->del_object($aroId, 'ARO');
    }
}

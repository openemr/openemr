...
class AclGroupMemberService
{
    ...
    /**
     * Return IDs of groups where given user is member
     */
    public function getUserGroupIds(array $user): array
    {
        Assert::keyExists($user, 'username');

        $userAroId = $this->acl->get_object_id('users', $user['username'], 'ARO');
        $groups = $this->acl->get_object_groups($userAroId, 'ARO', 'RECURSE');
        Assert::notFalse($groups, sprintf('Unknown error while getting ARO groups of user %s', $user['username']));

        return $groups;
    }

    public function updateUsersUsername(array $user, string $newUsername): void
    {

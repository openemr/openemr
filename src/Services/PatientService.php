...
class PatientService extends BaseService
{
    ...
    public function getPatientPictureDocumentId($pid)
    {
        ...
        $result = sqlQuery($sql, [\OpenEMR\Core\OEGlobalsBag::getInstance()->get('patient_photo_category_name'), $pid]);
    ...
    {
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('age_display_format') == '1') {
            $ageYMD = $this->getPatientAgeYMD($dobYMD, $asOfYMD);
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->has('age_display_limit') && $ageYMD['age'] <= \OpenEMR\Core\OEGlobalsBag::getInstance()->get('age_display_limit')) {
    ...
    public function touchRecentPatientList(array $patient): void
    {
        ...
        $rp = array_slice($rp, 0, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('recent_patient_count'));
        $_rp = serialize($rp);

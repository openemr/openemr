...
class EmployerService extends BaseService
{
    ...
    /**
     * @param $pid The pid of the patient whose employer data we want to update
     * @param $new the new employer data we want to save. This should be an associative array where the keys are the column names in the employer_data table and the values are the values we want to save for those columns.
     * @param $create boolean flag to indicate if we are creating a new record or updating an existing record. If true, we will not compare the new data to the old data and will just save it as is. If false, we will compare the new data to the most recent employer data for the patient and only save the values that have changed. This allows us to keep a history of changes for the patient without saving duplicate data.

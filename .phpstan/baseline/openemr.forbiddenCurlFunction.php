<?php declare(strict_types = 1);

// total 258 errors

$ignoreErrors = [];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRxXMLBuilder.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRxXMLBuilder.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRxXMLBuilder.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRxXMLBuilder.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 11,
    'path' => __DIR__ . '/../../interface/eRxXMLBuilder.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 11,
    'path' => __DIR__ . '/../../interface/eRx_xml.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/src/ClaimRevApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/src/ClaimRevApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 8,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/src/ClaimRevApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 9,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/src/ClaimRevApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 37,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-claimrev-connect/src/ClaimRevApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 3,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 18,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-dorn/src/ConnectorApi.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-prior-authorizations/src/Controller/AuthorizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-prior-authorizations/src/Controller/AuthorizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-prior-authorizations/src/Controller/AuthorizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt_array\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-prior-authorizations/src/Controller/AuthorizationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/scripts/file_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/scripts/file_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/scripts/file_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/scripts/file_download.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/DownloadWenoPharmacies.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/DownloadWenoPharmacies.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/DownloadWenoPharmacies.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 4,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/DownloadWenoPharmacies.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_errno\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../interface/modules/custom_modules/oe-module-weno/src/Services/LogProperties.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 7,
    'path' => __DIR__ . '/../../library/MedEx/API.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/maviq_phone_api.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/maviq_phone_api.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/maviq_phone_api.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../library/maviq_phone_api.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 12,
    'path' => __DIR__ . '/../../library/maviq_phone_api.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_clickatell.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_clickatell.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_clickatell.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_clickatell.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_tmb4.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_tmb4.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_tmb4.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../modules/sms_email_reminder/sms_tmb4.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 2,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 19,
    'path' => __DIR__ . '/../../src/Services/Cda/CdaValidateDocuments.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/ProductRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/ProductRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/ProductRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Services/ProductRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 5,
    'path' => __DIR__ . '/../../src/Services/ProductRegistrationService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_close\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_errno\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_error\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_exec\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_getinfo\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_init\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 1,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];
$ignoreErrors[] = [
    'message' => '#^Raw curl function curl_setopt\\(\\) is forbidden\\. Use a PSR\\-18 HTTP client instead\\.$#',
    'count' => 6,
    'path' => __DIR__ . '/../../src/Telemetry/TelemetryService.php',
];

return ['parameters' => ['ignoreErrors' => $ignoreErrors]];

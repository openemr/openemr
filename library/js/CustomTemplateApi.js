...
function doTemplateEditor(_this, event, oContext = '') {
    ...
    const url = `${window.top.webroot_url}/library/custom_template/custom_template.php?type=${encodeURIComponent(id)}&ccFlag=${encodeURIComponent(ccFlag)}`;
...
const bindTextArea = () => {
    ...
    console.log(`Bound text events: ['${window.location}']`);
    return false;
};

    ...
    $.widget("ui.sortableTree", $.extend($.ui.mouse, {
        ...
        serialize: function (o) {
            ...
            items.each(function () {
                ...
                if (res) {
                    str.push((o.key || res[1])+'[]='+(o.key ? res[1] : res[2]));
                }
            });
        ...
        intersectsWithEdge: function (item) {
            ...
            if (!this.intersectsWith(item.item.parents(".ui-sortableTree").data("sortableTree").containerCache)) {
                return false;
            }

            if (!( t < y1 + (this.helperProportions.height / 2)        // Bottom Half
                &&     y2 - (this.helperProportions.height / 2) < b )) {
                return false; // Top Half
            }

            if (y2 > t && y1 < t) {
                return 1; //Crosses top edge
            }
            if (y1 < b && y2 > b) {
                return 2; //Crosses bottom edge
            }
                    ...
                    if (!this.containers[i].containerCache.over) {
                        if (this.currentContainer != this.containers[i]) {
                            ...
                            for (var j = this.items.length - 1; j >= 0; j--) {
                                if (!this.containers[i].element[0].contains(this.items[j].item[0])) {
                                    continue;
                                }
        ...
        mouseStart: function (e,el) {
            ...
            if ($.data(e.target, 'sortableTree-item')) {
            ...
            if (this.options.handle) {
                ...
                $(this.options.handle, currentItem).each(function () {
                    if (this == e.target) {
                        validHandle = true;
                    } });
                if (!validHandle) {
            ...
            if (!this.helper.parents('body').length) {
                this.helper.appendTo("body"); //Add the helper to the DOM if that didn't happen already
            }
        ...
        mouseStop: function (e) {
            ...
            if (!dropped && this.newPositionAt) {
                this.newPositionAt[this.direction == 'down' ? 'before' : 'after'](this.currentItem); //Append to element to its new position
            }
            
            if (this.position.dom != this.currentItem.prev()[0]) {
                this.propagate("update", e); //Trigger update callback if the DOM position has changed
            }
        ...
        mouseDrag: function (e) {
            ...
            if (intersectsWithDroppable) {
                ...
                for (var i = this.items.length - 1; i >= 0; i--) {
                    if (this.currentItem[0].contains(this.items[i].item[0])) {
                        continue;
                    }

var clickmap = function (args) {
    ...
    var fn_buildMarker = function (x, y, pos, annotation) {
        var legendItem = $("<li class='legend-item'><strong>" + pos + "</strong> " + decodeURIComponent(annotation) + "</li>");
        $(".legend .body ul").append(legendItem);

        var marker = $(".marker-template").clone();
        marker.attr("data-x", x).attr("data-y", y).attr("data-pos", pos).attr("id", new Date().getTime()).attr("class", "marker").attr("style", "left:" + x + "px; top:" + y + "px;").find("span.count").text(pos);
    ...
    if (!hideNav) {
        container.click(function (e) {
            ...
            dialog.dialog({
                title: "Information",
                autoOpen: false,
                height: hasOptions ? 345 : 300,
                width: 350,
                modal: true,
                open: function () {
                    dialog.find(".detail").focus();
                },
                buttons: {
                    "Save": function () {
                        dialog.saved = true;

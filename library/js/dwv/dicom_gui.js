...
dwvOemr.gui.ToolboxContainer = function (app, infoController) {
    ...
    this.setup = function (list) {
        ...
        toolbox.onclick = function () {
            const toolList = app.getElement('toolList');
        ...
        draw.setAttribute('class', 'btn-sm btn-primary fa fa-edit');
        draw.title = dwv.i18n('basics.drawList');
        draw.onclick = function () {
            const drawList = app.getElement('drawList');
        ...
        image.onclick = function () {
            const layerDialog = app.getElement('layerDialog');
        ...
        info.onclick = function () {
            const infoLayer = app.getElement('infoLayer');
        ...
        if (dwvOemr.oemrDocumentUrl) {
            ...
            const node = app.getElement('openData');
            node.appendChild(saveButton);
            saveButton.onclick = function () {
                const oemrStateSaveUrl = dwvOemr.oemrDocumentStateUrl;
                const data = new FormData();
                const json = app.getState();
            ...
            const toggleSaveState = document.createElement('a');
            toggleSaveState.onclick = function () {
                const blob = new Blob([app.getState()], { type: 'application/json' });
        ...
        // add rest to form
        const node = app.getElement('toolbar');
        const spannode = app.getElement('editspan');

...
dwvOemr.browser.hasFileApi = function () {
    ...
    const isSafari5 = (navigator.appVersion.indexOf('Safari') !== -1)
        && (navigator.appVersion.indexOf('Chrome') === -1)
        && ((navigator.appVersion.indexOf('5.0.') !== -1) || (navigator.appVersion.indexOf('5.1.') !== -1));
    if (isSafari5) {
...
dwvOemr.browser.check = function () {
    ...
    const appnorun = 'The application cannot be run.';
    let message = '';
    ...
    if (!dwvOemr.browser.hasTypedArraySlice()) {
        ...
        Uint16Array.prototype.slice = function (begin, end) {
            ...
            const cloned = new Uint16Array(size);
        ...
        Int16Array.prototype.slice = function (begin, end) {
            const size = end - begin;
            const cloned = new Int16Array(size);
            for (let i = 0; i < size; i += 1) {
                cloned[i] = this[begin + i];
            }
            return cloned;
        ...
        Uint8Array.prototype.slice = function (begin, end) {
            const size = end - begin;
            const cloned = new Uint8Array(size);
            for (let i = 0; i < size; i += 1) {
                cloned[i] = this[begin + i];
            }
            return cloned;
        ...
        Int8Array.prototype.slice = function (begin, end) {
            const size = end - begin;
            const cloned = new Int8Array(size);
            for (let i = 0; i < size; i += 1) {
                cloned[i] = this[begin + i];
            }
            return cloned;

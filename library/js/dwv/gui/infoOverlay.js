...
dwvOemr.gui.info.Overlay = function (div, pos) {
    ...
    this.create = function () {
        ...
        if (pos === 'bc' || pos === 'tc'
            ...
            let li;
            let value;
            for (let n = 0; n < overlayData.length; n += 1) {
            ...
            for (let n = 0; n < overlayData.length; n += 1) {
                ...
                if (value === 'window-center') {
                    if (event.type === 'wl-center-change') {
                        ...
                        const wcStr = dwv.utils.replaceFlags2(format, [Math.round(event.wc)]);
                        if (li) {
                    ...
                    if (event.type === 'wl-width-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const wwStr = dwv.utils.replaceFlags2(format, [Math.round(event.ww)]);
                        if (li) {
                    ...
                    if (event.type === 'zoom-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const zoom = Number(event.scale).toPrecision(3);
                        const zoomStr = dwv.utils.replaceFlags2(format, [zoom]);
                        if (li) {
                    ...
                    if (event.type === 'zoom-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const offset = [Number(event.cx).toPrecision(3),
                            Number(event.cy).toPrecision(3)];
                        const offStr = dwv.utils.replaceFlags2(format, offset);
                        if (li) {
                    ...
                    if (event.type === 'position-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const valueStr = dwv.utils.replaceFlags2(format, [event.value]);
                        if (li) {
                    ...
                    if (event.type === 'position-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const posStr = dwv.utils.replaceFlags2(
                            format, [event.i, event.j, event.k],
                        );
                        if (li) {
                    ...
                    if (event.type === 'frame-change') {
                        ...
                        dwvOemr.html.cleanNode(li);
                        const frameStr = dwv.utils.replaceFlags2(format, [event.frame]);
                        if (li) {
...
dwvOemr.gui.info.createOverlayData = function (dicomElements) {
    ...
    const omaps = dwvOemr.gui.info.overlayMaps;
    if (!omaps) {
        return overlays;
    }
    const omap = omaps[modality] || omaps['*'];

    for (let n = 0; omap[n]; n += 1) {
        const { tags, pos } = omap[n];
        let { value, format } = omap[n];
        ...
        overlays[pos].push({ value: value.trim(), format });
    }

    // (0020,0020) Patient Orientation
    const valuePO = dicomElements.getFromKey('x00200020');
    if (typeof valuePO !== 'undefined' && valuePO !== null && valuePO.length === 2) {
        const po0 = dwv.dicom.cleanString(valuePO[0]);
        const po1 = dwv.dicom.cleanString(valuePO[1]);

...
function formHeader($title = "My Form"): void
{
    ...
    <body background="<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('backpic')?>" topmargin=0 rightmargin=0 leftmargin=2 bottommargin=0 marginwidth=2 marginheight=0>
    <?php
}
...
function formFetch($tableName, $id, $cols = "*", $activity = "1")
{
    ...
    return sqlQuery("select " . escape_sql_column_name(process_cols_escape($cols), [$tableName]) . " from `" . escape_table_name($tableName) . "` where id=? and pid = ? and activity like ? order by date DESC LIMIT 0,1", [$id,\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pid'),$activity]) ;
}

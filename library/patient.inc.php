...
function _set_patient_inc_count($limit, $count, $where, $whereBindArray = []): void
{
  // When the limit is exceeded, find out what the unlimited count would be.
    \OpenEMR\Core\OEGlobalsBag::getInstance()->set('PATIENT_INC_COUNT', $count);
  // if ($limit != "all" && $GLOBALS['PATIENT_INC_COUNT'] >= $limit) {
    if ($limit != "all") {
        ...
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set('PATIENT_INC_COUNT', $tmp['count']);
    }
}
...
function getPatientLnames($term = "%", $given = "pid, id, lname, fname, mname, providerID, DATE_FORMAT(DOB,'%m/%d/%Y') as DOB_TS", $orderby = "lname ASC, fname ASC", $limit = "all", $start = "0")
{
    ...
    if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field'))) {
        if ($session->get("authUser") != 'admin' || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_admin')) {
            $where .= " AND ( patient_data." . add_escape_custom(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field')) .
                " = ( SELECT facility_id FROM users WHERE username = ?) OR patient_data." .
                add_escape_custom(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field')) . " = '' ) ";
...
function getPatientId($pid = "%", $given = "pid, id, lname, fname, mname, providerID, DATE_FORMAT(DOB,'%m/%d/%Y') as DOB_TS", $orderby = "lname ASC, fname ASC", $limit = "all", $start = "0")
{
    ...
    if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field')) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_by_id')) {
        if ($session->get("authUser") != 'admin' || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_admin')) {
            $where .= "AND ( patient_data." . add_escape_custom(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field')) .
                    " = ( SELECT facility_id FROM users WHERE username = ?) OR patient_data." .
                    add_escape_custom(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('pt_restrict_field')) . " = '' ) ";
...
function fixDate($date, $default = "0000-00-00")
{
    ...
    if (preg_match("'^[0-9]{1,4}[/.-][0-9]{1,2}[/.-][0-9]{1,4}$'", $date)) {
        ...
        if ($dmy[0] > 99) {
            ...
            $using_mdy = empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format')) ?
                (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('phone_country_code') == 1) : (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('date_display_format') == 1);
            if ($using_mdy) {

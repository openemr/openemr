...
while($cnt < 4) {
	if($dt['ins_data_id'.$cnt]) {
		if($policies[$dt['ins_data_id'.$cnt]]['id']) {
			...
			$ins_error_msg = $dt['ins_data_id'.$cnt];
		}
	} else {
		$ins[$cnt] = array();
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td <?php echo $dt[$field_prefix . 'id'] ? 'style="color: grey;"' : ''; ?>><?php echo $dt[$field_prefix . 'id'] ? htmlspecialchars($dt{$field_prefix . 'id'},ENT_QUOTES) : '[ New Case ]'; ?><input name="case_id" id="case_id" type="hidden" tabindex="-1" value="<?php echo $dt['case_id']; ?>" /></td>
	...
	<tr>
		...
		<td class="label"><label><input name="<?php echo $field_prefix; ?>closed" id="<?php echo $field_prefix; ?>closed" type="checkbox" value="1" <?php echo $dt[$field_prefix . 'closed'] ? 'checked' : ''; ?> />&nbsp;&nbsp;<?php echo xl('Inactive'); ?></label></td>
		<td class="label"><?php echo xl('Referring Provider'); ?>:</td>
		<td style="width: 240px;"><select name="<?php echo $field_prefix; ?>referring_id" id="<?php echo $field_prefix; ?>referring_id">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr style="color: red;">
		<td>**</td>
		<td colspan="5">Error Condition: A policy (ID <?php echo $ins_error_msg; ?>) is attached to this case that is no longer valid by date</td>
	</tr>
	<?php } ?>
	<tr>
		<td>&nbsp;</td>
...
<?php echo $dt[$field_prefix . 'ins_data_id'.$cnt] ? htmlspecialchars($ins[$cnt]['subscriber'], ENT_QUOTES) : ''; ?>
	...
	<tr>
		<td colspan="2">Email Addresses:</td>
		<td colspan="4"><i>**  Please use a comma to separate multiple addresses</i></td>
	</tr>
	<tr>
		<td colspan="6"><textarea name="<?php echo $field_prefix; ?>notes" id="<?php echo $field_prefix; ?>notes" class="wmtFullInput" rows="3"><?php echo htmlspecialchars($dt[$field_prefix . 'notes'], ENT_QUOTES); ?></textarea></td>
	</tr>
</table>
</fieldset>
</div>

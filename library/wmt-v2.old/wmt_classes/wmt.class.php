...
class wmtPatient {
	...
	public function __construct($id = false) {
		...
		if ($this->DOB && strtotime($this->DOB) > 0) { // strtotime returns FALSE or -1 for invalid date
	...
	public function update() {
		...
		foreach ($this as $key => $value) {
			if ($key == 'id') continue;
			if (!in_array($key, $fields)) continue;
			if ($value == 'YYYY-MM-DD' || $value == "_blank") $value = "NULL";
	...
	public static function getPidPatient($pid, $update = false) {
		...
		if(!$data || !$data['id']) {
			throw new Exception('wmtPatient::getPidPatient - no patient with pid ('.$pid.').');
		}
...
class wmtInsurance {
	...
	public function __construct($id = false) {
		...
		$query = "SELECT a.*, i.*, i.id AS company_id, c.name AS company_name, c.id AS company_id, c.freeb_type AS plan_type, c.cms_id FROM insurance_data i ";
	...
	public static function getPidInsDate($pid, $date, $type = null) {
		...
		if(!$date || strtotime($date) < 1) // strtotime returns FALSE or -1 on invalid date
...
class wmtForm {
	...
	public function __construct($form_name, $id = false, $update = false) {
		...
		// retrieve data
		$query = "SELECT * FROM $this->form_table WHERE id = ?";
		$data = sqlQuery($query,array($id));
	...
	public function fields() {
		if (!$this->form_table)
			throw new Exception('wmtForm::fields - no form table name provided in object.');
...
class wmtOrder extends wmtForm {
	...
	*/
	 ...
	 */
	public static function fetchOrder($form_type = "order", $order_num, $pid, $pat_DOB) {
		if(! $order_num)
			throw new Exception ("wmtOrder::fetchOrder - no order number provided");

		$table = "form_".$form_type;

		if (!$pat_DOB) $pat_DOB = '0000-00-00';
		$order = sqlQuery("SELECT id FROM $table ".
			"WHERE order_number = ? AND pid = ? AND pat_DOB = ? ",
				array($order_num, $pid, $pat_DOB));
...
class wmtResultItem {
	...
	public function update() {
		...
		// run the update
		sqlInsert("UPDATE procedure_result SET $query WHERE procedure_result_id = ?",$params);

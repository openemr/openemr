...
$load_form = $frmn;
$load_id = $id;
if($frmn == 'form_wmt_ros') {
	$load_form = 'encounter';
	$load_id = $encounter;
}
$comments = LoadROSFormComments($load_id, $load_form);
...
foreach($ros_order_array as $ros_type) {
	...
	if(!isset($rs{'ros_'.$ros_type[1].'_hpi'}))
			$rs{'ros_'.$ros_type[1].'_hpi'} = '';
	if(!isset($rs{'ros_'.$ros_type[1].'_none'}))
			$rs{'ros_'.$ros_type[1].'_none'} = '';

...
class wmtPatData{
	...
	public function __construct($id = false, $ageWhen='') {
		...
		if ($data = sqlFetchArray($results)) {
			...
			$data = sqlQuery($query, array('primary', $this->pid));
			if($data) {
			...
			$data = sqlQuery($query, array('secondary', $this->pid));
			if($data) {
				if(!isset($data['area_code'])) $data['area_code'] = '';
				if(!isset($data['prefix'])) $data['prefix'] = '';
				if(!isset($data['number'])) $data['number'] = '';
			...
			$data = sqlQuery($query, array('tertiary', $this->pid));
			if($data) {
				if(!isset($data['area_code'])) $data['area_code'] = '';
				if(!isset($data['prefix'])) $data['prefix'] = '';
				if(!isset($data['number'])) $data['number'] = '';
   ...
   * generated by getPidPolicyByDate.
	...
	public static function pidPolicySelect($insId = '', $policies = array(), $emptyLabel = '') {
  	...
  	foreach ($policies as $policy) {
    	...
    	echo '>' . htmlspecialchars($policy{'name'}, ENT_QUOTES);
			echo '&nbsp;&nbsp;-&nbsp;';
    	echo htmlspecialchars($policy{'subscriber_lname'}, ENT_QUOTES);
			echo ', ';
    	echo htmlspecialchars($policy{'subscriber_fname'}, ENT_QUOTES);
    	echo '</option>';
  	}

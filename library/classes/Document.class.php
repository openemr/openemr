...
class Document extends ORDataObject
{
    ...
    */
    ...
    protected function get_filesystem_filepath()
    {
        ...
        $filepath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/' . $from_pathname . '/' . $from_filename;
        return $filepath;
    }
    /**
   ...
   */
    function createDocument(
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('generate_doc_thumb')) {
        ...
        $encounter_id = $eid;
        $this->storagemethod = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('document_storage_method');
        $this->mimetype = $mimetype;
        if ($this->storagemethod == self::STORAGE_METHOD_COUCHDB) {
            // Store it using CouchDB.
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_encryption')) {
            ...
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('documentStoredRemotely') ?? '') {
                return xlt("Document was uploaded to remote storage"); // terminate processing
            }

            // Storing document files locally.
            $repository = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('oer_config')['documents']['repository'];
            ...
            // Store the file.
            $storedData = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('drive_encryption') ? $cryptoGen->encryptStandard($data, null, KeySource::Database) : $data;
        ...
        if (
            (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('drive_encryption') && ($this->storagemethod != 1))
            || (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_encryption') && ($this->storagemethod == 1))
        ) {

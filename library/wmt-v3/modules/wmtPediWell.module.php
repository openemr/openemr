...
class PediWellModule extends BaseModule {
	...
	public function __construct(&$sec_data, &$form_data) {
		if (!$sec_data || !$sec_data['key'])
			throw new \Exception('wmtPediWellModule::No module key provided for construct.');
		...
		<div class='wmtPrnMainContainer'>
			...
			<div class='wmtPrnCollapseBox'>
				<table class='wmtPrnContent'>
					...
					<tr><td style="width:10px"></td><td style="width:10%"></td><td style="width:10%"></td><td colspan="3"></td></tr>
<?php 			
		...
		foreach ($rep_list AS $record) {
			...
			if ($group_label) { // first data for this group 
				self::end_row(); ?>
					...
					<tr>
						<td class="wmtPrnLabel" colspan="<?php echo $this->max_cells +1 ?>" <?php if (!$first_group) echo 'style="padding-top:10px"'?>>
...
<?php 
				...
				$this->cell_count += 3; ?>
						...
						<td><!-- spacer --></td>
						<td colspan='2' class="wmtPrnLabel" <?php if ($titlecols > 1) echo "colspan='".$titlecols."' "; ?> style="white-space:nowrap;padding-left:23px" >

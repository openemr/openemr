 ...
 * @subpackage modules
 */
class ImagesModule extends BaseModule {
	/**
	...
	public function __construct(&$sec_data, &$form_data) {
		if (!$sec_data || !$sec_data['key'])
			throw new \Exception('wmtImagesModule::No module key provided for construct.');
		...
		$this->images = GetImageHistory($form_data->pid, $limit);
	...
	public function display($open=false, $bottom=false) {
		...
		$dt['fyi_img_nt'] = $this->form_data->images_notes;
		$base_action = $GLOBALS['base_action'];
		$frmdir = $this->form_data->form_name;
		
		include($GLOBALS['srcdir'] . '/wmt-v2/image_summary.inc.php');

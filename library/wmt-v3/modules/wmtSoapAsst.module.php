...
class SoapAsstModule extends BaseModule {
	/**
	...
	public function __construct(&$sec_data, &$form_data) {
		if (!$sec_data || !$sec_data['key'])
			throw new \Exception('wmtSoapAsstModule::No module key provided for construct.');
				...
				<table style="width:100%">
					<tr>
						<td class="wmtLabel" colspan="2">
							<div style="display:inline-block;width:200px">Assessment Narrative:</div>
						</td>
						<td>
						<?php 
						// $snippet_type = isSnippetModule('assessment_notes');
						if($use_snippets && 
									($snippet_type = isSnippetModule('assessment_notes'))) {
							$field_prefix = '';
							$field_name = 'assessment_notes';
					...
					<tr>
						<td colspan="4">
							<textarea name="assessment_notes" id="assessment_notes" class="wmtFullInput" rows="4"><?php echo $this->form_data->assessment_notes; ?></textarea>
						</td>
					</tr>
				</table>
			</div>
...
<?php }
	...
	public function report() { 
		$output = false;
		if ($this->form_data->assessment_notes) $output = true;
		if (!$output) return; ?>
				...
				<table class='wmtPrnContent' style="margin:6px 0">
					...
					<tr>
						<td style='line-height:14px' colspan="2">
							<span class='wmtPrnLabel'>Assessment Narrative:</span>
						</td>
					</tr><tr>
						<td style="width:25px"></td><td style='line-height:14px'>
							<span class='wmtPrnBody'><?php echo $this->form_data->assessment_notes . "\n"; ?></span>
						</td>
					</tr>
...
<?php }
	...
	public function store() {
		...
		// Push data to form
		$this->form_data->assessment_notes = strip_tags($dt['assessment_notes']);

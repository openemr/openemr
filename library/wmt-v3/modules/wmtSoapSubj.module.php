...
class SoapSubjModule extends BaseModule {
	/**
	...
	public function __construct(&$sec_data, &$form_data) {
		if (!$sec_data || !$sec_data['key'])
			throw new \Exception('wmtSoapSubjModule::No module key provided for construct.');
				...
				<table style="width:100%">
					<tr>
						<td class="wmtLabel" colspan="2">
							<div style="display:inline-block;width:200px">Subjective Narrative:</div>
						</td>
						<td>
						<?php 
						if($use_snippets && isSnippetModule('subjective_notes')) {
							$field_prefix = '';
							$field_name = 'subjective_notes';
					...
					<tr>
						...
						<td colspan="4">
							<textarea name="subjective_notes" id="subjective_notes" class="wmtFullInput" rows="4"><?php echo $this->form_data->subjective_notes; ?></textarea>
						</td>
					</tr>
...
<?php }
	...
	public function report() { 
		$output = false;
		if ($this->form_data->subjective_notes) $output = true;
		if (!$output) return; ?>
				...
				<table class='wmtPrnContent'>
					...
					<tr>
						<td style='line-height:14px' colspan="2">
							<span class='wmtPrnLabel'>Subjective Narrative:</span>
						</td>
					</tr><tr>
						<td style="width:25px"></td><td style='line-height:14px'>
							<span class='wmtPrnBody'><?php echo $this->form_data->subjective_notes . "\n"; ?></span>
						</td>
					</tr>
...
<?php }
	...
	public function store() {
		...
		// Push data to form
		$this->form_data->subjective_notes = strip_tags($dt['subjective_notes']);

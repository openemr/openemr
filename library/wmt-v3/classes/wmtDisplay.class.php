...
class Display {
	...
	public static function chapter($title='', $bar_id='', $toggle, $collapsible='true', $class='wmtCollapseBar') {
		// old version support
		if ($toggle === 'none') $toggle = false;
		if ($toggle === 'block') $toggle = true;
		$arrow = ($toggle)? 'fill-090.png' : 'fill-270.png';
?>
		<div class="<?php echo $class ?> wmtColorBar <?php if (!$toggle) echo 'wmtBarClosed' ?> <?php if (!$collapsible) echo 'wmtNoCollapse'?>" id="<?php echo $bar_id ?>Bar" style="text-align:center">
			...
			<input type="hidden" id="tmp_<?php echo $bar_id ?>_disp_mode" name="tmp_<?php echo $bar_id ?>_disp_mode" value="<?php echo ($toggle)? '1' : '0' ?>" /> 

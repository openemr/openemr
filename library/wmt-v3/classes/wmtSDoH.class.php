...
class SDoH extends Form {
	...
	*/
	...
	public function __construct($id=false) {
		...
		parent::__construct('sdoh_screen', $id, 'Social Determinants of Health');

		// Initialize section parameters
		if (empty($this->sections_data)) {
			
			$section_list = new Options('SDoH_Sections');
	...
	public function fetchModules() {
		...
		if(! $pid)
			throw new \Exception ("wmtSDoH::fetchEncounter - no patient identifier provided");
	
		$query = "SELECT `form_id` FROM `forms` fr ";
		$query .= "LEFT JOIN `form_sdoh_screen` sdoh ON sdoh.id = fr.form_id ";
		$query .= "WHERE fr.`encounter` = ? AND fr.`pid` = ? AND fr.`formdir` = 'sdoh_screen' ";
		$binds[] = $enc;
		$binds[] = $pid;
	...
	public static function fetchEncounter($pid, $enc) {
		...
		if(! $enc)
			throw new \Exception ("wmtSDoH::fetchEncounter - no encounter number provided");
	...
	public function listFields($full=true) {
		$fields = sqlListFields('form_sdoh_screen');

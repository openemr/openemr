...
class PediWCC extends Form {
	...
	*/
	...
	public function __construct($id=false, $type=false) {
		...
		parent::__construct('pedi_wcc', $id, 'Well Child Exam');

		if ($type) $this->form_type = $type;
		if (!$this->form_type)
			throw new \Exception("wmtPediWCC::__construct - No form type provided...");
		
		// load form type data
		$vers_list = new Options('WCC_Version');
		$version = $vers_list->getItem($this->form_type);
		
		// store the wcc type
		$this->form_version = $version;
		$this->form_title = 'Well Child Exam - '.$version;
		
		// Initialize section parameters
		if (empty($this->section_data)) {
			
			$section_list = new Options('PediWCC_Sections');
	...
	public function fetchModules() {
		...
		if(! $pid)
			throw new \Exception ("wmtPediWCC::fetchEncounter - no patient identifier provided");
	
		$query = "SELECT `form_id` FROM `forms` fr ";
		$query .= "LEFT JOIN `form_pedi_wcc` wcc ON wcc.id = fr.form_id ";
		$query .= "WHERE fr.`encounter` = ? AND fr.`pid` = ? AND fr.`formdir` = 'pedi_wcc' ";
		$binds[] = $enc;
		$binds[] = $pid;
	 ...
	 * @return wmtPediWCC $object
	 */
	public static function fetchEncounter($pid, $enc, $type) {
		if(! $pid)
			throw new \Exception ("wmtPediWCC::fetchEncounter - no patient identifier provided");

		if(! $enc)
			throw new \Exception ("wmtPediWCC::fetchEncounter - no encounter number provided");
	...
	public function listFields($full=true) {
		$fields = sqlListFields('form_pedi_wcc');

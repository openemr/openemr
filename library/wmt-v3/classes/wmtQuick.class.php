...
class Quick extends Form {
	...
	*/
	...
	public $portal_flag;
	public $int_provider;
	public $int_clinician;
	public $ext_provider;
	
	/**
	 * Constructor for the 'quick' class which retrieves the requested
	...
	public function __construct($id = false) {
		...
		parent::__construct('quick', $id, 'Quick Pick Order');
		...
		if (!$this->order_number)
			throw new \Exception('wmtQuick::_construct - no procedure order number.');
		...
		if (!$data['procedure_order_id'])
			throw new \Exception('wmtQuick::_construct - no procedure order record with procedure_order_id ('.$this->order_number.').');
	...
	public function store() {
		...
		$status_list = array('i'=>'pending','c'=>'complete','a'=>'approved');
	 ...
	 * @return wmtQuick $object
	 */
	public static function fetchOrder($form_name = "order", $order_num, $lab_id, $pid, $pat_DOB = false) {
		if(! $order_num)
			throw new \Exception ("wmtQuick::fetchOrder - no order number provided");

		if(! $lab_id)
			throw new \Exception ("wmtQuick::fetchOrder - no lab identifier provided");
	...
	public static function fetchEncounter($enc, $pid, $lab, $form) {
		if(! $enc)
			throw new \Exception ("wmtQuick::fetchEncounter - no encounter number provided");

		if(! $form)
			throw new \Exception ("wmtQuick::fetchEncounter - no form name provided");

		if(! $pid)
			throw new \Exception ("wmtQuick::fetchEncounter - no patient identifier provided");

		if(! $lab)
			throw new \Exception ("wmtQuick::fetchEncounter - no laboratory identifier provided");
		...
		return new Order($form, $order['form_id']);
	}

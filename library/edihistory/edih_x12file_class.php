...
class edih_x12_file {
	...
	function __construct($file_path='', $mk_segs=true, $text=false ) {
		...
		if ( is_file($file_path) && is_readable($file_path) ) {
				...
				if ($this->isx12) {
					...
					if ($mk_segs) {
						...
						if ( is_array($this->segments) && count($this->segments) ) {
							...
							$this->message[] = 'edih_x12_file: error in creating segment array ' . $this->filename . PHP_EOL;
						}
					} else {
			...
			// invalid file path
			$this->message[] = 'edih_x12_file: invalid file path ' . $file_path;
		}
	...
	private function edih_file_text($file_text, $type=false, $delimiters=false, $segments=false) {
		...
		if ( !strpos($v, 's') ) {
			$this->message[] = 'edih_file_text(): failed scan of file text ('.$v.')';
			return $ret_ar;
		}
		//
		$this->constructing = true;
	...
	public function edih_filepath() { return $this->filepath; }
	public function edih_filename() { return $this->filename; }
	public function edih_type() { return $this->type; }
	public function edih_version() { return $this->version; }
	public function edih_text() { return $this->text; }
	public function edih_length() { return $this->length; }
	public function edih_valid() { return $this->valid; }
	public function edih_isx12() { return $this->isx12; }
	public function edih_hasGS() { return $this->hasGS; }
	public function edih_hasST() { return $this->hasST; }
	public function edih_delimiters() { return $this->delimiters; }
	public function edih_segments() { return $this->segments; }
	public function edih_envelopes() { return $this->envelopes; }
	...
	public function edih_x12_type($file_text='') {
		...
		if ($file_text) {
			...
			if (!$de) { $de = substr(reset($this->segments), 3, 1); }
			...
			if ($pr && count($matches)) {
				foreach($matches as $m) {
					...
					if ( array_key_exists($gs_ar1[1], $this->gstype_ar) ) {
						...
						$tp_tmp[] = $gs_ar1[1];
						$this->message[] = 'edih_x12_type: unknown x12 type '.$gs_ar1[1];
					}
				}
			} else {
	...
	public function edih_x12_delimiters($isa_str110='') {
		...
		for ($i = 0; $i < $isalen; $i++) {
			if ($isa_str[$i] == $de) {
				...
				if ($delim_ct == 11) { $dr = substr($s, 1, 1); }
				if ($delim_ct == 12) {
					if ( strpos($s, '501') === false ) { $dr = ''; }
				}
				//
				if ($delim_ct == 15) {
	...
	public function edih_x12_envelopes($file_text='') {
		...
		for ($i=$seg_first; $i<$seg_ct; $i++) {
			...
			if ( !in_array(substr($sn, 0, 3), $chk_segs) ) { continue; }
			...
			if ( strncmp($sn, 'GS'.$de, 3) == 0) {
				...
				if ( array_key_exists($seg_ar[1], $this->gstype_ar) ) {
					...
					$this->message[] = 'edih_x12_envelopes: Unknown GS type '.$seg_ar[1];
				}
				continue;
			}
			...
			if ( strncmp($sn, 'ST'.$de, 3) == 0 ) {
				...
				if ( $gs_fid != $st_type && strpos($st_type, '99') === false ) {
					$this->message[] = "edih_x12_envelopes: ISA $icn, GS $gsn $gs_fid ST $stn $st_type type mismatch".PHP_EOL;
				}
				//
				continue;
			}
			...
			if ($st_type == '835') {
				if ( strncmp($sn, 'TRN'.$de, 4) == 0) {
					...
					if (!isset($seg_ar[2]) || !isset($seg_ar[3]) ) {
						$this->message[] = 'error in 835 TRN segment '.$seg_text;
					}
				...
				if ( strncmp($sn, 'CLP'.$de, 4) == 0) {
					...
					if ( isset($seg_ar[1]) ) {
						...
						$this->message[] = 'error in 835 CLP segment '.$seg_text;
					}
					continue;
				}
			}
			...
			if ($st_type == '837') {
				...
				if ( strncmp($sn, 'CLM'.$de, 4) == 0) {
					...
					if ( isset($seg_ar[1]) ) {
						...
						$this->message[] = 'error in 837 CLM segment '.$seg_text;
					}
					continue;
				}
			}
			...
			if ( strncmp($sn, 'SE'.$de, 3) == 0 ) {
				...
				// errors
				if ($se_num != $stn) {
					$this->message[] = 'edih_x12_envelopes: ST-SE number mismatch '.$stn.' '. $se_num.' in ISA '.$icn.PHP_EOL;
				}
				if (intval($seg_ar[1]) != $st_segs_ct) {
					$this->message[] = 'edih_x12_envelopes: ST-SE segment count mismatch '.$st_segs_ct.' '.$seg_ar[1].' in ISA '.$icn.PHP_EOL;
				}
				continue;
			}
			//
			...
			if ( strncmp($sn, 'IEA'.$de, 4) == 0 ) {
				...
				if ($env_ar['ISA'][$icn]['icn'] !== $seg_ar[2]) {
					$this->message[] = 'edih_x12_envelopes: ISA-IEA identifier mismatch ISA '.$icn.' IEA '.$seg_ar[2];
				}
				if ($iea_ct == $isa_ct) {
					$trnset_seg_ct += $isa_segs_ct;
					//if ( $i+1 != $trnset_seg_ct ) {
					if ( $i != $trnset_seg_ct ) {
						$this->message[] = 'edih_x12_envelopes: IEA segment count error '.($i).' : '.$trnset_seg_ct;
					}
				} else {
					$this->message[] = 'edih_x12_envelopes: ISA-IEA count mismatch ISA '.$isa_ct.' IEA '.$iea_ct;
				}
				continue;
			}
		}
	 ...
	 */
 	public function edih_x12_transaction($clm01, $stn='', $filetext='') {
		...
		if ( substr($tp, 0, 5) == 'mixed' ) { $tp = substr($tp, -2); }
		...
		if ( count($env_ar) ) {
			foreach($env_ar['ST'] as $st) {
				if ( strlen($stn) && $st['stn'] != $stn ) { continue; }
		...
		if ($srch_ar[0]['type'] == 'NA' || !$srch_ar[0]['type']) {
			$this->edih_message('edih_x12_transaction(): invalid file type '.$srch_ar[0]['type']);
			return $ret_ar;
		}
		// segments we check
		...
		foreach($srch_ar as $srch) {
			...
			foreach($srch['array'] as $key=>$seg) {
				...
				if ( !in_array($test_str, $test_id, true) ) { continue; }
				//
	...
	public function edih_get_segment($segmentID, $srchStr='', $seg_array='') {
		...
		if ( is_array($seg_array) && count($seg_array) ) {
			if ( isset($seg_array[0]) && is_array($seg_array[0]) ) {
				foreach($seg_array as $ar) { $seg_ar = array_merge($seg_ar, $ar); }
			} else {
				$seg_ar = $seg_array;
			}
		} elseif ( count($this->segments) ) { 
	...
	function edih_x12_slice($arg_array, $file_text='') {
		...
		foreach($arg_array as $key=>$val) {
			switch((string)$key) {
				case 'trace': $trace = (string)$val; break;
				case 'ST02': $stn = (string)$val; break;
				case 'GS06': $gsn = (string)$val; break;
				case 'ISA13': $icn = (string)$val; break;
				case 'keys': $prskeys = (bool)$val; break;
			}
		}
		//	
		if ($trace && strpos('|HP|FA', $ft) === false) {
			$this->message[] = 'edih_x12_slice() incorrect type ['.$ft.'] for trace';
			return $ret_ar;
		}
		//
		if ($f_str) {
			...
			if ($icn) {
				...
				if ($icnpos === false) {
					// $icn not found
					$this->message[] = 'edih_x12_slice() did not find ISA13 '.$icn;
					// debug			
					//echo $this->edih_message().PHP_EOL;
					return $ret_ar;
				} elseif ($icnpos < 106) {
			...
			if ($gsn) {
				...
				if ($gspos === false) {
					// $gsn not found
					$this->message[] = 'edih_x12_slice() did not find GS06 '.$gsn;
					return $ret_ar;
				} else {
			...
			if ($stn) {	
				...
				if ($stpos === false) {
					// $stn not found
					$this->message[] = 'edih_x12_slice() did not find ST02 '.$stn;
					return $ret_ar;
				} else {
					$stpos = $stpos + 1;
				}

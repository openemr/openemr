...
class edih_x12_file
{
    ...
    public function edih_x12_delimiters($isa_str110 = '')
    {
        ...
        if (!$isa_str110 && $this->text) {
            $isa_str = substr((string) $this->text, 0, 106);
        } else {
    ...
    public function edih_x12_envelopes($file_text = '')
    {
        ...
        if ($file_text) {
            ...
            if (isset($this->delimiters['e'])) {
                ...
                $de = (substr((string) reset($segment_ar), 0, 3) == 'ISA') ? substr((string) reset($segment_ar), 3, 1) : '';
            }
        } else {
        ...
        if (count($seg_ar)) {
            ...
            foreach ($seg_ar as $key => $seg) {
                if (strncmp((string) $seg, $segid . $de, $cmplen) === 0) {
                    if ($srch) {
                        if (strpos((string) $seg, $srch) !== false) {
                            $ret_ar[$key] = $seg;
                        }
                    } else {
                        $ret_ar[$key] = $seg;

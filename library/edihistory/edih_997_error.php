    ...
    foreach ($err_array['err'] as $k => $v) {
        ...
        if ($icn && $stn && $rtp) {
            ...
            if (is_array($trn_ar) && count($trn_ar)) {
                ...
                $str_html .= text($pt_name) . " " . text($svcdate) . " <em>Trace</em> <a class='rpt' href='edih_main.php?gtbl=claim&fname=" . attr(urlencode($btfn)) . "&ftype=" . attr(urlencode($rtp)) . "&pid=" . attr(urlencode($clm01)) . "&fmt=seg&csrf_token_form=" .attr(urlencode(collectCsrfToken())) . "'>" . text($clm01) . "</a> <br />".PHP_EOL;
            } else {

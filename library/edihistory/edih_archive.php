        ...
        if ( is_dir($fdir) ) {
            ...
            if (is_array($dir_ar) && ((count($dir_ar)-2) > 0)) {
                ...
                foreach($dir_ar as $fn) {
                    if ($fn == 'README.txt') { $dir_ct--;
                        continue; }
                    if (substr($fn, 0, 1) == '.') { $dir_ct--;
                        continue; }
                    if (is_dir($fdir.DS.$fn)) { $subdir_ct++;
                        $dir_ct--;
                        continue; }

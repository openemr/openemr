...
function phimail_connect(&$phimail_error)
{
    ...
    $phimail_server = @parse_url((string) \OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_server_address'));
    ...
    if (\OpenEMR\Core\OEGlobalsBag::getInstance()->has('phimail_testmode_disabled') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_testmode_disabled') == '1') {
        $phimail_cafile = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . '/public/certs/phimail/phimail_server.pem';
    } else {
        $phimail_cafile = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('fileroot') . '/public/certs/phimail/EMRDirectTestCA.pem';
        (new SystemLogger())->debug("running phimail_connect in test mode.  This should not be used for production", ['ca' => $phimail_cafile, 'testmode' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_testmode_disabled')]);
    }
    ...
    switch ($phimail_server['scheme']) {
        ...
        case "sslv3":
        case "tls":
            $server = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_server_address');
            break;
        default:
            $phimail_error = 'C2';
...
function phimail_check(): void
{
    ...
    if (!($notifyUsername = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('phimail_notify'))) {
...
function phimail_read_blob($fp, $len): string|false
{
    ...
    $fpath = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir');

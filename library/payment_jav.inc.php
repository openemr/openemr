...
<script>
    ...
    function FormValidations() {//Screen validations are done here.
        ...
        if (document.getElementById('post_to_date').value == '') {
            ...
            document.getElementById('post_to_date').focus();
            return false;
        } else if (DateCheckGreater(document.getElementById('post_to_date').value, '<?php echo \OpenEMR\Core\OEGlobalsBag::getInstance()->get('post_to_date_benchmark') == '' ? date('Y-m-d', time() - (10 * 24 * 60 * 60)) : htmlspecialchars((string) oeFormatShortDate(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('post_to_date_benchmark')));?>',

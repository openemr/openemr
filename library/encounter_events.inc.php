...
function InsertEvent($args, $from = 'general')
{
    ...
    $parent_id = empty($args['parent_id']) ? '0' : $args['parent_id'];
    if ($from == 'general') {
        $pc_eid = sqlInsert(
            ...
            "pc_apptstatus, pc_prefcatid, pc_location, pc_eventstatus, pc_sharing, pc_facility,pc_billing_location,pc_room,pc_parent_eid " .
            ") VALUES (?,?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,?,?,?,?)",
            ...
            $args['locationspec'],(int)$args['facility'],(int)$args['billing_facility'],$form_room,$parent_id)
        );

...
function todaysEncounterCheck($patient_id, $enc_date = '', $reason = '', $fac_id = '', $billing_fac = '', $provider = '', $cat = '', $return_existing = true)
{
    ...
    if ($encounter && (int)\OpenEMR\Core\OEGlobalsBag::getInstance()->get('auto_create_new_encounters') !== 2) {
        if ($return_existing) {
            return $encounter;
        } else {
            return 0;
...
function InsertEvent($args, $from = 'general')
{
    ...
    if ($from == 'general') {
        ...
        if (!empty($form_pid)) {
            ...
            \OpenEMR\Core\OEGlobalsBag::getInstance()->set('temporary-eid-for-manage-tracker', $pc_eid); //used by manage tracker module to set correct encounter in tracker when check in

    ...
    if ($data_type == 1 || $data_type == 43 || $data_type == 46) {
        ...
        */
        ...
        while ($urow = sqlFetchArray($ures)) {
            ...
            if (empty($uname) || substr((string) $uname, 0, 1) == '(') {
                $uname = $urow['lname'];
                if ($urow['fname']) {
                    $uname .= ", " . $urow['fname'];
                }
...
function generate_print_field($frow, $currvalue, $value_allowed = true): void
{
    ...
    if ($data_type == 1 || $data_type == 26 || $data_type == 33 || $data_type == 43 || $data_type == 46) {
        ...
        if ($tmp === '') {
            ...
            $tmp = htmlspecialchars((string) $tmp, ENT_QUOTES);
        }
        echo $tmp;
...
function generate_display_field($frow, $currvalue)
{
    ...
    // A set of lab test results; Gestation, 3 radio buttons, test value, notes field:
        ...
        foreach ($values_array as $value) {
            ...
            if ($i > 0) {
                  $s = $s . ", " . htmlspecialchars((string) $lrow['name'], ENT_NOQUOTES);
            } else {
                $s = text($lrow['name'] ?? '');
            }
            $i++;
...
function accumActionConditions(&$frow, &$condition_str): void
{
    ...
    foreach ($conditions as $key => $condition) {
        ...
        if ($frow['data_type'] == BillingCodeType::OPTIONS_TYPE_INDEX && strpos((string) $frow['edit_options'], '2') !== false) {
            $billingCodeType = new BillingCodeType();
...
function getCodeDescription($codestring, $defaulttype = 'ICD10')
{
    ...
    list($ctype, $code) = explode(':', (string) $codestring);
    if (empty($code)) {
        $code = $ctype;
        $ctype = $defaulttype;
    }
...
function isSkipped(&$frow, $currvalue)
{
        ...
        if ($src_datatype == 21 && $src_list_id) { // array of checkboxes
            ...
            foreach ($tmp as $tmp2) {
                if (strpos($tmp2, "$itemid:") === 0) {
                    if ($datatype == 22) {
                        $srcvalue = substr($tmp2, strlen((string) $itemid) + 1);
                    } else {
                        $srcvalue = substr($tmp2, strlen((string) $itemid) + 1, 1);
                    }
                }
            }
        }
    ...
    if (substr((string) $action, 0, 6) == 'hsval=') {
...
{
    if ($sel != '*' && strpos((string) $sel, 'grp_group_id') === false) {
...
function display_layout_rows($formtype, $result1, $result2 = ''): void
{
    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            if ($formtype == 'DEM') {
                if (strpos((string) $field_id, 'em_') === 0) {
            ...
            if (!empty($currvalue) && !($currvalue == '0000-00-00 00:00:00')) {
                ...
                if (!$span_col_row) {
                    ...
                    if ($frow['title']) {
                        ...
                        if (strpos('?!.,:-=', substr((string) $tmp, -1, 1)) === false) {
                            echo ':';
                        }
                    } else {
                        echo "&nbsp;";
...
function lbf_current_value($frow, $formid, $encounter)
{
    ...
    if ($source == 'D' || $source == 'H') {
        ...
        } elseif (strpos((string) $field_id, 'em_') === 0) {

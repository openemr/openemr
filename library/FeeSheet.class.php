...
class FeeSheet
{
  ...
  // These variables are used to compute the initial consult service with highest CYP (IPPF).
    ...
    function __construct($pid = 0, $encounter = 0)
    {
        ...
        $this->ALLOW_COPAYS = empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific'));
        ...
        $this->match_services_to_products = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific') &&
  ...
  //  pricelevel
    public function addServiceLineItem($args)
    {
        ...
        if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('default_fee_sheet_line_item_provider')) && !empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('support_fee_sheet_line_item_provider'))) {
            if ($provider_id == 0) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_new_acceptor_policy') && $codetype == 'MA') {
            $codesrow = sqlQuery(
    ...
    public function addProductLineItem($args, $convert_units = true)
    {
        ...
        // the option is chosen to use or auto-generate Contraception forms.
        // It adds contraceptive method and effectiveness to relevant lines.
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('ippf_specific') && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_new_acceptor_policy')) {
    ...
    public function save(
        ...
        if (is_array($bill)) {
            foreach ($bill as $iter) {
                ...
                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('replicate_justification') == '1') {
                    if ($justify != '') {
                         $autojustify = $justify;
                    }
                }

                if ((\OpenEMR\Core\OEGlobalsBag::getInstance()->get('replicate_justification') == '1') && ($justify == '') && check_is_code_type_justify($code_type)) {
                    $justify =  $autojustify;
                }
            ...
            foreach ($prod as $iter) {
                ...
                if ($sale_id) {
                    ...
                    if ($del) {
                        ...
                        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_auto_create_rx') && $rxid && empty($iter['rx'])) {
            ...
            if (isset($newmauser)) {
                ...
                ******************************************************/
                ...
                if (empty($csrow)) {
                    ...
                    addForm($this->encounter, 'Contraception Summary', $newid, 'LBFcontra', $this->pid, \OpenEMR\Core\OEGlobalsBag::getInstance()->get('userauthorized'));

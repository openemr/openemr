...
<?php
	...
	if (flock($cron_lock, LOCK_EX | LOCK_NB)) { // do an exclusive lock
		...
		if(isset($isEventIdExists) && $isEventIdExists !== false) {
			$action_responce = @ActionEvent::actionReminderByEvent($event_type, $eventid_param, $configid_param);
		} else {
			$action_responce = @ActionEvent::actionReminder($event_type);
		}

		$total_items = 0;
		$total_sent_item = 0;

		if($action_responce['total_items'] > 0) {
			$total_items += $action_responce['total_items'];
		}

		if($action_responce['total_sent_item'] > 0) {
			$total_sent_item += $action_responce['total_sent_item'];
		}

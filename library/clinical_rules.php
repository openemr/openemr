...
function clinical_summary_widget($patient_id, $mode, $dateTarget = '', $organize_mode = 'default', $user = ''): void
{
  ...
  // Display the actions
    ...
    foreach ($actions as $action) {
        ...
        if ($mode == "reminders-due" && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_alert_log')) {
  ...
  // Only when $mode is reminders-due
    if ($mode == "reminders-due" && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_alert_log')) {
        ...
        if (!empty($new_targets) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_cdr_new_crp')) {
...
function active_alert_summary($patient_id, $mode, $dateTarget = '', $organize_mode = 'default', $user = '', $test = false)
{
  ...
  // Display the actions
    foreach ($actions as $action) {
        ...
        if (($mode == "reminders-due") && ($test === false) && (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_alert_log'))) {
            $target_temp = $action['category'] . ":" . $action['item'];
            $current_targets[$target_temp] =  ['rule_id' => $action['rule_id'],'due_status' => $action['due_status']];
        }
    }
...
function allergy_conflict($patient_id, $mode, $user, $test = false)
{
  ...
  // If there are conflicts, $test is FALSE, and alert logging is on, then run through compare_log_alerts
    $new_conflicts = [];
    if ((!empty($conflicts_unique)) && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_alert_log') && ($test === false)) {
...
function test_rules_clinic_batch_method($provider = '', $type = '', $dateTarget = '', $mode = '', $plan = '', $organize_mode = 'default', $options = [], $pat_prov_rel = 'primary', $batchSize = '', $report_id = null)
{
  ...
  // Set ability to itemize report if this feature is turned on
    if (
        ( ($type == "active_alert" || $type == "passive_alert")          && (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_standard')) ) ||
        ( (in_array($type, ["cqm", "cqm_2011", "cqm_2014"])) && (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_cqm'))      ) ||
        ( (CertificationReportTypes::isAMCReportType($type)) && (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_amc'))      )
    ) {
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set('report_itemizing_temp_flag_and_id', $report_id);
    } else {
        \OpenEMR\Core\OEGlobalsBag::getInstance()->set('report_itemizing_temp_flag_and_id', 0);
    }
    ...
    for ($i = 0; $i < $totalNumberBatches; $i++) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id')) {
            \OpenEMR\Core\OEGlobalsBag::getInstance()->set('report_itemized_test_id_iterator', 1);
        }
  ...
  //  a separate call to this function.
    ...
    foreach ($rules as $rowRule) {
        ...
        if ((count($targetGroups) == 1) || ($mode == "report")) {
            ...
            if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id'))) {
            ...
            foreach ($patientData as $rowPatient) {
                ...
                */
                    ...
                    foreach ($target_dates as $dateFocus) {
                        ...
                        */
                        ...
                        if ($passTarget) {
                            ...
                            if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id'))) {
                                insertItemReportTracker(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id'), \OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemized_test_id_iterator'), 1, $rowPatient['pid']);
                                $temp_track_pass = 1;
                            }
                ...
                if (!empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id')) && !($temp_track_pass)) {
                    insertItemReportTracker(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id'), \OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemized_test_id_iterator'), 0, $rowPatient['pid']);
                }
            }
        }
        ...
        if ($mode == "report") {
            ...
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id')) {
                $newRow = array_merge($newRow, ['itemized_test_id' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemized_test_id_iterator')]);
            }
                    ...
                    if ($passFilter) {
                        ...
                        foreach ($target_dates as $dateFocus) {
                            ...
                            if ($passTarget) {
                                ...
                                $pass_target++;
                                // If report itemization is turned on, then record the "passed" item and set the flag
                                if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id') ?? null) {
                                    insertItemReportTracker(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemizing_temp_flag_and_id'), \OpenEMR\Core\OEGlobalsBag::getInstance()->get('report_itemized_test_id_iterator'), 1, $rowPatient['pid']);
                                    $temp_track_pass = 1;
                                }

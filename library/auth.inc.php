...
if (
    ...
    && (
        ...
        || (filter_input(INPUT_POST, 'oidc_id_token') !== null && filter_input(INPUT_POST, 'oidc_id_token') !== '')
    )
) {
    // Attempt login
    ...
    if ($isOidcAttempt) {
        ...
        // Validate CSRF token on OIDC token POST to prevent login CSRF attacks.
        // The token is generated by the GCIP module when rendering the login form
        // and verified here using the same HMAC-based CsrfUtils mechanism.
        //
        // Aisle round-5 #11 (CWE-248). `verifyCsrfToken` calls
        // `collectCsrfToken` which throws RuntimeException when the
        // session lacks `csrf_private_key`. A pre-auth POST with a
        // non-empty `oidc_id_token` from a session that never rendered
        // the login form (so the key was never seeded) would otherwise
        // 500 instead of failing closed. Treat the throw as a normal
        // CSRF failure — same audit / rate-limit / re-render path as
        // a bad token.
        $oidcCsrfToken = filter_input(INPUT_POST, 'oidc_csrf_token') ?? '';
        try {
            $oidcCsrfOk = CsrfUtils::verifyCsrfToken($oidcCsrfToken, $session, 'oidc_login');

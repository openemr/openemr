...
foreach ($billkeys as $billkey) {
	...
	$args = array(new xmlrpcval($billkey['key'],XMLRPCSTRING), new xmlrpcval($format),new xmlrpcval($file_type));
	...
	if (!$r) { die("send failed"); }
  		...
  		if (!$r->faultCode()) {
			...
			$presult = $v->scalarval();
			echo "Claim for PatientID: $patient_id, Encounter: $billkey[key] processed successfully. Results are in file:\n " . basename("/" . $presult) . "\n";
			$sql = "UPDATE billing set process_date = now(), bill_process = 2, process_file = '" . basename("/" . $presult) . "' where encounter = $encounter AND pid = '" . $patient_id . "'";
			$results = $db->Execute($sql);
			if (!$results) {
				echo "There was an error with the database.\n";
				echo $db->ErrorMsg() . "\n";
			}
			elseif ($billkey['bill_process'] == 5) {
				//the bill was generated without an error now print it
				$fconfig = $GLOBALS['oer_config']['freeb'];
				$fname = $fconfig['claim_file_dir'] . basename("/" . $presult);
			...
			$sql = "UPDATE billing set process_date = now(), bill_process = 3, process_file = '" . mysql_real_escape_string(basename("/" . $presult)) . "' where encounter = $encounter AND pid = '" . $patient_id . "'";
			$results = $db->Execute($sql);
			if (!$results) {
				echo "There was an error with the database.\n";
				echo $db->ErrorMsg() . "\n";

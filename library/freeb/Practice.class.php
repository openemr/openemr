...
class Practice Extends DataObjectBase {
	...
	function Practice() {
		...
		$this->_addFunc("groupid",				array(	"name"	=>	"FreeB.FBPractice.GroupID",
															"sig"	=>	array(XMLRPCSTRING,XMLRPCINT,XMLRPCINT,XMLRPCINT),
															"doc"	=>	""));
		$this->_addFunc("x12idtype",			array(	"name"	=>	"FreeB.FBPractice.X12IdType",
															"sig"	=>	array(XMLRPCSTRING,XMLRPCINT,XMLRPCINT,XMLRPCINT),
															"doc"	=>	""));
	...
	*/
	...
	function x12idtype($m) {
		...
		if (!$results) {
			...
			while (!$results->EOF) {
				$vals[] = $results->fields['x12_id_type'];
				$results->MoveNext();
			}
		}
...
//'FreeB.FBPractice.X12IdType' 			=> \&FreeB_FBPractice_X12IdType,

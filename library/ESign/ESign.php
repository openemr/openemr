...
class ESign
{
    ...
    {
        $viewable = false;
        if (count($this->_signable->getSignatures()) > 0 && empty(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('esign_report_hide_all_sig'))) {
            ...
            if (($mode == "report") && (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('esign_report_hide_empty_sig'))) {
                $viewable = false;
            } else {

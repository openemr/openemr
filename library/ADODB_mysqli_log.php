...
class ADODB_mysqli_log extends ADODB_mysqli
{
    ...
    function Execute($sql, $inputarr = false, $insertNeedReturn = false)
    {
        ...
        if ($retval === false) {
            ...
            \OpenEMR\Core\OEGlobalsBag::getInstance()->set('last_mysql_error', $this->ErrorMsg());

            // Last error no
            \OpenEMR\Core\OEGlobalsBag::getInstance()->set('last_mysql_error_no', $this->ErrorNo());
        } else {
            $outcome = true;
        }
        ...
        // we insert into the audit log.
        if ($insertNeedReturn) {
            \OpenEMR\Core\OEGlobalsBag::getInstance()->set('lastidado', $this->Insert_ID());
        }

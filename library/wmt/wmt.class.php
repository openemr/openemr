...
class wmtInsurance {
	 ...
	 * @return array object list of insurance objects
	 */
	public static function getPidInsCase($pid, $case) {
		if(! $pid)
			throw new Exception('wmtInsurance::getPidInsCase - no patient identifier provided.');

		if(! $case)
			throw new Exception('wmtPatient::getPidInsCase - no case identifier provided.');
		...
		$query = "SELECT ins.id, ins.type, ins.date FROM `form_cases` cas ";
		$query .= "LEFT JOIN insurance_data ins ON cas.ins_data_id1 = ins.id ";
		$query .= "WHERE ins.pid = ? AND cas.id LIKE ? ";
		$query .= "AND provider != '' AND provider IS NOT NULL ";

		$primary = sqlQuery($query,$params);
		if ($primary) $list[] = new wmtInsurance($primary['id']);
// if ($pid == '56557') var_dump($params);
		$query = "SELECT ins.id, ins.type, ins.date FROM `form_cases` cas ";
		$query .= "LEFT JOIN insurance_data ins ON cas.ins_data_id2 = ins.id ";
		$query .= "WHERE ins.pid = ? AND cas.id LIKE ? ";
		$query .= "AND provider != '' AND provider IS NOT NULL ";

		$secondary = sqlQuery($query,$params);
		if ($secondary) $list[] = new wmtInsurance($secondary['id']);

		$query = "SELECT ins.id, ins.type, ins.date FROM `form_cases` cas ";
		$query .= "LEFT JOIN insurance_data ins ON cas.ins_data_id3 = ins.id ";
		$query .= "WHERE ins.pid = ? AND cas.id LIKE ? ";
		$query .= "AND provider != '' AND provider IS NOT NULL ";

		$tertiary = sqlQuery($query,$params);
		if ($tertiary) $list[] = new wmtInsurance($tertiary['id']);

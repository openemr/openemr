 ...
 */
		for ($i = 0; $i < count($map); $i++) {
			...
			while ($item['type'] == 'OBR') {
				...
				$report->specimen_datetime = $obr_data['collected_datetime'];
				...
				if ($message->lab_status == 'I') $message->lab_status = 'F'; // default final if we get any results
				if ($message->lab_status == 'F' && $obr_data['result_status'] != 'F') $message->lab_status = $obr_data['result_status'];

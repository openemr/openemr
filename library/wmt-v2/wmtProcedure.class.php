<?php
 /****************************************************************************
 *	wmtProcedure.class.php
 *
 *	Copyright (c)2018 - Williams Medical Technologies Inc. 
 *  www.williamsmedtech.com
 *
 ...
 * 
 *  or FITNESS FOR A PARTICULAR PURPOSE. DISTRIBUTOR IS NOT LIABLE TO USER FOR 
 *
 ...
 *
 *  AGREEMENT OR USE OF THIS SOFTWARE.
 *
 ...
 *  @author Rich Genandt <richg@williamsmedtech.com>
 *
 *****************************************************************************/
...
/**
 * Provides standardized processing for procedure orders and entry.
 *
 * @package WMT
 * @subpackage Forms
 */
class Procedure {
	...
	public $proc_title;
	public $proc_plan;
	public $proc_modifier;
	public $proc_units;
	public $proc_justify;
	
	/**
	 * Constructor for the 'Procedure' class which retrieves the requested
	 * information from the database or creates an empty object.
	 *
	 * @param int $id record identifier
	 * @return object instance of form class
	 */
	public function __construct($proc_id = false) {
		if (!$proc_id) return false;
		
		$query = 'SELECT * FROM lists WHERE id = ?';
		$results = sqlStatement($query, array($proc_id));
		...
		if ($data = sqlFetchArray($results)) {
			...
			throw new \Exception('wmtProcedure::_construct - no list item with id ('.$proc_id.').');
		}
	...
	public function store() {
		...
		if ($insert) { // do insert
			$this->id = sqlInsert("REPLACE lists SET $sql",$binds);
		} else { // do update
			$binds[] = $this->id;
			sqlStatement("UPDATE lists SET $sql WHERE id = ?",$binds);
		}
	 ...
	 * @return array $objectList list of selected objects
	 */
	public static function fetchEncounterProcedures($pid = false, $enc = false, $source = false) {

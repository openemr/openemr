...
function togglePanel(divid,imgid1,imgid2,barid)
{
  ...
  if(document.getElementById(divid).style.display == 'none') {
    ...
    document.getElementById(barid).style.borderBottom = 'solid 1px black';
		if(numargs >= 7) {
    	document.getElementById(arguments[6]).src = '../../../library/wmt-v2/fill-090.png';
		}
		if(numargs >= 8) {
    	document.getElementById(arguments[7]).src = '../../../library/wmt-v2/fill-090.png';
		}
		if(numargs >= 9) {
    	document.getElementById(arguments[8]).style.borderTop= 'solid 1px black';
    	// document.getElementById(arguments[8]).style.display= 'block';
		}
  } else {
    ...
    document.getElementById(barid).style.borderBottom = 'none';
		if(numargs >= 7) {
    	document.getElementById(arguments[6]).src = '../../../library/wmt-v2/fill-270.png';
		}
		if(numargs >= 8) {
    	document.getElementById(arguments[7]).src = '../../../library/wmt-v2/fill-270.png';
		}
		if(numargs >= 9) {
    	document.getElementById(arguments[8]).style.borderTop= 'none';
...
function ToggleDivDisplay(thisDiv, thisCheck)
{
  ...
  if(document.getElementById(thisCheck).checked == true) {
    document.getElementById(thisDiv).style.display = 'block';
		if(numargs > 2) document.getElementById(arguments[2]).value = 'block';
		if(numargs > 3) document.getElementById(arguments[3]).style.display = 'block';
  } else {
    document.getElementById(thisDiv).style.display = 'none';
		if(numargs > 2) document.getElementById(arguments[2]).value = 'none';
		if(numargs > 3) document.getElementById(arguments[3]).style.display = 'none';
	}
}

function ToggleDivHide(thisDiv, thisCheck)
{
	var numargs = arguments.length
  if(document.getElementById(thisCheck).checked == true) {
    document.getElementById(thisDiv).style.display = 'none';
		if(numargs > 2) document.getElementById(arguments[2]).value = 'none';
		if(numargs > 2) document.getElementById(arguments[3]).style.display = 'none';
  } else {
    document.getElementById(thisDiv).style.display = 'block';
		if(numargs > 2) document.getElementById(arguments[2]).value = 'block';
		if(numargs > 2) document.getElementById(arguments[3]).style.display = 'block';
	}
}
...
function VerifyYesFirstCheck()
{
  var numargs=arguments.length;
	var mode = 'list';
	if(numargs > 2) mode = arguments[1];
  // The First Item is the Yes Box
  if(document.getElementById(arguments[0]).checked == false) return(1);
	if(mode == 'name') {
		var test = '';
		if(numargs > 2) var test = arguments['2'];
		if(test == '') return(1);
		var l = document.forms[0].elements.length;
		var i;
		for(i=0; i<l; i++) {
			if(document.forms[0].elements[i].name.indexOf(test) != -1) {
				if(document.forms[0].elements[i].name != arguments[0]) {
...
{
  var fld = document.getElementById(thisDate);
  if(fld == null) return false;
  var thisStyle = fld.style.color;
  ...
  document.getElementById(thisDate).style.color = thisStyle;
}
...
function ClearTheseValues(nameMatch)
{
	var fldType = '';
	var f = document.forms[0];
	if(arguments.length > 1) fldType = arguments[1];
	var i;
	var l = f.elements.length;
	for( i=0; i<l; i++) {
		if(f.elements[i].name.indexOf(nameMatch) != -1) {
			if(fldType != '' && f.elements[i].type.indexOf(fldType) != -1) continue;
			if(f.elements[i].type.indexOf('select') != -1) {
				document.forms[0].elements[i].selectedIndex = 0;
			} else if(f.elements[i].type.indexOf('check') != -1) {
				document.forms[0].elements[i].checked = false;
			} else {
...
{
  if(thisField) {
    if(document.getElementById(thisField) == null) return false;
    document.getElementById(thisField).value= '';
  }
}

function ClearThis(fld)
{
  if(fld) if(document.getElementById(fld) == null) return false;
  if(fld) document.getElementById(fld).value = '';
}

function ShortTimeStamp(fld)
{
	var numargs = arguments.length;
  var currentTime = new Date();
  var myHours = "00" + currentTime.getHours();
  ...
  document.getElementById(fld).value = myTime;
	if(numargs >= 2) {
  	document.getElementById(arguments[1]).value = myTime;
	}
}
...
function SetScrollTop()
{
  ...
  // alert('Scroll: '+scroll);
	if(scroll != null) {
		// alert("Setting the scroll value: "+window.pageYOffset);
		scroll.value = parseInt(window.pageYOffset - 30);
...
function AutoSave(base, wrap, field, formID)
{
	var btn = '';
	var target = '';
	if(arguments.length > 4) btn = arguments[4];
	if(btn) var target = document.getElementById(btn);
	if(target != null) {
		target.style.pointerEvents = 'none';
	}
	var myAction = base+'&mode=save&wrap='+wrap+'&continue=true';
	if(field != '') myAction += '&focusfield='+field;
	if(formID != '') myAction += '&id='+formID;
	SetScrollTop();
	document.forms[0].action = myAction;
	
	document.forms[0].submit();
	if(target != null) {
		window.setTimeout("enableButton('"+btn+"')", 5000);
	}
}
...
function SubmitLinkBuilder(base,wrap)
{
  ...
  var num_links = 0;
	...
	if(numargs > 8) wmode = arguments[8];
...
function DeleteSurgery(base,wrap,itemID,formID)
{
	...
	if(confirm(warn)) {
  	document.forms[0].action=base+'&mode=delsurg&wrap='+wrap+'&itemID='+itemID;
		...
		document.forms[0].submit();
	} else hideDiv('save-notification');
	return false;
}
...
function DeleteSupplier(base,wrap,itemID,formID)
{
	...
	base = base+'&continue=true';
	if(!ValidateItem(itemID, 'sp_id_', 'Provider / Supplier')) return false;
	var warn="Delete This Entry From Providers/Suppliers?\n\nThis Action CAN NOT Be Reversed!";
	var numargs= arguments.length;
	if(numargs > 4) {
		var num_links = arguments[4];
		if(num_links > 0) {
...
function UpdatePrescriptionHistory(base,wrap,itemID,formID)
{
	SetScrollTop();
	base = base+'&continue=true';
	if(!ValidateItem(itemID, 'med_hist_id_', 'Prescription History')) return false;
...
function SubmitFavorite(base,wrap,itemID,formID,code_field,plan_field)
{
	var ptype = '';
	var no_alert = false;
	if(arguments.length > 6) ptype = arguments[6];
	if(arguments.length > 7) no_alert = arguments[7];
	SetScrollTop();
	base += '&continue=true';
	...
	if(!no_alert) alert("Plan Saved as a Favorite");
...
function clearBecause(thisField, triggerField, thisVal)
{
	var set_last_select = false;
	if(arguments.length > 3) set_last_select = arguments[3];
	var tst = document.getElementById(triggerField);
	if(tst == null) return;	
	var tst = document.getElementById(thisField);
	if(tst == null) return;	
	if(document.getElementById(triggerField).checked == thisVal) {
		if(tst.type.indexOf('select') != -1) {
			if(set_last_select) {
				tst.selectedIndex = tst.options.length;
			} else {
				tst.selectedIndex = 0;
			}
		} else {
...
{
	var js_root = '../../..';
	if(arguments.length > 2) js_root = arguments[2];
	wmtOpen(js_root+'/custom/add_list_entry_popup.php?thisItem='+itemField+'&thisList='+itemType, '_blank', 500, 300);
}
...
function submit_print_summary(base, wrap, field, formID)
{
	SetScrollTop();
	var myAction = base+'&mode=save&wrap='+wrap+'&continue=summary';
	if(field != '') myAction += '&focusfield='+field;
...
function getVitals(path, pid) {
	wmtOpen(path + '/custom/vital_choice_popup.v3.php?pid='+pid, '_blank', 700, 400);
}
...
function select_address(path, type)
{
 var href = path + '/interface/usergroup/addrbook_list.php?popup=true';
 href += '&select=true';
 if(type) href += '&form_abook_type=' + type;
 wmtOpen(href, '_blank', 700, 400);
}

function select_patient(path)
{
 wmtOpen(path + '/find_patient_popup.php', '_blank', 500, 400);
}
...
function popGrowthChart(format, data, today, stamp, weight, height, pid) {
	var tmp = document.forms[0].elements[stamp].value;
	var base = '../../..';
	if(arguments.length > 7) base = arguments[7];
	if(tmp == '' || tmp == 0) {
		var v_date = today;	
	} else {
		var v_date = tmp.substring(0,4)+tmp.substring(5,7)+tmp.substring(8,10);
	}
	var v_data = data+'~'+v_date+'-'+document.forms[0].elements[height].value+
		'-'+document.forms[0].elements[weight].value;
	var chartURL = base+'/interface/forms/vitals/growthchart/chart.php?pid='+
		pid + '&data=' + v_data +'&' + format + '=1';	
	top.restoreSession();
	window.open(chartURL, '_blank', "menubar=1,toolbar=1,scrollbars=1,resizeable=1,width=800,height=600");
}

...
<table width='100%' class="table table-borderless diagnosisTable text">
	<tr>
		<td class=' wmtBorder1B' style='width: 60px;'><?php echo ($use_sequence) ? 'Seq #' : '&nbsp;'; ?></td>
		<td class=' wmtBorder1L wmtBorder1B' style='width: 100px;'>Diagnosis</td>
		<td class=' wmtBorder1L wmtBorder1B' style='width: 100px;'>Start Date</td>
		<td class=' wmtBorder1L wmtBorder1B' style='width: 100px;'>End Date</td>
		<td class=' wmtBorder1L wmtBorder1B'>Title</td>
<?php if($show_unlink || !$suppress_plan || !$suppress_goal) { ?>
		<td class=' wmtBorder1L wmtBorder1B'>Description</td>
		<td class=' wmtBorder1B' style='width: 90px; padding: 0px; margin: 0px;'>&nbsp;</td>
<?php } else { ?>
		<td class=' wmtBorder1B' style='padding-right: 0px;'>Description</td>
<?php } ?>
	</tr>
<?php
$cnt=1;
...
if(count($diag) > 0) {
	foreach($diag as $prev) {
		...
		if($use_sequence) {
			echo "<td class='$suppress_class'><input name='dg_id_$cnt' id='dg_id_$cnt' type='hidden' readonly='readonly' value='".$prev['id']."' />#)&nbsp;<input name='dg_seq_$cnt' id='dg_seq_$cnt' class='form-control' type='text' style='width: 40px;' value='".htmlspecialchars($prev['seq'],ENT_QUOTES)."' /></td>\n";
		} else {
			echo "<td class='$suppress_class'><input name='dg_id_$cnt' id='dg_id_$cnt' type='hidden' readonly='readonly' value='".$prev['id']."' />&nbsp;$cnt&nbsp;).&nbsp;</td>\n";
		}
		echo "<td";
		echo $suppress_class ? ' class="'.$suppress_class.'"' : '';
		echo "><input name='dg_code_$cnt' id='dg_code_$cnt' class='dg_code_field form-control' type='text' value='".htmlspecialchars($prev['diagnosis'],ENT_QUOTES)."' onClick='".$diag_onclick."' ";
		echo "title='Click to select or clear a diagnosis' /></td>\n";
		echo "		<td"; 
		echo "><input name='dg_begdt_$cnt' id='dg_begdt_$cnt' class='form-control' type='text' value='".htmlspecialchars($prev['begdate'],ENT_QUOTES)."' /></td>\n";
		echo "		<td";
		echo $suppress_class ? ' class="'.$suppress_class.'"' : '';
		echo "><input name='dg_enddt_$cnt' id='dg_enddt_$cnt' class='form-control' type='text' value='".htmlspecialchars($prev['enddate'],ENT_QUOTES)."' /></td>\n";
		echo "		<td";
		echo $suppress_class ? ' class="'.$suppress_class.'"' : '';
		echo "><input name='dg_title_$cnt' id='dg_title_$cnt' class='form-control' type='text' readonly='readonly' value='".htmlspecialchars($prev['title'],ENT_QUOTES)."' /></td>\n";
		echo "		<td";
		echo $suppress_class ? ' class="'.$suppress_class.'"' : '';
		echo "><input name='tmp_dg_desc_$cnt' id='tmp_dg_desc_$cnt' class='form-control' type='text' tabindex='-1'";
		...
		if($show_unlink && $suppress_plan) {
			...
			if($diag_use_checkbox) {
				...
				echo "<div class='form-check'><input name='dg_link_$cnt' id='dg_link_$cnt' type='checkbox' class='form-check-input' value='1' ";
				echo ($dt['dg_link_'.$cnt]) ? 'checked' : '';
				echo " /><label for='dg_link_$cnt' class='form-check-label'>&nbsp;Linked&nbsp;</label></div>";
			} else {
		...
		if($GLOBALS['wmt::link_diag_education']) {
			...
			echo "<tr class='jumbotron'>\n";
			echo "<td class='' colspan='4'>\n";
			echo "&nbsp;&nbsp;&nbsp;<a style='cursor: pointer; color: blue;' ";
			echo "title='Click to Open Patient Education' ";
			echo "onclick=\"wmtOpen('$href', '_blank', 800, 600);\" >";
			echo "Click to View Patient Education</a></td>";
			echo "<td><div class='form-check'><input name='tmp_dg_ed_prn_$cnt' id='tmp_dg_ed_prn_$cnt' type='checkbox' class='form-check-input' value='1' ";
			...
			echo " /><label for='tmp_dg_ed_prn_$cnt' class='form-check-label'";
			if($frmdir == 'dashboard') echo "title='$ed_print_title' ";
			echo ">&nbsp;Printed &amp; Given to Patient&nbsp;</label></div>";
			if($frmdir == 'dashboard' && $ed_printed['id']) 
				echo "&nbsp;&nbsp;<span class='' style='color: blue;' title='" .
					$ed_print_title . "'>[&nbsp;Hover for Recent&nbsp;]</span>";
			echo "</td>";
			echo "<td class='wmtBorder1R'><div class='form-check'><input name='tmp_dg_ed_sent_$cnt' id='tmp_dg_ed_sent_$cnt' type='checkbox' class='form-check-input' value='1' ";
			...
			echo " /><label for='tmp_dg_ed_sent_$cnt' class='form-check-label'";
			if($frmdir == 'dashboard') echo "title='$ed_portal_title' ";
			echo ">&nbsp;Sent to Portal&nbsp;</label></div>";
		...
		if(!$suppress_plan) {
			echo "<tr class='jumbotron'>\n";
			echo "<td class='wmtT wmtR wmtBorder1B'>Plan:";
			if($frmdir != 'definable_fee') {
			...
			echo "</td>\n";
			echo "<td colspan='5' class='wmtBorder1B'><textarea name='dg_plan_$cnt' id='dg_plan_$cnt' class='form-control'";
			if($frmdir == 'definable_fee') echo " readonly='readonly'";
			echo '>' . htmlspecialchars($prev['comments'],ENT_QUOTES,'',FALSE);
			echo "</textarea></td>\n";
		
			echo "<td class='wmtBorder1L wmtBorder1B' style='vertical-align: top'>";
			if($show_unlink) {
				if($diag_use_checkbox) {
					if(!isset($dt['dg_link_'.$cnt])) {
						$dt['dg_link_'.$cnt] = '';
		...
		if(!$suppress_goal) {
			echo "<tr class='jumbotron'>\n";
			echo "<td class='wmtT wmtR wmtBorder1B'>Goals:";
			...
			echo "</td>\n";
			echo "<td colspan='5' class='wmtBorder1B'><textarea name='dg_goal_$cnt' id='dg_goal_$cnt' class='form-control'>",htmlspecialchars($prev['plan'],ENT_QUOTES),"</textarea></td>\n";
...
echo "	<tr class='jumbotron'>\n";
if($use_sequence) {
	echo "		<td class='wmtR $suppress_class'>#)<input name='dg_seq' id='dg_seq' class='wmtInput' type='text' style='width: 35px;' value='",htmlspecialchars($dt{'dg_seq'},ENT_QUOTES,'',FALSE),"' /></td>\n";
} else {
	echo "		<td class='$suppress_class'>&nbsp;$cnt&nbsp;).&nbsp;</td>\n";
}
if($suppress_class) $suppress_class = "class='wmtBorder1B'";
echo "		<td $suppress_class><input name='dg_code' id='dg_code' class='form-control' type='text' value='",htmlspecialchars($dt{'dg_code'},ENT_QUOTES,'',FALSE),"' onClick='".$diag_onclick1."' title='Click to select a diagnosis' /></td>\n";
echo "		<td $suppress_class><input name='dg_begdt' id='dg_begdt' class='form-control' type='text' value='",htmlspecialchars($dt{'dg_begdt'},ENT_QUOTES,'',FALSE),"' title='YYYY-MM-DD' /></td>\n";
echo "		<td $suppress_class><input name='dg_enddt' id='dg_enddt' class='form-control' type='text' value='",htmlspecialchars($dt{'dg_enddt'},ENT_QUOTES,'',FALSE),"' title='YYYY-MM-DD' /></td>\n";
echo "		<td $suppress_class><input name='dg_title' id='dg_title' class='form-control' type='text' value='",htmlspecialchars($dt{'dg_title'},ENT_QUOTES,'',FALSE),"' title='Enter a brief description of the problem here'/></td>\n";
echo "		<td $suppress_class><input name='tmp_dg_desc' id='tmp_dg_desc' class='form-control' type='text' readonly='readonly' value='",htmlspecialchars($dt{'tmp_dg_desc'},ENT_QUOTES,'',FALSE),"' /><input name='dg_type' id='dg_type' type='hidden' value='",$dt['dg_type']."' /></td>\n";
if($show_unlink || $use_favorites) {
	if($suppress_class) $suppress_class = 'wmtBorder1B';
	echo "		<td class=' wmtBorder1L $suppress_class'>&nbsp;</td>\n";
}
echo "	</tr>\n";
if(!$suppress_plan) {
	echo "	<tr class='jumbotron'>\n";
	...
	echo "</td>\n";
	echo "		<td class='wmtBorder1B' colspan='5'><textarea name='dg_plan' id='dg_plan' class='form-control'";
	if($frmdir == 'definable_fee') echo " readonly='readonly'";
...
if(!$suppress_goal) {
	echo "	<tr class='jumbotron'>\n";
	echo "	<td class='wmtT wmtR wmtBorder1B'>Goal:";
	echo "<br><br>";
	...
	echo "</td>\n";
	echo "	<td class='wmtBorder1B' colspan='5'><textarea name='dg_goal' id='dg_goal' class='form-control'>",htmlspecialchars($dt{'dg_goal'},ENT_QUOTES),"</textarea></td>\n";
	echo "	<td class=' wmtBorder1L wmtBorder1B'>&nbsp;";
	echo "<br>";

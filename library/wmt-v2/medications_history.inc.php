...
?>
	<tr>
		...
		<td class='wmtBody wmtBorder1L wmtBorder1B'><a href="javascript:;" class='css_button_small' tabindex='-1' onClick="return UpdatePrescriptionHistory('<?php echo $base_action; ?>','<?php echo $wrap_mode; ?>','<?php echo $cnt; ?>','<?php echo $id; ?>');" ><span><?php xl('Update','e'); ?></span></a>
		<?php if($unlink_allow) { ?>
		<a href="javascript:;" class='css_button_small' tabindex='-1' onClick="return UnlinkPrescriptionHistory('<?php echo $base_action; ?>','<?php echo $wrap_mode; ?>','<?php echo $cnt; ?>','<?php echo $id; ?>');" ><span><?php xl('Un-Link','e'); ?></span></a>
		<?php } ?>
		</td>
	</tr>
<?php
...
?>
	...
	<tr>
		...
		<?php	if(($dt['tmp_mhist_window_mode'] == 'limit') && 
													$max_med_hist && (count($med_hist) > $max_med_hist)) { ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="2">
		<div style="float: left; padding-left: 15px;"><a href="javascript:;" class='css_button_small' tabindex='-1' onClick="return ToggleWindowDisplayMode('<?php echo $base_action; ?>','<?php echo $wrap_mode; ?>','<?php echo $id; ?>','medhistwindow','all');" ><span><?php xl('Show ALL Prescription History','e'); ?></span></a></div>
		</td>
		<?php } else if(($dt['tmp_mhist_window_mode'] == 'all') && 
																(count($med_hist) > $max_med_hist) && $max_med_hist) { ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="2">
		<div style="float: left; padding-left: 15px;"><a href="javascript:;" class='css_button_small' tabindex='-1' onClick="return ToggleWindowDisplayMode('<?php echo $base_action; ?>','<?php echo $wrap_mode; ?>','<?php echo $id; ?>','medhistwindow','limit');" ><span><?php xl('Show '.$max_med_hist.' Most Recent','e'); ?></span></a></div>
		<?php } else { ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="2">&nbsp;</td>
		<?php } ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="1">&nbsp;</td>
		<?php if($unlink_all_rx_history) { ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="2">
		<div style="float: right; padding-right: 15px;"><a href="javascript:;" class='css_button_small' tabindex='-1' onClick="return UnlinkAllPrescriptionHistory('<?php echo $base_action; ?>','<?php echo $wrap_mode; ?>','<?php echo ($cnt); ?>','<?php echo $id; ?>');" ><span><?php xl('Un-Link ALL History','e'); ?></span></a></div>
		</td>
		<?php } else { ?>
		<td class="wmtCollapseBar wmtBorder1B" colspan="2">
		<?php } ?>
...
?>
	<tr>
		<td class='wmt wmtLabel wmtBorder1B'>&nbsp;</td>
		<td class='wmt wmtLabel wmtBorder1L wmtBorder1B'>&nbsp;<?php xl('None on File','e'); ?></td>

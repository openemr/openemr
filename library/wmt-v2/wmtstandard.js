...
{
	var numargs = arguments.length
	if(checked_mode == 'none') unchecked_mode = 'block';
...
function VerifyYesFirstCheck()
{
  var numargs=arguments.length;
	var name_mode = false;
  if(numargs > 1 && arguments[1] == 'name') name_mode = true;
  ...
  if(document.getElementById(arguments[0]).checked == false) return(1);
	if(name_mode) {
		...
		for (i=0; i<l; i++) {
			if(document.forms[0].elements[i].name.indexOf(arguments[2]) != -1) {
...
function SubmitMedicalHistory(base,wrap,formID)
{
	SetScrollTop();
	if(base.indexOf('?') == -1) {
		base += '?mode=pmh&wrap='+wrap;
	} else {
		base += '&mode=pmh&wrap='+wrap;
	}
  document.forms[0].action = base;
  if(formID != '' && formID != 0 && formID != null) {
 		document.forms[0].action=base+'&id='+formID;
	}
  document.forms[0].action=document.forms[0].action+'&continue=true';
	document.forms[0].submit();
}
...
function checkIf(chk, thisFld)
{
	if(thisFld.value) chk.ckecked = true;
}
...
function getVitals(path, pid) {
	wmtOpen(path+'/custom/vital_choice.v3.popup.php?pid='+pid, '_blank', 500, 400);
}
...
{
 wmtOpen(path + '/custom/find_patient_popup.php', '_blank', 500, 400);
}
...
function elementExists(id, err)
{
	var e = document.getElementById(id);
	if(!e || e == null) {
		if(err) alert('ERROR - Element [' + id + '] Does NOT Exist!');
		return false;
	}
	return e;
}

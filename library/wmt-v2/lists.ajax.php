...
if($action == 'del') {
	...
	if($type == 'dated_document') {
		if(!$item) {
			echo 'error';
			exit;
		}
		DeleteDatedDocument($pid, $item);
	}
	if(!$encounter) {
		echo 'error';
		exit;
...
if($action == 'update') {
	...
	} else if($type == 'dated_document') {
		UpdateDatedDocument($pid,$item,$cplan);
	}
...
if($action == 'add') {
	...
	if($type == 'medical_problem') {
		...
		AddDatedDocument($pid,$ctype,$s_dt,$title,$cplan,$code);
	}

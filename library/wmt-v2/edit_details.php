...
// Include Layout Based Williams Forms.
$lres = sqlStatement("SELECT * FROM list_options " .
		"WHERE list_id LIKE '%_Forms' ORDER BY title");
while ($lrow = sqlFetchArray($lres)) {
	$layouts[$lrow['option_id']] = $lrow['title'];
}
...
// Check authorization.
$thisauth = \OpenEMR\Common\Acl\AclMain::aclCheckCore('admin', 'super');
...
if ($_POST['formaction'] == "save" && $layout_id) {
    ...
    for ($lino = 1; isset($fld[$lino]['id']); ++$lino) {
        ...
        if ($field_id) {
            sqlStatement("UPDATE layout_options SET " .
                ...
                "WHERE form_id = '$layout_id' AND group_name = '".$iter['group']."' AND field_id = '$field_id'");
        }
    }
}
    ...
    if (substr($layout_id,0,3) != 'LBF' && $layout_id != "FACUSR" && $data_type != "6") {
    ...
    // criswell - added type '6' which is label_only so no database changes
    if (substr($layout_id,0,3) != 'LBF' && $layout_id != "FACUSR" && $data_type != "6") {
      // Add the field to the table too (this is critical)
      if ($layout_id == "DEM") { $tablename = "patient_data"; }
      else if ($layout_id == "HIS") { $tablename = "history_data"; }
...
//    }
    ...
    *****************************************************************/
    ...
    if ($linedata['data_type'] == 31) {
      echo "  <td align='center' class='optcell'>";
      echo "<textarea name='fld[$fld_line_no][desc]' rows='3' cols='35' class='optin' style='resize:both'>" .
...
<form method='post' name='theform' id='theform' action='edit_details.php'>
...
<td><input type="textbox"  name="gnewdefault" id="gnewdefault" value="" /> </td>
...
?>
   ...
   <td><input type="textbox"  name="newdefault" id="newdefault" value="" /> </td>
...
//        }
    ...
    var ShowLists = function(btnObj) {
        window.open("../../interface/super/show_lists_popup.php", "lists", "width=300,height=500,scrollbars=yes");
        selectedfield = btnObj;
    };
    
    // show the popup choice of groups
    var ShowGroups = function(btnObj) {
        window.open("../../interface/super/show_groups_popup.php?layout_id=<?php echo $layout_id;?>", "groups", "width=300,height=300,scrollbars=yes");
    };

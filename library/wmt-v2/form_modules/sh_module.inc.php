...
if(!isset($use_smk_program)) $use_smk_program = checkSettingMode('wmt::smoking_program','',$frmdir);
...
$local_fields = array('sex_active', 'sex_act_nt', 'bc', 'bc_chc', 
	...
	'coffee_note', 'coffee_dt', 'sex_nt', 'sex_orient_nt',
  'sex_sti', 'sex_sti_nt');
if($expanded_sh) {
...
if(!isset($dt['fyi_sh_nt'])) $dt['fyi_sh_nt'] = '';
if(!isset($pat_entries['fyi_sh_nt'])) $pat_entries['fyi_sh_nt'] = $portal_data_layout;
...
<!-- SELECTION OF 'option_id' 1, 2, 5, 15 or 16 
	...
	<?php
	if($use_smk_program) {
		$prow = sqlQuery('SELECT * FROM form_' . $use_smk_program .
				'_screen WHERE pid=? ORDER BY sent DESC',array($pid));
		if($prow{'sent'}) {
			echo "<tr><td class='wmtLabel' colspan='4'>Enrolled in " . 
				ucfirst($use_smk_program) .  ' ' . $prow{'sent'} . ' by ' .
				UserNameFromID($prow{'sent_by'},'first') .  "</td></tr>\n";
		} else {
			echo "<tr><td colspan='4'>Not currently referred to any program</td></tr>\n";
		}
	}
	?>
	...
	<tr>
		<td class="<?php echo ($portal_mode) ? 'bkkBody' : 'wmtBody'; ?>" colspan="3">&nbsp;&nbsp;&nbsp;
			<input name="tmp_cessation_check" id="tmp_cessation_check" type="checkbox" value="show" <?php echo ($dt['tmp_cessation_disp_mode'] == 'none') ? '' : 'checked'; ?> onchange="ToggleDivDisplay('smoke_cessation', 'tmp_cessation_check', 'tmp_cessation_disp_mode', 'smoke_cessation_btn');" />
			&nbsp;&nbsp;<label for="tmp_cessation_check"><i>Smoking Cessation Questionnaire</i></label>
		</td>
		<td colspan="3"><div id="smoke_cessation_btn" style="float: right; padding-right: 12px; display: <?php echo $dt['tmp_cessation_disp_mode']; ?>;"><a href="javascript:;" class="css_button_small" onclick="ClearTheseValues('lf_sc_');"><span>Clear Smoking Questionaire</span></a></div></td>
	</tr>
	<tr><td height="3"></td></tr>
</table>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="3">
      ...
      <tr>
        ...
        <td class="<?php echo ($portal_mode) ? 'bkkCalendarCell' : 'wmtCalendarCell'; ?>"><img src="<?php echo $GLOBALS['rootdir']; ?>/pic/show_calendar.gif" align="absbottom" width="20" height="20" id="img_<?php echo $field_prefix; ?>alcohol_dt" border="0" alt="[?]" style="cursor:pointer" title="Click here to choose a date"></td>
			...
			<tr>
				<td class="<?php echo ($portal_mode) ? 'bkkBody' : 'wmtBody'; ?>" colspan="3">&nbsp;&nbsp;&nbsp;
					<input name="tmp_ace_check" id="tmp_ace_check" type="checkbox" value="show" <?php echo ($dt['tmp_ace_disp_mode'] == 'none') ? '' : 'checked'; ?> onchange="ToggleDivDisplay('alcohol_ace', 'tmp_ace_check', 'tmp_ace_disp_mode','ace_btn');" />
					&nbsp;&nbsp;<label for="tmp_ace_check"><i>ACE Questionnaire</i></label>
				</td>
				<td colspan="3"><div id="ace_btn" style="float: right; padding-right: 12px; display: <?php echo $dt['tmp_ace_disp_mode']; ?>;"><a href="javascript:;" class="css_button_small" onclick="ClearTheseValues('lf_ace_');"><span>Clear ACE Questionaire</span></a></div></td>
			</tr>
			<tr><td height="3"></td></tr>
		</table>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="3">
			...
			<tr><td height="3"></td></tr>
				<td class="<?php echo ($portal_mode) ? 'bkkBody' : 'wmtBody'; ?>" colspan="3">&nbsp;&nbsp;&nbsp;
					<input name="tmp_t_ace_check" id="tmp_t_ace_check" type="checkbox" value="show" <?php echo ($dt['tmp_t_ace_disp_mode'] == 'none') ? '' : 'checked'; ?> onchange="ToggleDivDisplay('alcohol_t_ace', 'tmp_t_ace_check', 'tmp_t_ace_disp_mode','t_ace_btn');" />
					&nbsp;&nbsp;<label for="tmp_t_ace_check"><i>T-ACE Alcohol Questionnaire</i></label>
				</td>
				<td colspan="2"><div id="t_ace_btn" style="float: right; padding-right: 12px; display: <?php echo $dt['tmp_t_ace_disp_mode']; ?>; "><a href="javascript:;" class="css_button_small" onclick="ClearTheseValues('lf_t_ace_');"><span>Clear T-ACE Questionaire</span></a></div></td>
			</tr>
			<tr><td height="3"></td></tr>
		</table>
...
function total_t_ace(pre)
{
  ...
  if(document.getElementById(pre+'lf_t_ace_e').selectedIndex == 1) t = 1;
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="3">
			...
			<tr>
				<td class="<?php echo ($portal_mode) ? 'bkkBody' : 'wmtBody'; ?>" colspan="3">&nbsp;&nbsp;&nbsp;
					<input name="tmp_audit_check" id="tmp_audit_check" type="checkbox" value="show" <?php echo ($dt['tmp_audit_disp_mode'] == 'none') ? '' : 'checked'; ?> onchange="ToggleDivDisplay('alcohol_audit', 'tmp_audit_check', 'tmp_audit_disp_mode','audit_btn');" />
					&nbsp;&nbsp;<label for="tmp_audit_check"><i>Alcohol 'AUDIT' Questionnaire</i></label>
				</td>
				<td colspan="3"><div id="audit_btn" style="float: right; padding-right: 12px; display: <?php echo $dt['tmp_audit_disp_mode']; ?>;"><a href="javascript:;" class="css_button_small" onclick="ClearTheseValues('lf_alc_');"><span>Clear AUDIT Questionaire</span></a></div></td>
			</tr>
			<tr><td height="3"></td></tr>
		</table>
      ...
      <tr>
        ...
        <td class="<?php echo ($portal_mode) ? 'bkkCalendarCell' : 'wmtCalendarCell'; ?>"><img src="<?php echo $GLOBALS['rootdir']; ?>/pic/show_calendar.gif" align="absbottom" width="20" height="20" id="img_<?php echo $field_prefix; ?>coffee_dt" border="0" alt="[?]" style="cursor:pointer" title="Click here to choose a date"></td>
			...
			<tr class="wmtPortalData">
				...
				<td>&nbsp;</td>
				<td class="wmtBorderHighlight wmtBody" id="tmp_<?php echo $field_prefix; ?>drug_dt" onclick="AcceptPortalData(this.id);"><?php echo htmlspecialchars($pat_entries['coffee_dt']['content'], ENT_QUOTES, '', FALSE); ?></td>
			</tr>
			<?php
				}
			}
		...
		<?php if($field_prefix != 'ee1_') { ?>
			<tr>
				<td style="width: 120px;" class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody' ;?>">Gender&nbsp;&nbsp;&nbsp;</td>
				<td class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody'; ?>" style="width: auto">
					<select name="pat_sex" id="pat_sex" class="wmtInput">
						<?php ListSel($patient->sex,'sex'); ?>
					</select>
				</td>
        		<td colspan="5">
        			&nbsp;
			...
			<tr>
				<td class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody' ;?>">Orientation&nbsp;&nbsp;&nbsp;</td>
				<td class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody'; ?>" style="width: auto;">
					<select name="<?php echo $field_prefix; ?>sex_orient" id="<?php echo $field_prefix; ?>sex_orient" class="<?php echo ($portal_mode) ? 'bkkInput' : 'wmtInput'; ?>">
						<?php ListSel($dt[$field_prefix.'sex_orient'],'Sexual_Orientation'); ?>
					</select>
				</td>
        		<td colspan="5">
        			<input name="<?php echo $field_prefix; ?>sex_orient_nt" id="<?php echo $field_prefix; ?>sex_orient_nt" class="<?php echo $portal_mode ? 'bkkFullInput' : 'wmtFullInput'; ?>" type="text" value="<?php echo $dt[$field_prefix.'sex_orient_nt']; ?>" />
        		</td>
			</tr>
			<tr>
				<td class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody' ;?>">Sexually active?&nbsp;&nbsp;&nbsp;</td>
        ...
        <td colspan="5"><textarea name="<?php echo $field_prefix; ?>sex_nt" id="<?php echo $field_prefix; ?>sex_nt" class="<?php echo $portal_mode ? 'bkkFullInput' : 'wmtFullInput'; ?>" rows="3"><?php echo htmlspecialchars($dt{$field_prefix.'sex_nt'}, ENT_QUOTES, '', FALSE); ?></textarea></td>
			...
			<tr>
				<td style="width: 120px;" class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody' ;?>">Ever had a STI?&nbsp;&nbsp;&nbsp;</td>
				<td class="<?php echo $portal_mode ? 'bkkBody' : 'wmtBody'; ?>" style="width: 220px;">
					<select name="<?php echo $field_prefix; ?>sex_sti" id="<?php echo $field_prefix; ?>sex_sti" class="<?php echo ($portal_mode) ? 'bkkInput' : 'wmtInput'; ?>">
						<?php ListSel($dt[$field_prefix.'sex_sti'],'YN_DECLINE'); ?>
					</select>
				</td>
        		<td colspan="5">
        			<input name="<?php echo $field_prefix; ?>sex_sti_nt" id="<?php echo $field_prefix; ?>sex_sti_nt" class="<?php echo $portal_mode ? 'bkkFullInput' : 'wmtFullInput'; ?>" type="text" value="<?php echo $dt[$field_prefix.'sex_sti_nt']; ?>" />
        		</td>
			</tr>
...
<?php
$field_name = 'fyi_sh_nt';
$prefix_save = $field_prefix;
$field_prefix = '';
include($GLOBALS['srcdir'].'/wmt-v2/specified_text_box.inc.php');
$field_prefix = $prefix_save;


if($pat_entries_exist && !$portal_mode) {
	if($pat_entries['fyi_sh_nt']['content'] && (strpos($dt{'fyi_sh_nt'},$pat_entries['fyi_sh_nt']['content']) === false)) {
?>
		<div class="wmtLabel wmtPortalData wmtL" style="margin: 6px;">Notes input by the patient via the patient portal:</div>
		<div class="wmtPortalData wmtBorderHighlight wmtBodyi wmtL" id="tmp_fyi_sh_nt" onclick="AcceptPortalData(this.id);"><?php echo htmlspecialchars($pat_entries['fyi_sh_nt']['content'], ENT_QUOTES, '', FALSE); ?></div>
<?php
	}
}
?>

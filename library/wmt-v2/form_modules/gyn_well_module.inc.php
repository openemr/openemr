...
if(!isset($GLOBALS['wmt::use_dated_documents'])) $GLOBALS['wmt::use_dated_documents'] = '';
if($GLOBALS['wmt::use_dated_documents']) {
$dated_doc_id = array();
$linked_docs = array('last_bone', 'last_mamm', 'last_pap');
...
<fieldset style="border: solid 1px gray; margin: 6px;"><legend class="wmtLabel">&nbsp;Gynecological&nbsp;</legend>
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td colspan="3">&nbsp;</td>
		<?php // BEGINNING OF THE LAST BONE DENSITY LINK HANDLER
		$href_text = '';
		if($dated_doc_id['last_bone']) { 
			$d = new Document($dated_doc_id['last_bone']);
			$url = $d->get_url();
			//strip url of protocol handler
			$url = preg_replace("|^(.*)://|","",$url);
			$from_all = explode("/", $url);
			$from_filename = array_pop($from_all);
			$category = $d->get_ccr_type($dated_doc_id['last_bone']);
			$href_text = $from_filename . "  (" . $category . ")";
		}
		?>
			<td colspan="2" id="link_<?php echo $field_prefix; ?>last_bone_doc">
			<?php if($href_text) { ?>
			Document: <a href="javascript:;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/controller.php?document&retrieve&patient_id=<?php echo $pid; ?>&document_id=<?php echo $dated_doc_id['last_bone']; ?>&as_file=false', '_blank', 800, 600);"><i><?php echo htmlspecialchars($href_text, ENT_QUOTES); ?></i></a>
			<?php } else { ?>
			<i>No document is currently attached</i>
			<?php } ?>
			&nbsp;</td>
			<td><div id="img_<?php echo $field_prefix; ?>last_bone_div">
			<?php if($dated_doc_id['last_bone']) { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_break.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_bone_doc_unlink" border="0" alt="[x]" style="vertical-align: bottom; cursor:pointer;" onclick="breakLink('<?php echo $dated_doc_id['last_bone']; ?>', '<?php echo $pid; ?>', 'last_bone', '<?php echo $field_prefix; ?>');" title"Click here to remove the link">&nbsp;
			<?php } else { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_add.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_bone_doc" border="0" alt="[d]" style="vertical-align: bottom; cursor:pointer;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/custom/document_popup.php?pid=<?php echo $pid; ?>&task=link&link_type=last_bone&prefix=<?php echo $field_prefix; ?>', '_blank', 800, 600);" title"Click here to link a document">&nbsp;
			<?php } ?>	
			</div></td>

		<?php // BEGINNING OF THE LAST MAMMOGRAM LINK HANDLER
		$href_text = '';
		if($dated_doc_id['last_mamm']) { 
			$d = new Document($dated_doc_id['last_mamm']);
			$url = $d->get_url();
			//strip url of protocol handler
			$url = preg_replace("|^(.*)://|","",$url);
			$from_all = explode("/", $url);
			$from_filename = array_pop($from_all);
			$category = $d->get_ccr_type($dated_doc_id['last_mamm']);
			$href_text = $from_filename . "  (" . $category . ")";
		}
		?>
			<td colspan="2" id="link_<?php echo $field_prefix; ?>last_mamm_doc">
			<?php if($href_text) { ?>
			Document: <a href="javascript:;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/controller.php?document&retrieve&patient_id=<?php echo $pid; ?>&document_id=<?php echo $dated_doc_id['last_mamm']; ?>&as_file=false', '_blank', 800, 600);"><i><?php echo htmlspecialchars($href_text, ENT_QUOTES); ?></i></a>
			<?php } else { ?>
			<i>No document is currently attached</i>
			<?php } ?>
			&nbsp;</td>
			<td><div id="img_<?php echo $field_prefix; ?>last_mamm_div">
			<?php if($dated_doc_id['last_mamm']) { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_break.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_mamm_doc_unlink" border="0" alt="[x]" style="vertical-align: bottom; cursor:pointer;" onclick="breakLink('<?php echo $dated_doc_id['last_mamm']; ?>', '<?php echo $pid; ?>', 'last_mamm', '<?php echo $field_prefix; ?>');" title"Click here to remove the link">&nbsp;
			<?php } else { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_add.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_mamm_doc" border="0" alt="[d]" style="vertical-align: bottom; cursor:pointer;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/custom/document_popup.php?pid=<?php echo $pid; ?>&task=link&link_type=last_mamm&prefix=<?php echo $field_prefix; ?>', '_blank', 800, 600);" title"Click here to link a document">&nbsp;
			<?php } ?>	
			</div></td>
		</tr>
		<?php } ?>
		...
		<tr>
			<td colspan="3">&nbsp;</td>
			<td colspan="3">FRAX:&nbsp;&nbsp;<input name="<?php echo $field_prefix; ?>frax" id="<?php echo $field_prefix; ?>frax" class="wmtInput" style="width: 85%;" value="<?php echo attr($dt[$field_prefix.'frax']); ?>" /></td>
			<td>Dense Breast Mammogram Law Informed?</td>
			<td><select name="<?php echo $field_prefix; ?>mam_law" id="<?php echo $field_prefix; ?>mam_law" class="wmtInput"><?php echo ListSel($dt[$field_prefix.'mam_law'], 'YesNo'); ?></select></td>
			<td>&nbsp;</td>
		</tr>
		...
		if($pat_entries_exist && !$portal_mode) {
			...
			<tr class="wmtPortalData">
				...
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td colspan="3" class="wmtBorderHighlight" id="tmp_<?php echo $field_prefix; ?>frax" onclick="AcceptPortalData(this.id);"><?php echo attr($pat_entries['frax']['content']); ?></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
		...
		<tr>
			<td colspan="6">&nbsp;</td>
		<?php // BEGINNING OF THE LAST PAP LINK HANDLER
		$href_text = '';
		if($dated_doc_id['last_pap']) { 
			$d = new Document($dated_doc_id['last_pap']);
			$url = $d->get_url();
			//strip url of protocol handler
			$url = preg_replace("|^(.*)://|","",$url);
			$from_all = explode("/", $url);
			$from_filename = array_pop($from_all);
			$category = $d->get_ccr_type($dated_doc_id['last_pap']);
			$href_text = $from_filename . "  (" . $category . ")";
		}
		?>
			<td colspan="2" id="link_<?php echo $field_prefix; ?>last_pap_doc">
			<?php if($href_text) { ?>
			Document: <a href="javascript:;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/controller.php?document&retrieve&patient_id=<?php echo $pid; ?>&document_id=<?php echo $dated_doc_id['last_pap']; ?>&as_file=false', '_blank', 800, 600);"><i><?php echo htmlspecialchars($href_text, ENT_QUOTES); ?></i></a>
			<?php } else { ?>
			<i>No document is currently attached</i>
			<?php } ?>
			&nbsp;</td>
			<td><div id="img_<?php echo $field_prefix; ?>last_pap_div">
			<?php if($dated_doc_id['last_pap']) { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_break.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_pap_doc_unlink" border="0" alt="[x]" style="vertical-align: bottom; cursor:pointer;" onclick="breakLink('<?php echo $dated_doc_id['last_pap']; ?>', '<?php echo $pid; ?>', 'last_pap', '<?php echo $field_prefix; ?>');" title"Click here to remove the link">&nbsp;
			<?php } else { ?>
			<img src="<?php echo $GLOBALS['webroot']; ?>/images/link_add.png" width="20" height="20" id="img_<?php echo $field_prefix; ?>last_pap_doc" border="0" alt="[d]" style="vertical-align: bottom; cursor:pointer;" onclick="wmtOpen('<?php echo $GLOBALS['webroot']; ?>/custom/document_popup.php?pid=<?php echo $pid; ?>&task=link&link_type=last_pap&prefix=<?php echo $field_prefix; ?>', '_blank', 800, 600);" title"Click here to link a document">&nbsp;
			<?php } ?>	
			</div></td>
		</tr>
		<?php } ?>
...
<script type="text/javascript">
<?php
include_once(FORM_JS_DIR . 'gyn_well.js.php');
?>
</script>

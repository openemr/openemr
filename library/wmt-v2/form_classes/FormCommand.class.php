<?php
	...
	class FormCommand
	{
		...
		/*
			error_log("Debug (0)");
			foreach($dbug[0] as $key => $val) {
				if(is_string($val)) {
					error_log("   ($key) - [$val]\n");
				} else {
					$dbg_string = var_export($val, TRUE);
					error_log("  Not a string: ".$dbg_string);
				}
			}
			error_log("Debug (1)");
			foreach($dbug[1] as $key => $val) {
				if(is_string($val)) {
					error_log("   ($key) - [$val]\n");
				} else {
					$dbg_string = var_export($val, TRUE);
		...
		*/
			$callerFileName = isset($dbug[0]['file']) ? $dbug[0]['file'] : '';
			// error_log("Caller (1) [$callerFileName]");
			/*
			$callerFileName = isset(debug_backtrace()[0]['file']) ? debug_backtrace()[0]['file'] : '';
			error_log("Caller (2) [$callerFileName]");
			*/
			
			if (!preg_match('/form_commands\/([a-zA-Z0-9_]+).cmd.php$/', $callerFileName, $matches))
			{
				error_log(__FILE__ . ':' . __LINE__ . ' - $Caller file name wasn\'t a valid file name. Got - ' . var_export($callerFileName, true) . "\nThis is gunna cause a lot of problems so start looking here if something doesn't work right and getting this error");
				return;
			}

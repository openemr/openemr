...
class C_Prescriptiondutch extends Controller {
	...
	function multiprint_header(& $pdf, $p) {
		...
		$pdf->ezText(str_pad('Datum:', 26) . date('m/d/Y'), 12, array('left' => 30, 'spacing' => 2));
                $pdf->ezText(str_pad('Naam:', 25) . $p->patient->get_name_display(), 12, array('left' => 30, 'spacing' => 2));
                $pdf->ezText(str_pad('Geboortedatum:', 20) . $p->patient->date_of_birth, 12, array('left' => 30, 'spacing' => 2));
	...
	function get_prescription_body_text($p) {
		...
		$body .=  "\n" .
			'No.' . $p->get_quantity() . "\n" .
			//'S. ' . $p->get_dosage() . ' ' . $p->form_array[$p->get_form()] . ' ' .
                        'S. ' . $p->get_dosage() . ' ' .

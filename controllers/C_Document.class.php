...
class C_Document extends Controller {
	...
	function retrieve_action($patient_id="",$document_id,$as_file=true,$original_file=true,$disable_exit=false,$show_original=false) {
		...
		if($couch_docid && $couch_revid && $original_file){
			...
			if ( $doEncryption ) {
				...
				header("Content-Disposition: " . ($as_file ? "attachment" : "inline") . "; filename=\"" . $this->basename_nonenglish($d->get_url()) . "\"");
		...
		if (!file_exists($url)) {
			echo xl('The requested document is not present at the expected location on the filesystem or there are not sufficient permissions to access it.','','',' ') . $url;
			    ...
			    //special case when retrieving a document that has been converted to a jpg and not directly referenced in database
			   	$convertedFile = substr($this->basename_nonenglish($url), 0, strrpos($this->basename_nonenglish($url), '.')) . '_converted.jpg';
				if($couch_docid && $couch_revid){
			    ...
			    header("Expires: 0");
			    header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
			    header("Content-Disposition: " . ($as_file ? "attachment" : "inline") . "; filename=\"" . $this->basename_nonenglish($url) . "\"");
	...
	function queue_action($patient_id="") {
		...
		if (file_exists($this->_config['repository']) && is_dir($this->_config['repository'])) {
			...
			while (($file = readdir($dir)) !== false) {
				...
				if (is_file($file) && strpos($this->basename_nonenglish($file),".") !== 0) {
					$file_info['filename'] = $this->basename_nonenglish($file);
					$file_info['mtime'] = date("m/d/Y H:i:s",filemtime($file));
					$d = $this->Document->document_factory_url("file://" . $file);
					preg_match("/^([0-9]+)_/",$this->basename_nonenglish($file),$patient_match);
	...
	function queue_action_process() {
		...
		foreach($files as $doc_id=> $file) {
			...
			if (!file_exists($new_path)) {
		  	...
		  	preg_match("/^([0-9]+)_/",$this->basename_nonenglish($fname),$patient_match);
		  	...
		  	if (file_exists($new_path.$file['name'])) {
		  		...
		  		$fname = $this->basename_nonenglish($this->_rename_file($new_path.$file['name']));
		...
		elseif (strtolower($new_patient_id) == "q" && is_numeric($document_id)) {
			...
			//see if there is an existing file with the same name and rename as necessary
		  	if (file_exists($new_path.$d->get_url_file())) {
		  		...
		  		$fname = $this->basename_nonenglish($this->_rename_file($new_path.$d->get_url_file()));
		  		$messages .= "Current file name was changed to " . $fname ."\n";
		  	}
    ...
    function _rename_file($fname) {
        $path = dirname($fname);
        $file = $this->basename_nonenglish($fname);
 		...
 		foreach($array as $id => $ar) {
          ...
          if($this->tree->get_node_name($id) == "CCR"){
            $current_node->addItem(new HTML_TreeNode(array(
              'text' => $doc['docdate'] . ' ' . $this->basename_nonenglish($doc['url']),

...
class C_Document extends Controller
{
    ...
    public function __construct($template_mod = "general")
    {
        ...
        $this->assign("CURRENT_ACTION", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?" . "document&");
        ...
        $this->assign("IMAGES_STATIC_RELATIVE", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('images_static_relative'));

        //get global config options for this namespace
        $this->_config = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('oer_config')['documents'];
    ...
    public function upload_action($patient_id, $category_id)
    {
        ...
        $this->assign("hide_encryption", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('hide_document_encryption'));
        ...
        $activity = $this->fetch(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "documents/" . $this->template_mod . "_upload.html");
    ...
    {
        $zip = new ZipArchive();
        $zip_name = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/" . $study_name;
        if ($zip->open($zip_name, (ZipArchive::CREATE | ZipArchive::OVERWRITE)) === true) {
            foreach ($_FILES['dicom_folder']['name'] as $i => $name) {
                $zfn = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir') . "/" . $name;
    ...
    public function upload_action_process()
    {
        ...
        $couchDB = false;
        $harddisk = false;
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('document_storage_method') == 0) {
            $harddisk = true;
        }
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('document_storage_method') == 1) {
            $couchDB = true;
        }
        ...
        if (
            !\OpenEMR\Core\OEGlobalsBag::getInstance()->get('hide_document_encryption') &&
        ...
        if (!$skipUpload && count($_FILES['file']['name']) > 0) {
            ...
            foreach ($_FILES['file']['name'] as $key => $value) {
                ...
                } elseif (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('secure_upload') && !isWhiteFile($_FILES['file']['tmp_name'][$key])) {
                ...
                $upload_plugin = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documentUpload.plugin.php";
    ...
    public function note_action_process($patient_id)
    {
        ...
        if ($_POST['identifier'] == "no") {
            ...
            if ($couch_docid && $couch_revid) {
                ...
                $content = $resp->data;
                if ($content == '' && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_log') == 1) {
                ...
                $temp_couchdb_url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/temp/couch_' . date("YmdHis") . $d->get_url_file();
    ...
    public function view_action(?string $patient_id, $doc_id)
    {
        ...
        $this->assign("hide_encryption", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('hide_document_encryption'));
        $this->assign("assets_static_relative", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('assets_static_relative'));
        $this->assign("webroot", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot'));
        ...
        $activity = $this->fetch(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "documents/" . $this->template_mod . "_view.html");
        $this->assign("activity", $activity);
    ...
    public function retrieve_action(?string $patient_id, $document_id, $as_file = true, $original_file = true, $disable_exit = false, $show_original = false, $context = "normal")
    {
        ...
        if ($context === 'patient_picture') {
            if (!($session->has('patient_portal_onsite_two') && $session->has('pid'))) {
                $allowed_pid = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('pid') ?? 0;
        ...
        if ($couch_docid && $couch_revid) {
            ...
            if ($content == '') {
                ...
                if ($contentM == '' && \OpenEMR\Core\OEGlobalsBag::getInstance()->get('couchdb_log') == 1) {
                    $log_content = date('Y-m-d H:i:s') . " ==> Retrieving document\r\n";
                    $log_content = date('Y-m-d H:i:s') . " ==> URL: " . $url . "\r\n";
                    $log_content .= date('Y-m-d H:i:s') . " ==> CouchDB Document Id: " . $couch_docid . "\r\n";
                    $log_content .= date('Y-m-d H:i:s') . " ==> CouchDB Revision Id: " . $couch_revid . "\r\n";
                ...
                $to_file_tmp = tempnam(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('temporary_files_dir'), "oer");
                ...
                if (is_file($to_file_tmp_name)) {
                    $couchI = new CouchDB();
                    if ($d->get_encrypted() == 1) {
                        $document = $this->cryptoGen->encryptStandard(file_get_contents($to_file_tmp_name), null, KeySource::Database);
                    } else {
        ...
        if (!empty($from_filename)) {
            ...
            if ($couch_docid && $couch_revid) {
                ...
                $temp_url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/temp/' . $d->get_foreign_id() . '_' . $from_filename;
            } else {
                $temp_url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/' . $from_pathname . '/' . $from_filename;
            }
        ...
        if ($original_file) {
            ...
            $url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/' . $from_pathname . '/' . $convertedFile;
            if (!is_file($url)) {
                if ($d->get_encrypted() == 1) {
                    ...
                    if (is_file($to_file_tmp_name)) {
                        $to_file_encrypted = $this->cryptoGen->encryptStandard(file_get_contents($to_file_tmp_name), null, KeySource::Database);
    ...
    public function validate_action_process(?string $patient_id, $document_id)
    {
        ...
        if ($d->couch_docid && $d->couch_revid) {
            $file_path = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/documents/temp/';
    ...
    public function list_action($patient_id = "")
    {
        ...
        return $this->fetch(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "documents/" . $this->template_mod . "_list.html");
    }
    ...
    {
        $log_path = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/couchdb/";
        $log_file = 'log.txt';
        if (!is_dir($log_path)) {
    ...
    public function document_send($email, $body, $attfile, $pname)
    {
        if (empty($email)) {
          ...
          $mail = new MyMailer();
          $from_name = \OpenEMR\Core\OEGlobalsBag::getInstance()->get("practice_return_email_path");

...
class C_Prescription extends Controller
{
    ...
    {
        parent::__construct();
        $this->assign("TOP_ACTION", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . "/controller.php?" . "prescription" . "&");
        $this->assign("STYLE", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('style'));
        $this->assign("WEIGHT_LOSS_CLINIC", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('weight_loss_clinic'));
        $this->assign("SIMPLIFIED_PRESCRIPTIONS", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('simplified_prescriptions') === '1');
        $this->pconfig = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('oer_config')['prescriptions'];
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('inhouse_pharmacy')) {
    ...
    function default_action(): void
    {
        ...
        $vars['enable_amc_prompting'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_amc_prompting') ?? false;
        $vars['weno_rx_enable'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('weno_rx_enable') ?? false;
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('enable_amc_prompting')) {
    ...
    function edit_action($id = "", $patient_id = "")
    {
        ...
        $url = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('webroot') . '/interface/patient_file/encounter/select_codes.php?codetype=' . urlencode((string) $urlCodes);
        $this->assign('diagnosisCodes', $this->getDiagnosisCodesList($this->prescriptions[0]));
        $this->assign("addCodeUrl", $url);

        $this->assign("GBL_CURRENCY_SYMBOL", \OpenEMR\Core\OEGlobalsBag::getInstance()->get('gbl_currency_symbol'));
    ...
    function list_action($id, $sort = "", $printPrescriptionId = null)
    {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_drug_drug')) {
            $interaction = "";
        ...
        $vars = $this->getTemplateVars();
        $vars['pid'] = $id;
        $vars['rx_send_email'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_send_email') ?? false;
        $vars['rx_show_drug_drug'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_drug_drug') ?? false;
        $vars['rx_zend_pdf_template'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_zend_pdf_template') ?? false;
        $vars['baseModDir'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('baseModDir') ?? '';
        $vars['zendModDir'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('zendModDir') ?? '';
        $vars['printm'] = null; // TODO: figure out where printm is used or defined
        $vars['rx_zend_pdf_action'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_zend_pdf_action') ?? '';
        $vars['rx_zend_html_template'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_zend_html_template') ?? '';
        $vars['rx_zend_html_action'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_zend_pdf_action') ?? '';
        $vars['rx_use_fax_template'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_use_fax_template') ?? '';
        $vars['rx_send_email'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_send_email') ?? false;
    ...
    function block_action($id, $sort = "")
    {
        ...
        $this->display(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "prescription/" . $this->template_mod . "_block.html");
    }
    ...
    function fragment_action($id, $sort = "")
    {
        ...
        //print_r(Prescription::prescriptions_factory($id));
        $this->display(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "prescription/" . $this->template_mod . "_fragment.html");
    }
    ...
    function lookup_action()
    {
        ...
        $this->do_lookup();
        $this->display(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('template_dir') . "prescription/" . $this->template_mod . "_lookup.html");
    }
    ...
    function multiprint_header(&$pdf, $p)
    {
        ...
        //print header
        $pdf->ezImage(\OpenEMR\Core\OEGlobalsBag::getInstance()->get('oer_config')['prescriptions']['logo'], null, '50', '', 'center', '');
    ...
    //     Administration->Globals->Rx
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_DEA')) {
            if ($this->is_faxing || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_DEA')) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_NPI')) {
            if ($this->is_faxing || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_NPI')) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_SLN')) {
            if ($this->is_faxing || \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_SLN')) {
    ...
    function multiprintcss_header($p)
    {
        ...
        echo ("<td></td>\n");
        echo ("<td>\n");
        echo ("<img WIDTH='68pt' src='./interface/pic/" . \OpenEMR\Core\OEGlobalsBag::getInstance()->get('oer_config')['prescriptions']['logo_pic'] . "' />");
        echo ("</td>\n");
        echo ("</tr>\n");
        echo ("<tr>\n");
        echo ("<td>\n");
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_DEA')) {
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_DEA')) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_NPI')) {
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_NPI')) {
        ...
        if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_enable_SLN')) {
            if (\OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_show_SLN')) {
    ...
    function multiprint_body(&$pdf, $p)
    {
        ...
        $pdf->ezSetY($my_y);
        $pdf->ezText($d, 10);
        $pdf->ez['leftMargin'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_left_margin');
        $pdf->ez['rightMargin'] = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('rx_right_margin');
        $pdf->ezText('');
        $pdf->line($pdf->ez['leftMargin'], $pdf->y, $pdf->ez['pageWidth'] - $pdf->ez['rightMargin'], $pdf->y);
        $pdf->ezText('');
    }
    ...
    function getDefaultMailClientText_action()
    {
        ...
        $data = ob_get_clean();
        $result = [
            'subject' => \OpenEMR\Core\OEGlobalsBag::getInstance()->get('openemr_name') . " " . xl(" Prescription ")
            ,'message' => $data
        ];
    ...
    function email_prescription($id, $email, $sendAsPdf)
    {
        ...
        if ($sendAsPdf) {
            ...
            $mailBody = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('openemr_name') . " " . xl("Prescription attached to this email.") . " " . xl("Patient") . " " . $patient->get_name_display();
        } else {
...
//        $text_body  = $p->get_prescription_display();
        $mail->Body = $mailBody;
        $mail->Subject = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('openemr_name') . " " . xl("Prescription");
    ...
    function fax_prescription($p, $faxNum)
    {
        ...
        if (!empty($faxNum) && is_numeric($faxNum)) {
            ...
            if (empty($cmd)) {
                ...
                $fileName = \OpenEMR\Core\OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/documents/" . $p->get_id() .

...
- **The shared `.git/` in `<git-dir>/openemr/` is code-execution-relevant on the host.** Because the bind mount is writable by the agent, `.git/config` values (`core.sshCommand`, `core.hooksPath`, `credential.helper`, aliases, custom `filter` drivers), hook scripts under `.git/hooks/`, and directives in `.gitattributes` are all agent-writable. Any host-side git operation against this shared clone — `push`, `fetch`, `checkout`, commit (with or without signing) — can execute agent-planted code with your host credentials. This is consistent with the "bind mount is not protected" line above, but worth calling out on its own because it is easy to forget when doing routine host-side git work against a repo that also lives inside the appliance. For operations you want to insulate from this (typically: signed commits, or pushes to sensitive remotes from your host identity), use a separate host clone the agent cannot reach.
...
- **For stuff that must be strictly isolated from a compromised appliance** (such as personal finances), the honest answer is a physically separate machine — ideally powered off except when actively in use — not a tighter container config on this host. As AI-agent autonomy grows, that separation-by-hardware pattern is likely to matter more.
...
- **Optional followup:** an unprivileged-LXC variant is sketched at the end of this guide (see [Optional followup: unprivileged LXC](#optional-followup-unprivileged-lxc)). It would narrow kernel-escape blast radius but does not close the shared-`.git/` path above, and is not required for the workflow this guide targets.
...
> **Note on the bind-mount attack surface.** Host-side `git commit --amend -S` against this shared clone will execute any `core.sshCommand`, hook, or filter values the agent may have written to `.git/config` or `.git/hooks/`. See the [shared-`.git/` bullet](#threat-model) at the top of this guide. If you want signed commits fully insulated from that, pull the branch into a separate host clone of your fork (kept outside the appliance's bind mount), amend + sign there, and push from there.
...
## Optional followup: unprivileged LXC

This appendix documents an engineering direction, not a required next step. The setup above is a **privileged** LXC container (container root maps to host root, AppArmor unconfined because the `generated` profile blocks runc from writing per-container sysctls under Docker 26+, explained at Step 4). It is a coherent choice for the threat model at the top — the container is the trust boundary, the appliance is disposable, and the setup is short.

**What a user-unprivileged variant would close, and what it would not.** An unprivileged LXC migration reduces the blast radius of one specific attack: a kernel-level namespace escape from inside the container. In the current privileged posture, a successful kernel escape lands as host root. Under unprivileged LXC, the same escape typically lands as an unprivileged host UID (e.g. `100000`) that owns nothing on the host and can only read world-readable content — a much narrower blast radius. (Some kernel vulnerabilities cross user-namespace boundaries and still land as full root regardless; the UID shift addresses common-case escapes, not that class of exploit.)

That is the entire security delta. In particular, an unprivileged LXC variant **does not** address the shared-`.git/` bind-mount attack surface documented in the threat model. `.git/config`, `.git/hooks/`, and `.gitattributes` remain agent-writable in the bind-mounted directory under either posture, and host-side git operations against that clone will still execute agent-planted values. That is a bind-mount problem, not a privilege problem, and no repositioning of container-root's host UID fixes it. The mitigation for that path — doing signing and other trust-anchor host git operations from a separate clone outside the bind mount — is available under either posture.

**Kernel-escape risk in practice.** An off-the-shelf coding agent has no motive or built-in capability to weaponize a container-escape 0-day. The kernel-escape path is a real category of concern for a hostile actor with kernel-exploit development capacity, less so for a routine agent-writes-code workflow. Weigh that against the ~2–3 days of setup work and the ~5–10% bind I/O overhead below when deciding whether to pursue this.

If someone does pick this up, the design sketch:

- **Root maps to a shifted host UID** (`100000`) that owns nothing on the host and can only read world-readable content. A container-to-host kernel escape then typically lands as that unprivileged UID rather than as host root — userns-bypass kernel vulns still exist and would land as full root under either posture.
...
The variant adds a bindfs dependency, a system-level systemd mount unit for the bindfs shim, doc divergence from the current privileged setup, and roughly 5–10% bind-mount I/O overhead. Verification against nine gates covers nested Docker, per-worktree stacks, port mapping through LXC NAT, and reboot resilience.

Design decisions locked so far (subject to revision if the work is picked up):
...
If no one picks this up, the privileged posture stays. It is not a stopgap — it is a coherent choice for its intended threat model.

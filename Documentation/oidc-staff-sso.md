...
Users by Email** is on and the ID token has `email_verified` true) and stores
the provider `iss` + `sub` in `oidc_external_identity`. Later logins use that
binding. Existing ACL is unchanged.
...
`http://127.0.0.1:8480/realms/openemr`.

### Local MFA and identity matching

Staff with registered OpenEMR TOTP or U2F devices must finish the local
challenge after returning from the identity provider. Navigating directly to
another authenticated page returns the pending login to that challenge.

Issuer and subject bindings are byte-exact, including case and trailing spaces.
Run the SQL upgrade when updating an installation created with an earlier
version of this feature. If `email` is the configured username claim, the ID
token must also contain a verified email claim.

...
$msgId = trim(strip_tags($_REQUEST['messageId']));
$fromNumber = trim(strip_tags($_REQUEST['msisdn']));
...
$msgStatus = trim(strip_tags($_REQUEST['status']));
$msgText = trim(strip_tags($_REQUEST['text']));
...
try {
	if (!empty($msgStatus) || !empty($clientRef)) {
		...
		// Process new message
		$sms = new wmt\Nexmo();
		$sms->smsReceived($fromNumber, $toNumber, $msgId, $timestamp, $message);

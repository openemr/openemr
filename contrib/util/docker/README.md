...
setup input screen:
 - for `Server Host`, use either `mariadb` or `mysql` or `mariadb-dev` or `mariadb-old` or
   `mariadb-very-old` or `mysql-old` or `mysql-very-old` (you have all
   ...
   `mariadb` is version 10.3 and `mariadb-dev` is version 10.4; `mariadb-old` is version
   10.2; `mariadb-very-old` is version 10.1)

...
formHeader("Form:Medical Evaluation");
$returnurl = 'encounter_top.php';
$formid = 0 + (isset($_GET['id']) ? $_GET['id'] : '');
$obj = $formid ? formFetch("form_medical_evaluation", $formid) : array();
...
<body class="body_top">
<p><span class="forms-title"><?php echo xlt('Medical Evaluation'); ?></span></p>
</br>
<?php
echo "<form method='post' name='my_form' " .
  "action='$rootdir/forms/medical_evaluation/save.php?id=" . attr($formid) ."'>\n";
?>
...
<td align="left" class="forms" class="forms"><?php echo xlt('Client Name' ); ?>:</td>
   ...
   ?>
    ...
    <input type="hidden" name="client_number" value="<?php echo attr($patient_id);?>">
		...
		<td align="left" class="forms"><?php echo xlt('Service Date'); ?>:</td>
		<td class="forms">
			   <input type='text' size='10' name='service_date' id='service_date' <?php echo attr($disabled) ?>;
			   value='<?php echo attr($obj{"service_date"}); ?>'
...
?>
  ...
  <td colspan='3' nowrap style='font-size:8pt'>
   &nbsp;
	...
	<tr>
		<td align="left" class="forms"><?php echo xlt('CC'); ?>:</td>
		<td colspan="3"><textarea name="CC" rows="2" cols="60" wrap="virtual name"><?php echo text($obj{"CC"});?></textarea></td>

	</tr>
	<tr>
		<td align="left" class="forms"><?php echo xlt('PE'); ?>:</td>
		<td colspan="3"><textarea name="PE" rows="2" cols="60" wrap="virtual name"><?php echo text($obj{"PE"});?></textarea></td>

	</tr>
	<tr>

		<td align="left" class="forms"><?php echo xlt('Dx'); ?>:</td>
		<td colspan="3"><textarea name="Dx" rows="2" cols="60" wrap="virtual name"><?php echo text($obj{"Dx"});?></textarea></td>


	</tr>
	<tr>
		<td align="left" class="forms"><?php echo xlt('Plan'); ?>:</td>
		<td colspan="3"><textarea name="Plan" rows="2" cols="60" wrap="virtual name"><?php echo text($obj{"Plan"});?></textarea></td>
...
Calendar.setup({inputField:"service_date", ifFormat:"%Y-%m-%d", button:"img_admission_date"});

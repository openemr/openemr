...
$sets = "pid = {$_SESSION["pid"]},
  ...
  service_date             =  '" . add_escape_custom($_POST["service_date"]) . "',
  CC          = '" . add_escape_custom($_POST["CC"]) . "',
  PE            =  '" . add_escape_custom($_POST["PE"]) . "',
  Dx                = '" . add_escape_custom($_POST["Dx"]) . "',
  ...
  if (empty($id)) {
  $newid = sqlInsert("INSERT INTO form_medical_evaluation SET $sets");
  addForm($encounter, "Medical Evaluation", $newid, "medical_evaluation", $pid, $userauthorized);
}
else {
  sqlStatement("UPDATE form_medical_evaluation SET $sets WHERE id = '". add_escape_custom("$id"). "'");
}

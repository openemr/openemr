...
#
my $DBNAME = "oemr501"; # database name
my $DBUSER = "oemr501"; # database user name
my $DBPASS = "Welcome";  # database user's password
my $DBHOST = 'openemr-cluster.cluster-cygxe2gztgaf.us-east-1.rds.amazonaws.com';
my $DBPORT	= '3306';
my $DBUSER = 'openemr';
my $DBPASS = 'KK26V%rk!WIJkn$jS!oI4wtUs';
my $DBNAME = 'openemr';

# Log file location:
#
my $INSLOG = "/var/www/scan/prognocis_pat_scan_load2.log";

# Shared directory base where the scanners deposit files:
#
my $INPATH = "/mnt/PrognoCIS";

# Base directory for OpenEMR documents:
#
# my $OUTPATH = "/usr/local/apache2/htdocs/openemr/documents";
my $OUTPATH = "/var/www/html/openemr/sites/default/documents";
...
# print "Directory: $dname  and file ($fname)\n";
# return;

  # Get nut if the source file is open by any other process.  This
  ...
  if (! @prow) {
    ...
    rename "$path", "$dname/ERR-$fname" if (! $DEBUG);
    return;
  }
  ...
  my $catid = 43; # EMD Imported
  my $catid = 42; # Prognocis Imported
  ...
  if(uc(substr($fname, -3)) ne "PDF") {
		&tolog(" -- Not a PDF, Skipping ($fname)");
    return;
  }
  ...
  #
  my $count = 0;
  if (-e "$OUTPATH/$pid/$fname") {
    &tolog("Skipping processed file '$path'", 1);
	  return;
	}
...
sub scan_dir($) {
  ...
  while (my $dirent = readdir $dh) {
    ...
    my $thispath = $path ? "$path/$dirent" : $dirent;
		# print "Checking '$thispath'\n";
    if (-d "$thispath") {
      &scan_dir($thispath);
			# print "It is a directory\n";
			# print "Scanning [$thispath]\n";
    } else {
			print "Processing File ($thispath)\n";
			# print " -- From Directory ($path)\n";
...
&scan_dir($INPATH); # This makes everything happen.
